...
Deployment）へ atomically swap します。引数を 省略した場合は working directory の `.takosumi/manifest.yml` (旧
`.takos/app.yml` は deprecated alias、後方互換のため受理) から `name` を読み、 それを group として扱います。
