...
は作らず、`deployment_id` は `preview:<digest>`
を返します。`takos deploy --resolve-only` は resolved Deployment record
を作ったうえで apply を行いません。それ以外の `takos deploy` 形式は default の
...
resolved Deployment (`status="resolved"`) を `applied`
に進めます。`takos deploy
--resolve-only` で作った Deployment id
を渡して使います。PolicySpec が `require-approval` decision
を残している場合、`takos approve` で approval を attach するまで `takos apply`
は失敗します。
...
`require-approval` decision が記録されている場合、`takos apply` の前に 1
度実行する必要があります。
...
`--json` を受け付けます。`--version` は package catalog の release version
または tag を指定し、未指定時は latest package を使います。`--group` は manifest
の `name` 由来の deploy group 名を override し、`--env` は target env
...
`resource rotate-secret` も新しい secret value を stdout
に出力し、human-readable mode では rotate
時刻を補助行として表示します。structured response が必要な場合は `--json`
を使います。
