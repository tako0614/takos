...
- **Layer 1: primitive (foundation)** — compute (worker / service / attached) /
  storage / route / publish。primitive 個別操作は `takos resource` または
  `/api/services/*` HTTP API 経由で CRUD する
...
deploy（alternative）の両方を扱います。 preview は `takos deploy --plan`
を使います（standalone の `takos plan` コマンドは ありません）。`takos install`
は catalog 経由で `takos deploy` を呼ぶ sugar です。 public spec は
Cloudflare-native で、実行モデルは Takos runtime です。

`takos deploy` はローカル working tree 由来でも repo/ref source
由来でも、いずれも 同じ pipeline を通り、immutable な app deployment
record（snapshot）を作ります。 group の `source` field（`local` /
`repo:owner/repo@ref`）は manifest の出どころを 示す metadata であり、lifecycle
の差ではありません。どちらの経路で deploy された group も
`takos rollback GROUP_NAME` で snapshot を再適用できます。
...
| command                 | 説明                                                                                 |
| ----------------------- | ------------------------------------------------------------------------------------ |
| `takos resource ...`    | storage / resource primitive の個別 CRUD（binding / data plane / secret 操作を含む） |
...
> 行います。CLI には `takos service` サブコマンドは存在しません（legacy 名のみ、
> current surface では未提供）。

`takos api ...` は removed legacy surface です。current CLI には含めません。
`takos apply` は廃止され、`takos deploy` に統合されました。 `takos plan` という
standalone command はありません。preview は `takos deploy --plan` を使います。
...
`takos deploy` は Takos の唯一の deploy entrypoint です。ローカル manifest
からの deploy（primary）と repository URL からの
deploy（alternative）を一つのコマンドで 扱います。
...
primitive (compute / storage / route / publish) は個別に CRUD できます。group に
所属させない場合 standalone primitive となり、それぞれが独立した lifecycle unit
になります。

compute (worker / service) の CRUD は `/api/services/*` HTTP API
経由で行います。 storage primitive の CRUD と data plane 操作は `takos resource`
サブコマンドが 提供します。
...
takos resource update res_xxx --body '{...}'
takos resource remove res_xxx

# group 所属 / 解除
takos resource attach my-db --group my-app
takos resource detach my-db
...
# data plane 操作
takos resource sql <subcommand>          # SQL (tables / query / export)
takos resource object <subcommand>       # object-store (list / put / get / delete)
takos resource kv <subcommand>           # key-value (get / put / list / delete)
...
compute / route (custom domain) の個別操作は `/api/services/*` 配下の HTTP API
を直接呼び出します。代表的な endpoint:
