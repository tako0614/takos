...
Takos CLI は bearer token を使って API にアクセスします。トークンは Takosumi
Accounts の PAT または OIDC フローで取得します。
...
`takos login --token` は token と endpoint を `~/.takos/config.json`
に保存します。 session bearer から PAT を作る場合は、Takosumi Accounts の
account token API を使います。
...
を参照します。 `takos login` と `takos endpoint use`
はこのファイルを更新します。
