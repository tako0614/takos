...
service 名で、 forward 3-level dotted format
(`<ecosystem>.<area>.<function>@<version>`) を取ります。
...
- manifest top-level field 詳細 ([manifest-spec](./manifest-spec))
- AppBinding kind の field 詳細 ([binding-catalog](./binding-catalog))
...
3 階層を超えた構造 (例 `takosumi.platform.subsystem.subservice`) を要する 場合、
area / function を統合して 3 階層に収める guidance を取ります:
...
| `endpoints[]`               | provider deploy 時に operator URL で resolved した endpoint role list。 consumer 側 resource spec が `${imports.<alias>.endpoints.<role>.url}` で参照する |
...
type CrossInstanceShare = {
  ...
  resolvedDescriptor: ServiceDescriptor; // anchor 経由 fetch、resource metadata / WAL に pin
...
anchor は provider が `publish` で signed descriptor を upsert したものを relay
します。 anchor 自身は signature を生成しません (provider's privateKey で
署名されたまま relay)。
...
現状の mainline は consumer 側の manifest validation / anchor fetch / signature
verify / descriptor pinning を実装済みです。provider publish
automation、descriptor cache refresh / revoke、installer-only placeholder
