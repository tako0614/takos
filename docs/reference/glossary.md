...
binding 注入、manifest compile、kernel への deploy を担う上位 sibling product。
(see new.md §2.4, §12)
...
placeholder は takosumi-git / Takosumi Accounts が compile 時に materialize /
strip し、kernel に渡る最終 manifest には残さない。kernel は `${ref:...}` /
`${secret-ref:...}` / `${imports...}` を resource DAG / service import として
扱える。 (see new.md §6, [reference/manifest-spec](/reference/manifest-spec))
...
に物理化された状態。`takosumi materialize` で遷移する。 (see new.md §10.2,
[architecture/runtime-modes](/architecture/runtime-modes))

### self-hosted

runtime mode の 1 つ。`takosumi export` で installation bundle を取り出し、 別の
takosumi (Keycloak / MinIO / 自前 Postgres など任意 binding) に import
