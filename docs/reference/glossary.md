...
flat YAML の single-document deploy manifest。 トップレベルは Core 語彙のみ
(`components` / `routes` / `resources` / `bindings` / `publications` /
`environments` / `policy`) で、 値の意味は全て `ref: <descriptor>` と
descriptor schema に従う `config:` で表現する。 `.takos/app.yml` が既定の
deploy manifest path で、 `.takos/app.yaml` も受け付ける。 app catalog では
ない。 filename には `app` が残るが、 deploy model では primitive desired
declaration を書く manifest として扱う。
...
Store / UI 上の product label。 deploy model を説明するときは primitive /
group / component / route / publication / resource を使う。
...
Git, Store, Auth) は publication ではなく kernel API として直接提供される。
