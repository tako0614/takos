...
`takosumi-git` / `takosumi-cloud` が compile し、`workflowRef` を strip します。
`${artifacts.*}` / `${bindings.*}` などの installer-only placeholder が未解決で
残る場合、current takosumi-git は kernel request の前に compile error にします。
...
`takosumi-git` compiler は、これらの installer-only placeholder が entry
manifest に未解決で残っている場合、Accounts / kernel request の前に reject
します。compile 後の kernel-bound manifest には残してはいけません。
...
after anchor resolution and may remain in the compiled manifest:
...
substitute unresolved installer-only placeholders; it fails the compile. Service
import placeholders must use the `${imports.*}` family.
...
へ流さず compile error にします。kernel 内の resource 間配線は `${ref:...}` /
`${secret-ref:...}`、cross-instance service は `imports[]` /
`serviceResolvers[]` を使います。
