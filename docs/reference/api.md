...
manifest の `publish` と同じ publication catalog を扱う API。route publication
は manifest deploy が正本、provider publication はこの API からも管理できる。
...
status 上書きも未対応。詳細は `packages/actions-engine/README.md`。
...
があります。詳細は `packages/actions-engine/README.md` の compatibility table
を参照してください。
...
state transitions: `pending → building → deploying → active`、または途中で
`failed`。`active` deployment は後続 deploy で `archived`、rollback 操作で
`rolled_back` になる。
...
明示的に close するには
`DELETE /api/spaces/:spaceId/browser-sessions/:sessionId` を呼ぶ。 close 後の再
