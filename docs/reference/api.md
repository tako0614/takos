...
- `import-repository` は inventory の repository entry を Local DB に複製
  ...
  `{ "canonical_repo_url": "<repo actor URL>",
  "local_name": "<optional local repo name>" }`。`remote_owner`
  / `remote_repo_name` は使わない。`canonical_repo_url` は選択した Store の
...
互換 backend では Takos-managed implementation に解決されます。
...
`group_name` は必須です。`source` は `git_ref` と `manifest` の 2 種類を
...
を返します。`group_name` は必須ですが、DB row や group-scoped declaration
は作成・更新しません。
...
どちらの形式でも `group_name` は必須です。manifest の `name` は display metadata
であり、group 名として暗黙解決されません。
...
| GET    | `/ns/takos`                             | Takos 名前空間定義     |
| GET    | `/ap/stores/:store`                     | ストアアクター情報     |
| GET    | `/ap/stores/:store/inventory`           | Inventory              |
...
| GET    | `/ap/repos/:owner/:repoName`            | Repository actor       |
| POST   | `/ap/repos/:owner/:repoName/inbox`      | Repository Inbox       |
| GET    | `/ap/repos/:owner/:repoName/outbox`     | Repository Outbox      |
| GET    | `/ap/repos/:owner/:repoName/followers`  | Repository followers   |
| GET    | `/ap/repos/:owner/:repoName/stores`     | Stores collection      |
