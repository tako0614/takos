...
| family                                    | auth     | purpose                                     |
| ----------------------------------------- | -------- | ------------------------------------------- |
| [`seed-repositories`](#seed-repositories) | none     | workspace bootstrap 用 seed repo 一覧       |
...
| [`apps`](#apps)                                  | workspace-installed app inventory / install / launch         |
...
default distribution が workspace bootstrap のために返す seed
リポジトリ一覧。認証不要。
...
| PATCH  | `/api/spaces/:spaceId`               | ワークスペース更新 _(owner/admin)_ |
| DELETE | `/api/spaces/:spaceId`               | ワークスペース削除 _(owner)_       |
...
default distribution に含まれる first-party catalog app が使う、workspace-scoped
...
`type`: `d1` | `r2` | `kv` | `queue` | `vectorize` | `analyticsEngine` |
`secretRef` | `workflow` | `durableObject`
...
workspace に install された app の inventory / detail / launch surface。
first-party app と third-party app を同じ contract で扱います。

| method | path                                      | description                  |
| ------ | ----------------------------------------- | ---------------------------- |
| GET    | `/api/spaces/:spaceId/apps`               | installed app 一覧           |
| POST   | `/api/spaces/:spaceId/apps/install`       | app install                  |
| GET    | `/api/spaces/:spaceId/apps/:appId`        | installed app 詳細           |
| GET    | `/api/spaces/:spaceId/apps/:appId/launch` | shell launch descriptor 取得 |
| DELETE | `/api/spaces/:spaceId/apps/:appId`        | app uninstall                |

#### `GET /api/spaces/:spaceId/apps/:appId/launch`
...
```json
{
  "app": {
    ...
    "display_name": "Chat",
    "workspace_id": "ws_123",
    "canonical_href": "/chat/ws_123"
  },
  "launch": {
    "shell_href": "/apps/ws_123/open/chat",
    "canonical_href": "/chat/ws_123",
    "open_mode": "iframe",
    "iframe_src": "/chat/ws_123?takos_shell=1",
    "launch_token": "tak_launch_xxx"
  }
}
```

`open_mode` は `iframe` または `redirect` です。compatibility alias として残る
`/api/apps*` は public contract には含めません。
...
`/api/workspaces/:workspaceId/skills` は同じ操作。
...
`git_ref` source を使うアプリデプロイ管理。`takos install` は catalog metadata
から `repository_url + tag` を解決して同じ endpoint を使います。
...
`POST /api/spaces/:spaceId/groups/apply` は current authoring surface
です。`plan` は non-mutating preview、`apply` は必要なら group
を作成して反映します。`POST /api/spaces/:spaceId/groups/uninstall` は empty
desired state を apply して inventory を drain したあと group を削除します。

group には source projection が保存されますが、`currentAppDeploymentId`
が意味を持つのは app-deployments 経由で管理されている group
だけです。`groups/apply` や local working tree からの `local_upload` は source
projection を更新しても app deployment history record
を作らないため、`currentAppDeploymentId` は意図的に `null` のままです。
...
source と immutable snapshot rollback を public contract
として扱います。rollback は snapshot execution context を含み、既存 group
があることを前提にします。
