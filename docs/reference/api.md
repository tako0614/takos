...
<!-- docs:api-families seed-repositories,explore,profiles,public-share,mcp,setup,me,spaces,shortcuts,services,custom-domains,resources,publications,apps,threads,runs,search,index,memories,skills,sessions,git,repos,agent-tasks,notifications,events,pull-requests,app-deployments,browser-sessions,billing,auth,oauth,groups -->
...
::: tip Publication provider tokens
...
`storage:` は使わず、resource / provider-backed resource は control-plane の API
で管理します。 :::
...
| [`publications`](#publications)                  | publication catalog / provider-managed publication           |
...
| [`events`](#events)                              | space lifecycle SSE                                          |
...
::: tip MIME type detection ファイル作成時の `mimeType` field は client
...
::: warning Move / rename atomicity `PATCH /storage/:fileId` と `bulk-move` /
...
::: tip Subscription model Store registry の subscription は **pull-based**
です。

- リモートストア追加時 + `refresh` 呼び出し時に metadata を fetch (ActivityPub
  actor + outbox)
- 自動 polling は **行われません**。space owner が `/refresh` または `/poll`
  を叩くか、catalog app の cron で更新を取得する設計
- `updates` endpoint は last `mark-seen` 以降に追加された Add activity
  の差分を返す
...
| method | path                          | description   |
| ------ | ----------------------------- | ------------- |
| GET    | `/api/publications/providers` | provider 一覧 |
...
| DELETE | `/api/publications/:name` | publication 削除       |

manifest の `publish` と `compute.<name>.consume` が public contract の正本で、
この API は publication catalog と provider publication を管理する control-plane
surface。route publication は manifest deploy が正本、provider publication は
この API からも管理できる。legacy storage-side auto-injection は retired。
...
Cloudflare-native で、Takos runtime がその spec を各 backend 上で実現します。
Cloudflare backend では通常そのまま対応する Cloudflare resource に解決され、
互換 backend では provider-backed または Takos-managed implementation に
解決されます。
...
::: warning `type` field enum `type` field は schema-level enum check
...
すべての field は optional。runner はこれらを参考に tool
選択や出力形式を決定する。
...
lifecycle、`conclusion` は最終結果。terminal status (`completed` / `cancelled`)
になった瞬間に `conclusion` が確定。

### Trigger compatibility

GitHub Actions YAML の `on:` は **`push` / `pull_request` /
`workflow_dispatch`** のみ正常に dispatch されます。`schedule:` (cron) /
`repository_dispatch` / `workflow_call` 等は parser で受理されますが **kernel
側に scan / dispatch 経路が無く、silently never fires**。scheduled workflow
が必要な場合は、kernel の cron worker から直接 `dispatchWorkflowRun()`
を叩く設計が必要。
...
`contains()` / `startsWith()` / `==` / `&&` / `||` / 三項演算子 / 算術 /
composite expression は **未サポート**。未認識の expression は `false`
...
です。parser が受理する YAML であっても runner 側で ignore される field
があります。Actions engine の compatibility table を参照してください。
...
`status` field の値: `planned` / `in_progress` / `completed` / `cancelled` /
`failed`
...
| method | path                                                             | description                    |
| ------ | ---------------------------------------------------------------- | ------------------------------ |
| POST   | `/api/spaces/:spaceId/app-deployments/plan`                      | 非破壊のデプロイ差分 preview   |
...
| status    | 意味                                                          |
| --------- | ------------------------------------------------------------- |
| `applied` | manifest apply が完了し、snapshot / deployment row が保存済み |
| `failed`  | apply pipeline の一部が失敗した deployment record             |
| `deleted` | history 一覧から論理削除された deployment record              |

build / provider apply の進行中状態は現行の `app_deployments.status`
には保存しません。 各 workload の詳細は `apply_result.applied[].status` と
deployment / managed service 側の record を参照します。

#### `POST /api/spaces/:spaceId/app-deployments/plan`

`POST /api/spaces/:spaceId/app-deployments` と同じ `source` payload を受け取り、
repo / manifest source を解決して non-mutating preview を返します。DB row や
group desired state は作成・更新しません。
...
::: tip Browser session lifecycle Browser session は外部 `BROWSER_HOST` service
(Cloudflare backend では `takos-browser-host` の `BrowserSessionContainer`
Durable Object) に delegate される。session は作成後 idle で **5-15 分** (tier
依存) で自動 close される (CF Container DO の `sleepAfter` window)。

明示的に close するには `DELETE /api/browser-sessions/:id` を呼ぶ。 close 後の再
attach は **不可能** — session id は revoke され、新 session を
作成する必要がある。長時間の browser task は session の idle 延命を前提にせず、
reconnect ではなく新 session 作成で対応すること。 :::

::: warning 同時実行数上限 1 つの space が同時に持てる browser session 数は
**5** に固定されている (`MAX_BROWSER_SESSIONS_PER_SPACE`)。上限に達した状態で
`POST /api/spaces/:spaceId/browser-sessions` を呼ぶと、common error envelope の
`{ error: { code: 'RATE_LIMITED', message } }` shape と HTTP `429` で reject
される。既存 session を `DELETE /api/browser-sessions/:id` で 閉じてから再
create すること。cap の実装詳細は
[container-hosts § Per-space concurrent session cap](/architecture/container-hosts#browser-host)
を参照。 :::
...
app deployment record を作って `currentAppDeploymentId` を更新します。API source
kind は `manifest` / `git_ref`、group projection は `local_upload` / `git_ref`
です。CLI / UI の表示名として `local` / `repo:owner/repo@ref`
を使いますが、これは manifest の出どころを示す metadata であり、 lifecycle
の差では ありません。`groups/apply` も同じく snapshot を作ります。
...
(introspect / API auth middleware) では legacy compatibility のため `client_id`
/ `issuer` / `${issuer}/api` のいずれかを accept する fallback がある。 :::
