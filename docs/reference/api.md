...
`GET /api/publications/resolve?ref=<name-or-group/name>&consumerGroupId=<id>`
は group-local publication 名と `<group>/<name>` 形式の cross-group ref
...
し、group deployment history として plan / apply / rollback する public surface。
...
| POST   | `/api/spaces/:spaceId/group-deployment-snapshots`                                     | deployment record 作成 + apply   |
| GET    | `/api/spaces/:spaceId/group-deployment-snapshots`                                     | deployment history 一覧          |
| GET    | `/api/spaces/:spaceId/group-deployment-snapshots/:groupDeploymentSnapshotId`          | deployment record 詳細           |
| POST   | `/api/spaces/:spaceId/group-deployment-snapshots/:groupDeploymentSnapshotId/rollback` | 直前成功 record への rollback    |
| DELETE | `/api/spaces/:spaceId/group-deployment-snapshots/:groupDeploymentSnapshotId`          | history record 論理削除          |

### Deployment record status

| status        | 意味                                      |
| ------------- | ----------------------------------------- |
| `in_progress` | manifest apply 中の deployment record     |
| `applied`     | manifest apply が完了した deployment record |
| `failed`      | deploy pipeline の一部が失敗した deployment record |
| `deleted`     | history 一覧から論理削除された record     |
...
commit / manifest metadata を deployment record に保存します。API 互換のため
`snapshot.state` は `source_cached` を返しますが、client はこれを「source から
rollback 可能な record」として扱い、UI では Ready / rollback ready 相当に丸めます。
...
は指定 record より前の直前成功 record を既存 group に適用します。対象
...
です。group に所属した primitive は inventory、deployment history、rollback、uninstall
などの group 機能を使えます。
...
直前成功の deployment record を適用し、apply pipeline を通して reconcile します。
...
由来は bundled snapshot ではなく source metadata / resolved commit を保存します。API source kind は
`manifest` / `git_ref`、group projection は `local_upload` / `git_ref` です。
CLI / UI の表示名として `local` / `repo:owner/repo@ref` を使いますが、これは
manifest の出どころを示す metadata であり、lifecycle の差ではありません。
`groups/apply` も group deployment record を作ります。
...
`git_ref` / `manifest` の 2 つの source kind と group-level rollback を public
contract として扱います。rollback は既存 group があることを前提にします。
