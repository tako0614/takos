...
operator account plane の AppInstallation API が所有します。Takos product gateway は chat / agent / memory / space
...
Space 内の app / memory / member / repository compatibility route は必要に応じて control backend に forward されます。
新規 route を増やす場合は owning service の contract と apps/api gateway の責務を先に決めます。
...
`GET` の read route は `apps/api` が所有し、write / nested operation は compatibility backend に forward されます。

| method | path | 説明 |
| --- | --- | --- |
| GET | `/api/spaces/:spaceId/tools` | custom tool catalog |
| GET | `/api/spaces/:spaceId/tools/:toolName` | custom tool 詳細 |
| * | `/api/spaces/:spaceId/tools/*` | tool operation compatibility route |

## Explore / Catalog

Explore / catalog route は `apps/api` が所有します。unknown `/api/explore/*` は control backend に proxy せず 404 を返します。
...
## GitOps Deploy Intent

Takos product gateway は AppInstallation-owned lifecycle に接続するため、GitOps deploy intent を repository に書き込みます。
direct unmanaged deploy は Takosumi kernel API を直接使う経路です。

| method | path | 説明 |
| --- | --- | --- |
| POST | `/api/public/v1/deployments` | GitOps deploy intent 作成 |
...
## Runtime Services / Resources / Sessions

Runtime gateway は Takosumi runtime の internal API に actor context 付きで forward します。space を body / query
で渡す root route と、path で space を固定する scoped route の両方を提供します。
...
Takos のアプリインストールの lifecycle は、operator account plane の AppInstallation API
(リファレンス実装: Takosumi Accounts) が所有します。Takos product はインストール済みアプリの表示、起動、
app-local セッションを扱います。

| method | path | 説明 |
| --- | --- | --- |
| GET | `/api/apps` | app catalog / app detail compatibility route |
| GET | `/api/apps/*` | app nested compatibility route |
| GET | `/api/spaces/:spaceId/app-installations` | AppInstallation ledger 一覧 |
...
| DELETE | `/api/spaces/:spaceId/app-installations/:installationId` | AppInstallation 削除 |
