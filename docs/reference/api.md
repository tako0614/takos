...
`git_ref` deployment は bundled snapshot を永続化せず、source cache
として repository URL / ref / commit / manifest metadata を保存します。
その場合、`snapshot.state` は `source_cached`、`snapshot.format` は
`source-cache-v1` になります。
...
由来は bundled snapshot ではなく source cache を保存します。API source kind は
