...
等) を使います（[deployments section](#deployments)）。primitive (compute / storage / route / publish) を 個別に CRUD
したい場合は `/api/services/*` / `/api/resources/*` / `/api/services/:id/custom-domains/*` などの primitive surface
を直接呼び出し ます。`PATCH /api/services/:id/group` / `PATCH /api/resources/:id/group` で 既存 primitive を group
に所属させることもできます。
