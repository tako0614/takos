...
1. 応答の request ID を控えます。
2. 同じ request ID の Worker ログを探します。
3. D1 migration が適用されているか確認します。
4. binding 名と実際のリソース ID を確認します。
5. 一覧 API の入力件数が D1 / SQLite の変数上限を超えていないか確認します。

大量の ID を一つの `IN (...)` に渡す実装は、D1 の SQL 変数上限で失敗します。ページング、件数制限、または分割取得が必要です。

## エージェントが開始しない

1. Chat に実行状態が作られているか確認します。
2. エージェント実行サービスのヘルスを確認します。
3. Queue と callback の binding を確認します。
4. モデル接続と利用上限を確認します。
5. 実行が `queued`、`running`、`failed` のどこで止まったか確認します。
...
1. Connections で接続が有効か確認します。
2. アプリ由来なら、そのアプリのデプロイが完了しているか確認します。
3. MCP サーバーの `tools/list` が成功するか確認します。
4. Workspace の権限とツール利用設定を確認します。
5. Chat を再読み込みし、現在の一覧を取り直します。

`web_fetch` は Web 検索ではありません。Web 検索ツールが必要なら、対応する MCP サーバーを接続します。

## アプリが表示されない

1. Apps でインストール状態を確認します。
2. apply が成功しているか確認します。
3. 起動 URL または UI の公開情報が記録されているか確認します。
4. 公開 URL が安全な HTTPS URL として受理されているか確認します。
5. 対象 Workspace に利用権限があるか確認します。

画面を持たないサービスは、Apps に起動ボタンを表示しない場合があります。MCP ツールだけを提供するサービスは Connections も確認してください。

## 通知が届かない

1. 実行を依頼したアカウントか確認します。
2. 実行が完了または失敗として記録されているか確認します。
3. 通知設定と端末の許可を確認します。
4. 運営者は通知キューと push 配送のログを確認します。

現在、Takos のモバイル push はエージェント実行の完了と失敗が対象です。詳しくは [通知](/get-started/notifications) を参照してください。

## OpenTofu の plan / apply が失敗する

1. 使用した Git commit と module path を確認します。
2. `tofu validate` を実行します。
3. provider の認証と権限を確認します。
4. plan の診断を最初のエラーから読みます。
5. apply が失敗した場合は、一部のリソースだけ作成されていないか state とクラウドを確認します。
