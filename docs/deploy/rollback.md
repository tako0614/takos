...
ロールバックは「同じ group の前の成功 app deployment source を使って再 deploy する」操作です。
...
`rollout` 系 endpoint は current v1 では提供していません。URL は互換のため残っていますが、呼び出すと `410 Gone` を返します。
...
1. **デプロイ前に manifest を検証する**: `takos plan` で non-mutating preview を確認
2. **DB マイグレーションは後方互換に**: ロールバック時に旧コードが動くよう、カラム追加は nullable にする
3. **依存順を意識する**: 複数 Worker のロールバックでは、依存元（呼び出し先）から順に戻す
4. **Container は Worker と一体**: Container だけの個別ロールバックはできないことを覚えておく
