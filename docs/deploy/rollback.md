...
> [Current Implementation Note](https://github.com/tako0614/takosumi/blob/master/docs/reference/architecture/index.md#deploy-shell) を参照
...
([Install API §0.5](https://github.com/tako0614/takos-ecosystem/blob/master/docs/reference/install-api.md#status-enum))、rollback pipeline は
...
`resources[]` と AppBinding / namespace export materialization から作られた
Deployment.desired.bindings は target Deployment の値に戻ります。operator-owned
surface は AppInstallation / account plane の現在の grant と issuer 設定を
参照します。
...
- resources[] desired state (worker@v1 / web-service@v1 / database-postgres@v1
  など)
- AppBinding / namespace export materialization 由来の binding declarations
...
- account-plane namespace export の現在の issuer / billing endpoint 設定
...
`requires` の順序解決と resource 間参照は kernel 側で行われるため、ユーザーが
手動で順番を意識する必要はありません。OIDC / billing などの operator-owned
surface は namespace export と account API 側で扱います。
