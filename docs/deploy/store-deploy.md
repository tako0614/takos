...
更新や rollback に必要な repository URL / ref / 解決済み commit を deployment
record に残し、実行時に repository source から再解決します。
...
snapshot ではなく source metadata と resolved commit を保存します。
