...
に解決し、`source.kind = "git_ref"` / `ref_type = "tag"` として同じ
...
artifact 解決は Takos API / Takos CLI ではなく `takosumi-git init` /
`takosumi-git push` が担当します。旧 `source.kind = "inline"` + workflow
artifact payload は API gateway で拒否されます。
