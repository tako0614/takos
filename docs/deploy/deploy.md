...
::: tip Installable App Model 配下では 2 path に分岐 app deploy には **2
つの異なる surface** があります。

1. **Install from Git** (推奨、ユーザー / 開発者向け)
   `takosumi-git install <git-url> --ref <tag>` または operator-selected install
   UI URL (managed example: `takosumi.cloud/install?git=...`) から
   **AppInstallation を経由して** bundled / third-party app を install
   する経路。詳細は [Install Paths](/apps/install-paths) と
...
| `.takosumi/manifest.yml`       | takosumi-git compiler  | compile 前                 | compute resource declaration (installer-only placeholder を含み得る) |
| compiled manifest HTTP payload | takosumi kernel        | `POST /v1/deployments`     | installer-only placeholder / `workflowRef` strip 済み Shape manifest |
...
bundled / third-party app 利用者が使う path です。Takosumi Account に紐付く
AppInstallation を作り、source を git commit に pin して、**takosumi-git
installer** が build / compile / optional kernel apply を担当します。
...
# またはブラウザから (managed example。self-host/operator instance では base URL が変わる)
...
User
 ...
 │ Install App
 ▼
takosumi-git installer  (= takosumi-git install / operator install UI)
 │
 │ 1. Git URL fetch
 │ 2. ref → commit pin
 ...
 │
 ▼
Takosumi Accounts                operator provider materializers
 │ identity.oidc@v1 を provision  │ database.postgres@v1, object-store.s3-compatible@v1, ...
 │ install-launch-token@v1 を issue│
 │                                │
 └────────┬───────────────────────┘
          │ binding 宣言 / provisioning plan
          ▼
takosumi-git compiler
 │
 │ 9. workflow artifact と binding material を compile
 │ 10. 未解決 installer-only placeholder を拒否
 │ 11. compiled manifest digest を計算
 │
 ▼
takosumi kernel
 │
 │ 12. POST /v1/deployments で apply (Deployment record 作成)
 │
 ▼
...
Installed App
 ...
 │ local verify → Accounts consume → owner session 作成 → app 起動
```

step ごとの詳細は
[Installer Pipeline](https://github.com/tako0614/takosumi-git/blob/master/docs/architecture/installer-pipeline.md)
を 参照。AppInstallation 行に何が pin されるかは
[AppInstallation 台帳](https://github.com/tako0614/takos-ecosystem/blob/master/docs/platform/app-installation.md)
を参照。

### 1.3 installer-bound bindings

`.takosumi/app.yml` の `bindings:` で宣言された binding は、install preview /
AppInstallation ledger / Accounts 側 provisioning plan の入力です。current
`takosumi-git` compiler は `${bindings.*}` / `${secrets.*}` / `${artifacts.*}` /
`${installation.*}` / `${params.*}` / legacy `${refs.*}` が compile
後も残っていれば、Accounts / kernel request の前に失敗させます。binding
materializer が concrete value または secret ref を提供した場合だけ、compiled
manifest に入れられます。代表例:

| binding type                    | materializer が提供する値                                                         |
| ------------------------------- | --------------------------------------------------------------------------------- |
...
placeholder 文法は authoring-time reserved syntax です。current CLI は未解決の
installer-only placeholder を解決したふりで kernel に渡さず、deploy request
build 後も残る場合は kernel request 前に失敗します。完全な仕様 (placeholder 文法
/ rotate / revoke / lifecycle) は
...
kernel apply と Takosumi Accounts ledger update が成功すると AppInstallation 行は `ready`
に遷移し、次の値が **immutable に pin** されます。
...
CLI が呼ぶ HTTP API は
[Install API リファレンス](https://github.com/tako0614/takos-ecosystem/blob/master/docs/reference/install-api.md)
に 集約されています。

| API                                       | 用途                                               |
| ----------------------------------------- | -------------------------------------------------- |
| `POST /v1/install/preview`                | install preview を取得 (mutate なし)               |
| `POST /v1/installations`                  | Accounts が AppInstallation を作成し pipeline 起動 |
...
takos deploy --manifest .takosumi/manifest.yml --env staging --space SPACE_ID

# legacy repository URL から resolve + apply
takos deploy https://github.com/acme/my-app.git --legacy-repo-source --env staging --space SPACE_ID
...
takos deploy --manifest .takosumi/manifest.yml --resolve-only --space SPACE_ID
# → 出力された deployment id を `takos apply <id>` で適用
```

local deploy では `--manifest <path>` が必須です。positional argument を省略して
`.takosumi/manifest.yml` を自動探索する legacy default はありません。kernel direct
deploy path でも `takosumi deploy <compiled-shape-manifest>` のように explicit
path で compiled Shape manifest を渡してください。URL 経路は legacy
compatibility sugar で、明示的に `--legacy-repo-source` を付けた場合だけその
repository を source にします。
`TAKOS_SPACE_ID` または `.takos-session` で 既定 space が決まっている場合は
`--space` を省略できます。
...
| `--env <name>`             | 反映先環境                                                                                       |
| `--manifest <path>`        | local deploy manifest path。local deploy では必須。kernel direct path でも explicit compiled Shape manifest が必要              |
| `--legacy-repo-source`     | legacy repository source compatibility path を明示的に使う                                                                      |
...
1. takosumi-git authoring path では `.takosumi/manifest.yml` を compile する。kernel direct path では `--manifest`
   で指定した placeholder-free Shape manifest を読み込み、必要な worker bundle artifact input を添える
...
です。SQL / object-store / queue などの resource は app metadata catalog
...
app metadata catalog ではなく、Shape manifest `resources[]` または resource API
...
[Deploy System § GitOps deploy binding](https://github.com/tako0614/takosumi/blob/master/docs/reference/architecture/deploy-system.md)
と
[Binding Catalog](https://github.com/tako0614/takos-ecosystem/blob/master/docs/reference/binding-catalog.md)。
...
- [.takosumi/app.yml spec](https://github.com/tako0614/takosumi-git/blob/master/docs/reference/app-yml-spec.md)
  --- installer-bound manifest の field 定義
- [Binding Catalog](https://github.com/tako0614/takos-ecosystem/blob/master/docs/reference/binding-catalog.md)
  --- 6 種の binding type と materializer env contract
- [Install API](https://github.com/tako0614/takos-ecosystem/blob/master/docs/reference/install-api.md)
  --- `POST /v1/installations` 等の wire shape
- [Deploy System](https://github.com/tako0614/takosumi/blob/master/docs/reference/architecture/deploy-system.md)
  --- 3 種の deploy path と Core record 構造
