...
worker / service / resource / route / publication / consume の primitive declaration を
...
Takos API key / OAuth client は `takos.api-key` / `takos.oauth-client`
built-in provider publication を `consume` して受け取ります。SQL /
object-store / queue などの resource は manifest の `resources`、または
...
3. `takos deploy --space SPACE_ID` で primitive declaration
   を compile し、service / route / publication / consume へ apply する
...
6. 指定した group の deployment record と source provenance を更新する
...
- group deployment record がある primitive は、`takos rollback GROUP_NAME --space SPACE_ID`
  ...
  ではなく deployment record の source metadata から記録済み commit を再解決します。
...
| deployment record | group 指定時に manifest / artifacts を記録              | group 指定時に repository URL / ref / commit / manifest metadata を記録 |
