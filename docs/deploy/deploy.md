...
compile / optional kernel apply を担当します。
...
takosumi-git compiler
 │
 │ 9. workflow artifact と service import を compile
...
installer-only placeholder を解決したふりで kernel に渡さず、compile error
にします。完全な仕様 (placeholder 文法 / rotate / revoke / lifecycle) は
...
kernel apply と ledger update が成功すると AppInstallation 行は `ready`
