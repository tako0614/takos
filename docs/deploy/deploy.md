...
positional argument を省略した Takos compatibility wrapper の legacy default は
`.takosumi/manifest.yml` です。ただしこれは takosumi-git-owned authoring input
であり、kernel direct deploy path では使いません。kernel direct deploy では
`takosumi deploy <compiled-shape-manifest>` のように explicit path で compiled
Shape manifest を渡してください。URL を渡すとその repository を source にします。
...
| `--env <name>`             | 反映先環境                                                                                       |
| `--manifest <path>`        | manifest path。Takos compatibility surface の legacy default は `.takosumi/manifest.yml`。kernel direct path では explicit compiled Shape manifest が必要 |
