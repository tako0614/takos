...
`publish` は他の primitive へ共有する information sharing catalog です。
Takos API key / OAuth client は `takos.api-key` / `takos.oauth-client` system
publication source を `consume` して受け取ります。SQL / object-store / queue などの resource は
...
5. workload / routes の差分を計算し、publication は catalog として同期する。
   Takos system publication consume は validation / state sync する
