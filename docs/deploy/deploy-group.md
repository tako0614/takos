...
> が使える機能、AppInstallation 台帳との関係、`takos deploy` / `takos install`
> との関係。
...
**Installation は group の一種** であり、 `.takosumi/app.yml`
(`kind: InstallableApp`) で declaration された app を Takosumi Account に
install すると、 AppInstallation 台帳が ownership を提供し、 1 つの group
inventory が installation に対応します。 つまり:

- AppInstallation 台帳: 「誰の Takosumi Account に install されたか」 「どの
  Space に紐づくか」 「どの binding が承認されたか」 を持つ ownership ledger
- Group: その installation が apply した primitive の集合と、 deployment history
  / rollback / uninstall の compatibility state scope
- Primitive: group inventory に projection される実 record

詳細は [AppInstallation アーキテクチャ](/architecture/app-installation)
を参照して ください。

なお、 manifest を直接 `takos deploy` する従来の flow では、 AppInstallation
台帳を介さずに group だけで管理する経路もそのまま使えます。 group は
installation を伴わない primitive 集合としても扱えます。

Takos の group は **Deployment record を順序付ける compatibility state scope**
です。Shape resource / route projection は group に所属していても、
...
です。`jobs` のような worker bundle は takosumi-git が `workflowRef` を解決し、
kernel に届く前に concrete artifact hash を materialize します。

## Primitive と group の関係

control plane は Shape resource / route projection / binding report を
...
group + Deployment lifecycle で使う Takos compatibility public API
です。takosumi kernel の public surface ではありません。
