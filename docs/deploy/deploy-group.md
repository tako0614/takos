...
- 構成要素: 0..N 個の primitive (compute, route, publish)
- manifest authoring: `.takos/app.yml` に宣言した primitive 群が自動的に group
  としてまとめられる
...
- inventory: group に属する compute / route / publish の一覧
...
が使われるが、それは operator / backend 側の detail。current public CLI に
...
primitive (compute / route / publish) は 1st-class エンティティです。 group
に所属させずに standalone で作成することも、後から group に所属させる
こともできます。
