...
ボタンを押すと Takosumi Accounts の AppInstallation record が作られます。

詳細:

- InstallableApp の manifest (`.takosumi/app.yml`) と install path の詳細は
...
`entry.manifest` が指す authoring compute manifest (`.takosumi/manifest.yml`
...
- その Release のコミットに `.takosumi/app.yml` と `entry.manifest` が含まれている

`.takosumi/manifest.yml` だけでは owner / binding / permission を確認できない
ため、InstallableApp としては表示されません。Release の attachment はカタログ
...
InstallableApp ではありません。そのため Store package や Seed Repository として
扱いません。
...
- **package install**: Store package のソースを AppInstallation pipeline に渡す
...
`space_id` を付けた場合、レスポンスにはその space での AppInstallation summary
を付与できます。
...
1. リポジトリを public にする
2. installer-bound manifest (`.takosumi/app.yml`) と、そこから参照する authoring
   compute manifest (`.takosumi/manifest.yml`) を追加
3. non-draft / non-prerelease Release を作成

Store catalog service が release commit の `.takosumi/app.yml` と
