...
は添付ファイルとして扱われ、 Store の deployable 判定条件ではありません。Store
catalog service が Git object store (`GIT_OBJECTS`) を読めず `.takosumi/app.yml`
と `entry.manifest` を確認できない 場合、その release は deployable
として扱いません。
