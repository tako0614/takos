...
AppInstallation を **取り出して self-host する** ための bundle 生成。 ユーザの "退出する権利"
を機能として提供する。 ただし「完全に取り出す」 という framing は不正確で、 実態は data の移植と target instance での
identity 再生成。 詳細は §3.0 の honest framing を参照。

### 3.0 Export semantics (honest framing)

Export は「identity を migrate する」 のではなく、 「**data を移植し、 target で新 identity を生成する**」 操作です。
具体的に:

- ✅ Preserved: source commit pin / app manifest digest / compiled manifest digest / data dump (DB / blobs / memory /
  profiles) / binding template (config 構造)
- ⚠ Regenerated at target: OIDC client (新 `client_id` / `client_secret`)、 pairwise subject (salt 持ち越しなしの場合は
  新 issuer base で再計算)、 InstallationEvent ledger (新 chain start)、 launch token issuing context
- ❌ Lost: audit chain across instances (source ledger は source instance 内で frozen、 target は新規 chain)、 source
  instance での access history と target での continuity を link する protocol-level 手段

「主権を保ったまま移動する」 と言うより「主権を **再構築可能な形** で移植する」 が正確です。 user の identity は
source / target instance で別 entity として扱われ、 linked relationship は持ちません。 これは
...
の直接の帰結で、 「export ができる」 ことと「instance 間で identity が継続する」 ことは別の性質です。 federation を
要求する use case では app-layer (例: yurucommu の ActivityPub) で対応します。
