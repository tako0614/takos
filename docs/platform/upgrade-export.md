...
data restore や cross-provider data migration は current guarantee
としては扱わない。
...
`expected.planSnapshotDigest`、さらに deploy dry-run 時点の
...
2. Source resolution   (git / prepared / local input を pin し、汎用 repo metadata から InstallPlan snapshot を作成)
...
policy で扱います。Source input は Git URL / commit / tag / package metadata
などの汎用 repo metadata から resolve されます。

```yaml
# catalog/account-plane policy object; not source input
upgradePolicy:
...
は新しい Deployment を作らず、過去 Deployment の source pin / plan snapshot digest /
...
- reviewed InstallPlan snapshot digest (`planSnapshotDigest`)
...
Deployment pointer を過去の succeeded Deployment に戻す ledger revision
primitive です。runtime data の復元、database restore、provider data namespace
の移行は別の operator / provider evidence で扱います。

### 2.3 制限事項

- provider data copy / schema migration の巻き戻しは rollback の current
  guarantee ではない
- `service-binding` / `object-store` / `http-endpoint` material と activated
  HTTP domain projection の変更は binding-level review と user approval の対象
...
- ✅保持される: source identity pin / `planSnapshotDigest` / retained evidence refs
...
`source.json` / `source/source input` / `binding-template/*.yml` は restore
...
- [PlatformService bindings](https://takosumi.com/docs/reference/core-spec)
