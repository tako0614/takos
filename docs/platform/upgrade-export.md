...
installed service に渡す supporting projection route であり、deploy-control Capsule API ではありません。
...
| billing / OIDC / ServiceGrant 配布 | `/v1/installation-projections/*`                                                       | installed service への supporting projection。Workspace / Source / Capsule / Run の正本ではない。   |
...
3. plan Run: SourceSnapshot + dependency evidence + base StateVersion を pin
...
SourceSnapshot / plan digest / dependency evidence / base StateVersion / Output を pin した reviewed apply が
新しい StateVersion / Output ledger revision を作る唯一の update authority です。Accounts 台帳操作は OIDC client、billing usage
...
binding-level review は ServiceBinding / ServiceGrant / ProviderConnection / ProviderBinding / CredentialRecipe / output allowlist の変更を確認するための
