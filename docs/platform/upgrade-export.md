...
Installation の source revision を新しい ref に進める Accounts 台帳操作。同じ
...
mutable-looking ref は `takosumi` が apply 前に拒否する。

### 1.2 流れ

```txt
1. new ref fetch       (takosumi が source repo から指定 ref を pin)
2. AppSpec parse       (.takosumi.yml を parse し metadata / components / interfaces 抽出)
...
permission diff が **add** を含む場合は、UI が install dry-run と同等の
permission gate を再度通すことを要求する (新 `permissionDigest` の ack
必須)。

### 1.4 channel policy

`.takosumi.yml` の `upgrade.policy`:
...
直前 (または任意の過去 ref) の Installation 状態に戻す操作。
...
指定された Deployment が当該 Installation の Deployment 履歴に
存在しない場合は `422`。
...
- `appManifestDigest`
- Deployment evidence digest / outputs
...
dry-run は binding / grant / cost の差分を示し、必要な approval を要求する。
...
- ✅ 保持される: source commit pin / app manifest digest / Deployment evidence /
  binding template / provider 構成済みの場合のみ data dump (DB / blobs / memory / profiles)
...
(`takosumi.accounts.installation-export-bundle@v1`)。`takosumi import
...
IdP として接続し、Installation の issuer を直接外部 IdP に差し替えない。
...
payload を受け取り、target Accounts instance の Installation として登録する。
current CLI bridge として `takosumi import ./takos-export.tar.zst` は
...
```bash
takosumi import ./takos-export.bundle.json \
  --to https://my-takosumi.example.com \
  --account-id acct_self_host \
  --space-id space_self_host \
  --subject tsub_owner
...
- [Installation 台帳](https://github.com/tako0614/takosumi-cloud/blob/master/docs/architecture/app-installation.md)
