...
プランはユーザーの課金アカウント単位で 1 つ。変更は Stripe Billing Portal または
API から。
...
`/api/billing/webhook` で処理される Stripe event の一覧です。列挙された event
以外は signature 検証後に `200 OK` で ack しますが、state 更新は行いません。
...
| `/api/billing/usage`             | GET      | billing account 全体の当月使用量（メーターごとの units と cost_cents） |
