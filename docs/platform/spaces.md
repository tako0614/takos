...
1. Principal row が存在し、active です。
2. Workspace row が active で、その `owner_account_id` が Principal と一致します。
3. 同じ Principal を指す active owner witness が存在します。
...
Principal または owner witness が無効になった場合も、dispatch と tool bootstrap は安全側に停止します。
...
[課金](/platform/billing) と [Takosumi を自分で動かす](https://takosumi.com/docs/concepts/self-host) を参照してください。

## 関連ドキュメント

- [Takosumi を自分で動かす](https://takosumi.com/docs/concepts/self-host) — 外部 account-plane boundary
- [Capsule の実行モデル](https://takosumi.com/docs/concepts/run-model) — Workspace に追加する Capsule の plan / apply の記録
- [Takosumi の概念](https://takosumi.com/docs/concepts/) — Source / Capsule / Run / StateVersion / Output の全体像
