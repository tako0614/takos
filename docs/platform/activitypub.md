...
Takos 互換の catalog app-label は **ActivityPub** と **ForgeFed** をベースとした
Git リポジトリの分散カタログを実装できます。git
...
catalog app-label はリポジトリカタログとして機能します。git
データの複製は行いません。
...
inventory の管理 surface は catalog app-label ごとに持てます。Takos kernel
自体は inventory の編集 API を標準化しません。
...
する。`target` は更新された Git ref、`afterHash` は更新後 commit hash で必須。
`beforeHash` と commit メタデータは任意。
...
```json
{
  ...
  "beforeHash": "0000000000000000000000000000000000000000",
...
catalog app-label は購読している repo の activity を `Announce`
で再配信してよい。Store actor の follower は、Store actor をフォローすることで
inventory 内の全 repo の活動を受け取れる。
...
Store outbox は inventory 操作を activity として記録する。自動モードの場合は
repo の Create/Update activity を生成する。

- `Add` : 新しい repo 参照が inventory に追加された (明示モード)。`object` は
  canonical repo actor URI 文字列。
- `Remove` : repo 参照が inventory から削除された (明示モード)。`object` は
  canonical repo actor URI 文字列。
...
- Grant は `admin > write > read > visit` の階層として解釈する。たとえば `read`
  Grant は `visit` を満たし、`admin` Grant は全 capability を満たす。
- push 権限は `write` 以上の Grant でなければならない
- Grant は canonical repo に対して発行する
- private repo actor の GET は signed fetch 必須。`?actor=` の actor URL と HTTP
  Signature の `keyId` 由来 actor が一致し、その actor に `visit` 以上の Grant
  がある場合だけ actor document を返す。
...
`/ns/takos` は additive-only に拡張する。既存 term の意味変更や削除はしない。
...
1. 受信 activity の actor なりすましを検証しなければならない (HTTP Signature
   必須、署名なし or 検証失敗で 401)
...
- algorithm: **RSA-SHA256 (RFC 8017)**、Cavage draft
  (`draft-cavage-http-signatures-12`) compatible
- headers: `(request-target) host date digest`
- keyId format: `{actor-url}#main-key`
- signature header **必須**。欠落 or 検証失敗で 401 reject。
- `Digest` header が存在する場合は SHA-256(body) を再計算して照合する。mismatch
  は 400 Bad Request。signing string に digest が含まれていても body
  すり替えは別問題で、rehash が本当の binding を作る。

#### Replay 保護

Cavage §2.1.2 と Mastodon の慣行に合わせて、inbox は以下の replay
保護を追加で行う:

1. **`Date` header skew check** — `Date` が無い、parse できない、現在時刻 ±5
   分を超える場合は 401 reject
2. **Activity id dedup** — verification 後に `body.id` を bounded な in-memory
   set に記録 (worker instance ごと、最大 2,048 件、TTL 20 分)。同じ activity id
   の再 delivery は 200 を返して再処理しない (`{ duplicate: true }`)
3. **Actor public-key cache** — `keyId` で fetch した actor document は **24
   時間** in-memory にキャッシュ (worker instance ごと、最大 512 entries)。同じ
   remote actor からの burst で N 回 `apFetch` が走るのを防ぐ
4. **`Update` cache eviction** — remote actor の `Update` を inbox
   で受信したとき、object.id に対応する cache entry を `evictActorKeyByActorUrl`
   で明示的に 破棄する。これが無いと key rotation 後 24 時間 stale key で verify
   して しまう。

cache はすべて process-local なので CF Worker の cold start で flush
される。クロスインスタンス共有が必要なら routing KV / D1 に promote すること。
...
outbound delivery は `PLATFORM_PRIVATE_KEY` env が設定されている場合に署名付きで
delivery する。未設定時は warning / error ログを残し、remote inbox への POST
は試みない。outbox は公開 GET で引き続き読める。`Repository` actor
(`buildRepoActor`) も自身の `publicKey` を公開するため、他サーバーから signed
delivery を受け取って verify できる。

#### Key rotation runbook

現状、`PLATFORM_PRIVATE_KEY` env は **全 actor 共通の single key**
として使われる (per-actor key は未実装)。keyId は `{actor-url}#main-key`
固定。kernel は `PLATFORM_PRIVATE_KEY` env を 1 本しか読み込まない。rotation は
hard cut-over で行う:
...
3. **その瞬間から署名付きで in-flight の delivery / verification は全て壊れる**
   - 交換前の key で署名済みだが未配送の delivery は remote verifier に reject
     される
   - 交換前の keyId を `keyId` header に持つ受信 activity は 401 で reject
     される

この方式では key rotation の瞬間に互換性が切れる。破壊を最小化するなら、
rotation は traffic の少ない時間帯に実施し、delivery queue を drain してから
swap すること。
...
`import-repository` の request body は inventory / search 結果の `id` を
`canonical_repo_url` として渡す。

```json
{
  "canonical_repo_url": "https://a.example/ap/repos/alice/calc",
  "local_name": "calc"
}
```

import 時、kernel は `canonical_repo_url` が選択した Store の inventory
に存在することを確認してから repository actor を fetch する。inventory
に存在しない任意の actor URL は import できない。

`subscription_enabled` は outbox 差分を取得する対象として登録するフラグであり、
kernel による自動 polling を意味しない。更新取得は `/poll` または catalog app
側の cron が明示的に行う。
