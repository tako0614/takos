...
| ------------------------------------------ | -------- | ------------------------------------- | --------------------------------------------------------------- |
| [takos-docs](/platform/takos-docs)         | `master` | リッチテキストエディタ                | launcher / MCP endpoint / file handler / storage grant          |
| [takos-excel](/platform/takos-excel)       | `master` | スプレッドシート                      | launcher / MCP endpoint / file handler / storage grant          |
| [takos-slide](/platform/takos-slide)       | `master` | プレゼンテーション                    | launcher / MCP endpoint / file handler / storage grant          |
| [takos-computer](/platform/takos-computer) | `master` | sandbox computer / browser automation | launcher / MCP endpoint / sandbox web-service / Takos API grant |
| [yurucommu](/platform/yurucommu)           | `master` | ActivityPub / community social        | launcher / `identity.oidc@v1` / DB / object-store / queue       |

default app の launcher / MCP / file handler entry は kernel manifest の
...
office 系 default apps は **OIDC consumer** です。`identity.oidc@v1` AppBinding
を service identifier `takosumi.account.auth@v1` で解決される Takosumi Accounts
から 受け取り、ユーザー認証を OIDC issuer に委譲します。default app 自身は OAuth
provider ではありません ([OIDC Consumer](/apps/oidc-consumer) /
[binding-catalog](/reference/binding-catalog#_1-identity-oidc-v1) 参照)。

office 系 default apps は root UI、`/mcp`、`/files/:id` を同じ worker / web
...
object-store / queue / secret resources を持つため、office 系 default apps より
resource footprint が大きい。

## Office file contracts

office 系 default apps は Storage の file handler registry に登録され、Storage
...
default app の UI / API は通常 group subdomain で serve されます。Takos browser
session cookie は host-only の `__Host-tp_session` であり、`Domain` attribute を
持たないため、Takos host と group subdomain の間では共有されません。default app
...
2. default app は `identity.oidc@v1` AppBinding で発行された client credential
...
group が他 group のサーバー API や Takos API を呼ぶ場合は service import、 app
