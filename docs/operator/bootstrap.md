...
Takosumi Accounts に集約され、fresh operator は AppInstallation 用に
materialized された `OIDC_*` env を Takos に注入します。

## Prerequisites

- `takos-private/` の target deploy が完了している
- `ADMIN_DOMAIN` が public HTTPS で解決できる
- [OIDC Setup](/operator/oidc-setup) の Google OAuth callback (operator login)
  が登録済み
- AppInstallation の domain が `<TENANT_HOST>` として確定している
- AppInstallation 用の OIDC client (`OIDC_CLIENT_ID` / `OIDC_CLIENT_SECRET` /
...
| `OIDC_ISSUER_URL`       | no      | Takosumi Accounts | `${imports.account-auth.endpoints.oidc-issuer.url}` 経由で anchor resolve された operator-injected issuer URL |
...
経路です。username/password login を使う環境では `/auth/password` が同じ Web
session を作ります。

end user 向けの login (Takos chat への入口) はここでは扱いません。end user は
Takosumi Accounts の resolved OIDC issuer でログインし、Takos 側は
`/auth/oidc/login` + `/auth/oidc/callback` で受けます
...
1. 対象 AppInstallation の OIDC client を発行する (managed deploy では install
   pipeline が自動発行。self-host では選択した issuer 側で手動登録)
