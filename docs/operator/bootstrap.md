...
| `OIDC_ISSUER_URL`       | no      | Takosumi Accounts | `identity.primary.oidc` / OIDC discovery から materialize された operator-selected issuer URL |
...
AppSpec install / Deployment apply は `takosumi` が扱います。Takos product は
Web UI と public API から space / catalog / app-local product API を扱い、OAuth
/ billing は operator account plane (リファレンス実装: Takosumi Accounts) の OIDC / BillingPort を consume します。
