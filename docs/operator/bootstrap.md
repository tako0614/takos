...
で参照される Takosumi Accounts に集約され、fresh operator は AppInstallation 用に
...
1. 対象 AppInstallation の OIDC client を Takosumi Accounts で発行する (managed
   deploy では install pipeline が自動発行。self-host では operator-owned
   Takosumi Accounts に登録し、Keycloak / Authentik 等は upstream IdP として接続)
