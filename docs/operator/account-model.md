...
Takos product routes は OIDC consumer であり、credential issuer / billing owner にはなりません。同一 origin に
compose された embedded Takosumi Accounts plane が OIDC issuer と client projection を所有し、Takos はその subject から
...
| 対象                                 | 管理元                                           |
| ------------------------------------ | ------------------------------------------------ |
| account / credential / upstream IdP  | embedded Takosumi Accounts plane                 |
| billing / Installation ledger        | embedded Takosumi Accounts / deploy-control      |
| OIDC issuer / client registration    | embedded Takosumi Accounts plane                 |
| Takos の app-local profile / session | Takos app                                        |
| dedicated runtime mode / source pin  | Installation + operator-private runtime evidence |

Keycloak / Authentik / Auth0 などを使う場合も、Takos product runtime へ直接 issuer として渡しません。embedded Accounts
plane の upstream IdP として接続し、Takos runtime の `OIDC_ISSUER_URL` は同一 origin Accounts issuer を指します。
...
- embedded Accounts plane の issuer が同一 origin の `/.well-known/openid-configuration` で解決できること
...
public install 導線では、dedicated runtime も最初から Installation 経由で作成します。既に動いている dedicated runtime
を後から台帳に採用する作業は、公開 contract ではなく private operator evidence shaping です。この公開 docs では
手順化しません。
...
- deploy を戻す場合も Takosumi Accounts issuer / Installation ledger は維持する
