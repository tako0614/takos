# Operations: Cost Monitoring and Tenant Attribution

> このページでわかること: Takos operated environments の cloud spend を
> tenant / Space 単位で追跡する dashboard、metric contract、reconciliation
> cadence。

この runbook は Phase E GA readiness の cost monitoring 正本です。Takos の
primary customer surface は Web/API であり、Takos CLI を独立した customer
capacity / cost surface として扱いません。`takosumi` / `takosumi-git` CLI
から発生する deploy traffic は、最終的に Takos Web/API または Takosumi API の
Space-scoped usage として集計します。
...
対象は Takos-operated production / staging environments の tenant-attributed
cost です。
...
2. Takosumi Space attribution labels:
   `takosumi_cost_center`, `takosumi_project_code`,
   `takosumi_customer_segment`.
...
- Top tenants by cloud spend.
- Cloud spend by provider and service.
- Top tenants by billed usage.
...
- Check attribution coverage. Unattributed spend must stay below 2% of daily
  cloud spend.
- Review top 20 tenants by cloud spend and billed usage.
...
- Compare `sum(increase(takos_billing_usage_cost_cents_total[30d]))` with
  Takos app `usage_rollups.cost_cents` for the same period.
...
| Tenant spend spike | tenant spend > 3x its 30-day p95 daily spend | inspect abuse, quota, or noisy workload |
