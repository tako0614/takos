...
この runbook は Takos product の運用正本です。Takosumi kernel の signal
contract は `takosumi/docs/reference/observability-stack.md` と
`takosumi/docs/reference/incident-model.md` を参照し、Takos 側では誰がいつ受け、
どの順序で判断・連絡・復旧するかを固定します。

## Scope

対象は Takos operated environments の `takos-app` / `takos-git` /
`takos-agent` / `takosumi` / default apps です。Self-hosted operator は同じ
SEV vocabulary を使えますが、paging provider、担当者、customer comms は各
operator が所有します。

Takos は基本 Web/API surface として運用します。CLI は operator/debug 補助で
あり、customer-facing primary UX ではありません。Takosumi CLI は manifest
deploy engine の explicit manifest path を扱うだけで、Takos product の
...
SEV-2 以上として開始し、customer data exposure の可能性がある場合は
調査完了まで SEV-1 として扱います。
...
3. Primary が ack しない場合、paging provider は 5 分後に secondary、さらに
   10 分後に incident commander backup / product owner へ escalate する。
...
4. Capture timeline events every material change:
   alert, ack, mitigation attempt, rollback, customer update, recovery signal.
5. Prefer reversible mitigation:
   rollback, traffic shift, feature flag off, queue drain pause, credential
   disablement, dependency failover.
6. Resolve only when monitoring is green for two consecutive observation
   windows and no new customer impact is reported.
...
Primary cannot hand off an active SEV-1 without an explicit synchronous
handoff in the incident channel.

## SEV-1 Staging Simulation

Phase E GA readiness requires at least one SEV-1 simulation in staging. The
simulation is not a production outage and must not page customers.
...
Takos-operated private environments record the evidence in `takos-private`
run logs. Public docs must not contain secret names, provider account ids,
...
Postmortem review happens within 5 business days for SEV-1 and 10 business
days for qualifying SEV-2.
