...
> このページでわかること: Takos operated environments の現 traffic baseline、
> 1 年 growth forecast、headroom 計算、capacity review cadence。

この baseline は Phase E GA readiness 用です。Kernel orchestration の実測値は
[Performance Baseline](/performance/baseline) を参照します。このページでは
Takos product 全体の運用 capacity floor と、telemetry が集まり始めた後の
更新ルールを固定します。
...
As of 2026-05-07, Takos managed production is pre-GA. Customer production traffic
baseline is therefore 0 customer RPS. Capacity planning uses:
...
Do not use single-day spikes as the baseline. Use 30-day p95 peak and record
the largest single-day spike separately as stress evidence.
