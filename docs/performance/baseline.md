...
- プロバイダーアダプター: `SYNTHETIC_PROVIDER_ADAPTER`
  (常時成功、クラウド往復なし)
- トランスポート: kernel-api-bench は `Deno.serve` + 同 isolate からの loopback
  `fetch`
...
`Deno.serve` + 同 isolate からの `fetch` で計測します。 auth / catalog hash
...
- deployment dry-run スループット **3,556 req/sec** (target > 500 req/sec) —
  達成
...
- 30 % POST /v1/installations/{id}/deployments/dry-run → POST
