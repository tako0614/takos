...
- **P-Phase 1.6.1 (partial)**: Materialize/export lifecycle
  ...
  archive writer は covered)。残りは export bundle integrity
  (`takosumi-git
  export` の HTTP worker wiring が installation.json /
  source.json / manifest.compiled.yml / data dump / bindings template を持つ
  bundle を生成し、別 takosumi instance への data restore で同 source commit /
  digest を再現できる)。
