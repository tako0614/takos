...
- **P-Phase 1.6.1 (partial)**: Materialize/export lifecycle
  ...
  operation を idempotent に request する部分と、Accounts bundle/import planner
  が別 issuer への OIDC rewrite を行う部分は covered)。残りは export bundle
  integrity (`takosumi-git export` の tar.zst bundle が installation.json /
  source.json / manifest.compiled.yml / data dump / bindings template を持ち、別
  takosumi instance への import API/CLI で同 source commit / digest
  を再現できる)。
