...
accepts only the documented Takosumi process roles in `takos.io/process-role` labels
and `TAKOSUMI_PROCESS_ROLE` env values, and fails if any documented role is
