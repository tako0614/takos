...
データベースを持つ module の例です。DB の接続情報は module が Output として返し、
apply の成功後に Takosumi の Output から確認できます。
