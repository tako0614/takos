...
の 5 endpoint で、dry-run / apply / rollback は Installation 中心で記録されます。
...
AppSpec は **形 (component graph)** を固定するだけで、provider の materialization は
profile が決定します。同じ AppSpec を Cloudflare profile に当てれば
...
Installation の rollback は `takosumi rollback <installation-id> <deployment-id>`
で permission review / apply を通して実行します。kernel 側の rollback は
