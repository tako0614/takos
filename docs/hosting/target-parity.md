...
| provider provisioning proof     | `cd takosumi && bun run opentofu:live-local-proof` (local deploy-control proof; per-provider provisioning は plan / apply / destroy run + Connection / ProviderBinding / policy で証明)                                                          | no           |
