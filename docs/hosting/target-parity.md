...
| `aws`          | `smoke-only` | schema validation, Terraform/Helm refs, provider fixture proof command, dry-run service smoke    | `helm template` with `values-aws.yaml`, `terraform plan` for aws-staging, provider live proof, service live smoke         |
| `gcp`          | `smoke-only` | schema validation, Terraform/Helm refs, provider fixture proof command, dry-run service smoke    | `helm template` with `values-gcp.yaml`, `terraform plan` for gcp-staging, provider live proof, service live smoke         |
