...
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **Managed (Takosumi Accounts)**         | `operator.identity.oidc` namespace export で resolve される Takosumi Accounts が per-Installation で発行・rotation | Takosumi Accounts が one-shot opaque token を発行し、`/consume` で redeem (token 本体は signing key を持たない)         | `identity.oidc@v1` / `install-launch-token@v1` use edge 経由で env (`ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`) を注入 |
| **Self-host (operator-owned Accounts)** | import 先の Takosumi Accounts が client を登録・rotation。Keycloak / Authentik / Auth0 等は upstream IdP として接続   | operator-owned Takosumi Accounts が opaque token を発行・consume を担う。launch token を使わない構成では env 自体が不要 | `takos-private` の `.secrets/<env>/` から use edge env として注入                                                    |
| **Takos runtime (consumer)**            | (生成しない。consumer として受け取るだけ)                                                                             | (生成しない。受け取った opaque token を Accounts `/consume` で redeem する relying party)                               | use edge ベースで env (`OIDC_*` / `ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`) として消費                               |

::: info OAuth client secret は Takos が発行しない
Takos は OIDC consumer です。OAuth client 自体は **Takosumi Accounts** が
installation ごとに発行し、`identity.oidc@v1` use edge 経由で
`OIDC_CLIENT_SECRET` を Takos runtime に渡します。

Takos オペレーターがやるのは「Takosumi Accounts が発行した値を取り込んで
runtime に配信する」ことだけです。具体的には:
...
| `OIDC_CLIENT_SECRET` | Takosumi Accounts (managed / self-host) | OIDC consumer の client secret。`identity.oidc@v1` use edge 経由 |
...
| name                             | 由来                                 | 用途                                                                                                                                            |
| -------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `ACCOUNTS_BASE_URL`              | `install-launch-token@v1` use edge | Takosumi Accounts service の base URL (例: `https://accounts.example.com`)。`${ACCOUNTS_BASE_URL}${INSTALL_LAUNCH_CONSUME_PATH}` へ redeem 要求 |
| `INSTALL_LAUNCH_INSTALLATION_ID` | `install-launch-token@v1` use edge | この Installation の id (`inst_xxx`)。redeem 要求の identity context                                                                         |
| `INSTALL_LAUNCH_REDIRECT_URI`    | `install-launch-token@v1` use edge | Accounts が token 発行時に bind した redirect URI。redeem 時に完全一致比較                                                                      |
...
`runtimeConfig` 側に置きます (TLS + digest pin で context を運ぶだけで、token の
verify に必要な公開鍵は存在しません)。
...
| ---------------------------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `OIDC_CLIENT_ID`                                                       | Takosumi Accounts                                   | operator-owned Takosumi Accounts                                                  | use edge (`identity.oidc@v1`) 経由で自動更新を受け取るだけ                                                    |
| `OIDC_CLIENT_SECRET`                                                   | Takosumi Accounts                                   | operator-owned Takosumi Accounts                                                  | use edge (`identity.oidc@v1`) 経由で自動更新を受け取るだけ                                                    |
| launch token redeem context (`ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`) | Takosumi Accounts (installation lifecycle で再計算) | operator-owned Takosumi Accounts (launch token を使わない構成では env 自体が無い) | use edge (`install-launch-token@v1`) 経由で context env を受け取るだけ。token 自体は `/consume` で都度 redeem |
| その他 Takos runtime secret                                            | `takos-private` operator                            | `takos-private` operator                                                          | use edge / wrangler secret / Helm `existingSecrets.*` 経由で注入                                              |
...
1. rotation owner (managed / self-host Takosumi Accounts) 側で新
   `OIDC_CLIENT_SECRET` を発行する (launch token は 都度 opaque token
   を発行するため、rotation 対象は OIDC client secret と TLS / digest pin の 1
   signing domain のみ)
...
managed mode では 1 と 2 の間に Takosumi Accounts が use edge
(`identity.oidc@v1` / `install-launch-token@v1`) を更新し、`takos-private`
の取り込み自動化が 新値 (OIDC client secret や launch token redeem context) を
pull する 流れになります。self-host mode では operator が手動で 1 と 2
を繋ぎます。
