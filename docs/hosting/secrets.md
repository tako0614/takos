...
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| **Managed (Takosumi Accounts)**         | `operator.identity.oidc` namespace export で resolve される Takosumi Accounts が per-AppInstallation で発行・rotation | Takosumi Accounts が launch token signing key を発行・rotation し、公開鍵を `INSTALL_LAUNCH_PUBLIC_KEY` として配信 | `identity.oidc@v1` / `install-launch-token@v1` AppBinding 経由で env 注入  |
| **Self-host (operator-owned Accounts)** | import 先の Takosumi Accounts が client を登録・rotation。Keycloak / Authentik / Auth0 等は upstream IdP として接続   | self-host Accounts が launch token signing key を発行・rotation。launch token を使わない構成では env 自体が不要    | `takos-private` の `.secrets/<env>/` から AppBinding 互換の env として注入 |
...
が触るのは consumer 側 secret (`OIDC_CLIENT_ID` / `OIDC_CLIENT_SECRET` / `OIDC_REDIRECT_URI` /
`INSTALL_LAUNCH_PUBLIC_KEY`) の **配置と rotation 反映** だけで、client 自身の発行・revoke は Takosumi Accounts 側の
...
| `INSTALL_LAUNCH_PUBLIC_KEY` | Takosumi Accounts (launch token signing key の公開鍵) | `/_takosumi/launch` で launch token JWS を local verify し、Accounts consume 前に claim を確認する |

`OIDC_ISSUER_URL` / `OIDC_REDIRECT_URI` / `INSTALL_LAUNCH_AUDIENCE` は 非 secret として `wrangler.toml` の `[vars]` や
Helm `runtimeConfig` 側に 置きます。
...
| `INSTALL_LAUNCH_PUBLIC_KEY` | Takosumi Accounts (launch token signing key の rotation 起点) | operator-owned Takosumi Accounts (launch token を使わない構成では env 自体が無い) | AppBinding (`install-launch-token@v1`) 経由で公開鍵を受け取るだけ  |
...
1. rotation owner (managed / self-host Takosumi Accounts) 側で新 `OIDC_CLIENT_SECRET` / launch signing key を発行する
...
を更新し、`takos-private` の取り込み自動化が 新値を pull する流れになります。self-host mode では operator が手動で 1 と
2 を繋ぎます。
...
- [Takosumi Accounts](https://github.com/tako0614/takosumi-cloud/blob/master/docs/architecture/takosumi-accounts.md) ---
  OIDC client / launch token signing key を発行する正本 plane
- [Launch Token](https://github.com/tako0614/takosumi-cloud/blob/master/docs/apps/launch-token.md) ---
  `INSTALL_LAUNCH_PUBLIC_KEY` で検証する one-time JWS の仕様と `/_takosumi/launch` 挙動
