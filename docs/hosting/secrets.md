...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **Managed (Takosumi Accounts)**         | `identity.primary.oidc` platform service で resolve される Takosumi Accounts が per-Installation で発行・rotation  | Takosumi Accounts が one-shot opaque token を発行し、`/consume` で redeem (token 本体は signing key を持たない)         | `identity.oidc@v1` material と Cloud launch-token projection / binding profile 経由で env (`ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`) を注入 |
...
| **Takos runtime (consumer)**            | (生成しない。consumer として受け取るだけ)                                                                           | (生成しない。受け取った opaque token を Accounts `/consume` で redeem する relying party)                               | binding material ベースで env (`OIDC_*` / `ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`) として消費                               |
...
- launch token redeem の context: `ACCOUNTS_BASE_URL` /
...
| name                             | 由来                                       | 用途                                                                                                                                            |
| -------------------------------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `ACCOUNTS_BASE_URL`              | Cloud launch-token projection / binding profile | Takosumi Accounts service の base URL (例: `https://accounts.example.com`)。`${ACCOUNTS_BASE_URL}${INSTALL_LAUNCH_CONSUME_PATH}` へ redeem 要求 |
...
`OIDC_ISSUER_URL` / `OIDC_REDIRECT_URI` / `ACCOUNTS_BASE_URL` /
...
| launch token redeem context (`ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`) | Takosumi Accounts (installation lifecycle で再計算) | operator-owned Takosumi Accounts (launch token を使わない構成では env 自体が無い) | Cloud launch-token projection / binding profile 経由で context env を受け取るだけ。token 自体は `/consume` で都度 redeem |
