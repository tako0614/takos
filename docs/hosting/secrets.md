...
| **Self-host (operator-owned Accounts)** | import 先の Takosumi Accounts が client を登録・rotation。Keycloak / Authentik / Auth0 等は upstream IdP として接続   | self-host Accounts が launch token signing key を発行・rotation。launch token を使わない構成では env 自体が不要    | `takos-private` の `.secrets/<env>/` から AppBinding env として注入 |
