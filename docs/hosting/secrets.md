...
1. `bun run validate:terraform-secrets` が green
2. `bun run terraform:helm-values:check` が sensitive output を Helm values
   に流していない
3. `bun run terraform:plan-gate` の `.terraform-plan/summary.md` は
