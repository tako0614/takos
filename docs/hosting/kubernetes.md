...
::: tip 対象範囲本ページは Helm chart と k8s operator implementation evidence
が提供する範囲を扱います。 :::
...
deploy runtime の lifecycle は operator distribution と selected runtime implementation の
ownership であり、 chart 側に standalone runtime / executor / orchestrator
workload は作りません。

---

## Section 2: k8s operator implementation evidence
...
`k8s-ingress-router` runtime handler は次の lifecycle を扱います:
