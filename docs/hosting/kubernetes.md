...
2. **k8s runtime-agent connector** ― namespace / Deployment / Service
...
::: tip 対象範囲本ページは Helm chart と k8s runtime-agent connector
が提供する範囲を扱います。 :::
...
Service が release source に載ったことを検査します。Takos product の
...
deploy runtime の lifecycle は operator distribution と selected runtime-agent connector の
ownership であり、 chart 側に standalone runtime / executor / orchestrator
workload は作りません。

---

## Section 2: k8s runtime-agent connector

### 構成

Takosumi operator binding profile の k8s connector は次の runtime-agent lifecycle
を提供します:
...
`clients.provider: "k8s-provider-gateway"` を設定すると、operator が選んだ runtime-agent connector が k8s
API server (kubectl proxy / API gateway) 経由で resource lifecycle を実行します。
...
echo "$KUBE_TOKEN" | bun run control:secrets put K8S_API_TOKEN --env production
echo "https://k8s-api.takos.example.com" | bun run control:secrets put K8S_API_SERVER --env production

# CA cert (base64)
kubectl get secret -n takos-system takos-provider-token -o json \
  | jq -r '.data["ca.crt"]' \
  | bun run control:secrets put K8S_API_CA_CERT --env production
```
...
`k8s-ingress-router` connector は次の lifecycle を扱います:
