...
App runtime lifecycle は [Deploy](/deploy/) を参照してください。

::: info deploy manifest author へ deploy manifest author 向けの current surface
は `.takosumi/manifest.yml` / workflow を `takosumi-git` で解決し、Takosumi
kernel に explicit manifest を投下する path です。tracked reference Workers
backend は provider materialization の参照実装で、他環境でも同じ manifest
contract を使います。ただし runtime behavior や resource existence の完全一致は
contract ではありません。 :::
...
を正本とします。このページの target を選ぶには `kernel_host.target` を
`cloudflare` に設定するだけです:
...
Takosumi manifest の `publications` は app metadata / binding graph
の一部ですが、SQL / object-store / queue などの resource を `publications`
から自動作成しません。 resource record / binding は resource API / runtime
binding 側で扱い、Takos product / API gateway 本体の Cloudflare resource（D1 /
...
::: danger production / staging では audit-replication sink が必須 (Phase 18.3
M5)
...
::: info app lifecycle deploy とは別です ここでの deploy は Takos product / API
gateway 自体の deploy です。App runtime lifecycle は `takosumi-git push` または
`takosumi deploy --manifest <path>` を使い、[Deploy](/deploy/)
を参照してください。 :::
...
| 環境変数                                 | 形式       | 用途                                                                                                                        |
| ---------------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------- |
| `TAKOSUMI_DEFAULT_APP_DISTRIBUTION_JSON` | JSON array | `[{ ref, digest, version?, publisher?, kind? }]` を registry descriptor に投入                                              |
| `TAKOSUMI_REGISTRY_TRUST_ROOTS_JSON`     | JSON array | `[{ id, packageRef, packageDigest, packageKind?, trustLevel?, conformanceTier?, verifiedBy? }]` を trusted registry の seed |
...
::: tip self-host での issuer 解決 Takos 自身は OIDC consumer です。 Installable
App Model では self-host でも Takosumi Accounts を運用し、
`takosumi.account.auth@v1` を自前 anchor で resolve した issuer URL を
...
self-host Takosumi Accounts 側で Takos installation 用の OIDC client を作り、
...
1. Takosumi Accounts で Takos installation 用の OIDC client を発行し、
   ...
   `OPENAI_API_KEY` を配置（Phase 11B の `generate:keys:*` で作る platform
   secret に追加する形） `OIDC_ISSUER_URL` / `OIDC_CLIENT_ID` /
...
takos オペレーターが takos
をどのクラウドにホストするかはインストール時の設定で決まる。manifest author は
`.takosumi/manifest.yml` と workflow を書き、`takosumi-git` または explicit
manifest submit を使うだけで、デプロイ先を意識する必要はない:

```bash
# manifest author のコマンド（どの環境でも同じ）
takosumi-git push --env production
takosumi deploy --manifest ./dist/takosumi-manifest.yml --env production
```
...
詳細な runbook と各 cloud の credential / IAM / DNS 設計、provider plugin
profile 設定、runtime-agent placement は [Multi-cloud](/hosting/multi-cloud)
を参照してください。

## 次に読むページ

- [deploy](/deploy/deploy) --- AppInstallation / manifest deploy の詳細
