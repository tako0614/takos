...
shape、dry-run smoke 定義です。実際に Takos product gateway (`takos-worker`) が
Cloudflare Worker として応答していることは、operator が deploy 後に
`distribution:smoke --live` または `takos-private` の real backend smoke
を実行して private evidence に保存する必要があります。 local-substrate の Worker
smoke は Takosumi service / Accounts の Worker path を検証するもので、Takos
product gateway の live Worker proof ではありません。
...
`takos-private/.secrets/<env>` と `bun run secrets:sync:*` /
...
です。このページの target を選ぶには `takosumi_service_host.target` を `cloudflare`
に設定するだけです:

```yaml
# takos-private/distribution.yml
distribution:
  takosumi_service_host:
    target: cloudflare
...
# distribution.yml を編集 (takosumi_service_host.target = cloudflare)
...
operator approval / staged rehearsal が必要です。
...
::: tip トークンのスコープトークンは使うアカウントだけに限定しよう。「All
...
runtime が consume する 5 つの operator/Takosumi runtime secret を生成します:

- `PLATFORM_PRIVATE_KEY` / `PLATFORM_PUBLIC_KEY` ― RSA 2048 PKCS#8 / SPKI PEM
  (Takos product-internal runtime signing key; not a Takosumi public trust
  domain)
...
つ以上含むことを **必須** とします。signal が無い場合は `process exit 1` で
fail-closed し、unencrypted DB に対して serve しません。

Cloudflare D1 backend はプロバイダ側で暗号化されているため、`d1://...` の URL は
自動で `d1-managed-encryption` として認識されます。Postgres backend (RDS / Cloud
SQL / Neon 等) を使う場合は `?sslmode=require` (または `verify-ca` /
`verify-full`) を URL に付与してください。SQLite backend を使う運用の場合は
`sqlcipher://` または `sqlite://...?key=...` を使います。

ローカル開発で encryption signal の無い DB を使う場合は明示的な opt-in
`TAKOS_ALLOW_UNENCRYPTED_DB=1` を渡してください (production / staging では
...
を改竄できる脅威モデルでは、**off-DB の immutable replication sink** が独立した
tamper evidence の独立した記録先となります。Takosumi は production / staging
boot で `TAKOS_AUDIT_REPLICATION_KIND` を要求します:

- `TAKOS_AUDIT_REPLICATION_KIND=s3` ― S3 versioning + Object Lock (COMPLIANCE /
  GOVERNANCE) の bucket に各 audit event を 1 object として
  append。`TAKOS_AUDIT_REPLICATION_S3_BUCKET` 必須、
  `TAKOS_AUDIT_REPLICATION_S3_PREFIX` /
  `TAKOS_AUDIT_REPLICATION_S3_RETENTION_MODE` /
  `TAKOS_AUDIT_REPLICATION_S3_RETENTION_DAYS` で挙動を制御
- `TAKOS_AUDIT_REPLICATION_KIND=stdout` ― append-only log line として stdout に
  出力。test / single-node smoke 用 (production には推奨しない)

boot 時には更に SQL chain と external chain を
`verifyAuditReplicationConsistency` で照合します。hash mismatch / sequence
mismatch を検出した場合は production / staging で fail-closed します。:::

`takos-private/scripts/generate-platform-keys.ts` がこの 5 ファイルを
`takos-private/.secrets/<env>/` に書き出します。既存ファイルが
...
production の `PLATFORM_PRIVATE_KEY` を rotate する場合は事前に rotation runbook
に対象 runtime、token TTL、rollback point、verification command
を記録してください。current runtime が参照しない公開鍵 secret や追加 secret
は作成しないでください。 :::

### Secrets の設定

Takos 本体の private 運用では `takos-private/.secrets/<env>`
...
staging 用の secrets は `takos-private/.secrets/staging/`
...
app registry seeds: <count>
```

この bootstrap は Takos app-local PAT を発行しません。automation 用の long-lived
credential は Takosumi Accounts の account settings / API で発行した
`takpat_...`、または Accounts OIDC flow で得た bearer token を使い、安全な場所
(1Password 等の operator secret store) に保管してください。

### Staging integration test

**staging deploy pipeline integration test** は、上記 4 ステップ (key
generation, control deploy, DB migration, bootstrap) を **dry-run**
...
固定されており、`takos-private/.secrets/staging/` の本物の secret
を **絶対に上書きしません**。
...
| `PROXY_BASE_URL`                               | 同一 `takos-worker` 上の Cloudflare Containers callback base URL      |
...
Takosumi Installation Lifecycle では Takos は **OIDC consumer**
として動きます。OAuth client の発行・管理は **Takosumi Accounts** が
installation ごとに行い、 `OIDC_CLIENT_SECRET` などの値は `identity.oidc@v1`
binding material 経由で Takos runtime に注入されます。Takos worker
側に書かれるのは consumer 側の env だけです。
...
| `TAKOSUMI_ACCOUNTS_INTERNAL_URL`                          | Takosumi Accounts service の internal base URL。`/_takosumi/launch` で受けた opaque launch token を TLS 越しに `/consume` へ redeem |
| `TAKOSUMI_ACCOUNTS_URL`                                   | public account-plane base URL。internal URL が無い self-host 構成で使う |
| `INSTALL_LAUNCH_INSTALLATION_ID`                          | redeem 時に渡す Installation id (`inst_xxx`)                                                                               |
...
| `/_takosumi/launch`   | install 直後の opaque launch token を Accounts `/consume` へ HTTPS で redeem し、初回 owner session を bootstrap する |
...
self-host Takosumi Accounts 側で Takos runtime / bundled app 用の OIDC client
を作り、 `OIDC_CLIENT_ID` / `OIDC_CLIENT_SECRET` / `OIDC_REDIRECT_URI` を
`takos-private` の `.secrets/<env>/` に投入してから `secrets:sync:*` で Worker
secret に流します。 :::
...
を実行して、login flow と Accounts bearer / agent run が staging
環境でも動くか確認できます。`takos-private/scripts/auth-smoke.ts` がそのための
runbook script です。
...
| 2 | Accounts API URL   | apiUrl + Accounts bearer shape validate   | apiUrl 健全性 probe                                                     |
...
1. Takosumi Accounts で Takos runtime / bundled app 用の OIDC client を発行し、
   `takos-private/.secrets/<env>/` に `OIDC_CLIENT_SECRET` /
   `OPENAI_API_KEY` を配置する。`OIDC_ISSUER_URL` / `OIDC_CLIENT_ID` /
   `OIDC_REDIRECT_URI` は `wrangler.toml` の env vars または secret sync
   対象に設定する
2. Takosumi Accounts で automation 用 bearer を発行し、`TAKOS_ACCOUNTS_TOKEN`
   環境変数に export （Accounts bearer check と agent run check で必要）
...
- `[FAIL]` HTTP 5xx / 接続失敗等。exit code 1。runbook を再確認

production に対しても `--env=production` で同じスモークが実行できますが、 agent
run check は OpenAI API の課金対象なので、production では運用 window を
決めて実行してください。
...
Cloudflare を Takosumi service host にしたまま tenant runtime や特定 resource (Postgres /
S3 / Pub/Sub) を別 cloud に切り出すには `distribution.yml` の
`tenant_runtime.targets` に追加 target を並べます:
...
詳細な runbook と各 cloud の credential / IAM / DNS 設計、reference provider
adapter profile 設定、runtime-agent placement は
