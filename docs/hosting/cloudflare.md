...
は tenant runtime の基準 backend で、backend-neutral public spec
の参照実装です。
...
`takos-private/` を管理元とし、`takos-private-*` の worker / resource 名と
`cd ~/Desktop/takos/takos-private/apps/control && deno task deploy:staging` /
...
route publication と Takos capability grant は deploy 時に catalog / grant state
へ同期されますが、operator が管理する Cloudflare resource
の作成とは別の扱いです。
...
`takos-private/apps/control` から実行します:
...
実装では OCI deployment adapter を通る。他環境では ECS / Cloud Run / k8s /
Docker などの backend-specific adapter で解決する。
