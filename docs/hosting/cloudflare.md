...
であり、 Takosumi kernel の特権プロバイダではありません。 Cloudflare
に関する詳細はこのページで扱い、`architecture/` 章では collapsible
節に簡略版を置きます。

source-controlled gate が証明するのは `takos-worker` / Takosumi kernel / Takosumi
...
smoke は Takosumi kernel / Accounts の Worker path を検証するもので、Takos
...
`takos-dispatch` worker (`takos/deploy/cloudflare/wrangler.dispatch.toml`) と
takos-worker (`takos/deploy/cloudflare/wrangler.toml`) で同じ値を設定する
...
Cloudflare を kernel host にしたまま tenant runtime や特定 resource (Postgres /
