...
source-controlled gate が証明するのは `takos-app` / Takosumi kernel / Takosumi
Accounts の Cloudflare distribution profile、wrangler template、 binding
shape、dry-run smoke 定義です。実際に Takos product gateway (`takos-app`) が
...
`takos-private/apps/control/.secrets/<env>` と `deno task secrets:sync:*` /
...
`apps/control/scripts/deploy.mjs` (wrangler) を呼び出します。
...
Accounts の account-plane capability record / binding material から materialize します。
...
この bootstrap は Takos app-local PAT を発行しません。CLI / automation 用の
long-lived credential は Takosumi Accounts の account settings / API で発行した
...
Takosumi Installation Lifecycle では Takos は **OIDC consumer** として動きます。OAuth
client の発行・管理は **Takosumi Accounts** が installation ごとに行い、
`OIDC_CLIENT_SECRET` などの値は `identity.oidc@v1` binding material 経由で Takos
runtime に注入されます。Takos worker 側に書かれるのは consumer 側の env
だけです。

| 変数                                                      | 用途                                                                                                                       |
| --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `OIDC_ISSUER_URL`                                         | OIDC issuer URL。`operator.identity.oidc` external publication / OIDC discovery から得た operator-selected hostname            |
...
::: tip self-host での issuer 解決 Takos 自身は OIDC consumer です。 Takosumi installation lifecycle
App Model では self-host でも Takosumi Accounts を運用し、
`operator.identity.oidc` / OIDC discovery で得た issuer URL を `OIDC_ISSUER_URL`
に注入します。Keycloak / Authentik / Auth0 などは Takosumi Accounts の upstream
IdP として接続します。
