...
Takos product / API gateway の deploy は target に関わらず
`takos-private/distribution.yml` を正本とします。このページの target を選ぶには
`kernel_host.target` を `cloudflare` に設定するだけです:
...
`takos deploy` は deploy manifest の `publications` を catalog
に保存しますが、SQL / object-store / queue などの resource を `publications`
から自動作成しません。 resource record / binding は resource API / runtime
binding 側で扱い、control plane 本体の Cloudflare resource（D1 / R2 / KV /
Dispatch / Queues / Vectorize など）は operator が事前に作成・設定します。
...
::: info group の deploy とは別です ここでの deploy は Takos product / API
gateway 自体の deploy です。Takos 上で動く group の deploy は `takos deploy`
