...
::: info deploy manifest author へ migration window 中に残る deploy manifest author 向けの Takos compatibility surface
は backend-neutral な manifest を書く `takos deploy` です。Installable App Model の primary path は
`POST /v1/installations` / takosumi-git です。tracked reference Workers backend は compatibility spec
を実現する参照実装で、 他環境でも同じ schema / translation surface を使います。ただし runtime behavior や resource
...
`takos deploy` compatibility surface は app metadata catalog から SQL / object-store / queue などの resource
を自動作成しません。 resource record / binding は resource API / runtime binding 側で扱い、control plane 本体の
Cloudflare resource（D1 / R2 / KV / Dispatch / Queues / Vectorize など）は operator が事前に作成・設定します。

legacy route publication は互換 catalog state へ同期されますが、operator が管理する Cloudflare resource
の作成とは別の扱いです。Takos built-in provider publication consume (`takos.api-key`) は retired で、current app
credential は Takosumi Accounts の AppGrant/AppBinding から materialize します。
...
::: danger production / staging では DB at-rest encryption が必須 (Phase 18.3 M7)
...
::: danger production / staging では audit-replication sink が必須 (Phase 18.3 M5)
...
::: info group の deploy とは別です ここでの deploy は Takos product / API gateway 自体の deploy です。Takos 上で動く
group の deploy は `takos deploy` を使い、[Deploy](/deploy/) を参照してください。 :::
...
この PAT は **初回ログイン時のみ表示される** ので、安全な場所 (1Password 等の operator secret store)
に保管してください。再実行時は同じ PAT が `[reused]` として再表示されます (`pat:admin:<email>` secret
に保存済みの場合)。

### Staging integration test

Phase 14 の **staging deploy pipeline integration test** は、上記 4 ステップ (key generation, control deploy, DB
...
| `INSTALL_LAUNCH_PUBLIC_KEY`                               | launch token JWS local verification 用の公開鍵 (`/_takosumi/launch` → Accounts consume)                         |
| `INSTALL_LAUNCH_AUDIENCE`                                 | launch token の audience。v1 は AppInstallation の `appId` に pin され、installation id ではない                |
...
| `/_takosumi/launch`   | install 直後の launch token JWS を local verify し、Accounts consume 成功後に初回 owner session を bootstrap する |
...
が staging 環境でも動くか確認できます。Phase 15 で追加した `takos-private/scripts/auth-smoke.ts` がそのための runbook
script です。
...
1. Takosumi Accounts で Takos runtime / bundled app 用の OIDC client を発行し、
   `takos-private/apps/control/.secrets/<env>/` に `OIDC_CLIENT_SECRET` / `OPENAI_API_KEY` を配置（Phase 11B の
   `generate:keys:*` で作る platform secret に追加する形） `OIDC_ISSUER_URL` / `OIDC_CLIENT_ID` / `OIDC_REDIRECT_URI` は
...
takos オペレーターが takos をどのクラウドにホストするかはインストール時の設定で決まる。migration window 中の deploy
manifest author は compatibility manifest を書いて `takos deploy --group GROUP_NAME`
