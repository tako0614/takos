...
| `rust-agent` | `8082` | Rust 製のエージェント executor container |
...
Takos で `all Rust` と呼ぶ対象はこの executor container の内側です。agent loop、memory、skill selection、local tool は Rust 正本ですが、run queue、remote tool 実体、workspace state は control plane のままです。詳しくは [Agent Runtime](/architecture/agent-runtime) を参照。
