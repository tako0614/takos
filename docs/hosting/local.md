...
| `executor-host` | `8790` | エージェント executor host（agent app 側コンポーネント） |
| `rust-agent` | `8082` | Rust 製のエージェント executor container（agent app） |
| `browser-host` | `8791` | ブラウザ自動化 host（agent app 側コンポーネント） |
| `browser` | `8083` | ブラウザ自動化 container（agent app） |
| `oci-orchestrator` | `9002` | コンテナライフサイクル管理 |
| `postgres` | `15432` | PostgreSQL（D1 互換） |
| `redis` | `16379` | Redis（KV 互換） |
| `minio` | `19000` / `19001` | MinIO（R2 互換） |

`executor-host` は `TAKOS_LOCAL_EXECUTOR_URL` で executor container に forward する。既定値は `.env.local.example` の `http://rust-agent:8080/`。

Rust executor の source of truth は `packages/rust-agent-engine` と `apps/rust-agent`。前者が agent core、後者が control RPC / tool / skill bridge を持つ container app。

Takos で `all Rust` と呼ぶ対象はこの executor container の内側です。agent loop、memory、skill selection、local tool は Rust 正本ですが、remote tool 実体と space state は control plane のままです。run queue は agent app 側の責務です。詳しくは agent-engine リポジトリの docs を参照。
...
```bash
deno task --cwd apps/control dev:local:runtime-host   # テナントランタイム host
deno task --cwd apps/control dev:local:executor-host  # エージェント executor host（agent app）
deno task --cwd apps/control dev:local:browser-host   # ブラウザ自動化 host（agent app）
```
...
```json
{
  ...
  "workspace_id": "your-workspace-id",   // space ID
...
| `workspace_id` | no | デフォルトの space ID |
...
| `TAKOS_WORKSPACE_ID` | デフォルトの space ID |
