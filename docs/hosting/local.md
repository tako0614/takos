...
| `takosumi`    | `8788`  | Installation / Run / Deployment ledger engine |
...
runtime-agent handler 経由で扱います。Installation、typed Runs、Deployment /
OutputSnapshot の ledger 記録は `takosumi`、provider allowlist / credentials / state
backend / 実行は Connection / ProviderBinding / policy、infra provisioning は operator-owned workflow、build は
