...
| `browser-host`     | `8791`            | ブラウザ自動化の control-plane host                      |
...
`executor-host` は `TAKOS_LOCAL_EXECUTOR_URL` で executor container に forward
する。OSS local stack の既定値は `.env.local.example` の `http://rust-agent:8080/`。

private server stack の正本は `takos-private/`
で、`takos-private/.env.server.example`、`takos-private/compose.server.yml`、`takos-private/apps/executor`、`takos-private/apps/browser`
...
deno task --cwd apps/control dev:local:browser-host   # ブラウザ自動化 host
```
...
- Cloudflare-native spec を local compatibility backend
  上で実現するが、Cloudflare backend と完全同一ではない
- provider-native な queue consumer / scheduler / workflow semantics
  は再現しきれない
