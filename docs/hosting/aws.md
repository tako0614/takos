...
import { buildOperatorConnectorRegistry } from "./operator-connectors";

const registry = buildOperatorConnectorRegistry({
  aws: {
    region: "ap-northeast-1",
...
Connector implementation は operator distribution の実装です。OpenTofu が AWS
resource graph / state / credential を管理し、runtime-agent は Takosumi から受けた
lifecycle RPC を operator-owned connector registry に dispatch します。
