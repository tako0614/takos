...
Legal continuity rule: Capsule export, dedicated materialization, or
re-binding may move the Capsule owner role within the Customer's scope, but
the **DPA Customer remains the Takosumi Account holder**. Transferring the DPA
Customer to a different Takosumi Account (e.g., Capsule export to a
...
| Capsule owner                      | Customer's delegated agent for app-local processing decisions; does not change Customer's controller/processor role |
...
does not make the Takos product the OIDC issuer, billing owner, or Capsule
ledger owner.
