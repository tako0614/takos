...
Takosumi
kernel は generic PaaS の AppSpec / Deployment / resource graph / provider materialization surface、takosumi は
installer / Deployment lifecycle であり、Takos の個人データ access
...
> **Usage / billing metadata の分担**: Export API が返す usage metadata は **Takos app-local の
> `app_usage_events` / `app_usage_rollups` mirror** に限定されます。Takosumi Account level の billing account /
> Stripe customer / subscription / invoice は **operator account plane / BillingPort** が管理しており、
