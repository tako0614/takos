...
Takosumi は OpenTofu-native deploy control plane として Capsule / `plan` type Run /
`apply` type Run / StateVersion / Output の run ledger を記録し、infra
...
**Region 制約**: Export API は Capsule の **residency profile** を尊重し、
...
として記録され、export bundle は Capsule の primary region
