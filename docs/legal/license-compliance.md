...
| `takos-id/`                  | `UNLICENSED`    | private identity/OIDC package metadata; no OSS `LICENSE`                         |
...
| `takosumi-hosted/`           | `UNLICENSED`    | plan entitlement and Takoserver sponsorship policy; no OSS `LICENSE`             |
...
- lockfiles with root package license metadata do not contradict package
  metadata
- `takos-id/` and `takosumi-hosted/` stay `UNLICENSED` and do not publish an OSS `LICENSE`
