...
本 table は kernel-bound manifest と account-plane AppBinding
を分けて評価します。
...
| cross-instance imports                       | yes                      | no                  | `imports[]` + `serviceResolvers[]`                |
...
durable-object などの access は、current kernel-bound manifest では Shape
