...
| ----------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------ |
| `takosumi.account.auth@v1`    | OIDC issuer / install launch / pairwise subject | `oidc-issuer` / `authorize` / `token` / `jwks` / `userinfo` / `revoke` / `introspect` / `install-launch` | `pairwiseSubjectMode: true`    |
| `takosumi.account.billing@v1` | Stripe checkout / Stripe webhook                | `stripe-checkout` / `stripe-webhook`                                                                     | `stripeBillingSupported: true` |
| `takosumi.dashboard.web@v1`   | install dashboard / web UI                      | `installations`                                                                                          | —                              |
| `takosumi.platform.deploy@v1` | install preview / AppInstallation API           | `install-preview` / `installations-api` / `launch-token`                                                 | —                              |
| `takosumi.platform.anchor@v1` | service registry anchor                         | `service-descriptor-anchor`                                                                              | —                              |
...
services:
  ...
  - id: takosumi.platform.anchor
    ...
    endpoints:
      - role: service-descriptor-anchor
  ...
  - shape: web-service@v1
    ...
    spec:
      ...
      env:
        ...
        BILLING_CHECKOUT_URL: ${imports.account-billing.endpoints.stripe-checkout.url}
```
...
される `AppBinding` (`identity.oidc@v1` / `install-launch-token@v1` / database /
object-store / domain / deploy-intent) と billing service import は、
...
AppInstallation.serviceImports[]
  └── account-billing: takosumi.account.billing@v1
        checkoutUrl       ←──── ${imports.account-billing.endpoints.stripe-checkout.url}
        stripeWebhookUrl  ←──── ${imports.account-billing.endpoints.stripe-webhook.url}
```
...
cross-instance binding は **resolution layer** で挿入される拡張で、
AppInstallation contract には影響しません。
