...
Container host RPC は runtime-service container と PaaS control plane の間を仲介
する **agent-control RPC + Proxy** layer。process role としては runtime host
(`takos-runtime-service` container を扱う) と executor host (agent run の
container を扱う) の 2 種類があり、どちらも Deployment lifecycle の provider
operation (apply 側) で起動する container と control plane の
...
- [Runtime Service](/architecture/runtime-service) — runtime host 内で 動く Deno
  HTTP server
- [Control plane](/architecture/control-plane) — kernel 側の queue / cron 全体図
