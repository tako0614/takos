...
**明示** に consume する。 Takos API key / OAuth client は `publications[]`
ではなく `takos.api-key` / `takos.oauth-client` built-in provider publication
として `bindings[]` で consume する。
...
Takos API key / OAuth client は built-in provider publication として
...
kernel features (Agent / Chat, Git, Store, Auth) は publication ではなく
...
built-in provider publication として扱う。 `bindings[]` で request する。
