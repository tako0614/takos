...
routing management を担う。旧 publication の space-wide fanout や legacy
`storage` manifest は internal history としてのみ扱う。
...
publication に TTL はない。group が存在する限り publication も存在する。 必須
field は kind によって異なる。

- route publication: `type` + `path`
- provider publication: `name` + `provider` + `kind` + `spec`

route publication は URL を持つ。 provider publication の `spec` は kind ごとの
required / optional field を持つ。 kernel features (Agent / Chat, Git, Storage,
Store, Auth) は kernel API として直接提供されるため、publication の対象外。
...
2. kernel が space template を読み、default groups (computer, docs, excel,
   slide) を deploy する
3. deploy 時に kernel が各 group の `publish` / `compute.<name>.consume`
...
2. publications を削除（deploy DB から除去。legacy `storage` 由来の記録は
   internal history のみ）
...
hostname から叩くと同じ maintenance job を実行できる (`/internal/scheduled`
endpoint)。
