# Tenant Runtime

Tenant runtime は deploy された group の workload が実際の HTTP request
を処理する面。

## Canonical artifact

tenant の canonical artifact は `worker-bundle`。

- Cloudflare: Workers backend 上で実行
- local / self-host: Workers-compatible な local adapter 上で実行
- AWS / GCP / k8s: runtime-host compatibility layer で実行

`container-image` は OCI 系 backend 用の artifact であり、tenant の canonical
worker path そのものではない。

## Container runtime

`compute.<name>` の `kind="service"` (image-only) と
`compute.<name>.containers.<sub>` の `kind="attached-container"` はどちらも
`container-image` artifact を持つ container-service workload だが、group の中
での役割が異なる。

- service (`compute.<name>` with `image`): standalone な long-running HTTP
  service
- attached container (`compute.<name>.containers.<sub>`): worker に紐づく
  attached container workload

共通する runtime contract:

- routing は dispatch が `service-ref` または `http-url` target として到達する
- local backend では oci-orchestrator が Docker Engine API 経由で lifecycle
  を管理する
- Cloudflare backend でも current 実装では OCI deployment adapter を使う
- AWS / GCP / k8s では provider-aware adapter (`ecs`, `cloud-run`, `k8s`)
  に解決される
- `image` は digest pin (`@sha256:...`) 必須。rollback は deployment snapshot に
  保存された image ref と execution context を再適用する

container runtime は v1 では HTTP routable な service に限定される。

## Attached containers

`containers` は worker に紐づく image-backed workload。manifest 上は
`compute.<name>.containers` で参照する。

```yaml
compute:
  browser-host:
    build:
      fromWorkflow:
        path: .takos/workflows/deploy.yml
        job: build-browser-host
        artifact: browser-host
        artifactPath: dist/browser-host.js
    containers:
      browser:
        image: ghcr.io/example/browser-service@sha256:...
        port: 8080
```

worker 側には attached container binding が注入される。current 実装では
DurableObjectNamespace-compatible な namespace binding として expose
されるが、これは current runtime detail であり、Cloudflare product surface と 1
対 1 に固定された契約ではない。

## Dispatch の役割

tenant request は直接 bundle に届くのではなく、dispatch を経由する。

dispatch は次を行う。

- hostname から group を特定する
- hostname で kernel or group に振り分ける
  - `{KERNEL_DOMAIN}` → kernel（`/api/*`, `/auth/*`, `/settings`）
  - `{space-slug}-{group-slug}.{TENANT_BASE_DOMAIN}` (auto) / custom slug /
    custom domain → group の worker
- group 内では `service-ref` または `http-url` に request を振り分ける
- tenant request に内部ヘッダを付与する
- control plane と tenant runtime の境界を固定する

## Cloudflare backend

Cloudflare では `worker-bundle` を Workers backend に載せ、tenant runtime
として実行する。

- deploy backend は Cloudflare provider
- worker artifact は native
- image-backed service / container workload は OCI deployment adapter 経由
- tenant routing は dispatch と deployment routing contract に従う
- health / deploy / snapshot-based rollback は Cloudflare 側の backend
  能力も使って成立する

## Local backend

local でも tenant の canonical artifact は `worker-bundle` のまま。 local は
Cloudflare account なしで tenant worker contract を検証するための backend。

- control plane は Node-backed
- tenant runtime は Workers-compatible adapter
- local でも `worker-bundle` を materialize して実行する
- image-backed workload は local OCI deployment adapter で解決する

## Provider-backed compatibility backends

AWS / GCP / k8s では public spec は変わらないが、runtime topology は
provider-aware になる。

- worker workload: runtime-host compatibility path
- image-backed workload: `ecs` / `cloud-run` / `k8s` など provider-backed
  container service
- routing: Takos-managed hostname routing + provider ingress

## Routing contract

tenant runtime が受ける target は dispatch が RoutingRecord から解決した結果。

- `service-ref` (`routeRef`): dispatch namespace 内の worker 参照。RoutingRecord
  の deployments[].routeRef と同義
- `http-url`: 外部 backend や container endpoint のための URL。RoutingRecord の
  endpoints[].target.baseUrl と同義

dispatch が canary weight の解決と path routing を行った後、 tenant runtime
は単一の target を受け取る。

## Snapshot-based execution

deployment は実行 contract を deployment ごとの snapshot として持つ。

- runtime config
- bindings
- env vars

この snapshot により、local と Cloudflare は同じ deployment 入力をもとに tenant
runtime を再現する。

## Compatibility

Takos は Cloudflare を基準に tenant contract をできるだけ揃えるが、backend
は同一ではない。詳細は [互換性と制限](./compatibility.md) を参照。
