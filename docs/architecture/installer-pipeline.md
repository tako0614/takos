...
最終的に takosumi kernel の `POST /v1/deployments` に投下する。本ページは その
...
- compiled manifest への binding 注入は **installer 内部** で行い、 workflow
  からは触れない。
- runtime secret (OIDC client secret / DB password) は kernel への
  `Deployment.spec.env` として渡るが、build log には出ない (secret ref → 値の
  resolution は kernel apply 直前)。
