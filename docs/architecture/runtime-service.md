...
4. Rust wrapperはbounded model/tool loopだけを実行します。tool callはすべてWorkerへ戻し、permission、schema、
   idempotency、timeout、result sizeをWorkerが検証します。各nodeのcheckpointはagent-control RPC経由でRunへlease-fenced保存します。
...
- crash / stale-lease recoveryはWorker-ownedのlease-fenced engine checkpointからidempotent nodeをresumeします。toolのside effectは
  operation ledgerでdedupe / `uncertain` fenceします。provider-neutralなexactly-onceを保証できない`run_model`中断点は自動再発行せず
  Runをfail closedにします。
...
- productionでは短命なAI Gateway credentialがdefaultです。deployment-global `OPENAI_API_KEY`をagent containerへ渡す経路は
  ...
  として有効になります。
- terminal transcriptのlarge messageと1 MiBを超えるengine checkpointはobject storageへstageします。commit-ambiguous時に参照中objectを
  消さないことを優先するため、未参照stage objectやsuperseded leaseのcheckpoint objectの回収はbucket lifecycle policyに依存します。
