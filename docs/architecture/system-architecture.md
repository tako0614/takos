...
Takos は AI software creation product です。Takosumi kernel は generic PaaS
kernel、Takosumi Accounts は account / billing / AppInstallation ledger、
takosumi-git は Git URL install と manifest compile を担当します。
...
- identity / OIDC issuer / billing / AppInstallation ownership は Takosumi Accounts
  が持つ
- deploy / runtime lifecycle は Takosumi kernel が持つ
- Git URL install、`.takosumi/` project convention、workflow は takosumi-git が持つ
- Takos product は UI、public API、AI agent workflow、bundled app experience を持つ
