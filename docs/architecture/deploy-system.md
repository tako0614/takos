...
- `.takosumi/manifest.yml` は `workflowRef` や `${artifacts.*}` /
  ...
  unresolved installer-only placeholder は compile error になる。kernel-owned
  `${ref:...}` / `${secret-ref:...}` と service import placeholder は deploy
  route が解決する (詳細は [.takosumi/app.yml spec](/reference/app-yml-spec) と
...
- kernel が apply するのは **compiled manifest** です。image digest / OIDC
  client / AppInstallation 値など installer-only の値は unresolved なら compile
  error になります。 resource 間参照 (`${ref:...}` / `${secret-ref:...}`) と
  service import placeholder は kernel deploy route の責務として残り得ます。
...
unresolved `${bindings.*}` / `${secrets.*}` を kernel に渡さず、compile error
にします。詳しくは [Binding Catalog](/reference/binding-catalog) を参照。
