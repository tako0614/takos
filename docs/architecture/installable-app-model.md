...
- ecosystem の 5 entity (Takosumi Accounts / takosumi kernel / takosumi-cloud /
  takosumi-git / Takos) の責務分離
...
ecosystem は 5 つの独立 entity から構成されます。各 entity は持たないものを
明確に持たないことで、後工程の混入を防ぎます。
...
- 既存 [System Architecture](./system-architecture.md) は Takos service set の
  repository boundary を扱います。本ページの 5 entity 責務分離と整合します。
