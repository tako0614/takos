...
`publications` で `type: publication.http-endpoint@v1` を宣言する。`publication.http-endpoint@v1`
catalog は deploy manifest の `publications` entry で管理します。
...
を含まない `publication.http-endpoint@v1` publication は storage の handler catalog には出ない。current
...
`publications` に複数の `publication.http-endpoint@v1` entry を並べればよい。同一 manifest
内で publication 名は一意である必要があります。

## フィールド

route publication は core では generic object です。`publication.http-endpoint@v1`
を解釈する platform / app は `spec` 内で以下の field を使います。

| field             | required    | 説明                                                                               |
| ----------------- | ----------- | ---------------------------------------------------------------------------------- |
| `type`            | yes         | standard type 名 (`publication.http-endpoint@v1`)。canonical ref は [Publication types](/reference/glossary#publication-types) 参照 |
...
kernel は多くの `type` の意味を解釈しない。`publication.http-endpoint@v1` は platform /
app が解釈する standard route publication type です。`publication.http-endpoint@v1`
を理解するのは利用する側（agent runtime や storage UI 等）の役割。`publications` は
...
しません。control plane API で `publication.http-endpoint@v1` route publication を直接
作る運用は推奨しません。
