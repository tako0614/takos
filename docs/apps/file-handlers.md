...
特定の MIME type や拡張子のファイルをアプリで開けるようにするには `publish` で
`type: FileHandler` を宣言する。
...
ユーザーがファイルを開くと `path` にリダイレクトされる。`:id` がファイル ID
に置換される。
...
publication は generic object であり固定の schema はないが、`FileHandler`
では以下の field を使う。
...
kernel は `type` の意味を解釈しない。 `FileHandler`
を理解するのは利用する側（takos-agent 等）の役割。

他の app が FileHandler を使うには、その publication を `consume` するか
`/api/publications` から discovery します。kernel は未参照の publication
を自動で space 全体へ inject しません。
