...
launch token の詳細は
[Launch Token](https://github.com/tako0614/takosumi-cloud/blob/main/docs/apps/launch-token.md)、
...
issuer URL は Cloud workload external publication / OIDC discovery で得た
...
plane) が `operator.identity.oidc` を operator-owned external publication として
offer し、AppSpec の `listen.oidc.from: operator.identity.oidc`
...
| `ACCOUNTS_BASE_URL`              | optional | Takosumi Accounts service の base URL。`/_takosumi/launch` で受けた opaque token を HTTPS の `/consume` endpoint で redeem する relying party context (詳細→ [Launch Token (opaque + /consume)](https://github.com/tako0614/takosumi-cloud/blob/main/docs/apps/launch-token.md)) |
...
| Env 種類                                                             | 参照先                                                                                                                    |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| Database (`DATABASE_URL`) / Object Store (`OBJECT_STORE_*`) / Domain | [AppSpec publish/listen](https://takosumi.com/docs/reference/app-spec)                     |
| Installation identifier (`TAKOS_INSTALLATION_ID`, `BASE_URL`)        | [Environment 変数](/deploy/environment)                                                                                   |
| Account-plane deploy automation                                      | [Takosumi Cloud deploy facade](https://github.com/tako0614/takos-ecosystem/blob/main/takosumi-cloud/docs/spec.md#deploy-facade) |
| Launch Token (`ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`)              | [Launch Token (opaque + /consume)](https://github.com/tako0614/takos-ecosystem/blob/main/takosumi-cloud/docs/apps/launch-token.md) |
...
- [Install Paths](/apps/install-paths) — Use / Install from Git / Self-host の 3
  経路
- [.takosumi.yml spec](https://takosumi.com/docs/reference/app-spec)
  — `listen.oidc.from: operator.identity.oidc` の宣言
- [Takosumi Accounts](https://github.com/tako0614/takosumi-cloud/blob/main/docs/architecture/takosumi-accounts.md)
  — issuer 側の責務
