...
`identity.oidc@v1` AppBinding が compile 時に上記 env を解決します (`new.md`
§5.1, §6 を参照)。`OIDC_CLIENT_SECRET` は Takosumi Accounts
が発行し、takosumi-git / Takosumi Accounts が provider secret reference として
materialize してから kernel に渡します。kernel-bound manifest に `${secrets.*}`
placeholder は残しません。
