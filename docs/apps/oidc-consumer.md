...
Accounts の upstream IdP として broker し、Takos runtime が Installation
...
ここでの managed は operator-opened / rehearsal path を指します。public managed
offering は `takosumi-cloud` の launch-readiness evidence、`acceptedReady: true` topology reports、saved live audit、
operator approval が揃い、`managed-offering:status` が `canOpenManagedOffering: true` を返すまで closed です。
:::

managed (Use Takos / Install from Git) では、 takosumi-cloud (operator account plane)
が `operator.identity.oidc` namespace を provider として publish し、 AppSpec の
`listen: { operator.identity.oidc: { as: env } }` 宣言を経由して per-Installation OIDC
client (`OIDC_ISSUER_URL` / `OIDC_CLIENT_ID` / `OIDC_REDIRECT_URI` /
`OIDC_CLIENT_SECRET`) が consumer worker の env に注入されます。 deploy request build
後も未解決なら kernel request 前に失敗します。 `OIDC_CLIENT_SECRET` は Takosumi
Accounts が発行し、 Deployment build にはそのまま materialized value として渡します。
...
| Env 種類                                                             | 参照先                                                                                                                                                                                                                                                                              |
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Database (`DATABASE_URL`) / Object Store (`OBJECT_STORE_*`) / Domain | [AppSpec namespace pub/sub](https://github.com/tako0614/takosumi/blob/master/docs/reference/app-spec.md)                                                                                                                                                                           |
| Installation identifier (`TAKOS_INSTALLATION_ID`, `BASE_URL`)        | [Environment 変数](/deploy/environment)                                                                                                                                                                                                                                             |
| GitOps Deploy (`DEPLOY_INTENT_*`)                                    | [AppSpec namespace pub/sub (deploy-intent namespace)](https://github.com/tako0614/takosumi/blob/master/docs/reference/app-spec.md)                                                                                                                                                 |
| Launch Token (`ACCOUNTS_BASE_URL` / `INSTALL_LAUNCH_*`)              | [Launch Token (opaque + /consume)](https://github.com/tako0614/takosumi-cloud/blob/master/docs/apps/launch-token.md) / [AppSpec namespace pub/sub (install-launch-token namespace)](https://github.com/tako0614/takosumi/blob/master/docs/reference/app-spec.md) |
...
を持ちます。標準 OIDC endpoint と Installation-owned client の発行は
...
参照して Installation ledger を迂回する構成は採用しません。
...
- [.takosumi.yml spec](https://github.com/tako0614/takosumi/blob/master/docs/reference/app-spec.md)
  — `listen: { operator.identity.oidc: { as: env } }` の宣言
