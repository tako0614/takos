...
親 Worker / Service を `routes[].target` と `publish[].publisher` にして、親から
attached container を呼び出します。
...
この意味をより明示するため、将来は互換を保ったまま `inject.env` alias map を
追加する可能性があります。current contract では `consume.env` を使います。
