...
`publish[].spec` / `publications[].spec` に入れます。`publish` と
`publications` を同時に書く manifest は invalid です。
...
します。`UiSurface` publication が `display.icon` を持たない場合、アプリ一覧 /
...
Attached container も Worker / Service と同じ `consume` contract に参加できます。
ただし public route publication の publisher にはしません。外部に公開する場合は、
親 Worker / Service を `routes[].target` にし、publication は
`outputs.*.routeRef` でその route を参照します。親から attached container を
呼び出します。
...
provider が公開する publication（例: `takos.api-key`, `takos.oauth-client`）です。
`as` はその compute 内での local consume 名で、stored consume record の識別に
使われます。Takos built-in provider publication では default env 名にも反映されます。
`request` は provider publication へ渡す要求です。
`inject.env` は output 名 -> env 名 の explicit inject map です。明示した output
だけが注入されます。全 outputs を default env 名で注入したい場合だけ
`inject.defaults: true` を指定します。

`consume.env` は legacy shorthand として受け付けますが、docs では
`inject.env` を推奨します。legacy `env` も現在は `inject.env` と同じく明示 output
だけを注入します。
...
`routes[].id` を参照するため、公開したい endpoint は stable な `id` を持つ
1 つの route にまとめて必要な method を `methods` に列挙します。
...
`outputs.url` です。legacy 互換として `publisher + outputs.*.route` も受け付けます。
`type` は custom string ですが、Takos 標準 type は
`takos.mcp-server.v1` / `takos.file-handler.v1` / `takos.ui-surface.v1`
のように namespaced にします。短縮名 `McpServer` / `FileHandler` /
`UiSurface` は alias として受け付けます。

`outputs.*.kind` は output の型です。manifest route publication の route-backed
output は `kind: url` です。Takos built-in provider は `secret` / `string` / `url`
を内部 catalog で定義します。`spec` は consumer-facing metadata、`auth` は
platform-managed behavior です。

`UiSurface` は Takos の app launcher / アプリ一覧でも使われます。`spec` には
任意で `launcher` などの behavior metadata を置けます。表示 metadata は
...
`compute.<name>.icon` は画像 URL または publisher root-relative path として扱われます。
`spec.launcher: false` を指定した `UiSurface` は launcher には表示されません。

各 `outputs.*.routeRef` は `routes[].id` の 1 件に一致する必要があります。
manifest 全体で同じ route target/path を複数 publication が公開する状態は invalid
です。route output の値は assigned hostname と参照先 route の `path` から
...
Takos 自体が公開する情報は `publish[]` に書く特別な裏口ではなく、Takos
built-in provider が公開する publication として `consume[]` から参照します。
未知の `request` field は deploy validation で invalid です。SQL /
object-store / queue などの resource type はここには入りません。

| publication            | required request fields  | outputs                              |
| ---------------------- | ------------------------ | ------------------------------------ |
| `takos.api-key`        | `scopes`                 | `endpoint` (`url`), `apiKey` (`secret`) |
| `takos.oauth-client`   | `redirectUris`, `scopes` | `clientId` (`string`), `clientSecret` (`secret`), `issuer` (`url`) |
