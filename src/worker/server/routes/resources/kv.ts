...
async function loadKvResource(
  ...
  const backingResourceId = resource.backing_resource_id;
  if (!backingResourceId) {
    throw new BadRequestError("key-value store not provisioned");
  }

  return { ...resource, backing_resource_id: backingResourceId };
}
