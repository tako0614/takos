...
const resourcesBindings = new Hono<AuthenticatedRouteEnv>()
  .post("/:id/bind", async (c) => {
    ...
    const hasAccess = resource.owner_id === user.id ||
      await checkResourceAccess(c.env.DB, resourceId, user.id, [
        "write",
        "admin",
      ]);
    ...
    const service = await db.select().from(services).where(
      and(
        ...
        eq(services.accountId, principalId),
      ),
    ).get();
    ...
    const bindingType = bindingTypeForResource(resource.type);
...
function bindingTypeForResource(type: string): string {
  switch (type) {
    case "worker":
      return "service";
    case "sql":
