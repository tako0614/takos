...
import { deleteManagedCustomHostname } from "../../../application/services/platform/custom-domains.ts";
import {
  deleteService,
  type ServiceRow,
...
async function cleanupServiceArtifacts(
  ...
  if (!wfp) {
    logWarn(
      "Skipping WFP artifact cleanup because Cloudflare provider adapter is not configured",
      {
        module: "routes/services/base",
