...
import type {
  ...
  DeploymentStatus,
  RoutingStatus,
} from "../../../application/services/deployment/models.ts";
...
} from "../../../application/services/deployment/store.ts";
...
const workersDeployments = new Hono<AuthenticatedRouteEnv>()
  .post(
    ...
    async (c) => {
      ...
      // Helper: run the pipeline inline with an AbortController registered so
      // the cancel route can reach it. The registry is per-isolate; queue-
      // dispatched deploys run in a different isolate and are managed by the
      // queue handler's own registration.
      const runInlineWithCancellation = async (): Promise<void> => {
        const controller = registerDeploymentController(deployment.id);
        try {
          await deploymentService.executeDeployment(
            deployment.id,
            controller.signal,
          );
        } finally {
          unregisterDeploymentController(deployment.id);
        }
      };
      ...
      if (c.env.DEPLOY_QUEUE) {
        try {
          ...
          logWarn(
            "message queue enqueue failed, falling back to inline execution",
            {
              module: "deployment",
              ...{
                deploymentId: deployment.id,
          ...
          if (c.executionCtx) {
            c.executionCtx.waitUntil(runInlineWithCancellation());
          } else {
            await runInlineWithCancellation();
          }
        }
      } else {
   ...
   *
   * Distinct from the canary `abort` route above (which rolls back a
   * completed canary): this route triggers the AbortSignal on a deployment
   * that is still progressing through the pipeline.
   *
   * Per-provider cancellation behavior:
   * - Phase boundaries (lock / preflight / prepare / pre-commit) honor the
   *   signal immediately.
   * - The readiness probe honors the signal via its `externalSignal` option.
   * - The route writes a DB-backed cancellation flag, so queue-dispatched
   *   and cross-isolate deployments stop at the next phase boundary they
   *   poll.
   * - HTTP orchestrator provider deploy calls also propagate the signal into
   *   fetch when the in-process controller is reachable. Cloudflare WFP
   *   deploy calls are pre-abort aware but do not propagate mid-call abort
   *   yet, so an in-flight WFP deploy may continue until the provider
   *   returns or its own timeout fires. The pipeline then aborts at the next
   *   phase boundary.
   * - Once routing commits, the signal is no longer honored — completing
   *   cleanup is safer than backing out a committed routing change.
   * - The controller registry is still in-process; when present, it gives
   *   same-isolate deploys an immediate abort signal in addition to the DB
   *   flag.
   *
   * The route returns 200 once the cancellation flag is recorded, and 409 if
   * the deployment has already reached a terminal state.
   */
  .post("/:id/deployments/:deploymentId/cancel", async (c) => {
    const user = c.get("user");
    const serviceId = c.req.param("id");
    const deploymentId = c.req.param("deploymentId");
    ...
    const deployment = await getDeploymentById(c.env.DB, deploymentId);
    if (!deployment || deployment.service_id !== worker.id) {
      throw new NotFoundError("Deployment");
    }
    if (
      ...
      return c.json({
        ...
        cancelled: false,
        status: deployment.status,
        message: "Deployment is already in a terminal state",
      }, 409);
    }

    await requestDeploymentCancellation(c.env.DB, deploymentId);
    const triggered = cancelDeployment(
      deploymentId,
      `cancelled by user ${user.id}`,
    );

    return c.json({
      deployment_id: deploymentId,
      cancelled: true,
      in_process_signal_triggered: triggered,
      status: deployment.status,
    });
  });
