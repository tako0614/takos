...
export default new Hono<{ Bindings: Env; Variables: { user: User } }>()
  ...
  .get("/", async (c) => {
    ...
    const results = await listShortcuts(
      c.env.DB,
      user.id,
      spaceId,
    );
...
>()
  ...
  .get("/spaces/:spaceId/shortcuts/groups", async (c) => {
    ...
    const groups = await listShortcutGroups(
      c.env.DB,
      access.space.id,
    );
  ...
  .post(
    ...
    async (c) => {
      ...
      const group = await createShortcutGroup(
        c.env.DB,
        access.space.id,
        {
          name: body.name,
  ...
  .get("/spaces/:spaceId/shortcuts/groups/:groupId", async (c) => {
    ...
    const group = await getShortcutGroup(
      c.env.DB,
      access.space.id,
      groupId,
    );
  ...
  .patch(
    ...
    async (c) => {
      ...
      const group = await updateShortcutGroup(
        c.env.DB,
        access.space.id,
        groupId,
        {
  ...
  .post(
    ...
    async (c) => {
      ...
      const item = await addItemToGroup(
        c.env.DB,
        access.space.id,
        groupId,
