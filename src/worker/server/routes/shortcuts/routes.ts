...
async function getSpaceId(c: ShortcutContext): Promise<string> {
  ...
  if (spaceIdentifier) {
    ...
    const access = await shortcutsRouteDeps.requireSpaceAccess(
      c,
      spaceIdentifier,
      user.id,
    );
...
export default new Hono<{ Bindings: Env; Variables: { user: User } }>()
  ...
  .get("/", async (c) => {
    ...
    const results = await shortcutsRouteDeps.listShortcuts(
      c.env.DB,
      user.id,
      spaceId,
    );
  ...
  .post("/", zValidator("json", createShortcutSchema), async (c) => {
    ...
    if (!body.name || !body.resourceType || !body.resourceId) {
      throw new BadRequestError(
        "Name, resourceType, and resourceId are required",
      );
    }

    if (!shortcutsRouteDeps.isShortcutResourceType(body.resourceType)) {
      throw new BadRequestError(
        `Invalid resourceType. Allowed values: ${
          ALLOWED_SHORTCUT_RESOURCE_TYPES.join(", ")
        }`,
      );
    }
    ...
    const created = await shortcutsRouteDeps.createShortcut(
      c.env.DB,
      user.id,
      spaceId,
      body,
  ...
  .put("/:id", zValidator("json", updateShortcutSchema), async (c) => {
    ...
    const updated = await shortcutsRouteDeps.updateShortcut(
      c.env.DB,
      user.id,
      spaceId,
      id,
  ...
  .post("/reorder", zValidator("json", reorderSchema), async (c) => {
    ...
    // Batch update all shortcut positions in a single SQL store batch call
    await shortcutsRouteDeps.reorderShortcuts(
      c.env.DB,
      user.id,
      spaceId,
      body.order,
...
export const shortcutGroupRoutes = new Hono<
  { Bindings: Env; Variables: { user: User } }
>()
  ...
  .get("/spaces/:spaceId/shortcuts/groups", async (c) => {
    ...
    const groups = await shortcutsRouteDeps.listShortcutGroups(
      c.env.DB,
      access.space.id,
    );
  ...
  .post(
    ...
    async (c) => {
      ...
      const group = await shortcutsRouteDeps.createShortcutGroup(
        c.env.DB,
        access.space.id,
        {
          name: body.name,
  ...
  .get("/spaces/:spaceId/shortcuts/groups/:groupId", async (c) => {
    ...
    const group = await shortcutsRouteDeps.getShortcutGroup(
      c.env.DB,
      access.space.id,
      groupId,
    );
  ...
  .patch(
    ...
    async (c) => {
      ...
      const group = await shortcutsRouteDeps.updateShortcutGroup(
        c.env.DB,
        access.space.id,
        groupId,
        {
      ...
      if (!group) {
        throw new NotFoundError(
          "Shortcut group (or managed by deployment snapshot)",
        );
      }
  ...
  .delete("/spaces/:spaceId/shortcuts/groups/:groupId", async (c) => {
    ...
    const deleted = await shortcutsRouteDeps.deleteShortcutGroup(
      c.env.DB,
      access.space.id,
      groupId,
    );
  ...
  .post(
    ...
    async (c) => {
      ...
      const item = await shortcutsRouteDeps.addItemToGroup(
        c.env.DB,
        access.space.id,
        groupId,
        body as Omit<ShortcutItem, "id">,
      );
  ...
  .delete(
    ...
    async (c) => {
      ...
      const removed = await shortcutsRouteDeps.removeItemFromGroup(
        c.env.DB,
        access.space.id,
        groupId,
        itemId,
      ...
      if (!removed) {
        throw new NotFoundError(
          "Shortcut group or item (or managed by deployment snapshot)",
        );
      }
