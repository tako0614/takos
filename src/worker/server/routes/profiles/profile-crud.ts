...
import type { ProfileRepoResponse, UserProfileResponse } from "./dto.ts";
...
export const profileCrudRoutes = new Hono<OptionalAuthRouteEnv>()
  .get("/:username", async (c) => {
    ...
    const followRequested = !isSelf && !following
      ...
      : false;
    const muted = !isSelf
      ? await isMutedByViewer(db, currentUser?.id, profileUser.id)
      : false;
  ...
  .get(
    ...
    async (c) => {
      ...
      const validStars = starsData.filter(
        (sd): sd is typeof sd & { ownerSlug: string } => !!sd.ownerSlug,
      );
