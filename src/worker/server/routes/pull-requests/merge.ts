...
export function validateConflictResolutionPath(path: unknown): string | null {
  ...
  if (!takosGit.isValidGitPath(normalized)) {
    return null;
  }
  return normalized;
}
...
async function createRebaseMergeTarget(params: {
  ...
  const mergeBase = await takosGit.findMergeBase(
    params.env.DB,
    params.bucket,
    params.repoId,
    params.baseSha,
  ...
  const commitsToRebase: takosGit.GitCommit[] = [];
  let cursorSha = params.headSha;
  while (cursorSha !== mergeBase) {
    const commit = await takosGit.getCommitData(params.bucket, cursorSha);
    if (!commit) {
  ...
  const baseCommit = await takosGit.getCommitData(
    params.bucket,
    params.baseSha,
  );
  if (!baseCommit) {
  ...
  for (const original of commitsToRebase) {
    ...
    const originalParentCommit = await takosGit.getCommitData(
      params.bucket,
      originalParentSha,
    );
    ...
    const commit = await takosGit.createCommit(
      params.env.DB,
      params.bucket,
      params.repoId,
      {
...
async function createMergeOrSquashTarget(params: {
  ...
  if (params.canFastForward) {
    const headCommit = await takosGit.getCommitData(
      params.bucket,
      params.headSha,
    );
    if (!headCommit) {
    ...
    const [baseCommit, localCommit, incomingCommit] = await Promise.all([
      takosGit.getCommitData(params.bucket, mergeBase),
      takosGit.getCommitData(params.bucket, params.baseSha),
      takosGit.getCommitData(params.bucket, params.headSha),
    ]);
...
export async function performPullRequestMerge(params: {
  ...
  const baseBranch = await takosGit.getBranch(
    params.env.DB,
    params.repoId,
    params.pullRequest.baseBranch,
  );
  const headBranch = await takosGit.getBranch(
    params.env.DB,
    params.repoId,
  ...
  const headIsAncestorOfBase = await takosGit.isAncestor(
    params.env.DB,
    bucket,
    params.repoId,
    headSha,
  ...
  const canFastForward = await takosGit.isAncestor(
    params.env.DB,
    bucket,
    params.repoId,
    baseSha,
