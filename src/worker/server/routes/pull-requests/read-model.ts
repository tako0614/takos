...
async function buildCommitMetrics(
  ...
  await Promise.all(pullRequests.map(async (pullRequest) => {
    const [baseSha, headSha] = await Promise.all([
      takosGit.resolveRef(env.DB, repoId, pullRequest.baseBranch),
      takosGit.resolveRef(env.DB, repoId, pullRequest.headBranch),
    ]);
    if (!baseSha || !headSha) {
    ...
    try {
      const { ahead } = await takosGit.countCommitsBetween(
        env.DB,
        bucket,
        repoId,
        baseSha,
