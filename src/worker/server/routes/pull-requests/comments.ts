...
import { prComments, pullRequests } from "../../../infra/db/schema.ts";
...
export default new Hono<AuthenticatedRouteEnv>()
  .post(
    ...
    async (c) => {
      ...
      try {
        ...
        logWarn("Failed to create PR comment notification", {
  ...
  .get("/repos/:repoId/pulls/:prNumber/comments", async (c) => {
    ...
    const userIds = comments
      .filter(
        (comment): comment is typeof comment & { authorId: string } =>
          comment.authorType === "user" && comment.authorId !== null,
      )
