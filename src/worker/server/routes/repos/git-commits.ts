...
/**
 * Validate client-supplied file entries before any blob is uploaded so that
 * malformed-but-attacker-controlled input (missing/garbage `content`, invalid
 * git path) yields a 400 Bad Request instead of an unhandled 500.
 *
 * - `path` / `content` must be strings.
 * - `path` must be a valid git path (rejects `../x`, leading `/`, control
 *   chars, etc.) — checked up front to avoid orphan blob writes that would
 *   otherwise happen when the deep `assertValidGitPath` runs after upload.
 * - `content` must decode as base64. We only fail when `atob` actually throws,
 *   so we don't impose a stricter alphabet than what callers/export emit.
 */
function validateFileEntries(files: FileEntry[]): void {
  for (const file of files) {
    if (
      typeof file?.path !== "string" || typeof file?.content !== "string"
    ) {
      throw new BadRequestError(
        "each file requires string path and content",
      );
    }
    if (!gitStore.isValidGitPath(file.path)) {
      throw new BadRequestError(`Invalid git path: ${file.path}`);
    }
    try {
      atob(file.content);
    } catch {
      throw new BadRequestError(`Invalid base64 content for ${file.path}`);
    }
  }
}
...
const gitCommits = new Hono<AuthenticatedRouteEnv>()
    ...
    async (c) => {
      ...
      try {
        ...
        while (cursor) {
          ...
          cursor = await gitStore.getCommit(
            c.env.DB,
            bucket,
            repoId,
            parentSha,
  ...
  .post("/repos/:repoId/import", async (c) => {
    ...
    validateFileEntries(body.files);

    const commitMessage = body.message || "Update via API";
    ...
    try {
      ...
      if (
        err instanceof Error && err.message === "Repository not initialized"
      ) {
