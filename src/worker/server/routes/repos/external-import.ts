...
export default new Hono<AuthenticatedRouteEnv>()
  ...
  .post("/repos/import-external", async (c) => {
    ...
    // Authorize the caller against the named space before importing. This
    // resolves the canonical space id (so slugs/aliases keep working) and
    // closes the privilege-escalation hole where any authenticated caller
    // could import into an arbitrary space id taken straight from the body.
