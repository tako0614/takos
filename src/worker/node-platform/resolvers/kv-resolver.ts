/**
 * kv store resolver — selects DynamoDB/Firestore/persistent/in-memory.
 */
import path from "node:path";
import { optionalEnv } from "./env-utils.ts";
import { createResolver } from "./resolver-factory.ts";
import { createInMemoryKvStoreBinding } from "../../local-platform/in-memory-bindings.ts";
import { createPersistentKvStoreBinding } from "../../local-platform/persistent-bindings.ts";
...
export const resolveKvStore = createResolver({
  ...
  createInMemory: () => createInMemoryKvStoreBinding(),
});
