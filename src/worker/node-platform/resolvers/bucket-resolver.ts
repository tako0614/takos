...
import { createInMemoryObjectStore } from "../../local-platform/in-memory-bindings.ts";
import { createPersistentObjectStore } from "../../local-platform/persistent-bindings.ts";
...
function bucketResolverConfig(
  name: BucketName,
): ResolverConfig<ObjectStoreBinding> {
  return {
    cloudAdapters: [
      // S3 (including MinIO / S3-compatible)
      {
        async tryCreate() {
          ...
          const { createS3ObjectStore } = await import(
            "../../adapters/s3-object-store.ts"
          );
      ...
      // GCS
      {
        async tryCreate() {
          ...
          const { createGcsObjectStore } = await import(
            "../../adapters/gcs-object-store.ts"
          );
    ...
    createPersistent: (dataDir) =>
      createPersistentObjectStore(
        path.join(dataDir, "buckets", PERSISTENT_BUCKET_MAP[name]),
      ),
    createInMemory: () => createInMemoryObjectStore(),
  };
}
