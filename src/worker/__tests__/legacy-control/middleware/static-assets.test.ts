...
function createApp() {
  ...
  app.get("/oauth", (c) => c.json({ oauthRoot: true }));
...
Deno.test("staticAssetsMiddleware - passes through /oauth without checking assets", async () => {
  const requests: Request[] = [];
  const app = createApp();
  const res = await app.fetch(
    new Request("http://localhost/oauth"),
    createEnvWithAssetFetch(async (request) => {
      requests.push(request);
      return new Response("unexpected");
    }),
  ...
  assertEquals(res.status, 200);
  assertEquals(await res.json(), { oauthRoot: true });
  assertEquals(requests.length, 0);
});
