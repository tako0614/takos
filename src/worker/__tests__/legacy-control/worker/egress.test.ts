...
// The egress handler accepts a private `Env` interface from its source module.
// Every field is optional, so an empty object satisfies the contract. We capture
// the parameter type from the handler itself so the test mirrors the runtime
// signature without redeclaring the private interface here.
type EgressEnv = Parameters<typeof handler.fetch>[1];
...
function createEnv(overrides: Partial<EgressEnv> = {}): EgressEnv {
  return {
    ...overrides,
  };
}
