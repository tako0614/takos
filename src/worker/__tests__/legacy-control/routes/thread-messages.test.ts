...
const mocks: {
  ...
  createMessage: LooseAsyncFn;
  searchThreadMessages: LooseAsyncFn;
  getThreadTimeline: LooseAsyncFn;
  getThreadHistory: LooseAsyncFn;
} = {
  checkThreadAccess: asyncNoopDep("threadMessagesRouteDeps.checkThreadAccess"),
  createMessage: asyncNoopDep("threadMessagesRouteDeps.createMessage"),
  searchThreadMessages: asyncNoopDep(
    "threadMessagesRouteDeps.searchThreadMessages",
  ),
  getThreadTimeline: asyncNoopDep("threadMessagesRouteDeps.getThreadTimeline"),
  getThreadHistory: asyncNoopDep("threadMessagesRouteDeps.getThreadHistory"),
};
...
function createApp(user: User) {
  ...
  function bindMock<K extends keyof typeof threadMessagesRouteDeps>(
    mockKey: keyof typeof mocks,
  ): typeof threadMessagesRouteDeps[K] {
    return ((...args: unknown[]) => {
      type AnyFn = (...a: unknown[]) => unknown;
      return (mocks[mockKey] as AnyFn)(...args);
    }) as typeof threadMessagesRouteDeps[K];
  }
  Object.assign(threadMessagesRouteDeps, {
    checkThreadAccess: bindMock<"checkThreadAccess">("checkThreadAccess"),
    createMessage: bindMock<"createMessage">("createMessage"),
    searchThreadMessages: bindMock<"searchThreadMessages">(
      "searchThreadMessages",
    ),
    getThreadTimeline: bindMock<"getThreadTimeline">("getThreadTimeline"),
    getThreadHistory: bindMock<"getThreadHistory">("getThreadHistory"),
  });
...
Deno.test("thread-messages routes - GET /api/threads/:id/messages - returns 404 when thread not found or access denied", async () => {
  ...
  assertEquals(mocks.getThreadTimeline.calls?.length ?? 0, 0);
});
...
Deno.test("thread-messages routes - GET /api/threads/:id/messages/search - returns 400 when query is missing", async () => {
  ...
  assertEquals(mocks.searchThreadMessages.calls?.length ?? 0, 0);
});
...
Deno.test("thread-messages routes - POST /api/threads/:id/messages - returns 404 when thread not found or not writable", async () => {
  ...
  assertEquals(res.status, 404);
  assertEquals(mocks.createMessage.calls?.length ?? 0, 0);
});
