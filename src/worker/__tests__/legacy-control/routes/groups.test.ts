...
import { groupsRouteDeps } from "../../../server/routes/groups/routes.ts";
...
// db mock is a structural partial of DrizzleD1Database. `asTestDatabase`
// wraps it in a throwing Proxy that satisfies the full `Database` surface
// (returning a throwing function for any unstubbed method), so the
// production dep type accepts it without an unsafe cast.
function installGroupDbMock(db: object) {
  const wrapped = asTestDatabase(db);
  groupsRouteDeps.getDb = () => wrapped;
}

// SpaceAccess returned by `requireSpaceAccess` includes a `membership`
// field that some tests don't need; provide a default-role membership so
// the closure conforms to the typed signature.
function installSpaceAccessMock(space: { id: string }) {
  routeAuthDeps.requireSpaceAccess = (async (
    ..._args: Parameters<typeof routeAuthDeps.requireSpaceAccess>
  ) => ({
    space,
...
Deno.test("groups routes - rejects legacy build metadata in manifest input", async () => {
  const originalDeps = {
    getDb: groupsRouteDeps.getDb,
    getGroupState: groupsRouteDeps.getGroupState,
    planManifest: groupsRouteDeps.planManifest,
  ...
  try {
    ...
    assertStringIncludes(json.error?.message ?? "", "no longer supported");
    assertStringIncludes(json.error?.message ?? "", "takosumi init");
    assertStringIncludes(
      json.error?.message ?? "",
      "takosumi install dry-run/apply",
    );
    assertSpyCalls(planManifestSpy, 0);
  } finally {
    groupsRouteDeps.getDb = originalDeps.getDb;
...
Deno.test("groups routes - local_upload source projection clears git source fields", async () => {
  const originalDeps = {
    getDb: groupsRouteDeps.getDb,
    getGroupState: groupsRouteDeps.getGroupState,
    planManifest: groupsRouteDeps.planManifest,
