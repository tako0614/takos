...
const mocks: {
  ...
  listReminders: createRecordedMock(async () => undefined),
  getReminderById: createRecordedMock(async () => undefined),
  createReminder: createRecordedMock(async () => undefined),
  updateReminder: createRecordedMock(async () => undefined),
  deleteReminder: createRecordedMock(async () => undefined),
  triggerReminder: createRecordedMock(async () => undefined),
  checkWorkspaceAccess: createRecordedMock(async () => undefined),
};
...
function invokeMock(mockKey: keyof typeof mocks, args: unknown[]): unknown {
  type AnyFn = (...a: unknown[]) => unknown;
  const fn: AnyFn = mocks[mockKey] as AnyFn;
  return fn(...args);
}

function bindMock<K extends keyof typeof remindersRouteDeps>(
  mockKey: keyof typeof mocks,
): typeof remindersRouteDeps[K] {
  const callMock = (...args: unknown[]) => invokeMock(mockKey, args);
  return callMock as typeof remindersRouteDeps[K];
}

function applyMockBindings() {
  remindersRouteDeps.requireSpaceAccess = bindMock<"requireSpaceAccess">(
    "requireSpaceAccess",
  );
  remindersRouteDeps.listReminders = bindMock<"listReminders">("listReminders");
  remindersRouteDeps.getReminderById = bindMock<"getReminderById">(
    "getReminderById",
  );
  remindersRouteDeps.createReminder = bindMock<"createReminder">(
    "createReminder",
  );
  remindersRouteDeps.updateReminder = bindMock<"updateReminder">(
    "updateReminder",
  );
  remindersRouteDeps.deleteReminder = bindMock<"deleteReminder">(
    "deleteReminder",
  );
  remindersRouteDeps.triggerReminder = bindMock<"triggerReminder">(
    "triggerReminder",
  );
  remindersRouteDeps.checkSpaceAccess = bindMock<"checkSpaceAccess">(
...
Deno.test("reminders routes - GET /api/reminders/:id - returns 404 when reminder not found", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  env = createMockEnv();
  mocks.getReminderById = createRecordedMock(async () => null);
...
Deno.test("reminders routes - GET /api/reminders/:id - returns 403 when user has no workspace access", async () => {
  ...
  mocks.checkWorkspaceAccess = createRecordedMock(async () => null);
