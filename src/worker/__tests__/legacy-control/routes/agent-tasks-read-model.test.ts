...
import { agentTaskRouteDeps } from "@/routes/agent-tasks";
import { createMockEnv } from "../../../test/integration/setup.ts";
import { agentTaskHandlerDeps } from "../../../server/routes/agent-tasks-handlers.ts";
...
const mocks: {
  ...
  getDb: noopDep("agentTaskRouteDeps.getDb"),
  checkWorkspaceAccess: asyncNoopDep("agentTaskRouteDeps.checkWorkspaceAccess"),
};
...
const originalCheckSpaceAccess = agentTaskRouteDeps.checkSpaceAccess;
const originalGetDb = agentTaskRouteDeps.getDb;
const originalHandlerGetDb = agentTaskHandlerDeps.getDb;
...
Deno.test("agent task read-model routes - returns latest run summary and resume target for each task", async () => {
  ...
  try {
    ...
    agentTaskRouteDeps.checkSpaceAccess = mocks
      .checkWorkspaceAccess as typeof agentTaskRouteDeps.checkSpaceAccess;
    agentTaskRouteDeps.getDb = mocks.getDb as typeof agentTaskRouteDeps.getDb;
    agentTaskHandlerDeps.getDb = mocks
      .getDb as typeof agentTaskHandlerDeps.getDb;
    ...
    const payload = await response.json() as {
      tasks: Array<{
        ...
        latest_run?:
          | { run_id: string; status: string; artifact_count: number }
          | null;
        resume_target?: {
          thread_id: string;
          run_id: string | null;
    ...
    agentTaskRouteDeps.checkSpaceAccess = originalCheckSpaceAccess;
    agentTaskRouteDeps.getDb = originalGetDb;
    agentTaskHandlerDeps.getDb = originalHandlerGetDb;
  }
});
