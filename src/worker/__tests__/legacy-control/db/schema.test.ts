...
// ---- schema-app-usage ----
import { appUsageEvents, appUsageRollups } from "@/db/schema-app-usage";
...
// ---- schema-oauth ----
import { mcpOauthPending, mcpServers } from "@/db/schema-oauth";
...
Deno.test('schema-app-usage - appUsageEvents table - is named "app_usage_events"', () => {
  assertEquals(getTableName(appUsageEvents), "app_usage_events");
});
Deno.test("schema-app-usage - appUsageEvents table - has owner, meter, idempotency, and unit columns", () => {
  assertEquals(hasColumn(appUsageEvents, "ownerAccountId"), true);
  assertEquals(hasColumn(appUsageEvents, "meterType"), true);
  assertEquals(hasColumn(appUsageEvents, "idempotencyKey"), true);
  assertEquals(hasColumn(appUsageEvents, "units"), true);
});
Deno.test("schema-app-usage - appUsageEvents table - has usage lookup indexes", () => {
  const idxs = indexNames(appUsageEvents);
  assertHasEntry(idxs, "idx_app_usage_events_owner_account_id");
  assertHasEntry(idxs, "idx_app_usage_events_meter_type");
  assertHasEntry(idxs, "idx_app_usage_events_created_at");
});
Deno.test("schema-app-usage - appUsageRollups table - has a unique owner/scope/meter/month index", () => {
  const uniq = uniqueIndexNames(appUsageRollups);
  assertHasEntry(uniq, "idx_app_usage_rollups_scope");
});
// ===================================================================
// schema-agents
// ===================================================================
...
const expectedNames: [Parameters<typeof getTableName>[0], string][] = [
  ...
  [appUsageRollups, "app_usage_rollups"],
