...
function emptySqlMeta(changes = 0) {
  return {
    duration: 0,
    size_after: 0,
    rows_read: 0,
...
function sqlResult<T>(results: T[], changes = 0): SqlResultBinding<T> {
  return { results, success: true, meta: emptySqlMeta(changes) };
}

const mockKV: KvStoreBinding = noopKvStoreBinding();

const mockR2: ObjectStoreBinding = noopObjectStoreBinding();

function rawStub<T = unknown[]>(
...
function rawStub(
  ...
  if (options?.columnNames) return Promise.resolve([[]]);
  return Promise.resolve([]);
}

const mockStmt: SqlPreparedStatementBinding = {
  bind: () => mockStmt,
  all: <T = Record<string, unknown>>() => Promise.resolve(sqlResult<T>([])),
  run: <T = Record<string, unknown>>() => Promise.resolve(sqlResult<T>([], 1)),
  first: <T = Record<string, unknown>>(_col?: string) =>
...
function makeContext(overrides: Partial<ToolContext> = {}): ToolContext {
  return {
    ...
    storage: noopObjectStoreBinding(),
...
Deno.test("createSqlHandler - provisions through managed resource lifecycle", async () => {
  ...
  let receivedInput:
    | Parameters<typeof storageResourceToolDeps.provisionManagedResource>[1]
    | undefined;
...
Deno.test("listResourcesHandler - lists portable resource records from registry", async () => {
  ...
  storageResourceToolDeps.getDb = () => db;
...
Deno.test("keyValueGetHandler - returns value when key exists", async () => {
  mockKV.get =
    (<T = string>(_k: string) =>
      Promise.resolve("my-value" as T)) as KvStoreBinding["get"];
...
Deno.test("keyValueGetHandler - returns not found when key is null", async () => {
  mockKV.get =
    (<T = string>(_k: string) =>
      Promise.resolve(null as T | null)) as KvStoreBinding["get"];
...
Deno.test("keyValuePutHandler - stores a value", async () => {
  ...
  mockKV.put = (...args: Parameters<KvStoreBinding["put"]>) => {
    receivedArgs = args;
    return Promise.resolve();
  };
...
Deno.test("keyValueListHandler - reports empty namespace", async () => {
  mockKV.list = () => Promise.resolve({ keys: [], list_complete: true });
...
Deno.test("sqlQueryHandler - executes a SELECT query and formats results", async () => {
  mockStmt.all = (<T = Record<string, unknown>>() =>
    Promise.resolve(
      sqlResult<T>(
        [
          { id: 1, name: "Alice" },
    ...
    )) as SqlPreparedStatementBinding["all"];
...
Deno.test("sqlQueryHandler - returns no results message for empty SELECT", async () => {
  mockStmt.all =
    (<T = Record<string, unknown>>() =>
      Promise.resolve(sqlResult<T>([]))) as SqlPreparedStatementBinding["all"];
...
Deno.test("sqlQueryHandler - executes INSERT and reports changes", async () => {
  mockStmt.run =
    (<T = Record<string, unknown>>() =>
      Promise.resolve(sqlResult<T>([], 1))) as SqlPreparedStatementBinding[
        "run"
      ];
...
Deno.test("sqlDescribeHandler - throws when table does not exist", async () => {
  mockStmt.first =
    (<T = Record<string, unknown>>(_col?: string) =>
      Promise.resolve(null as T | null)) as SqlPreparedStatementBinding[
        "first"
      ];
...
Deno.test("sqlDescribeHandler - describes table with columns and indexes", async () => {
  mockStmt.first =
    (<T = Record<string, unknown>>(_col?: string) =>
      Promise.resolve(
        { name: "users" } as T,
      )) as SqlPreparedStatementBinding["first"];
  let allCallCount = 0;
  mockStmt.all = (<T = Record<string, unknown>>() => {
    allCallCount++;
    if (allCallCount === 1) {
...
Deno.test("objectStoreListHandler - shows truncation notice", async () => {
  mockR2.list = () =>
    Promise.resolve({
      objects: [
        { key: "f.txt", size: 10, uploaded: new Date() },
      ] as ObjectStoreObject[],
      truncated: true,
      delimitedPrefixes: [],
    });
...
Deno.test("objectStoreInfoHandler - throws when object not found", async () => {
  mockR2.head = () => Promise.resolve(null);
