...
Deno.test("encrypt / decrypt - rejects unsupported algorithm", async () => {
  // Build via Record to assign an alg literal outside the type's narrowed
  // union; `decrypt` validates the alg at runtime.
  const bad: Record<keyof EncryptedData, unknown> = {
    ciphertext: "aaa",
    iv: "bbb",
    alg: "AES-128-CBC",
    v: 1,
  ...
  await assertRejects(async () => {
    await decrypt(bad as EncryptedData, MASTER_SECRET, SALT);
