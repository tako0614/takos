...
Deno.test("validateRuntimeHostEnv - rejects JWT_PUBLIC_KEY runtime-host input", () => {
  const err = expectError(validateRuntimeHostEnv({
    RUNTIME_CONTAINER: {},
    PLATFORM_PUBLIC_KEY: "public-key",
    JWT_PUBLIC_KEY: "public-key",
  }));
  assertStringIncludes(err, "JWT_PUBLIC_KEY is not a supported");
});
...
Deno.test("validateExecutorHostEnv - returns null when all bindings present", () => {
  const env = {
    EXECUTOR_CONTAINER: {},
    TAKOS_WORKER: {},
    TAKOS_AGENT_CONTROL_RPC_BASE_URL: "http://localhost",
    EXECUTOR_PROXY_SECRET: "executor-proxy-secret",
  };
...
Deno.test("validateExecutorHostEnv - reports all missing bindings", () => {
  ...
  assertStringIncludes(err, "TAKOS_WORKER");
  assertStringIncludes(err, "TAKOS_AGENT_CONTROL_RPC_BASE_URL");
