...
/**
 * Test factory returns a Hono `Context` but only with the minimum slots the
 * rate-limiter middleware touches (req.header, header, json). The factory
 * captures recorded headers in `_resHeaders` so tests can assert on them.
 */
type TestContext = Context & { _resHeaders: Record<string, string> };
...
// The rate-limiter middleware only invokes `req.header`, `header`, and
// `json` on its Hono context argument. We build a partial that implements
// those slots, capture written headers in `_resHeaders`, and structurally
// cast to TestContext at the boundary. Hono's `json` overload signature
// makes a strict partial declaration unwieldy, so we keep the partial typed
// against the call surface (input parameters) and let the cast bridge it.
function createMockContext(ip?: string): TestContext {
...
Deno.test("InMemoryRateLimiter#middleware - returns 429 with Retry-After header when rate limited", async () => {
  ...
  const result = await mw(c, next) as
    | { body: RateLimitErrorBody; status: number }
    | undefined;
  ...
  const limitedBody = limited.body;
