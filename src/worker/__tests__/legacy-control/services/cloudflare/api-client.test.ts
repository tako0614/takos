...
function mockFetchSuccess<T>(result: T) {
  const fetchMock = spy(async (..._args: FetchArgs) =>
    new Response(
      JSON.stringify({
        success: true,
        result,
...
Deno.test("CloudflareApiClient - fetch - throws classified error when success is false in body", async () => {
  try {
    ...
    const cfErr = err as CloudflareAPIError;
    assertEquals(cfErr.statusCode, 400);
    assertEquals(cfErr.code, 100);
    assertStringIncludes(cfErr.message, "Validation error");
  } finally {
    restoreFetch();
  }
});
