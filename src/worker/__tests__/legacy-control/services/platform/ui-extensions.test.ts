import type { ObjectStoreBinding } from "@/shared/types/bindings.ts";

import { assert, assertEquals, assertNotEquals } from "@std/assert";

// [Deno] vi.mock removed - manually stub imports from '@/db'
import {
  getUIExtensionBundle,
  getUIExtensionByPath,
  getUIExtensionPaths,
  getUISidebarItems,
  isUIExtensionPath,
  listUIExtensions,
  uiExtensionDeps,
} from "@/services/platform/ui-extensions";
import {
  noopObjectStoreBinding,
  noopSqlDatabaseBinding,
} from "@test/binding-stubs";
import { asTestDatabase, asTestSqlDatabaseBinding } from "@test/db-stubs";
import { makeObjectStoreObjectBody } from "../../../../test/integration/setup.ts";

const originalUiExtensionDeps = { ...uiExtensionDeps };

function setUiDb(drizzle: ReturnType<typeof createDrizzleMock>) {
  const wrappedDb = asTestDatabase(drizzle);
  uiExtensionDeps.getDb =
    ((..._args: Parameters<typeof uiExtensionDeps.getDb>) =>
      wrappedDb) as typeof uiExtensionDeps.getDb;
}

function restoreUiDeps() {
  Object.assign(uiExtensionDeps, originalUiExtensionDeps);
}

type MockFn = (...args: unknown[]) => unknown;

interface DrizzleMockApi {
  get: MockFn;
  all: MockFn;
}

interface DrizzleMockChain {
  from(): DrizzleMockChain;
  where(): DrizzleMockChain;
  orderBy(): DrizzleMockChain;
  get: MockFn;
  all: MockFn;
}

function createDrizzleMock() {
  const api: DrizzleMockApi = {
    get: () => undefined,
    all: () => undefined,
  };
  const chain: DrizzleMockChain = {
    from() {
      return chain;
    },
    where() {
      return chain;
    },
    orderBy() {
      return chain;
    },
    get: (...args: unknown[]) => api.get(...args),
    all: (...args: unknown[]) => api.all(...args),
  };
  return {
    select: () => chain,
    _: api,
  };
}

function createPublicationDb(rows: unknown[]) {
  return asTestSqlDatabaseBinding({
    select: () => ({
      from: function (this: any) {
        return this;
      },
      where: function (this: any) {
        return this;
      },
      orderBy: function (this: any) {
        return this;
      },
      all: async () => rows,
      get: async () => rows[0] ?? null,
    }),
    insert: () => ({
      values: () => ({
        run: async () => undefined,
      }),
    }),
    update: () => ({
      set: () => ({
        where: async () => ({ meta: { changes: 0 } }),
      }),
    }),
    delete: () => ({
      where: async () => ({ meta: { changes: 0 } }),
    }),
  });
}

const makeExtRow = (overrides: Record<string, unknown> = {}) => ({
  id: "ext-1",
  accountId: "ws-1",
  path: "/custom-panel",
  label: "Custom Panel",
  icon: "star",
  bundleR2Key: "ui-ext/ws-1/ext-1.js",
  sidebarJson: JSON.stringify({
    label: "Panel",
    icon: "star",
    path: "/custom-panel",
  }),
  bundleDeploymentId: "bd-1",
  createdAt: "2026-01-01T00:00:00.000Z",
  ...overrides,
});

Deno.test("listUIExtensions - returns empty array when no extensions", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.all = async () => [];
  setUiDb(drizzle);

  try {
    const result = await listUIExtensions(
      noopSqlDatabaseBinding(),
      "ws-1",
    );
    assertEquals(result, []);
  } finally {
    restoreUiDeps();
  }
});
Deno.test("listUIExtensions - maps extension rows correctly", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.all = async () => [makeExtRow()];
  setUiDb(drizzle);

  try {
    const result = await listUIExtensions(
      noopSqlDatabaseBinding(),
      "ws-1",
    );
    assertEquals(result.length, 1);
    assertEquals(result[0].id, "ext-1");
    assertEquals(result[0].path, "/custom-panel");
    assertEquals(result[0].label, "Custom Panel");
    assert(result[0].sidebar !== undefined);
    assertEquals(result[0].sidebar!.label, "Panel");
    assertEquals(result[0].bundleDeploymentId, "bd-1");
  } finally {
    restoreUiDeps();
  }
});

Deno.test("getUIExtensionByPath - returns null when not found", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => undefined;
  setUiDb(drizzle);

  try {
    const result = await getUIExtensionByPath(
      noopSqlDatabaseBinding(),
      "ws-1",
      "/nonexistent",
    );
    assertEquals(result, null);
  } finally {
    restoreUiDeps();
  }
});
Deno.test("getUIExtensionByPath - returns extension when found", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => makeExtRow();
  setUiDb(drizzle);

  try {
    const result = await getUIExtensionByPath(
      noopSqlDatabaseBinding(),
      "ws-1",
      "/custom-panel",
    );
    assertNotEquals(result, null);
    assertEquals(result!.path, "/custom-panel");
  } finally {
    restoreUiDeps();
  }
});
Deno.test("getUIExtensionByPath - handles extension without sidebar JSON", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => makeExtRow({ sidebarJson: null });
  setUiDb(drizzle);

  try {
    const result = await getUIExtensionByPath(
      noopSqlDatabaseBinding(),
      "ws-1",
      "/custom-panel",
    );
    assertNotEquals(result, null);
    assertEquals(result!.sidebar, undefined);
  } finally {
    restoreUiDeps();
  }
});

Deno.test("getUIExtensionBundle - returns null when extension not found", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => undefined;
  setUiDb(drizzle);

  try {
    const storage = noopObjectStoreBinding();
    const result = await getUIExtensionBundle(
      noopSqlDatabaseBinding(),
      storage,
      "ws-1",
      "/nonexistent",
    );
    assertEquals(result, null);
  } finally {
    restoreUiDeps();
  }
});
Deno.test("getUIExtensionBundle - returns null when object store entry is not found", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => makeExtRow();
  setUiDb(drizzle);

  const storage: ObjectStoreBinding = {
    ...noopObjectStoreBinding(),
    get: () => Promise.resolve(null),
  };

  const result = await getUIExtensionBundle(
    noopSqlDatabaseBinding(),
    storage,
    "ws-1",
    "/custom-panel",
  );
  assertEquals(result, null);
});
Deno.test("getUIExtensionBundle - returns content and content type", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => makeExtRow();
  setUiDb(drizzle);

  const content = new ArrayBuffer(10);
  const storage: ObjectStoreBinding = {
    ...noopObjectStoreBinding(),
    get: () =>
      Promise.resolve(
        makeObjectStoreObjectBody(content, {
          httpMetadata: { contentType: "application/javascript" },
        }),
      ),
  };

  try {
    const result = await getUIExtensionBundle(
      noopSqlDatabaseBinding(),
      storage,
      "ws-1",
      "/custom-panel",
    );
    assertNotEquals(result, null);
    assertEquals(result!.contentType, "application/javascript");
  } finally {
    restoreUiDeps();
  }
});

Deno.test("getUISidebarItems - returns empty array when no sidebar extensions", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.all = async () => [];
  setUiDb(drizzle);

  try {
    const result = await getUISidebarItems(
      createPublicationDb([]),
      "ws-1",
    );
    assertEquals(result, []);
  } finally {
    restoreUiDeps();
  }
});
Deno.test("getUISidebarItems - filters out invalid sidebar JSON", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.all = async () => [
    makeExtRow(),
    makeExtRow({ id: "ext-2", sidebarJson: '{"invalid": true}' }),
  ];
  setUiDb(drizzle);

  const publicationRows = [
    {
      id: "pub-1",
      name: "panel-1",
      sourceType: "manifest",
      groupId: null,
      ownerServiceId: null,
      catalogName: null,
      publicationType: "UiSurface",
      specJson: JSON.stringify({
        name: "panel-1",
        publisher: "external",
        type: "ui-surface",
        path: "/custom-panel",
        outputs: { default: { kind: "url", routeRef: "panel-1-route" } },
        spec: {
          sidebar: {
            label: "Panel",
            icon: "star",
            path: "/custom-panel",
          },
        },
      }),
      resolvedJson: JSON.stringify({ url: "https://example.com/custom-panel" }),
      createdAt: "2026-01-01T00:00:00.000Z",
      updatedAt: "2026-01-01T00:00:00.000Z",
    },
    {
      id: "pub-2",
      name: "panel-2",
      sourceType: "manifest",
      groupId: null,
      ownerServiceId: null,
      catalogName: null,
      publicationType: "UiSurface",
      specJson: JSON.stringify({
        name: "panel-2",
        publisher: "external",
        type: "ui-surface",
        path: "/custom-panel-2",
        outputs: { default: { kind: "url", routeRef: "panel-2-route" } },
        spec: {
          sidebar: {
            label: "Panel 2",
            icon: "sparkles",
            path: "/custom-panel-2",
          },
        },
      }),
      resolvedJson: JSON.stringify({}),
      createdAt: "2026-01-01T00:00:00.000Z",
      updatedAt: "2026-01-01T00:00:00.000Z",
    },
  ];

  try {
    const result = await getUISidebarItems(
      createPublicationDb(publicationRows),
      "ws-1",
    );
    // The first has valid sidebar, the second is missing label/icon
    assertEquals(result.length, 1);
    assertEquals(result[0].extensionId, "publication:pub-1");
  } finally {
    restoreUiDeps();
  }
});

Deno.test("isUIExtensionPath - returns true when extension exists at path", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => ({ count: 1 });
  setUiDb(drizzle);

  try {
    const result = await isUIExtensionPath(
      noopSqlDatabaseBinding(),
      "ws-1",
      "/custom-panel",
    );
    assertEquals(result, true);
  } finally {
    restoreUiDeps();
  }
});
Deno.test("isUIExtensionPath - returns false when no extension at path", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.get = async () => ({ count: 0 });
  setUiDb(drizzle);

  try {
    const result = await isUIExtensionPath(
      noopSqlDatabaseBinding(),
      "ws-1",
      "/nonexistent",
    );
    assertEquals(result, false);
  } finally {
    restoreUiDeps();
  }
});

Deno.test("getUIExtensionPaths - returns all registered paths", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const drizzle = createDrizzleMock();
  drizzle._.all = async () => [{ path: "/panel-a" }, { path: "/panel-b" }];
  setUiDb(drizzle);

  try {
    const result = await getUIExtensionPaths(
      noopSqlDatabaseBinding(),
      "ws-1",
    );
    assertEquals(result, ["/panel-a", "/panel-b"]);
  } finally {
    restoreUiDeps();
  }
});
