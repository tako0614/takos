...
import { mergeTrees3Way } from "@/application/services/takos-git/local/core/merge.ts";
import {
  buildTreeFromPaths,
  createEmptyTree,
  flattenTree,
...
async function buildTree(
  ...
  return buildTreeFromPaths(bucket, entries);
}
...
Deno.test("mergeTrees3Way - no conflicts (disjoint changes) - merges when local adds file A and upstream adds file B", async () => {
  ...
  const localSha = await buildTree(bucket, {
    "shared.txt": "base",
    "local-new.txt": "local content",
  });
  const upstreamSha = await buildTree(bucket, {
    "shared.txt": "base",
    "upstream-new.txt": "upstream content",
  });
  ...
  const merged = await flattenTree(bucket, result.tree_sha!);
...
Deno.test("mergeTrees3Way - no conflicts (disjoint changes) - merges when local modifies file A and upstream modifies file B", async () => {
  bucket = new MockObjectStoreBinding();
  const baseSha = await buildTree(bucket, {
    "a.txt": "original-a",
    "b.txt": "original-b",
  });
  const localSha = await buildTree(bucket, {
  ...
  const localA = await flattenTree(bucket, localSha);
  const upstreamB = await flattenTree(bucket, upstreamSha);
  assertEquals(
    fileMap.get("a.txt"),
    localA.find((f) => f.path === "a.txt")!.sha,
  );
  assertEquals(
    fileMap.get("b.txt"),
    upstreamB.find((f) => f.path === "b.txt")!.sha,
  );
});
...
Deno.test("mergeTrees3Way - no conflicts (disjoint changes) - merges when both sides make the same change to a file", async () => {
  ...
  const result = await mergeTrees3Way(bucket, baseSha, sameSha, sameSha);
...
Deno.test("mergeTrees3Way - conflict detection - detects add-add conflict when both sides add the same path with different content", async () => {
  ...
  const localSha = await buildTree(bucket, {
    "existing.txt": "base",
    "new.txt": "local version",
  });
  const upstreamSha = await buildTree(bucket, {
    "existing.txt": "base",
    "new.txt": "upstream version",
  });
...
Deno.test("mergeTrees3Way - added and deleted files - accepts when both sides delete the same file", async () => {
  ...
  const result = await mergeTrees3Way(bucket, baseSha, bothSha, bothSha);
...
Deno.test("mergeTrees3Way - added and deleted files - accepts when both sides add the same file with identical content", async () => {
  bucket = new MockObjectStoreBinding();
  const baseSha = await buildTree(bucket, { "base.txt": "base" });
  const withNew = await buildTree(bucket, {
    "base.txt": "base",
    "new.txt": "identical",
  });

  const result = await mergeTrees3Way(bucket, baseSha, withNew, withNew);
...
Deno.test("mergeTrees3Way - edge cases - merges identical trees with no changes", async () => {
  ...
  const result = await mergeTrees3Way(bucket, treeSha, treeSha, treeSha);
...
Deno.test("mergeTrees3Way - edge cases - merges three empty trees", async () => {
  bucket = new MockObjectStoreBinding();
  const emptySha = await createEmptyTree(bucket);
...
Deno.test("mergeTrees3Way - edge cases - merges nested directory trees with disjoint changes", async () => {
  ...
  const localFiles = await flattenTree(bucket, localSha);
  const upstreamFiles = await flattenTree(bucket, upstreamSha);
  assertEquals(
    fileMap.get("dir/a.txt"),
    localFiles.find((f) => f.path === "dir/a.txt")!.sha,
  );
  assertEquals(
    fileMap.get("dir/b.txt"),
    upstreamFiles.find((f) => f.path === "dir/b.txt")!.sha,
  );
});
...
Deno.test("mergeTrees3Way - edge cases - handles mode change as a modification", async () => {
  ...
  const baseSha = await buildTreeFromPaths(bucket, [
    { path: "run.sh", sha: blobSha, mode: "100644" },
  ]);
  const localSha = await buildTreeFromPaths(bucket, [
