import {
  ...
  encodeTree,
  hashBlob,
} from "@/application/services/takos-git/local/core/object.ts";
import type {
  GitSignature,
  TreeEntry,
...
Deno.test("encodeTree / decodeTree - roundtrips multiple entries", () => {
  ...
  assertEquals(names.includes("b.sh"), true);
  assertEquals(names.includes("c-dir"), true);
});
...
Deno.test("decodeCommit error cases - throws on missing required fields", () => {
  ...
  assertThrows(() => decodeCommit(content), Error, "missing required fields");
});
