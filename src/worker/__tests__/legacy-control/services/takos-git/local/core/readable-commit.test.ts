...
} from "@/application/services/takos-git/local/core/readable-commit.ts";
import type { GitTree } from "@/application/services/takos-git/local/git-objects.ts";
import {
  noopObjectStoreBinding,
  noopSqlDatabaseBinding,
} from "@test/binding-stubs";

const db = noopSqlDatabaseBinding();
const bucket = noopObjectStoreBinding();
...
Deno.test("resolveReadableCommitFromRef - returns ref_not_found when ref does not resolve", async () => {
  ...
  const result = await resolveReadableCommitFromRef(
    db,
    bucket,
    REPO_ID,
    "main",
...
Deno.test("resolveReadableCommitFromRef - returns non-degraded when primary commit has a valid tree", async () => {
  ...
  readableCommitDeps.getTree =
    async () => ({ sha: "ttt", entries: [] } as GitTree);
...
Deno.test("resolveReadableCommitFromRef - falls back to ancestor when primary tree is missing", async () => {
  ...
  readableCommitDeps.getTree = async (_bucket, treeSha) =>
    treeSha === "ttt-ok" ? ({ sha: "ttt-ok", entries: [] } as GitTree) : null;
...
Deno.test("resolveReadableCommitFromRef - respects fallbackLimit option", async () => {
  ...
  await resolveReadableCommitFromRef(db, bucket, REPO_ID, "main", {
    fallbackLimit: 10,
  });
