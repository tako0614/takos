...
import {
  applyDelta,
  readPackfileAsync,
} from "@/application/services/takos-git/local/protocol/packfile-reader.ts";
import {
  getObject,
  putBlob,
...
import {
  concatBytes,
  sha1Bytes,
} from "@/application/services/takos-git/local/core/sha1.ts";
...
async function buildTestPackfile(
  ...
  const TYPE_NUMS: Record<string, number> = {
    commit: 1,
    tree: 2,
    blob: 3,
    tag: 4,
...
Deno.test("readPackfileAsync - throws on invalid signature", async () => {
  ...
  await assertRejects(async () => {
    await readPackfileAsync(bad, bucket);
...
Deno.test(
  ...
  async () => {
    ...
    const originalSha = await putBlob(bucket, content);
    ...
    const receiveBucket = new MockObjectStoreBinding();
    const storedShas = await readPackfileAsync(pack, receiveBucket);
    ...
    const obj = await getObject(receiveBucket, storedShas[0]);
...
Deno.test("readPackfileAsync - throws when maxObjectCount is exceeded", async () => {
  ...
  const error = await assertRejects(async () => {
    await readPackfileAsync(pack, new MockObjectStoreBinding(), {
      maxObjectCount: 0,
    });
  }, Error);
  assertStringIncludes(
...
Deno.test(
  ...
  async () => {
    ...
    assertStringIncludes(error.message, "Inflated total");
    assertStringIncludes(error.message, "exceeds limit of 1");
  },
);
...
Deno.test("readPackfileAsync - throws when maxPackfileBytes is exceeded", async () => {
  ...
  assertStringIncludes(error.message, "Packfile size");
  assertStringIncludes(error.message, "exceeds limit of 10");
});
