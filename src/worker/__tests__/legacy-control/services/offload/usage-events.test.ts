...
const mocks = {
  gzipCompressString: spy(
    async (..._args: unknown[]) => new ArrayBuffer(0),
  ),
  gzipDecompressToString: (..._args: unknown[]): unknown => undefined,
};
...
// ---------------------------------------------------------------------------
// write usage event segment
// ---------------------------------------------------------------------------

Deno.test("usage event offload - does nothing when events array is empty", async () => {
...
Deno.test("usage event offload - compresses events to JSONL and writes to the object store with metadata", async () => {
  restoreUsageEventsDeps();
  const bucket = new MockObjectStoreBinding();
  const compressed = new ArrayBuffer(4);
  mocks.gzipCompressString = spy(async () => compressed);
...
Deno.test("usage event offload - returns empty array when no usage segments exist", async () => {
  const bucket = new MockObjectStoreBinding();
  const events = await getUsageEventsFromR2(bucket as never, "no-run");
  assertEquals(events, []);
});
Deno.test("usage event offload - decompresses and parses events from multiple segments", async () => {
  restoreUsageEventsDeps();
  const bucket = new MockObjectStoreBinding();
  ...
  usageEventsDeps.gzipDecompressToString = mocks
    .gzipDecompressToString as typeof usageEventsDeps.gzipDecompressToString;
...
Deno.test("usage event offload - respects maxEvents option", async () => {
  restoreUsageEventsDeps();
  const bucket = new MockObjectStoreBinding();
...
Deno.test("usage event offload - clamps maxEvents to at least 1", async () => {
  restoreUsageEventsDeps();
  const bucket = new MockObjectStoreBinding();
...
Deno.test("usage event offload - skips malformed lines during parsing", async () => {
  restoreUsageEventsDeps();
  const bucket = new MockObjectStoreBinding();
...
Deno.test("usage event offload - skips entries missing required fields", async () => {
  restoreUsageEventsDeps();
  const bucket = new MockObjectStoreBinding();
...
Deno.test("usage event offload - skips entries with non-finite units", async () => {
  restoreUsageEventsDeps();
  const bucket = new MockObjectStoreBinding();
