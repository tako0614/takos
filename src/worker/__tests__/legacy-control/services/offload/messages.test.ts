...
Deno.test("message offload key - builds the expected key", () => {
  assertEquals(
    messageR2Key("thread-1", "msg-1"),
    "threads/thread-1/messages/msg-1.json",
  );
});
Deno.test("message offload key - handles ids containing special characters", () => {
...
// ---------------------------------------------------------------------------
// write/read offloaded messages
// ---------------------------------------------------------------------------

Deno.test("message offload - writes a JSON-serialised message and returns the key", async () => {
...
Deno.test("message offload - reads and parses a stored message", async () => {
  const bucket = new MockObjectStoreBinding();
  const payload: PersistedMessage = {
    id: "msg-2",
    thread_id: "thread-2",
...
Deno.test("message offload - returns null for missing key", async () => {
  const bucket = new MockObjectStoreBinding();
  const result = await readMessageFromR2(bucket as never, "nonexistent");
  assertEquals(result, null);
});
Deno.test("message offload - returns null for invalid JSON", async () => {
  const bucket = new MockObjectStoreBinding();
  await bucket.put("bad-key", "not json");
  const result = await readMessageFromR2(bucket as never, "bad-key");
  assertEquals(result, null);
});
Deno.test("message offload - returns null when parsed object is missing required fields", async () => {
  const bucket = new MockObjectStoreBinding();
...
Deno.test("message offload - returns null when id field is missing", async () => {
  const bucket = new MockObjectStoreBinding();
  await bucket.put(
    "no-id",
...
Deno.test("message offload - returns null when content field is missing", async () => {
  const bucket = new MockObjectStoreBinding();
  await bucket.put(
    "no-content",
