...
Deno.test(
  "WfpClient - rejects with parent-signal reason when signal is pre-aborted",
  async () => {
    try {
      // Real native fetch synchronously rejects with AbortError when the
      // signal is already aborted; the mock mirrors that contract.
      ...
      await assertRejects(
        async () => {
          await client.fetch("/cancelled", {}, { signal: controller.signal });
        },
        Error,
        "user-cancelled",
      );
...
Deno.test(
  "WfpClient - mid-call abort surfaces parent reason (not timeout)",
  async () => {
    try {
      // fetch resolves only after the (composed) signal aborts. This mirrors a
      // long-running Cloudflare API call cancelled mid-flight.
      const fetchMock = spy((_url: string, init?: RequestInit) => {
      ...
      const controller = new AbortController();
      const client = new WfpClient(config);
      // Use a generous timeout so the rejection is unambiguously from the
      // parent signal, not the internal 600s budget.
      ...
      queueMicrotask(() => controller.abort(new Error("deploy-cancelled")));

      await assertRejects(async () => await pending, Error, "deploy-cancelled");
    } finally {
      restoreFetch();
    }
  },
