...
function installFetchMock(fetchMock: FetchLike): () => void {
  const originalFetch = globalThis.fetch;
  (globalThis as { fetch?: FetchLike }).fetch = fetchMock;
  const restore = () => {
    if (originalFetch) {
...
Deno.test("WFPService - createWorker - sends PUT request with FormData containing runtime script and metadata", async () => {
  try {
...
Deno.test("WFPService - createWorker - serializes vectorize bindings into runtime metadata", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
      mockSuccessResponse({})
    );
    ...
    const [, init] = fetchMock.calls[0].args as [
      string,
      RequestInit,
    ];
...
Deno.test("WFPService - createWorker - serializes message queue and analytics bindings into runtime metadata", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
      mockSuccessResponse({})
    );
...
Deno.test("WFPService - createWorker - serializes workflow bindings into runtime metadata", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
      mockSuccessResponse({})
    );
...
Deno.test("WFPService - listWorkers - returns runtime records from API", async () => {
  try {
    const workers = [
      {
        id: "w1",
...
Deno.test("WFPService - createD1Database - returns the SQL database uuid from API response", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
...
Deno.test("WFPService - createR2Bucket - sends POST request to create object store", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
      mockSuccessResponse(null)
    );
...
Deno.test("WFPService - listD1Tables - extracts table names from SQL query result", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
      mockSuccessResponse([{
...
Deno.test("WFPService - uploadToR2 - sends PUT request to object-store endpoint", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
...
Deno.test("WFPService - getR2Object - reads object bytes from the object-store endpoint", async () => {
  try {
    const fetchMock = spy(async (..._args: Parameters<typeof fetch>) =>
      new Response("hello", {
        status: 200,
    ...
    const svc = new WFPService(config);
    const result = await svc.r2.getR2Object(
      "my-bucket",
      "path/to/file.txt",
    );
...
Deno.test("WFPService - getR2Object - returns null when the object does not exist", async () => {
  try {
    ...
    const svc = new WFPService(config);
    await assertEquals(
      await svc.r2.getR2Object("bucket", "missing.txt"),
      null,
    );
  } finally {
    restoreFetchMock();
...
Deno.test("getTakosWorkerScript - throws when runtime bundle is missing from object store", async () => {
...
Deno.test("getTakosWorkerScript - returns runtime script content from object store", async () => {
  const bucket = new MockObjectStoreBinding();
