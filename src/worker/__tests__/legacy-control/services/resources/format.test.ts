import {
  ...
  toApiResourceAccess,
  toApiServiceBinding,
} from "@/services/resources/format";
...
Deno.test("toApiResource - maps internal resource to API format", () => {
  ...
  assertEquals(result.backing_resource_id, "cf-123");
  assertEquals(result.backing_resource_name, "my-db");
  assertEquals(result.config, '{"implementation":"d1"}');
...
Deno.test("toApiResource - handles null fields", () => {
  ...
  assertEquals(result.type, "object-store");
  assertEquals(result.capability, "object_store");
  assertEquals(result.implementation, "r2");
  assertEquals(result.backing_resource_id, null);
  assertEquals(result.backing_resource_name, null);
