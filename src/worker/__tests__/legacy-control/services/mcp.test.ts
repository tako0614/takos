...
// Widened stub signature: the dep expects `(db: SqlDatabaseLike) =>
// Database`; tests only need to override the return value, so we accept any
// args here and let the call site re-narrow when assigning to the dep.
type GetDbStub = (...args: never[]) => object;
const mocks: { getDb: GetDbStub } = {
  getDb: noopDep<GetDbStub>("mcp.getDb"),
};

function setMcpAuthSecretGetDb(impl: () => object): void {
  mcpAuthSecretDeps.getDb = impl as typeof mcpAuthSecretDeps.getDb;
}
...
function makeDb(): SqlDatabaseBinding {
  const stmt = {
    bind: () => stmt,
    all: async () => ({ results: [] }),
...
function syncMcpDeps() {
  mcpServiceDeps.getDb = mocks.getDb as typeof mcpServiceDeps.getDb;
}
...
// ---------------------------------------------------------------------------
// MCP publication auth.bearer.secretRef
// ---------------------------------------------------------------------------
...
Deno.test("readPublicationAuthSecretRef - normalizes bearer secretRef", () => {
  assertEquals(
    readPublicationAuthSecretRef({
      publication: {
        name: "tools",
        publisher: "web",
        type: "McpServer",
        auth: { bearer: { secretRef: " mcp_auth_token " } },
      } as AppPublication,
    }),
    "MCP_AUTH_TOKEN",
  );
...
Deno.test("resolvePublicationAuthToken - reads bearer token from service env", async () => {
  ...
  setMcpAuthSecretGetDb(() => drizzleMock);
  try {
...
Deno.test("resolvePublicationAuthToken - falls back to portable secret binding", async () => {
  ...
  mcpAuthSecretDeps.getPortableSecretValue = async () =>
    "portable-secret-token";
