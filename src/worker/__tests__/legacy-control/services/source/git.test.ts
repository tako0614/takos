...
function createBucketMock(value?: string | null): ObjectStoreBinding {
  return {
    ...noopObjectStoreBinding(),
    get: () =>
      Promise.resolve(
        value === null || value === undefined
          ? null
          : makeObjectStoreObjectBody(value),
      ),
...
Deno.test("GitService.log - returns empty array when no commits", async () => {
  const service = new GitService(
    createFakeSqlDatabaseBinding([{ all: [] }]),
    createBucketMock(),
  );
...
Deno.test("GitService.getCommit - returns null when commit not found", async () => {
  const service = new GitService(
    createFakeSqlDatabaseBinding([{ get: null }]),
    createBucketMock(),
  );
...
Deno.test("GitService.restore - returns failure for deleted files", async () => {
  const service = new GitService(
    createFakeSqlDatabaseBinding([{ get: { changeType: "deleted" } }]),
    createBucketMock(),
  );
