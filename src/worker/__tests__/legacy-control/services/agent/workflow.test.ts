...
const mocks: {
  ...
  now: AnyMockFn;
  callRuntimeRequest: AnyMockFn;
  buildPRDiffText: AnyMockFn;
  SnapshotManager: AnyMockFn;
  logError: AnyMockFn;
} = {
  LLMClient: noopDep("workflowDeps.LLMClient"),
  getDb: noopDep("workflowDeps.getDb"),
  generateId: noopDep("workflowDeps.generateId"),
  now: noopDep("workflowDeps.now"),
  callRuntimeRequest: noopDep("workflowDeps.callRuntimeRequest"),
  buildPRDiffText: noopDep("workflowDeps.buildPRDiffText"),
  SnapshotManager: noopDep("workflowDeps.SnapshotManager"),
  logError: noopDep("workflowDeps.logError"),
};
...
function setMockLLMClient(
  ...
  workflowDeps.LLMClient = ClassMock as typeof workflowDeps.LLMClient;
}
...
Deno.test("executeCodeChangeWorkflow - returns success for a simple code change without PR", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.generateId = () => "gen-id";
  mocks.now = () => "2025-01-01T00:00:00Z";
  const plan: TaskPlan = {
    type: "code_change",
    tools: ["file_write"],
    needsRepo: false,
...
Deno.test("executeCodeChangeWorkflow - handles errors gracefully and marks steps as failed", async () => {
  ...
  const context: WorkflowContext = {
    env: { DB: noopSqlDatabaseBinding() } as Env,
    spaceId: "ws-1",
    userId: "user-1",
    threadId: "thread-1",
    runId: "run-1",
...
Deno.test("executeReview - throws when PR not found", async () => {
  ...
  const context: WorkflowContext = {
    env: { OPENAI_API_KEY: "key", DB: noopSqlDatabaseBinding() } as Env,
    spaceId: "ws-1",
    userId: "user-1",
    threadId: "thread-1",
    runId: "run-1",
...
Deno.test("orchestrateWorkflow - returns tool steps for tool_only plan type", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.generateId = () => "step-id";
  workflowDeps.generateId = mocks.generateId as typeof workflowDeps.generateId;
  const mockChat = async () => ({
    content: JSON.stringify({
      type: "tool_only",
