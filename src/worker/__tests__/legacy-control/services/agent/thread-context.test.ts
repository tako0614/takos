...
/**
 * Build a `ThreadContextEnv`-shaped env from partial overrides. Tests only
 * exercise the AI/VECTORIZE bindings; unused fields are left to the caller.
 */
function makeThreadContextEnv(
  overrides: { AI?: unknown; VECTORIZE?: unknown },
): Parameters<typeof queryRelevantThreadMessages>[0]["env"] {
  return overrides as Parameters<typeof queryRelevantThreadMessages>[0]["env"];
}
...
Deno.test("queryRelevantThreadMessages - returns empty array when AI or VECTORIZE is missing", async () => {
  const result = await queryRelevantThreadMessages({
    env: makeThreadContextEnv({ AI: undefined, VECTORIZE: undefined }),
    spaceId: "ws-1",
    threadId: "thread-1",
    query: "test query",
    topK: 5,
...
Deno.test("queryRelevantThreadMessages - returns empty array for empty query", async () => {
  const result = await queryRelevantThreadMessages({
    env: makeThreadContextEnv({ AI: {}, VECTORIZE: {} }),
    spaceId: "ws-1",
    threadId: "thread-1",
    query: "   ",
    topK: 5,
...
Deno.test("queryRelevantThreadMessages - queries vector store and filters by minScore", async () => {
  ...
  const result = await queryRelevantThreadMessages({
    env: makeThreadContextEnv({ AI: mockAI, VECTORIZE: mockVectorize }),
    spaceId: "ws-1",
    threadId: "thread-1",
    query: "test query",
    topK: 5,
