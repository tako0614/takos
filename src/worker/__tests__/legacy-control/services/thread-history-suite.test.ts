...
function createQueuedDrizzle(responses: QueuedResponse[]): Database {
...
Deno.test("getThreadHistory - assembles a parent-centric root snapshot across child threads", async () => {
  ...
  threadHistoryDeps.getDb = () => db;
  threadHistoryDeps.listThreadMessages = (async (
    ..._args: Parameters<typeof threadHistoryDeps.listThreadMessages>
  ) => ({
    messages: [
      {
        id: "msg-1",
    ...
    total: 1,
    runs: [],
  })) as typeof threadHistoryDeps.listThreadMessages;
