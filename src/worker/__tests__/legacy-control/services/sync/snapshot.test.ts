...
function makeMockEnv(overrides: Partial<Env> = {}): Env {
  return {
    DB: {} as never,
    TENANT_SOURCE: new MockObjectStoreBinding() as never,
    ...overrides,
  } as Env;
}
...
Deno.test("SnapshotManager - createSnapshot - stores compressed tree in object store, inserts snapshot row, and increases blob refcount", async () => {
  ...
  const stored = await env.TENANT_SOURCE!.get(snapshot.tree_key);
...
Deno.test("SnapshotManager - writeBlob - writes blob to object store and inserts DB row", async () => {
  const drizzle = createDrizzleMock();
...
Deno.test("SnapshotManager - createBlobFetcher - returns null when blob is not found in object store", async () => {
  const mgr = makeManager();
  const fetcher = mgr.createBlobFetcher();
