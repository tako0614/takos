...
function makeManager(
  options: {
    ...
    storageOverride?: MockObjectStoreBinding | undefined;
  } = {},
) {
  const env = {
    DB: db,
    RUNTIME_HOST: {
      fetch: noopDep<(...args: never[]) => Promise<Response>>(
        "RUNTIME_HOST.fetch",
      ),
    },
...
const db = new MockSqlDatabaseBinding();
const storage = new MockObjectStoreBinding();
    ...
    depsOverrides: {
      callRuntimeRequest: (async () =>
        new Response(
          ...
          { status: 200 },
        )) as RuntimeSessionDeps["callRuntimeRequest"],
    },
  });
...
Deno.test("RuntimeSessionManager - getWorkDir - returns session directory path on success", async () => {
  const mgr = makeManager({
    depsOverrides: {
      callRuntimeRequest: (async () =>
        new Response("{}", { status: 200 })) as RuntimeSessionDeps[
          "callRuntimeRequest"
        ],
    },
  });
...
Deno.test("RuntimeSessionManager - getWorkDir - returns null on failed response", async () => {
  const mgr = makeManager({
    depsOverrides: {
      callRuntimeRequest: (async () =>
        new Response("error", { status: 500 })) as RuntimeSessionDeps[
          "callRuntimeRequest"
        ],
    },
  });
...
Deno.test("RuntimeSessionManager - syncSnapshotToRepo - returns error when storage bucket is not configured", async () => {
  ...
  assertStringIncludes(
    result.error ?? "",
    "object-store bucket not configured",
  );
});
...
Deno.test("RuntimeSessionManager - destroySession - calls runtime destroy endpoint without throwing", async () => {
  const mgr = makeManager({
    depsOverrides: {
      callRuntimeRequest: (async () =>
        new Response("ok", { status: 200 })) as RuntimeSessionDeps[
          "callRuntimeRequest"
        ],
    },
  });
