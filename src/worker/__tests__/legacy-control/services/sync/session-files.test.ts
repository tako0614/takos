...
function makeManager(
  storage: MockObjectStoreBinding,
...
Deno.test("SessionFilesManager - writeFile - computes hash and size, writes blob to object store, and inserts session_file", async () => {
  storage = new MockObjectStoreBinding();
  const drizzle = makeDrizzle();
...
Deno.test("SessionFilesManager constructor - creates a SessionFilesManager instance", () => {
  const mgr = makeManager(new MockObjectStoreBinding());
