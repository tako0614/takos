...
Deno.test("run-failure-events helpers - persists run.failed event when offload is disabled", async () => {
  ...
  try {
    ...
    const inserted = valuesCapture.calls[0].args[0] as {
      runId: string;
      type: string;
      data: string;
      createdAt: string;
...
Deno.test("run-failure-events helpers - skips SQL event persistence when offload is enabled", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const originalGetDb = runFailureEventDeps.getDb;
...
Deno.test("run-failure-events helpers - notifies run notifier with event_id when present", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const fetchSpy = spy(
    async (_request: Request): Promise<Response> => new Response("ok"),
  );
  const runNotifier = {
    idFromName: () => ({ toString: () => "run-3" }),
    get: () => ({ fetch: fetchSpy }),
  };
  const env = createMockEnv({ RUN_NOTIFIER: runNotifier });
  ...
  const request = fetchSpy.calls[0].args[0] as Request;
