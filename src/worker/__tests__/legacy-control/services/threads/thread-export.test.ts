...
const mocks: { getDb: AnyMockFn; logError: AnyMockFn } = {
  getDb: noopDep("threadExportDeps.getDb"),
  // logError is exercised indirectly by error-path tests; default to a
  // silent stub instead of `noopDep` so unrelated tests don't have to
...
type MockDb = SqlDatabaseBinding;
...
Deno.test("exportThread - returns null when thread is not found", async () => {
  ...
  mocks.getDb = () => buildDrizzleMock({ threadGet: undefined });
...
Deno.test("exportThread - exports thread as JSON with correct headers", async () => {
  ...
  mocks.getDb = () =>
    buildDrizzleMock({ threadGet: thread, messagesAll: msgs });
...
Deno.test("exportThread - returns 503 when PDF format requested without renderPdf", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const thread = makeThreadRow();
  mocks.getDb = () => buildDrizzleMock({ threadGet: thread, messagesAll: [] });
