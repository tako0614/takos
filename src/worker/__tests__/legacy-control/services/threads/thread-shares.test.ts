...
// Loose mock signatures (the underlying deps demand specific shapes; the
// indirection in `threadShareDeps.X = ((args) => mocks.X(...args)) as
// typeof threadShareDeps.X` is responsible for re-narrowing).
type AnyMockFn = (...args: unknown[]) => unknown;
const mocks: {
  getDb: AnyMockFn;
  hashPassword: AnyMockFn;
  verifyPassword: AnyMockFn;
  now: AnyMockFn;
  base64UrlEncode: AnyMockFn;
  randomUUID: AnyMockFn;
} = {
  getDb: noopDep("threadShareDeps.getDb"),
  hashPassword: noopDep("threadShareDeps.hashPassword"),
  verifyPassword: noopDep("threadShareDeps.verifyPassword"),
  now: noopDep("threadShareDeps.now"),
  base64UrlEncode: noopDep("threadShareDeps.base64UrlEncode"),
  randomUUID: noopDep("threadShareDeps.randomUUID"),
};
...
Deno.test("generateThreadShareToken - returns a string token", () => {
  ...
  mocks.base64UrlEncode = () => "abcdefghijklmnopqrstuvwx";
...
Deno.test("createThreadShare - creates a public thread share", async () => {
  ...
  mocks.now = () => "2026-03-01T00:00:00.000Z";
  mocks.base64UrlEncode = () => "generated-token-abc";
  mocks.randomUUID = () => "generated-uuid-1";
...
Deno.test("createThreadShare - creates a password-protected share", async () => {
  ...
  mocks.hashPassword = async (...args: unknown[]) => {
...
Deno.test("listThreadShares - returns mapped share records", async () => {
  ...
  const result = await listThreadShares(
...
Deno.test("revokeThreadShare - returns true when revocation succeeds", async () => {
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.now = () => "2026-03-02T00:00:00.000Z";
...
Deno.test("getThreadShareByToken - returns share record with password_hash for valid token", async () => {
  ...
  const result = await getThreadShareByToken(
    noopSqlDatabaseBinding(),
    "token-abc123",
  );
...
Deno.test("markThreadShareAccessed - updates lastAccessedAt on the share", async () => {
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.now = () => "2026-03-05T00:00:00.000Z";
  let updateCalls = 0;
  const drizzle = {
  ...
  await markThreadShareAccessed(noopSqlDatabaseBinding(), "share-1");
...
Deno.test("verifyThreadShareAccess - returns forbidden when password is incorrect", async () => {
  ...
  const drizzle = buildDrizzleMock({ selectGet: row });
  mocks.getDb = () => drizzle;
  mocks.verifyPassword = async () => false;
...
Deno.test("verifyThreadShareAccess - succeeds with correct password on password-protected share", async () => {
  ...
  mocks.getDb = () => drizzle;
  mocks.verifyPassword = async () => true;
