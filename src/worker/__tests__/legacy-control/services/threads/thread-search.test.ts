...
const mocks: {
  getDb: AnyMockFn;
  queryRelevantThreadMessages: AnyMockFn;
  logWarnCalls: unknown[][];
  logWarn: (...args: unknown[]) => void;
} = {
  getDb: noopDep("threadSearchDeps.getDb"),
  queryRelevantThreadMessages: noopDep(
...
function makeEnv(options: { ai?: boolean; vectorize?: boolean } = {}): Env {
  ...
  if (options.ai) {
    env.AI = {
      run: () => Promise.resolve({ data: [[0.1, 0.2, 0.3]] }),
    } satisfies Env["AI"];
  }
  if (options.vectorize) {
    env.VECTORIZE = {
      ...
      insert: () =>
        Promise.reject(new Error("VectorizeIndex.insert not stubbed")),
      upsert: () =>
        Promise.reject(new Error("VectorizeIndex.upsert not stubbed")),
      deleteByIds: () =>
        Promise.reject(new Error("VectorizeIndex.deleteByIds not stubbed")),
    } satisfies Env["VECTORIZE"];
  }
  return env as Env;
}
...
Deno.test("searchSpaceThreads - deduplicates results across keyword and semantic results", async () => {
  ...
  env.AI!.run = (async () => ({ data: [[0.1, 0.2]] })) as NonNullable<
    Env["AI"]
  >["run"];
  ...
  })) as NonNullable<Env["VECTORIZE"]>["query"];
...
Deno.test("searchSpaceThreads - skips deleted threads from semantic results", async () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  const env = makeEnv({ ai: true, vectorize: true });
  env.AI!.run = (async () => ({ data: [[0.1]] })) as NonNullable<
    Env["AI"]
  >["run"];
  env.VECTORIZE!.query = (async () => ({
    matches: [{
