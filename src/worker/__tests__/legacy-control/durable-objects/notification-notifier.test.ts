...
Deno.test("NotificationNotifierDO - /emit - broadcasts to connected WebSocket clients", async () => {
  ...
  (doInstance.connections as Map<string, unknown>).set(
    "conn-1",
    ws,
  );
  ...
  // Canonical WS envelope: generic clients pin against this shape across
  // notifier subclasses (run + notification), so created_at and event_id
  // must always be present.
  assertEquals(typeof sent.eventId, "number");
  assertEquals(typeof sent.event_id, "string");
  assertEquals(sent.event_id, String(sent.eventId));
  assertEquals(typeof sent.created_at, "string");
  assert(!Number.isNaN(Date.parse(sent.created_at)));
});
Deno.test("NotificationNotifierDO - /emit - removes failed WebSocket connections on broadcast", async () => {
  ...
  const connections = doInstance.connections;
...
Deno.test("NotificationNotifierDO - WebSocket handling - rejects WebSocket connections with mismatched user ID", async () => {
  const state = createMockState();
  // Pre-populate persisted state so the DO loads with userId = "user-1"
  state.storage._store.set("bufferState", {
    eventBuffer: [],
    eventIdCounter: 0,
    userId: "user-1",
  });
  const { doInstance } = createDO(state);
  // Wait for the DO's blockConcurrencyWhile-wrapped load to settle.
...
Deno.test("NotificationNotifierDO - webSocketMessage - responds to ping with pong", async () => {
  ...
  await doInstance.webSocketMessage(ws, "ping");
...
Deno.test("NotificationNotifierDO - webSocketClose - removes connection from map on close", async () => {
  const { doInstance } = createDO();
  const connections = doInstance.connections as Map<string, unknown>;

  const ws = new MockWebSocket();
  ws.connectionId = "conn-close";
  connections.set("conn-close", ws);

  await doInstance.webSocketClose(ws);
