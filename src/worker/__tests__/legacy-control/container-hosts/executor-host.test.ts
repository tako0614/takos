...
function createEnv(overrides: Partial<Record<string, unknown>> = {}) {
  ...
  const env = {
    ...
    TAKOS_AGENT_CONTROL_RPC_BASE_URL: "https://executor-host.example",
...
Deno.test("executorHost dispatch rejects missing runId", async () => {
  ...
  const response = await executorHost.fetch(
    new Request("http://localhost/dispatch", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ serviceId: "svc-1", workerId: "svc-1" }),
    }),
    env as never,
  );
...
Deno.test("executorHost proxy rejects missing auth headers", async () => {
  const { env } = createEnv();
  const response = await executorHost.fetch(
    createProxyRequest("/api/internal/v1/agent-control/api-keys", {
...
Deno.test("executorHost rejects removed binding proxy paths", async () => {
  ...
  const response = await executorHost.fetch(
    createProxyRequest("/proxy/heartbeat", {
  ...
  assertEquals(tier1.getLastToken(), null);
});
...
Deno.test("executorHost control RPC forwards to TAKOS_WORKER", async () => {
  const { env, controlRequests, tier1 } = createEnv();
  const response = await executorHost.fetch(
    createProxyRequest("/api/internal/v1/agent-control/api-keys", {
      bearerToken: "control-token",
...
Deno.test("executorHost heartbeat touches the active control token", async () => {
  const { env, tier1 } = createEnv();
  const response = await executorHost.fetch(
    createProxyRequest("/api/internal/v1/agent-control/heartbeat", {
      bearerToken: "control-token",
      executorContainerId: "tier1-warm-0",
      body: { runId: "run-1", serviceId: "svc-1" },
    }),
...
Deno.test("executorHost revokes the control token after terminal status update", async () => {
  const { env, tier1 } = createEnv();
  const response = await executorHost.fetch(
    createProxyRequest("/api/internal/v1/agent-control/update-run-status", {
      bearerToken: "control-token",
      executorContainerId: "tier1-warm-0",
      body: {
        runId: "run-1",
...
Deno.test("executorHost keeps the control token after non-terminal status update", async () => {
  const { env, tier1 } = createEnv();
  const response = await executorHost.fetch(
    createProxyRequest("/api/internal/v1/agent-control/update-run-status", {
      bearerToken: "control-token",
