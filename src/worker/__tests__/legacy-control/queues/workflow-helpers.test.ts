...
import type { WorkflowEngine } from "../../../application/services/execution/workflow-engine.ts";
...
function makeRuntimeEnv(
  ...
  return {
    ...
    RUNTIME_HOST: { fetch: async (req) => await fetchImpl(req) },
  };
}
...
Deno.test("runtimeJson - sends a POST request with space_id and parses JSON", async () => {
  ...
  const env = makeRuntimeEnv(fetch);
  ...
  const request = fetch.calls[0]?.args[0];
...
Deno.test("runtimeDelete - swallows 404s and failures", async () => {
  ...
  const env404 = makeRuntimeEnv(fetch404);
  const env500 = makeRuntimeEnv(fetch500);
...
Deno.test("getStepDisplayName - prefers explicit names and falls back cleanly", () => {
  assertEquals(getStepDisplayName({ name: "Build" } as Step, 1), "Build");
  assertEquals(
    getStepDisplayName({ uses: "actions/checkout@v4" } as Step, 1),
    "actions/checkout@v4",
  );
  assertEquals(getStepDisplayName({ run: "echo ok" } as Step, 1), "echo ok");
  assertEquals(getStepDisplayName({} as Step, 3), "Step 3");
});
...
Deno.test("failJobWithResults - calls engine.onJobComplete with failure conclusion", async () => {
  // `failJobWithResults` only invokes `onJobComplete` on the engine. We build a
  // minimal stub that returns void and structurally satisfies the WorkflowEngine
  // contract via the Pick used at the call site.
  const onJobComplete = spy(
    (
      _jobId: Parameters<WorkflowEngine["onJobComplete"]>[0],
      _result: Parameters<WorkflowEngine["onJobComplete"]>[1],
    ) => Promise.resolve(),
  );
  const engine: Pick<WorkflowEngine, "onJobComplete"> = { onJobComplete };
  ...
  assertEquals(onJobComplete.calls.length, 1);
  assertEquals(onJobComplete.calls[0]?.args[0], "job-1");
});
