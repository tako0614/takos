...
// Minimal env stub: `executeStep` only reads `RUNTIME_HOST`. Other DbEnv /
// queue bindings throw if invoked so accidental use surfaces as a clear error.
const failingDb = (): SqlDatabaseBinding => {
  const fail = (m: string) => () => {
    throw new Error(`DB.${m} should not be called in workflow-steps tests`);
  };
  return {
...
const failingDo = (): DurableObjectNamespace => ({
  idFromName: () => ({}),
  get: () => {
    throw new Error("DurableObjectNamespace.get should not be called");
  },
});
...
Deno.test("executeStep - executes a run step via runtime", async () => {
  ...
  const ctx = createContext({
    env: makeQueueEnv({ RUNTIME_HOST: { fetch: recorder.fetch } }),
  });
