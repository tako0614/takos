...
/**
 * Map every method on `JobQueueContext["engine"]` to a `Spy` typed with the
 * same parameters and return value. This lets the spy-based test mock be
 * passed directly as `engine` (it satisfies the call signatures) while still
 * exposing `.calls`, `.original`, etc. for `assertSpyCalls` assertions.
 */
type SpiedEngine = {
  [K in keyof JobQueueContext["engine"]]: JobQueueContext["engine"][K] extends
    (...args: infer Args) => infer Return ? Spy<unknown, Args, Return>
    : JobQueueContext["engine"][K];
};
...
function createEngine(overrides: Partial<SpiedEngine> = {}): SpiedEngine {
  type Engine = JobQueueContext["engine"];
  const base: SpiedEngine = {
    startRun: spy<
      unknown,
      Parameters<Engine["startRun"]>,
      ReturnType<Engine["startRun"]>
    >(
      (..._args) =>
        Promise.resolve(
          { id: "run-1" } as Awaited<ReturnType<Engine["startRun"]>>,
        ),
    ),
    enqueueJob: spy<
      unknown,
      Parameters<Engine["enqueueJob"]>,
      ReturnType<Engine["enqueueJob"]>
    >(
      (..._args) => Promise.resolve(),
    ),
    onJobStart: spy<
      unknown,
      Parameters<Engine["onJobStart"]>,
      ReturnType<Engine["onJobStart"]>
    >(
      (..._args) => Promise.resolve(),
    ),
    onJobComplete: spy<
      unknown,
      Parameters<Engine["onJobComplete"]>,
      ReturnType<Engine["onJobComplete"]>
    >((..._args) => Promise.resolve()),
    updateStepStatus: spy<
      unknown,
      Parameters<Engine["updateStepStatus"]>,
      ReturnType<Engine["updateStepStatus"]>
    >((..._args) => Promise.resolve()),
    storeJobLogs: spy<
      unknown,
      Parameters<Engine["storeJobLogs"]>,
      ReturnType<Engine["storeJobLogs"]>
    >((..._args) => Promise.resolve("artifact-1")),
    cancelRun: spy<
      unknown,
      Parameters<Engine["cancelRun"]>,
      ReturnType<Engine["cancelRun"]>
    >(
      (..._args) => Promise.resolve(),
    ),
    createArtifact: spy<
      unknown,
      Parameters<Engine["createArtifact"]>,
      ReturnType<Engine["createArtifact"]>
    >((..._args) => Promise.resolve({ id: "artifact-1", r2Key: "artifact-1" })),
  };
...
Deno.test("handleJobSkipped - completes the job when the condition evaluates to false", async () => {
  ...
  const skippedResult = engine.onJobComplete.calls[0]?.args[1];
...
Deno.test("completeJobSuccess - persists evaluated job outputs", async () => {
  ...
  assertSpyCalls(engine.storeJobLogs, 1);
  assertSpyCalls(engine.onJobComplete, 1);
  const completedResult = engine.onJobComplete.calls[0]?.args[1];
...
Deno.test("completeJobSuccess - reports success for continue-on-error jobs", async () => {
  ...
  assertSpyCalls(engine.onJobComplete, 1);
  const continueOnErrorResult = engine.onJobComplete.calls[0]?.args[1];
...
Deno.test("completeJobFailure - swallows storeJobLogs failures", async () => {
  const engine = createEngine({
    storeJobLogs: spy<
      unknown,
      Parameters<JobQueueContext["engine"]["storeJobLogs"]>,
      ReturnType<JobQueueContext["engine"]["storeJobLogs"]>
    >((..._args) => {
...
Deno.test("completeJobFailure - rethrows persistence failures", async () => {
  const engine = createEngine({
    onJobComplete: spy<
      unknown,
      Parameters<JobQueueContext["engine"]["onJobComplete"]>,
      ReturnType<JobQueueContext["engine"]["onJobComplete"]>
    >((..._args) => {
