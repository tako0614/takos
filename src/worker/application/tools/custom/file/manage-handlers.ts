...
export const fileDeleteHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === "rejected") {
    logWarn(`object-store backup delete failed for ${path}`, {
      module: "tools/custom/file/manage-handlers",
      detail: r2Result.reason,
    });
  }

  if (runtimeResult.status === "rejected") {
    logError("Runtime delete failed", runtimeResult.reason, {
      module: "tools/custom/file/manage-handlers",
    });
    throw new Error(`Failed to delete file ${path}: ${runtimeResult.reason}`, {
      cause: runtimeResult.reason,
    });
  }
...
export const fileRenameHandler: ToolHandler = async (args, context) => {
  ...
  const r2KeyOld =
    `session-files/${context.spaceId}/${context.sessionId}/${oldPath}`;
  const r2KeyNew =
    `session-files/${context.spaceId}/${context.sessionId}/${newPath}`;
  ...
  if (context.storage) {
    try {
      ...
      logWarn(`object-store backup rename write failed for ${newPath}`, {
        module: "tools/custom/file/manage-handlers",
        detail: err,
      });
    }
    try {
      ...
      logWarn(`object-store backup rename delete failed for ${oldPath}`, {
        module: "tools/custom/file/manage-handlers",
        detail: err,
      });
    }
