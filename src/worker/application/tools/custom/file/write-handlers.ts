...
export const fileWriteHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === "rejected") {
    logWarn(`object-store backup write failed for ${path}`, {
  ...
  if (runtimeResult.status === "rejected") {
    ...
    throw new Error(`Failed to write file ${path}: ${runtimeResult.reason}`, {
...
export const fileWriteBinaryHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === "rejected") {
    logWarn(`object-store backup binary write failed for ${path}`, {
      module: "tools/custom/file/write-handlers",
      detail: r2Result.reason,
    });
  }
  ...
  if (runtimeResult.status === "rejected") {
    ...
    throw new Error(
      `Failed to write binary file ${path}: ${runtimeResult.reason}`,
      { cause: runtimeResult.reason },
    );
  }
...
export const fileCopyHandler: ToolHandler = async (args, context) => {
  ...
  if (context.storage) {
    try {
      ...
      logWarn(`object-store backup copy write failed for ${destPath}`, {
        module: "tools/custom/file/write-handlers",
        detail: err,
      });
    }
