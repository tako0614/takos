...
import {
  ...
  getPathsForClaim,
  searchClaims,
} from "../../services/memory-graph/claim-store.ts";
...
export const memoryGraphRecallHandler: ToolHandler = async (args, context) => {
  ...
  switch (mode) {
    ...
    case "evidence":
      return handleEvidenceMode(context.db, context.spaceId, claimId, limit);
    default:
...
async function handleClaimsMode(
  ...
  const lines = claims.map((c, i) => {
    ...
    }] ${c.subject} ${c.predicate} ${c.object}${status} (id: ${
      c.id.slice(0, 8)
    }...)`;
  });
...
async function handlePathSearchMode(
  db: SqlDatabaseBinding,
  accountId: string,
  ...
  const lines = paths.map((p, i) =>
    `${i + 1}. ${
      p.pathRelations.join(" -> ")
    } (${p.hopCount} hops, confidence: ${p.minConfidence.toFixed(2)})`
  );

  return `Found ${paths.length} paths:\n\n${lines.join("\n")}`;
}

async function handleEvidenceMode(
  db: SqlDatabaseBinding,
  accountId: string,
  claimId: string | undefined,
  limit: number,
  ...
  const evidence = await getEvidenceForClaim(db, accountId, claimId, limit);
  ...
  const lines = evidence.map((e, i) => {
    ...
    const kindIcon = e.kind === "supports"
      ? "+"
      : e.kind === "contradicts"
      ? "-"
      : "~";
