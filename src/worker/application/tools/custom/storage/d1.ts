...
export const SQL_TABLES: ToolDefinition = {
  name: "sql_tables",
...
export const SQL_DESCRIBE: ToolDefinition = {
  name: "sql_describe",
...
function hasValidWhereClause(
  ...
  if (operation === "delete") {
    ...
    const updateMatch = normalizedSql.match(
      /update\s+\S+\s+set\s+[\s\S]+\s+where\s+/,
    );
...
export const sqlQueryHandler: ToolHandler = async (args, context) => {
  ...
  if (!startsWithAllowed) {
    ...
    if (
      normalizedSql.includes("delete from") || normalizedSql.includes("delete ")
    ) {
  ...
  if (statementCount > 1) {
    throw new Error(
      "Multiple SQL statements not allowed. Execute one query at a time.",
    );
  }
  ...
  if (
    normalizedSql.startsWith("select") || normalizedSql.startsWith("explain")
  ) {
  ...
  return `Query executed successfully.\nChanges: ${affectedRowCount(result)}`;
};

export const sqlTablesHandler: ToolHandler = async (_args, context) => {
  ...
  const tables = items.filter((item: { name: string; type: string }) =>
    item.type === "table"
  );
  const views = items.filter((item: { name: string; type: string }) =>
    item.type === "view"
  );
  ...
  if (tables.length > 0) {
    ...
    output += tables.map((table: { name: string; type: string }) =>
      `  - ${table.name}`
    ).join("\n");
  }
  if (views.length > 0) {
    ...
    output += views.map((view: { name: string; type: string }) =>
      `  - ${view.name}`
    ).join("\n");
  }
...
export const sqlDescribeHandler: ToolHandler = async (args, context) => {
  ...
  if (!columns.results || columns.results.length === 0) {
    throw new Error(
      `Could not retrieve column info for table: ${safeName} (PRAGMA table_info returned ${
        columns.results ? "empty rows" : "null results"
      })`,
    );
  }
