...
function requireContainer(context: ToolContext): void {
  if (!context.sessionId) {
    throw new Error(
      buildContainerUnavailableMessage(context, "using runtime_exec"),
    );
  }
}
...
export const runtimeExecHandler: ToolHandler = async (args, context) => {
  ...
  const timeout = Math.min(
    (args.timeout as number) || DEFAULT_TOOL_TIMEOUT_SECONDS,
...
export const runtimeStatusHandler: ToolHandler = async (args, context) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to get status for runtime ${runtimeId}: ${response.status} - ${error}`,
    );
  }
