  ...
  parameters: {
    ...
    properties: {
      ...
      timeout_ms: {
        type: "number",
        description:
          `How long to wait in milliseconds (optional, default: ${DEFAULT_WAIT_TIMEOUT_MS}, max: ${MAX_WAIT_TIMEOUT_MS})`,
      },
    },
    required: ["run_id"],
  },
...
async function loadChildRun(
  ...
  if (!childRun) {
    throw new Error(
      `Child run not found or not owned by this parent run: ${childRunId}`,
    );
  }
...
function buildWaitAgentResponse(
  result: { run: ChildRunRow; artifacts: ArtifactSummary[] },
  timedOut: boolean,
): string {
...
export const spawnAgentHandler: ToolHandler = async (args, context) => {
  ...
  try {
    const parsed = safeJsonParseOrDefault<unknown>(
      parentThread?.keyPoints ?? "[]",
      [],
    );
  ...
  const { packet, observability } = buildDelegationPacket({
    ...
    productHint: typeof args.product_hint === "string"
      ? args.product_hint
      : null,
  ...
  if (!childThread) {
    throw new Error(
      "Cannot spawn sub-agent: child thread could not be created",
    );
  }
