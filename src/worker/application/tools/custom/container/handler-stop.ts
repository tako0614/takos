...
export const containerStopHandler: ToolHandler = async (args, context) => {
  ...
  const result = await db.update(sessions).set({
    ...
    .where(
      and(eq(sessions.id, context.sessionId), eq(sessions.status, "running")),
    );
  ...
  if (context.env.RUNTIME_HOST) {
    try {
      ...
      logError("Failed to destroy runtime session", e, {
        module: "tools/custom/container/handler-stop",
      });
    }
  }

  const contextStorage = context.storage;
  if (contextStorage && context.sessionId) {
    try {
      const prefix = `session-files/${context.spaceId}/${context.sessionId}/`;
      const listed = await contextStorage.list({ prefix });
      if (listed.objects && listed.objects.length > 0) {
        await Promise.all(
          listed.objects.map((obj: { key: string }) =>
            contextStorage.delete(obj.key)
          ),
        );
        logInfo(
          `Cleaned up ${listed.objects.length} object-store session files on stop`,
          { module: "tools/custom/container/handler-stop" },
        );
      }
    } catch (e) {
      logError("Failed to cleanup object-store session files", e, {
        module: "tools/custom/container/handler-stop",
      });
    }
  }
