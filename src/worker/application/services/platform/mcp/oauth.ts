...
export async function discoverOAuthMetadata(
  ...
  if (!meta.issuer || !meta.authorization_endpoint || !meta.token_endpoint) {
    throw new Error(
      "OAuth metadata missing required fields (issuer, authorization_endpoint, token_endpoint)",
    );
  }
...
export async function createMcpOAuthPending(
  ...
  const expiresAt = new Date(clock.now() + 10 * 60 * 1000).toISOString(); // 10 minutes
...
export async function refreshMcpToken(
  ...
  const masterSecret = env.ENCRYPTION_KEY;
  if (!masterSecret || !server.oauthRefreshToken || !server.oauthIssuerUrl) {
    return;
  }
  ...
  try {
    ...
    if (!tokenResp.ok) {
      logError(
        `Token refresh failed for server ${server.id}: ${tokenResp.status}`,
        undefined,
        { module: "mcp" },
      );
      return;
    ...
    logError(`Token refresh error for server ${server.id}`, err, {
      module: "mcp",
    });
  }
}
