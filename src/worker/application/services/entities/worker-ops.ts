/**
 * Workload entity operations for the control plane.
 *
 * Records workload deployment intent in the canonical services table and deletes
 * provider service scripts when removing managed workload entities.
 *
 * Runs inside the control runtime without subprocess or CLI access.
 */
...
export async function deployWorker(
  ...
  if (!opts.skipUpload) {
    throw new Error(
      "Service entity deploy requires a real worker artifact from the deployment pipeline or pass skipUpload",
    );
  }
