...
export async function deleteResource(
  ...
  try {
    ...
    // The real resource may already have been deleted externally; record
    // the failure but continue to drop the DB row so the canonical state
    // converges. A leaked backing resource is preferable to a stuck entry.
    logWarn(`Failed to delete managed resource for "${name}"`, {
      module: "entities.resource_ops",
