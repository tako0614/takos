...
import * as takosGit from "../takos-git/index.ts";
...
export async function resolveConflictsAndMerge(
  ...
  const baseBranchRef = await takosGit.getBranch(
    dbBinding,
    repoId,
    baseBranch,
  );
  const headBranchRef = await takosGit.getBranch(
    dbBinding,
    repoId,
    headBranch,
  );
...
export async function checkConflicts(
  ...
  const baseBranch = await takosGit.getBranch(db, repoId, baseBranchName);
  const headBranch = await takosGit.getBranch(db, repoId, headBranchName);
  ...
  const detailedConflicts = await Promise.all(
    mergeResult.conflicts.map(async (conflict) => {
      const [baseBlob, oursBlob, theirsBlob] = await Promise.all([
        takosGit.getBlobAtPath(bucket, baseCommit.tree, conflict.path).catch(
          () => null,
        ),
        takosGit.getBlobAtPath(bucket, localCommit.tree, conflict.path).catch(
          () => null,
        ),
        takosGit.getBlobAtPath(bucket, incomingCommit.tree, conflict.path)
          .catch(() => null),
      ]);
