...
/**
 * Narrow subset of {@link RunTerminalStatus} used by the run lifecycle's
 * auto-close path. `cancelled` runs are excluded because the auto-close
 * implementation discards changes the same way as `failed`, so callers
 * collapse the cancellation case onto `"failed"` before invoking it.
 */
export type AutoCloseStatus = Exclude<RunTerminalStatus, "cancelled">;
...
export const TERMINAL_STATUS_BY_EVENT_TYPE: Readonly<
  Record<RunTerminalEventType, RunTerminalStatus>
> = {
  completed: "completed",
  error: "failed",
  cancelled: "cancelled",
