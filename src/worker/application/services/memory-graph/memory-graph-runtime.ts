...
export class AgentMemoryRuntime {
  ...
  async bootstrap(): Promise<ActivationResult> {
    ...
    try {
      ...
      const [evidenceCounts, pathsArrays] = await Promise.all([
        countEvidenceForClaims(this.db, this.context.spaceId, claimIds),
        Promise.all(
          topClaims.map((c) =>
  ...
  private async flushOverlay(): Promise<void> {
    ...
    // Persist overlay atomically. Previously a crash mid-loop left the DB
    // in a partially-persisted state with `overlay.clear()` skipped, so the
    // next run would replay the surviving overlay entries and produce
    // duplicates (insertEvidence is not idempotent). Wrapping the persist
    // in a single transaction makes it all-or-nothing: any error rolls
    // back the writes and `overlay.clear()` is skipped, leaving the
    // overlay intact for the caller to handle.
    const tx = new D1TransactionManager(this.db);
