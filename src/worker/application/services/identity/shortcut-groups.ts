...
// Standardize on the package-wide ID generator (`generateId`) instead of
// nanoid so this module matches the rest of control. `generateId` is the
// documented canonical generator (crypto-random, a-z0-9, unbiased).
...
export async function createShortcutGroup(
  ...
  if (!created) {
    throw new Error(
      `createShortcutGroup invariant violated: group ${id} not found after insert`,
    );
  }
...
export async function updateShortcutGroup(
  ...
  if (!updated) {
    throw new Error(
      `updateShortcutGroup invariant violated: group ${groupId} not found after update`,
    );
  }
