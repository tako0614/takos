...
//
// This integration targeted the retired Takosumi Installer API (the
// 5-endpoint `install` / `deploy` model behind `@takos/takosumi-installer`).
// That surface no longer exists: Takosumi is now an OpenTofu-native deploy
// control plane where installs are driven by Installation + PlanRun / ApplyRun
// against a runner profile, which is not a synchronous install-and-return
// facade and is not orchestrated from the Takos app today.
//
// The dual-write was always opt-in via Installer API env config that was unset
// in every configured environment, so this path already returned `null` /
// `false` at runtime; the corresponding `TAKOSUMI_INSTALLER_*` Env fields have
...
export async function createTakosumiInstallation(
  ...
  // Retired Installer API integration; OpenTofu-native re-wiring is not yet
...
export async function deleteTakosumiInstallation(
  ...
  // Retired Installer API integration; no-op until the OpenTofu-native deploy
