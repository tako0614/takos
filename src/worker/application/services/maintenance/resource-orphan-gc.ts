...
/**
 * Garbage-collect resources orphaned by manifest updates.
 * Resources are marked as orphaned (orphaned_at set) when removed from a manifest during update.
 * After the grace period, this job permanently deletes the provider resource and DB record.
 */
export async function gcOrphanedResources(
  ...
  const cutoffTime = new Date(clock.now() - gracePeriodMs).toISOString();
