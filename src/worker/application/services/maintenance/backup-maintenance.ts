...
async function resolveD1DatabaseUuid(
  ...
  if (!match?.uuid) {
    throw new Error(`SQL database not found: ${databaseName}`);
  }
  return match.uuid;
}
...
async function exportD1DatabaseSql(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    if (poll.status === "complete") {
      const signedUrl = poll.result?.signed_url
        ...
        : "";
      if (!signedUrl) {
        throw new Error("SQL export completed but no signed_url was returned");
      }
      return {
    ...
    if (poll.status === "error") {
      throw new Error(poll.error || "SQL export failed");
    }
  ...
  throw new Error("SQL export did not complete within polling budget");
}
...
export async function runD1DailyBackup(
  ...
  if (env.CF_ACCOUNT_ID && env.CF_API_TOKEN) {
    ...
    if (!download.ok) {
      throw new Error(
        `SQL export download failed (${download.status}): ${download.statusText}`,
      );
    }
...
export async function runD1BackupIntegrityCheck(
  ...
  if (expected && expected !== sha256) {
    throw new Error(
      `SQL backup integrity check failed: sha256 mismatch for ${key}`,
    );
  }
