    ...
    try {
      ...
      if (domain.status === "ssl_pending") {
        ...
        if (hostnameStatus && hostnameStatus.sslStatus === "active") {
          // SSL provisioned — promote to active and ensure kv store routing
          await db.update(serviceCustomDomains)
            .set({
              status: "active",
              sslStatus: "active",
          ...
          // SSL failed/expired — mark as ssl_failed and remove kv store routing
          await db.update(serviceCustomDomains)
            .set({
              status: "ssl_failed",
              sslStatus: "failed",
...
export async function reconcileStuckDomains(
  ...
  const oneHourAgo = new Date(clock.now() - thresholdMs).toISOString();
