/**
 * Commit creation, SQL store indexing, and DAG traversal.
 *
...
function indexedToCommit(row: CommitIndexRow): GitCommit {
  const parentShasRaw = row.parentShas;
  return {
    sha: row.sha,
    tree: row.treeSha,
    parents: parentShasRaw
      ? (() => {
        try {
          return JSON.parse(parentShasRaw) as string[];
        } catch {
          return [];
        }
      })()
...
/**
 * Index an existing commit (received via push) into SQL store.
 */
