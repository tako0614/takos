/**
 * Git ref management (branches & tags) — SQL store with Drizzle + fallback.
 *
...
// Accept both SHA-1 (40) and SHA-256 (64) Git object ids.
const COMMIT_SHA_PATTERN = /^[a-f0-9]{40}([a-f0-9]{24})?$/i;

const BRANCH_COLUMNS =
  `id, repo_id, name, commit_sha, is_default, is_protected, created_at, updated_at`;
const TAG_COLUMNS =
  `id, repo_id, name, commit_sha, message, tagger_name, tagger_email, created_at`;
...
async function withDrizzleFallback<T>(
  ...
  try {
    ...
    // Fall back to SQL store raw queries for Drizzle adapter issues:
    // - Invalid array buffer length (large payload)
    // - SQL store transient errors (SQLITE_BUSY, timeout)
    if (
...
function branchFromDrizzle(b: SelectOf<typeof branches>): GitBranch {
  return {
    ...
    created_at: textDate(b.createdAt),
    updated_at: textDate(b.updatedAt),
  };
}
...
function toTag(row: D1TagRow): GitTag {
  return {
    ...
    commit_sha: row.commit_sha,
    message: row.message,
    tagger_name: row.tagger_name,
    tagger_email: row.tagger_email,
    created_at: row.created_at,
  };
}
...
function tagFromDrizzle(t: SelectOf<typeof tags>): GitTag {
  return {
    ...
    message: t.message,
    tagger_name: t.taggerName,
    tagger_email: t.taggerEmail,
    created_at: textDate(t.createdAt),
  };
}
...
export async function getBranch(
  ...
  return withDrizzleFallback(
    ...
    async () => {
      const row = await dbBinding.prepare(
        `SELECT ${BRANCH_COLUMNS} FROM branches WHERE repo_id = ? AND name = ? LIMIT 1`,
      )
...
export async function getBranchesByNames(
  dbBinding: SqlDatabaseBinding,
  repoId: string,
  names: string[],
  ...
  const result = await withDrizzleFallback(
    async () => {
      const rows = await db.select().from(branches)
        .where(
          and(eq(branches.repoId, repoId), inArray(branches.name, validNames)),
        )
        .all();
...
export async function getDefaultBranch(
  ...
  return withDrizzleFallback(
    ...
    async () => {
      const row = await dbBinding.prepare(
        `SELECT ${BRANCH_COLUMNS} FROM branches WHERE repo_id = ? AND is_default = 1 LIMIT 1`,
      )
...
export async function listBranches(
  ...
  return withDrizzleFallback(
    ...
    async () => {
      const result = await dbBinding.prepare(
        `SELECT ${BRANCH_COLUMNS} FROM branches WHERE repo_id = ? ORDER BY is_default DESC, name ASC`,
      )
...
export async function createBranch(
  ...
  if (!isValidRepoId(repoId) || !isValidRefName(name)) {
  ...
  try {
    ...
    if (existing) {
      return {
        success: false,
        current: existing.commit_sha,
        error: "Branch already exists",
      };
    }
    throw new Error(
      `Failed to create branch '${name}' on repo ${repoId} at ${commitSha}: ${
...
export async function updateBranch(
  ...
  if (
    (oldSha !== null && !isValidCommitSha(oldSha)) || !isValidCommitSha(newSha)
  ) return { success: false, error: "Invalid commit SHA" };
  ...
  if (result.length === 0) {
    ...
    return {
      success: false,
      current: current.commit_sha,
      error: "Ref conflict: branch was modified by another process",
    };
  }
...
export async function deleteBranch(
  ...
  await db.delete(branches).where(
    and(eq(branches.repoId, repoId), eq(branches.name, name)),
  );
  return { success: true };
}
...
export async function setDefaultBranch(
  ...
  const db = getDb(dbBinding);
  await db.update(branches).set({ isDefault: false }).where(
    eq(branches.repoId, repoId),
  );
  await db.update(branches).set({ isDefault: true }).where(
    and(eq(branches.repoId, repoId), eq(branches.name, name)),
  );
  return { success: true };
}
...
export async function getTag(
  ...
  return withDrizzleFallback(
    ...
    async () => {
      const row = await dbBinding.prepare(
        `SELECT ${TAG_COLUMNS} FROM tags WHERE repo_id = ? AND name = ? LIMIT 1`,
      )
...
export async function listTags(
  ...
  return withDrizzleFallback(
    ...
    async () => {
      const result = await dbBinding.prepare(
        `SELECT ${TAG_COLUMNS} FROM tags WHERE repo_id = ? ORDER BY created_at DESC`,
      )
...
export async function createTag(
  ...
  try {
    ...
    if (existingNow) {
      return {
        success: false,
        current: existingNow.commit_sha,
        error: "Tag already exists",
      };
    }
    throw new Error(
      `Failed to create tag '${name}' on repo ${repoId} at ${commitSha}: ${
        cause instanceof Error ? cause.message : String(cause)
      }`,
      { cause },
    );
...
export async function deleteTag(
  ...
  await db.delete(tags).where(
    and(eq(tags.repoId, repoId), eq(tags.name, name)),
  );
  return { success: true };
}
...
export async function listAllRefs(
  ...
  Array<{ name: string; target: string; type: "branch" | "tag" | "remote" }>
> {
  const [branchList, tagList] = await Promise.all([
    listBranches(dbBinding, repoId),
    listTags(dbBinding, repoId),
  ]);
