/**
 * Remote Git protocol client — fetch remote refs.
 *
 * Speaks the client side of the git smart HTTP info/refs protocol for import
 * fallback code. Takos-owned repository hosting lives in the takos-git service.
 *
 * Usage:
...
export async function fetchRemoteRefs(
  ...
  if (!response.ok) {
    const body = await response.text().catch((err) => {
      logWarn("Failed to read error response body from remote git server", {
        module: "fetch-refs",
    ...
    throw new Error(
      `Failed to fetch refs from ${normalizedUrl}: HTTP ${response.status}${
