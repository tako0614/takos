/**
 * Remote Git protocol client — fetch pack (clone/fetch).
 *
 * Speaks the client side of the git upload-pack protocol for import fallback
 * code. Takos-owned repository hosting lives in the takos-git service.
 *
 * The returned packfile bytes can be passed directly to `readPackfileAsync`
 * from `protocol/packfile-reader.ts` to unpack objects into object store.
 */
