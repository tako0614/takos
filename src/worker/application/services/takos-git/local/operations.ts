...
import type { GitBranch, GitCommit, GitSignature } from "./git-objects.ts";
import {
  createBranch,
  getBranch,
  getDefaultBranch,
...
import { countCommitsBetween, createCommit } from "./core/commit-index.ts";
import { applyTreeChanges, createEmptyTree } from "./core/tree-ops.ts";
import { getCommitData, putBlob } from "./core/object-store.ts";
import {
  branches,
  getDb,
  repoForks,
...
export async function initRepository(
  ...
  if (!branch) {
    throw new Error(
      `Failed to create default branch '${defaultBranch}' for repo ${repoId} (commit ${commit.sha})`,
    );
  }
...
export async function forkRepository(
  ...
  const sourceBranches = await db.select().from(branches).where(
    eq(branches.repoId, sourceRepoId),
  ).all();
  const sourceTags = await db.select().from(tags).where(
    eq(tags.repoId, sourceRepoId),
  ).all();
...
export async function checkSyncStatus(
  ...
  const db = getDb(dbBinding);
  const fork = await db.select().from(repoForks).where(
    eq(repoForks.forkRepoId, forkRepoId),
  ).get();
  ...
  const defaultBranchName = branchName ||
    (await getDefaultBranch(dbBinding, forkRepoId))?.name || "main";
  const forkBranch = await getBranch(dbBinding, forkRepoId, defaultBranchName);
  const upstreamBranch = await getBranch(
    dbBinding,
    fork.upstreamRepoId,
    defaultBranchName,
  );
  ...
  const { ahead, behind, has_merge_base } = await countCommitsBetween(
    dbBinding,
    bucket,
    forkRepoId,
...
export async function commitFile(
  ...
  if (!updateResult.success) {
    throw new Error(
      `Failed to update branch '${branchName}' on repo ${repoId} from ${branch.commit_sha} to ${commit.sha}: ${
        updateResult.error ?? "unknown reason"
      }`,
    );
  }
