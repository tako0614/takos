...
import {
  insertRerunWithDerivedRunAttempt,
  insertRunWithDerivedRunNumber,
} from "./run-number.ts";
...
async function safeEnqueueFirstPhaseJobs(options: {
  queue: MessageQueueBinding<WorkflowJobQueueMessage> | undefined;
  workflow: Workflow;
  workflowPath: string;
  ...
  try {
    ...
    logError(`Failed to enqueue ${logLabel}`, err, {
      module: "services/workflow-runs/commands",
    });
  }
}
...
export async function dispatchWorkflowRun(
  ...
  if (!bucket) {
    return {
      ok: false as const,
      status: 500 as const,
      error: "Git storage not configured",
    };
  }
  if (!env.WORKFLOW_QUEUE) {
    return {
      ok: false as const,
      status: 500 as const,
      error: "Workflow queue not configured",
    };
  }

  const resolved = await resolveWorkflowBlob(
    env.DB,
    bucket,
    params.repoId,
    params.refName,
  ...
  const existingWorkflow = await db.select({ id: workflows.id })
    ...
    .where(
      and(
        eq(workflows.repoId, params.repoId),
        eq(workflows.path, params.workflowPath),
      ),
    )
    .get();

  // run_number is a per-(repo, workflow) monotonic counter derived as
  // MAX(run_number)+1. Two concurrent dispatches can read the same MAX and
  // compute the same run_number; the
  // idx_workflow_runs_repo_path_number_attempt unique index (migration 0079)
  // rejects the losing insert with a UNIQUE constraint violation. Re-derive
  // MAX and retry so the loser is assigned the next free number instead of
  // failing the request.
  const runNumber = await insertRunWithDerivedRunNumber({
    db,
...
export async function cancelWorkflowRun(
  ...
  const run = await db.select()
    ...
    .get();
  if (!run) {
    return { ok: false as const, status: 404 as const, error: "Run not found" };
  }
  if (run.status === "completed" || run.status === "cancelled") {
    return {
      ok: false as const,
      status: 400 as const,
      error: "Run is already completed or cancelled",
    };
  }
  ...
  const runningJobs = await db.select({ id: workflowJobs.id })
    ...
    .where(
      and(
        eq(workflowJobs.runId, params.runId),
  ...
  // Guard the run UPDATE with a non-terminal status predicate so a cancel
  // racing a concurrent completion cannot clobber a freshly-finalized terminal
  // state. The pre-read 400 above handles the common case; this predicate
  // closes the SELECT-then-UPDATE window where finalizeRunIfComplete flips the
  // run to completed after the pre-read. Mirrors the sibling cancelRun guard in
  // workflow-run-lifecycle.ts; on a lost race the UPDATE matches zero rows and
  // the cancel becomes a no-op on the run row.
  ...
  if (runningJobs.length > 0 && env.RUNTIME_HOST) {
    for (const job of runningJobs) {
      try {
        await callRuntimeRequest(env, `/actions/jobs/${job.id}`, {
          method: "DELETE",
        });
      } catch (err) {
        logWarn(`Failed to cancel runtime job ${job.id}`, {
          module: "services/workflow-runs/commands",
          detail: err,
        });
      }
...
export async function rerunWorkflowRun(
  ...
  if (run.status !== "completed" && run.status !== "cancelled") {
    return {
      ok: false as const,
      status: 400 as const,
      error: "Can only re-run completed or cancelled runs",
    };
  }
  ...
  const refValue = run.ref?.replace("refs/heads/", "") || run.ref ||
    params.defaultBranch || "main";
  // run.ref is already the prefixed form from a prior run; only re-prefix the
  // null case so the persisted/returned ref matches dispatchWorkflowRun and the
  // read-model branch filter (refs/heads/<branch>).
  const storedRef = run.ref ?? `refs/heads/${refValue}`;
  const commitSha = run.sha ||
    await gitStore.resolveRef(env.DB, params.repoId, refValue);
  if (!commitSha) {
    return {
      ok: false as const,
      status: 404 as const,
      error: "Ref not found for workflow rerun",
    };
  }
  ...
  // Reruns reuse run_number and increment run_attempt. The
  // idx_workflow_runs_repo_path_number_attempt unique index (migration 0079)
  // rejects a concurrent double-rerun that derives the same run_attempt;
  // re-derive MAX(run_attempt)+1 and retry on the run_attempt axis so the loser
  // gets the next free attempt instead of a 500.
  const newAttempt = await insertRerunWithDerivedRunAttempt({
    db,
    repoId: params.repoId,
