...
export async function insertResource(
  ...
  const drizzle = resourceStoreDeps.getDb(db);
  const configWithImpl = input.implementation
    ? { ...(input.config || {}), implementation: input.implementation }
    : (input.config || {});
  await drizzle.insert(resources).values({
    ...
    config: JSON.stringify(configWithImpl),
    metadata: "{}",
...
export async function markResourceDeleting(
  db: SqlDatabaseBinding,
  resourceId: string,
) {
  const drizzle = resourceStoreDeps.getDb(db);
