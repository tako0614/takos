...
const PROCESSING_LEASE_TTL_MS: TtlMs = ttlMs(5 * 60_000);
const STALE_PROCESSING_FALLBACK_MS: TtlMs = ttlMs(15 * 60_000);
...
export class CommonEnvReconcileJobStore {
  ...
  async markProcessing(jobId: string): Promise<boolean> {
    ...
    const leaseExpiresAt = processingLeaseExpiresAt(this.clock.now());
  ...
  async markRetry(
    ...
    const nextAttemptAt = new Date(this.clock.now() + backoffMs(attempts))
      .toISOString();
    await db
      .update(t)
      .set({
  ...
  private async listStaleProcessing(
    ...
    const fallbackCutoff = staleProcessingCutoff(nowMs);
    return db
      .select({
        id: t.id,
        attempts: t.attempts,
