...
/**
 * Snapshot lifecycle status. Distinct from the run / deployment status
 * unions: the success terminal is `"complete"` (not `"completed"`) and
 * there is no `"cancelled"` state — sync snapshots are immutable once
 * the writer commits them.
 */
