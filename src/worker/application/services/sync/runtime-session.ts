 ...
 * Sessions use the Git store (SQL store + object store) for file management.
 *
 * Flow:
...
export class RuntimeSessionManager {
  ...
  constructor(
    ...
    private storage: ObjectStoreBinding | undefined,
  ...
  async initSession(
    ...
    try {
      ...
      if (this.repositories.length > 0) {
        ...
        if (!this.repoId) {
          throw new Error(
            "initSession invariant violated: repoId must be set when repositories is empty",
          );
        }
        repos = [{
          repoId: this.repoId,
  ...
  private async _doInitSessionFromRepos(
    ...
    if (!bucket) {
      throw new Error(
        "object-store bucket not configured (need GIT_OBJECTS or TENANT_SOURCE)",
      );
    }
