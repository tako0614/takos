...
/**
 * Handles object store (TENANT_SOURCE) storage operations for snapshot trees and blobs.
 */
export class SnapshotStorage {
  ...
  /** Build the object store key for a snapshot tree. */
  buildTreeKey(snapshotId: string): string {
    return `trees/${this.spaceId}/${snapshotId}.json.gz`;
  }

  /** Put a compressed tree into object store and return the key. */
  async putTree(
    snapshotId: string,
    tree: SnapshotTree,
  ...
  /** Get a tree from object store, decompressing if needed. Returns null if not found. */
  ...
  /** Delete a tree from object store. */
  ...
  /** Build the object store key for a blob. */
  private buildBlobKey(hash: string): string {
    return `blobs/${this.spaceId}/${hash}`;
  }

  /** Check if a blob exists in object store. */
  ...
  /** Get blob content from object store, with integrity verification. Returns null if not found or integrity check fails. */
  ...
  /** Delete blobs from object store by their hashes. */
