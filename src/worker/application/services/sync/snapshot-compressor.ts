    ...
    try {
      while (true) {
        ...
        if (totalDecompressedSize > this.maxDecompressedSize) {
          reader.cancel();
          throw new Error(
            `Decompression bomb detected: decompressed size exceeds ${
              this.maxDecompressedSize / 1024 / 1024
            }MB limit`,
          );
        }
        ...
        if (
          currentRatio > this.compressionRatioLimit && compressedSize > 1024
        ) {
          reader.cancel();
          throw new Error(
            `Decompression bomb detected: compression ratio ${
              currentRatio.toFixed(0)
            }:1 exceeds limit of ${this.compressionRatioLimit}:1`,
          );
        }
      ...
      try {
        ...
        logWarn("Failed to release reader lock (non-critical)", {
          module: "snapshot-compressor",
