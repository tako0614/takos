/**
 * Deployment orchestration methods for the Cloudflare provider adapter.
 *
 * Coordinates high-level tenant provisioning workflows: fetching a runtime
 * bundle from the object store, mapping generic binding descriptors into
 * Cloudflare provider binding records, and deploying the assembled runtime
 * into the dispatch namespace. Also provides the canonical SQL migration for
 * new tenants.
 */
...
/**
 * Deploy a tenant runtime with bindings from a bundle URL or pre-built script.
 */
export async function deployWorkerWithBindings(
  ...
  if (options.bundleScript) {
    ...
    // 60s caps a stalled bundle fetch (deploy critical path) without
    // truncating realistic transfer times for large bundles.
...
/**
 * Get the Takos tenant runtime bundle.
 *
 * Priority:
 * 1. object-store binding (required)
 *
 * Embedded fallback is intentionally disabled to avoid silently deploying
 * stale runtime bundles that drift from yurucommu source.
 */
  ...
  if (!env.WORKER_BUNDLES) {
    throw new InternalError(
      "WORKER_BUNDLES is not configured. " +
        "Provisioning requires an explicit runtime bundle in the object store.",
    );
  }

  const object = await env.WORKER_BUNDLES.get("worker.js");
  if (!object) {
    throw new NotFoundError("runtime bundle is missing in WORKER_BUNDLES");
  }
  try {
    return await object.text();
  } catch (e) {
    throw new InternalError(
      `Failed to read runtime bundle: ${
...
/**
 * Get the SQL migration for the Takos tenant database.
 */
