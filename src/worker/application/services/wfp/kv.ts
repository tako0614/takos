/**
 * kv store methods for the Cloudflare provider adapter.
 *
 * Manages Cloudflare provider kv stores that are bound to tenant runtimes.
 * Provides creation and deletion of kv stores via the Cloudflare API.
 */
...
export async function createKVNamespace(
  ...
  if (!response.result?.id) {
    throw new InternalError(
      `Failed to create KV store namespace: no ID returned from API`,
    );
  }
...
export async function getKVValue(
  ...
  // The KV value endpoint returns raw value, not the standard API wrapper.
...
/**
 * Delete a value from a kv store.
 */
