/**
 * Tenant-runtime methods for the Cloudflare provider adapter.
 *
 * Handles creation, deletion, listing, existence checks, settings management,
 * and WASM-module deployment of tenant runtimes within a Cloudflare dispatch
...
/**
 * Create or update a tenant runtime in the dispatch namespace.
 */
...
/**
 * Create an assets upload session for a tenant runtime.
 */
...
/**
 * Delete a tenant runtime from the dispatch namespace.
 */
...
/**
 * Check if a tenant runtime exists.
 * @returns true if present, false if 404, throws on other errors
 */
...
/**
 * List all tenant runtimes in the dispatch namespace.
 */
...
/**
 * Update tenant runtime settings (bindings, environment variables, limits).
 * This updates provider metadata without changing the script.
 */
...
/**
 * Create or update a tenant runtime with WASM module support.
 * Used for deployments that require WASM modules (like yurucommu).
 */
export async function createWorkerWithWasm(
  ...
  options: {
    ...
    assetsJwt?: string;
    /**
     * Optional caller signal. When aborted mid-call, the underlying
     * Cloudflare HTTP fetch is torn down and retries stop.
     */
    signal?: AbortSignal;
  },
): Promise<void> {
