/**
 * Cloudflare provider adapter service.
 *
 * Manages tenant runtime deployment, resource binding, and lifecycle.
 * Uses the Cloudflare API to interact with dispatch namespaces.
 *
...
function abortReasonToError(signal: AbortSignal, fallback: string): Error {
...
function waitForRetryDelay(
  ...
  if (signal.aborted) {
    return Promise.reject(
      abortReasonToError(signal, "Cloudflare API retry delay aborted"),
    );
  }
...
function buildWfpContext(config: WFPConfig, client: WfpClient): WfpContext {
  ...
  async function cfFetch<T>(
    ...
    return client.fetch<T>(path, options, { timeoutMs, signal });
  }
  ...
  async function cfFetchWithRetry<T>(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        return await cfFetch<T>(path, options, timeoutMs, signal);
      } catch (error) {
        lastError = error as Error;
        // If the caller's signal aborted, propagate immediately without
        // retrying — retrying after a user-requested cancellation defeats the
        // purpose of mid-call cancellation.
        if (signal?.aborted) {
          throw error;
        }
...
function bindSubmodules(ctx: WfpContext, config: WFPConfig, client: WfpClient) {
  return {
    workers: {
      createWorker: (
        options: Parameters<typeof workerOps.createWorker>[1],
    ...
    queues: {
      ...
      upsertQueueConsumerByQueueName: (
        ...
        options?: Parameters<typeof queueOps.upsertQueueConsumerByQueueName>[3],
      ) =>
        queueOps.upsertQueueConsumerByQueueName(ctx, queueName, input, options),
    },
    vectorize: {
...
// ---------------------------------------------------------------------------
// WFPService class -- minimal facade over context-bound WFP operations
// ---------------------------------------------------------------------------
