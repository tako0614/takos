/**
 * Static-asset upload helpers for the Cloudflare provider adapter.
 *
 ...
 * and receive a completion JWT that is attached to tenant runtime deployment
 * metadata so the assets are served alongside the runtime.
 */

import type { CFAPIResponse, WFPConfig, WfpFetcher } from "./client.ts";
...
export async function uploadAssets(
  ...
  for (const [hash, file] of Object.entries(files)) {
    // Create Blob with the content type used when serving the asset.
