/**
 * Pure helper functions shared across Cloudflare provider adapter submodules.
 *
 * Contains utilities that have no side effects and no dependency on the
 * Cloudflare API client: SQL table-name sanitisation, provider SQL result
 * extraction, and metadata assembly for dispatch namespace deployments.
 */
...
/**
 * Extract the results array from a provider SQL query response.
 * The provider API returns Array<{ results: T[] }>; this unwraps the first
 * element.
 */
...
/**
 * Build metadata for Cloudflare dispatch namespace deployment.
 * Shared between the standard and WASM deployment paths.
 */
