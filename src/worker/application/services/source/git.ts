...
import { buildHunks } from "../../../shared/utils/unified-diff.ts";
...
export class GitService {
  private d1: SqlDatabaseLike;
  private storage: ObjectStoreBinding;

  constructor(d1: SqlDatabaseLike, storage: ObjectStoreBinding) {
    this.d1 = d1;
  ...
  async diff(
    ...
    for (const change of changes) {
      ...
      if (diff.oldContent || diff.newContent) {
        diff.hunks = this.buildFileHunks(
          diff.oldContent || "",
          diff.newContent || "",
        );
      }
  ...
  private toGitCommit(commit: {
    ...
    return {
      ...
      created_at: textDate(commit.createdAt),
    };
  }
  ...
  /**
   * Build aligned diff hunks for a single file via the shared LCS aligner.
   *
   * The LCS aligner is O(n*m) in memory, so very large blobs are guarded with
   * a naive fall-back that emits a single delete-all/add-all hunk instead of
   * materializing an (n+1)*(m+1) DP table.
   */
  private buildFileHunks(
    oldContent: string,
    newContent: string,
  ): DiffHunk[] {
    const oldLineCount = oldContent.length === 0
      ? 0
      : oldContent.split("\n").length;
    const newLineCount = newContent.length === 0
      ? 0
      : newContent.split("\n").length;

    if (oldLineCount * newLineCount > GitService.MAX_LCS_CELLS) {
      return GitService.fallbackHunks(oldContent, newContent);
    }

    return buildHunks(oldContent, newContent);
  }

  /** Upper bound on the LCS DP-table cell count before falling back. */
  private static readonly MAX_LCS_CELLS = 4_000_000;

  /**
   * Memory-safe fall-back for blobs too large for the O(n*m) LCS aligner:
   * represent the change as a whole-file delete followed by a whole-file add.
   */
  private static fallbackHunks(
    oldContent: string,
    newContent: string,
  ): DiffHunk[] {
    ...
    oldLines.forEach((content, i) => {
      lines.push({ type: "delete", content, oldLineNumber: i + 1 });
    });
    newLines.forEach((content, i) => {
      lines.push({ type: "add", content, newLineNumber: i + 1 });
    });
