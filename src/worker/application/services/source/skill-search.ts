...
export async function indexSkillVector(
  ...
  if (!embedding) {
    throw new Error(
      `Failed to generate embedding for skill ${skill.id} (space=${spaceId}, textLength=${content.length}): AI returned empty embedding`,
    );
  }
...
export async function searchSkills(
  env: { AI?: Ai; VECTORIZE?: VectorizeIndex; DB: SqlDatabaseBinding },
  ...
  const canVector = Boolean(ai && vectorize);

  let textResults: SkillSearchResult[] = [];
  if (hasTextQuery && params.query) {
  ...
  if (canVector && ai && vectorize && (hasTextQuery || params.vector)) {
    try {
