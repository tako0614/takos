...
function readConfiguredCustomSlug(
  config: { managedBy?: string; customSlug?: string },
): string | null {
  if (config.managedBy !== "group") return null;
  return slugifyGroupHostnameSegment(config.customSlug ?? "") || null;
}
...
export async function getGroupCustomSlugHostname(
  ...
  for (const row of rows) {
    ...
    const slug = readConfiguredCustomSlug(config);
    if (!slug) continue;
    return {
