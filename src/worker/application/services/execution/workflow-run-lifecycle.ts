...
import { insertRunWithDerivedRunNumber } from "../workflow-runs/run-number.ts";
...
export async function startRun(
  ...
  // Concurrent event triggers (push / pull_request / schedule) can read the
  // same MAX and collide; the idx_workflow_runs_repo_path_number_attempt unique
  // index (migration 0079) rejects the loser. insertRunWithDerivedRunNumber
  // re-derives MAX and retries on a UNIQUE constraint violation so a legitimate
  // triggered run is assigned the next free number instead of being dropped.
  await insertRunWithDerivedRunNumber({
    db: drizzle,
    repoId,
    workflowPath,
    insertValues: {
