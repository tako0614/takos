...
/**
 * Delete expired workflow artifacts (object store + DB row).
 *
 ...
 * rows accumulate in SQL store and orphan object store objects pile up indefinitely (the
  ...
  for (const row of expired) {
    if (bucket && row.r2Key) {
      try {
        ...
        errors++;
        logError(
          "Failed to delete expired workflow artifact from object store",
          err,
          {
        ...
        // make us retry the same object store delete every cron tick.
      }
    }
    try {
