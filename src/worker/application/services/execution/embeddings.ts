...
import { flattenTree, getBlob } from "../takos-git/index.ts";
...
export class EmbeddingsService {
  ...
  constructor(ai: Ai, vectorize: VectorizeIndex, db: SqlDatabaseBinding) {
    this.ai = ai;
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    if (!result.data || result.data.length === 0) {
      throw new Error(
        `Failed to generate embedding (model=${EMBEDDING_MODEL}, textLength=${text.length}): AI returned empty data`,
      );
    }
  ...
  async removeFile(spaceId: string, fileId: string): Promise<void> {
    // Look up the file's size from SQL store to derive the actual upper bound
