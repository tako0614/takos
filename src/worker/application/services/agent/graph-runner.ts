...
async function executeLangGraphEngine(
  ...
  if (!deps.openAiKey) {
    throw new Error(
      "executeLangGraphEngine invariant violated: openAiKey must be set when langgraph engine is selected",
    );
  }
  if (!deps.toolExecutor) {
    throw new Error(
      "executeLangGraphEngine invariant violated: toolExecutor must be provided",
    );
  }
...
async function dispatchSimpleLoop(deps: EngineDispatchDeps): Promise<void> {
  if (!deps.llmClient) {
    throw new Error(
      "dispatchSimpleLoop invariant violated: llmClient must be set when simple engine is selected",
    );
  }
