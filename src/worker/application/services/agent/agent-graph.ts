...
import { ServiceUnavailableError } from "@takos/worker-platform-utils/errors";
import {
  type AIMessage,
  type BaseMessage,
  SystemMessage,
...
// ── Message limits for runtime memory safety (128MB heap) ───────────────
    ...
    for (let attempt = 0; attempt < LLM_MAX_RETRIES; attempt++) {
      try {
        const response = await withTimeout(
          (timeoutSignal) => {
            const signal = abortSignal && timeoutSignal
              ? combineSignals(abortSignal, timeoutSignal)
        ...
        if (attempt < LLM_MAX_RETRIES - 1) {
          ...
          // Wake the backoff sleep early if the run is cancelled, so a
          // cancelled run does not have to wait out the full retry delay
          // (which can be up to LLM_MAX_DELAY ms = 30s per attempt).
          ...
          // After waking, re-check cancellation so the next attempt is not
          // launched against an aborted run.
          throwIfAborted(abortSignal, "langgraph-agent-node:retry-backoff");
        }
      }
    }
