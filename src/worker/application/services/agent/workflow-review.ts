...
import type { ReviewResult, WorkflowContext } from "./workflow-types.ts";
import { REVIEW_PROMPT } from "./workflow-types.ts";
import { LLMClient } from "./llm.ts";
import { getDb, prReviews, pullRequests } from "../../../infra/db/index.ts";
...
export async function executeReview(
  ...
  const pr = await db.select().from(pullRequests).where(
  ...
  try {
    ...
    logError("Failed to parse review JSON", parseError, {
      module: "services/agent/workflow-review",
    });
    reviewResult = {
      status: "commented",
...
export async function getPRDiff(
  ...
  if (!diffResult.diffText) {
    const skippedInfo = diffResult.skipped.length > 0
      ...
      : "";
    throw new Error(
      `No textual diff available for PR #${pr.number}${skippedInfo}`,
    );
  }
