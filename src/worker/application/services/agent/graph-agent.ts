/**
 * LangGraph Agent for fetch-based runtimes
 *
 * Uses @langchain/langgraph/web for web runtime compatibility.
 * Implements a ReAct-style agent with tool calling.
 *
 * This file is the public facade. Implementation is split into:
 *   - graph-tools.ts        : shared helpers, tool creation, public types
 *   - agent-graph.ts        : state definition, graph construction, agent factory
 *   - graph-checkpointer.ts : SQL-binding checkpoint persistence
 */
...
export { D1CheckpointSaver } from "./graph-checkpointer.ts";
...
import type { AgentStateType, createLangGraphAgent } from "./agent-graph.ts";
...
export async function runLangGraph(options: RunLangGraphOptions): Promise<{
  ...
  for await (
    const event of await agent.graph.stream(initialState, {
      ...config,
        ...
        await onEvent?.({
          ...
          data: {
            ...
            message: `Thinking (step ${
              output.iteration ?? lastIteration + 1
            })...`,
          },
        });
