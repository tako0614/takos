...
// Public deployment artifact exports.
...
export class DeploymentService {
  ...
  async createDeployment(input: CreateDeploymentInput): Promise<Deployment> {
    ...
    if (!isContainerDeploy) {
      if (input.bundleContent === undefined) {
        throw new BadRequestError(
          "bundleContent is required for worker-bundle deployments",
        );
      }
    ...
    const bundleHash = isContainerDeploy || bundleContent === undefined
      ? null
      : await computeSHA256(bundleContent);
    const bundleSize = isContainerDeploy || bundleContent === undefined
      ? null
      : new TextEncoder().encode(bundleContent).byteLength;
    ...
    try {
      ...
      if (!isContainerDeploy) {
        ...
        if (this.env.WORKER_BUNDLES) {
          await this.env.WORKER_BUNDLES.put(bundleR2Key, bundleContent);
      ...
      if (this.env.WORKER_BUNDLES) {
        for (const key of uploadedR2Keys) {
          try {
            ...
            logError(
              `Failed to clean up object-store artifact ${key}`,
              cleanupErr,
              {
                module: "deployment",
              },
  ...
  async resumeDeployment(
    ...
    return this.executeDeployment(deploymentId, signal);
  }
