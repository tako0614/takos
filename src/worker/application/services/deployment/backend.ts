...
import { throwIfAborted } from "@takos/worker-platform-utils/abort";

/**
 * Race a pending wfp/Cloudflare operation against a caller's cancellation
 * signal. As soon as the signal aborts the returned promise rejects with the
 * caller's reason. The underlying HTTP request is also torn down at the same
 * time because the workers-dispatch deploy callers now forward `signal` into
 * `WfpService.workers.{createWorker,createWorkerWithWasm}` and the queue-
 * consumer helpers, which thread it down to `WfpClient.fetch` via
 * `WfpFetchOptions` (composed into the actual `fetch` AbortSignal). This
 * wrapper is still useful for an immediate caller-visible rejection independent
 * of HTTP layer behavior.
 */
function raceWithSignal<T>(
    ...
    if (signal.aborted) {
      ...
      reject(
        reason instanceof Error ? reason : new Error(
          typeof reason === "string" ? reason : `${context}: aborted`,
        ),
      );
      return;
    }
  ...
  return Promise.race([operation, cancel]).finally(() => {
...
export function createWorkersDispatchDeploymentBackend(
  ...
  async function syncQueueConsumers(
    ...
    throwIfAborted(input.signal, "workers-dispatch-sync-queue-consumers");
    ...
    const signalOptions = input.signal ? { signal: input.signal } : undefined;
  ...
  return {
    ...
    async deploy(input) {
      throwIfAborted(input.signal, "workers-dispatch-deploy");
      ...
      if (input.wasmContent) {
        await raceWithSignal(
          input.signal,
          "workers-dispatch-deploy",
          wfp.workers.createWorkerWithWasm(
            input.artifactRef,
            input.bundleContent || "",
            input.wasmContent,
            {
...
async function restorePreviousQueueConsumers(input: {
  ...
  for (const consumer of input.previous) {
    ...
    await input.wfp.queues.upsertQueueConsumerByQueueName(consumer.queueName, {
      ...
      logWarn(
        "Failed to restore previous message queue consumer after sync failure",
        {
          module: "deployment",
  ...
  for (const consumer of input.desired) {
    ...
    await input.wfp.queues.deleteQueueConsumerByQueueName(consumer.queueName, {
      ...
      logWarn(
        "Failed to delete new message queue consumer after sync failure",
        {
          module: "deployment",
          queueName: consumer.queueName,
...
function resolveQueueNameForConsumer(
  ...
  if (!queueName) {
    throw new Error(
      `Message queue trigger binding "${bindingName}" does not resolve to a queue binding`,
    );
  }
  return queueName;
}
...
export function createRuntimeHostDeploymentBackend(): DeploymentBackend {
  return {
    name: "runtime-host",
    async deploy(input) {
      // runtime-host has no outgoing fetch/RPC, so there is no in-flight
      // request to abort. We still honor a pre-aborted signal to keep the
      // cancellation contract uniform across drivers.
      throwIfAborted(input.signal, "runtime-host-deploy");
      const runtime = normalizeDeployRuntime(input);
...
function createOrchestratedDeploymentBackend(
  ...
  return {
    ...
    async deploy(input) {
      throwIfAborted(input.signal, `${config.backendName}-deploy`);
      const runtime = normalizeDeployRuntime(input);
