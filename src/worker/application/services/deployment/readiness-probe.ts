...
export type ReadinessProbeOptions = {
  ...
  timeoutMs?: number;
  /**
   * Optional external AbortSignal. If aborted, the probe is cancelled and
   * `{ ok: false, reason: "timeout" }` is returned (caller treats this as a
   * fail-fast). Independent of the internal 10s timeout.
   */
...
/**
 * Workload に HTTP GET probe を送って readiness を判定する。
 *
 * - **HTTP 200 OK のみ** を ready とみなす
...
export async function probeWorkerReadiness(
  ...
  // Propagate external cancellation (e.g. deploy-wide AbortSignal) into the
  // probe's controller. Already-aborted signals fire abort() immediately.
  ...
  try {
    ...
    if (response.status === READY_STATUS_CODE) {
      // body は読まずに drain (TCP 接続を解放し、fetch handler をリーク
      // させない)。
      try {
...
export function describeReadinessFailure(
  ...
  switch (outcome.reason) {
    case "non-200":
      return `Service readiness probe failed: ${url} returned HTTP ${outcome.status} ` +
        `(only HTTP 200 is treated as ready; 201/204/3xx/4xx/5xx all fail)`;
    case "timeout":
      return `Service readiness probe failed: ${url} did not respond within ` +
        `${READINESS_PROBE_TIMEOUT_MS / 1000}s`;
    case "error":
      return `Service readiness probe failed: ${url} request error: ${outcome.error}`;
  }
}
