...
// Entrypoints that need only a subset can compose from these fragments
...
export interface ContainerHostEnv {
   ...
   * `executor-utils.ts`). Production platforms provide this as a service
   * binding; local-platform tests substitute a passthrough fetch.
   */
  TAKOS_EGRESS?: FetchBinding;
}

// ---------------------------------------------------------------------------
// Narrow Env types — used by individual runtime entrypoints that only need a
...
export interface Env extends DbEnv, StorageEnv, AiEnv, AgentConfigEnv, ContainerHostEnv {
  ...
  // executor workers omit it. Callers already cast via `as ... | undefined`
  // so optionality matches existing runtime behavior.
  SESSION_DO?: DurableNamespaceBinding;
  RUN_NOTIFIER: DurableNamespaceBinding;
  NOTIFICATION_NOTIFIER?: DurableNamespaceBinding;
  RATE_LIMITER_DO?: DurableNamespaceBinding;
  // ROUTING_DO is bound in workers that own routing (web app + workflow
  // runner) but not in pure executors. `cache.ts` / `sharding.ts` already
   ...
   * origin and OIDC redirect origin; Takosumi Accounts owns token issuing.
   */
  TAKOS_INTERNAL_API_URL?: string;
  TENANT_BASE_DOMAIN: string;
  // PLATFORM_* are wrangler secrets, present when the operator runs
  // `wrangler secret put`. Callers handle the unset case via `normalizePem`
  // and fail-closed JWT verification; declaring them optional matches that
  // runtime contract instead of pretending they are always provided.
