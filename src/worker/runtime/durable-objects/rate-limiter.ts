...
export class RateLimiterDO {
  private state: DurableObjectStateBinding;
  /**
   * Sliding-window entries keyed by rate-limit key. Exposed for Durable
   * Object tests that need to seed expired/fresh timestamps before
   * invoking alarm() or to assert eviction behavior; production callers
   * should not mutate this map outside the class methods.
   */
  readonly entries: Map<string, number[]> = new Map();
  /**
   * Token bucket state keyed by rate-limit key. Exposed for tests for the
   * same reason as `entries`; production callers should not mutate this
   * map outside the class methods.
   */
  readonly tokenBuckets: Map<string, TokenBucketState> = new Map();
    ...
    this.state.blockConcurrencyWhile(async () => {
      ...
      if (stored?.entries) {
        for (const [k, v] of Object.entries(stored.entries)) {
          this.entries.set(k, v.timestamps);
        }
      }
      if (stored?.tokenBuckets) {
        for (const [k, v] of Object.entries(stored.tokenBuckets)) {
          this.tokenBuckets.set(k, v);
        }
      }
  ...
  private async handleHit(data: RateLimitRequest): Promise<Response> {
    const { key, maxRequests, windowMs } = data;
    const algorithm: RateLimiterAlgorithm = data.algorithm ?? "sliding_window";
    const rawSampleRate = data.shadowSampleRate;
    const shadowSampleRate = typeof rawSampleRate === "number" &&
