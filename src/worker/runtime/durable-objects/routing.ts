...
export class RoutingDO {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      // All state-mutating handlers are wrapped in blockConcurrencyWhile so the
      // load -> compute version -> save read-modify-write sequence is atomic per
      // DO. Durable Objects can dispatch concurrent fetch() requests, and the
      // awaited storage read in load() yields the event loop; without
      // serialization two concurrent put/delete calls for the same hostname
      // would both read the same prev record, compute the same version, and the
      // second would silently overwrite the first (lost update). This matches
      // SessionDO / RateLimiterDO / NotifierBase.
      if (path === "/routing/get" && request.method === "POST") {
        const body = (await request.json()) as { hostname: string };
        // handleGet may evict an expired tombstone (delete + persist), so it
        // shares the same read-modify-write hazard as put/delete.
      ...
      if (path === "/routing/rollout/schedule" && request.method === "POST") {
        ...
        return await this.state.blockConcurrencyWhile(() =>
          this.handleRolloutSchedule(body)
        );
      }
      if (path === "/routing/rollout/cancel" && request.method === "POST") {
        const body = (await request.json()) as { hostname: string };
        return await this.state.blockConcurrencyWhile(() =>
          this.handleRolloutCancel(body)
        );
      }
