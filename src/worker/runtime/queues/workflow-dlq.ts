...
export async function handleWorkflowJobDlq(
  ...
  // Cancel sibling jobs and their pending steps when a job enters the DLQ.
  // Without this, sibling jobs that were queued but never started keep their
  // `queued` / `in_progress` rows forever: the run is marked `failure` below,
  // but the UI still shows "running" jobs.
  //
