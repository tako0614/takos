...
export const blobs = sqliteTable(
  "blobs",
  {
    ...
    isBinary: integer("is_binary", { mode: "boolean" })
      .notNull()
      .default(false),
...
export const branches = sqliteTable(
  ...
  (table) => ({
    uniqRepoName: uniqueIndex("idx_branches_repo_name").on(
      table.repoId,
      table.name,
    ),
...
export const commits = sqliteTable(
  ...
  (table) => ({
    uniqRepoSha: uniqueIndex("idx_commits_repo_sha").on(
      table.repoId,
      table.sha,
    ),
    ...
    idxRepoCommitDate: index("idx_commits_repo_commit_date").on(
      table.repoId,
      table.commitDate,
    ),
  }),
...
export const files = sqliteTable(
  ...
  (table) => ({
    uniqAccountPath: uniqueIndex("idx_files_account_path").on(
      table.accountId,
      table.path,
    ),
...
export const indexJobs = sqliteTable(
  "index_jobs",
  {
    ...
    error: text("error"),
    /** Stable Cloudflare Queue message id that owns a running delivery. */
...
export const prComments = sqliteTable(
  ...
  (table) => ({
    ...
    idxAuthorTypeId: index("idx_pr_comments_author_type_id").on(
      table.authorType,
      table.authorId,
    ),
  }),
...
export const prReviews = sqliteTable(
  ...
  (table) => ({
    ...
    idxReviewerTypeId: index("idx_pr_reviews_reviewer_type_id").on(
      table.reviewerType,
      table.reviewerId,
    ),
...
export const pullRequests = sqliteTable(
  ...
  (table) => ({
    uniqRepoNumber: uniqueIndex("idx_pull_requests_repo_number").on(
      table.repoId,
      table.number,
    ),
    ...
    idxAuthorTypeId: index("idx_pull_requests_author_type_id").on(
      table.authorType,
      table.authorId,
    ),
  }),
...
// 73. RepoFork
export const repoForks = sqliteTable(
  "repo_forks",
  {
    id: text("id").primaryKey(),
  ...
  (table) => ({
    idxUpstream: index("idx_repo_forks_upstream_repo_id").on(
      table.upstreamRepoId,
    ),
...
// 74. RepoReleaseAsset
export const repoReleaseAssets = sqliteTable(
  "repo_release_assets",
  {
    id: text("id").primaryKey(),
  ...
  (table) => ({
    uniqReleaseAssetKey: uniqueIndex(
      "idx_repo_release_assets_release_asset_key",
    ).on(table.releaseId, table.assetKey),
    idxRelease: index("idx_repo_release_assets_release_id").on(table.releaseId),
  }),
);

// 75. RepoRelease
export const repoReleases = sqliteTable(
  "repo_releases",
  {
    id: text("id").primaryKey(),
    repoId: text("repo_id")
  ...
  (table) => ({
    uniqRepoTag: uniqueIndex("idx_repo_releases_repo_tag").on(
      table.repoId,
      table.tag,
    ),
    idxTag: index("idx_repo_releases_tag").on(table.tag),
    idxRepo: index("idx_repo_releases_repo_id").on(table.repoId),
    idxPublishedAt: index("idx_repo_releases_published_at").on(
      table.publishedAt,
    ),
  }),
);
...
// 76. RepoRemote
export const repoRemotes = sqliteTable(
  "repo_remotes",
  {
    id: text("id").primaryKey(),
    repoId: text("repo_id")
  ...
  (table) => ({
    uniqRepoName: uniqueIndex("idx_repo_remotes_repo_name").on(
      table.repoId,
      table.name,
    ),
...
export const repositories = sqliteTable(
  "repositories",
  {
    ...
    gitEnabled: integer("git_enabled", { mode: "boolean" })
      .notNull()
      .default(false),
  ...
  (table) => ({
    uniqAccountName: uniqueIndex("idx_repositories_account_name").on(
      table.accountId,
      table.name,
    ),
    idxVisibilityUpdatedAt: index("idx_repositories_visibility_updated_at").on(
      table.visibility,
      table.updatedAt,
    ),
    idxVisibility: index("idx_repositories_visibility").on(table.visibility),
    idxPrimaryLanguage: index("idx_repositories_primary_language").on(
      table.primaryLanguage,
    ),
    idxLicense: index("idx_repositories_license").on(table.license),
    idxForkedFrom: index("idx_repositories_forked_from_id").on(
      table.forkedFromId,
    ),
    idxFeatured: index("idx_repositories_featured").on(table.featured),
    idxAccountVisibility: index("idx_repositories_account_visibility").on(
      table.accountId,
      table.visibility,
    ),
...
export const tags = sqliteTable(
  ...
  (table) => ({
    uniqRepoName: uniqueIndex("idx_tags_repo_name").on(
      table.repoId,
      table.name,
    ),
