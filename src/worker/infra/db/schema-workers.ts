...
export const bundleDeploymentEvents = sqliteTable("bundle_deployment_events", {
  ...
  idxReplacedBundleDeploymentId: index(
    "idx_bundle_deployment_events_replaced_bundle_deployment_id",
  ).on(table.replacedBundleDeploymentId),
}));
...
const deploymentTable = sqliteTable("deployments", {
  ...
  startedAt: text("started_at"),
  completedAt: text("completed_at"),
  // Cross-isolate cancellation request marker (epoch ms). Written by the
  // cancel route; polled at phase boundaries by `executeDeploymentPipeline`.
  // Cleared back to NULL on terminal state by the pipeline cleanup hook.
  ...
  // Baseline SQL creates `deployments_worker_id_created_at_idx` with DESC
  // order (`created_at DESC`). Drizzle cannot express column-level ASC/DESC
  // inside `index()`, so the physical order is determined by the migration.
...
// 100. ServiceBinding projection with a workerId field for Worker-facing code
export const serviceBindingsWithWorkerIdAlias = Object.assign(serviceBindings, {
  workerId: serviceBindings.serviceId,
});

export const workerBindings = serviceBindingsWithWorkerIdAlias;

// 101. ServiceCommonEnvLink projection with a workerId field for Worker-facing code
export const serviceCommonEnvLinksWithWorkerIdAlias = Object.assign(
  serviceCommonEnvLinks,
  {
...
export const workerCommonEnvLinks = serviceCommonEnvLinksWithWorkerIdAlias;
...
export const serviceEnvVarsWithWorkerIdAlias = Object.assign(
  serviceEnvVarsTable,
  {
...
export const workerEnvVars = serviceEnvVarsWithWorkerIdAlias;
...
// 107. ServiceRoute projection for Worker-facing code
export const servicesWithWorkerAliases = services;
export const workers = servicesWithWorkerAliases;
