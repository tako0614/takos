...
export const accountEnvVars = sqliteTable("account_env_vars", {
  ...
  uniqAccountName: uniqueIndex("idx_account_env_vars_account_id_name").on(
    table.accountId,
    table.name,
  ),
...
export const accountMemberships = sqliteTable("account_memberships", {
  ...
  uniqAccountMember: uniqueIndex("idx_account_memberships_account_member").on(
    table.accountId,
    table.memberId,
  ),
...
export const accountModeration = sqliteTable("account_moderation", {
  ...
  idxSuspendedUntil: index("idx_account_moderation_suspended_until").on(
...
export const accountSettings = sqliteTable("account_settings", {
  ...
  privateAccount: integer("private_account", { mode: "boolean" }).notNull()
    .default(false),
...
export const accountStats = sqliteTable("account_stats", {
  ...
  lastCalculatedAt: text("last_calculated_at").notNull().$defaultFn(() =>
    new Date().toISOString()
  ),
  ...updatedAtColumn,
}, (table) => ({
  idxTotalSizeBytes: index("idx_account_stats_total_size_bytes").on(
    table.totalSizeBytes,
  ),
...
export const accountStorageFiles = sqliteTable("account_storage_files", {
  ...
  uniqAccountPath: uniqueIndex("idx_account_storage_files_account_path").on(
    table.accountId,
    table.path,
  ),
  idxParent: index("idx_account_storage_files_parent_id").on(table.parentId),
  idxAccountType: index("idx_account_storage_files_account_type").on(
    table.accountId,
    table.type,
  ),
...
// 12. Account
//
// NOTE: The baseline migration `0001_baseline.sql` created two obsolete
// columns and three obsolete indexes that are *not* declared here:
...
// same baseline) and are removed by migration 0073. The local-platform
// sqlite-rewrite layer (`local-platform/d1-sql-rewrite.ts`) still filters out
// the baseline index creates while replaying the baseline. New code should use
// `authIdentities` for OAuth provider linking.
//
...
//
// NOTE: `ai_model` column default. The baseline migration
// (`db/migrations-control/migrations/0001_baseline.sql:153`) declares the column
...
// `gpt-5-mini` here is a legacy literal that exists only so drizzle does not
// emit a no-op default-change migration.
export const accounts = sqliteTable("accounts", {
  ...
  idxDefaultRepo: index("idx_accounts_default_repository_id").on(
    table.defaultRepositoryId,
  ),
}));
