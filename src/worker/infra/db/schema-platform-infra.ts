...
export const infraEndpoints = sqliteTable("infra_endpoints", {
  ...
  uniqAccountName: uniqueIndex("idx_infra_endpoints_account_name").on(
    table.accountId,
    table.name,
  ),
  idxBundleDeployment: index("idx_infra_endpoints_bundle_deployment_id").on(
...
const serviceRuntimesTable = sqliteTable("service_runtimes", {
  ...
  accountId: text("account_id").notNull().references(() => accounts.id),
  name: text("name").notNull(),
  runtime: text("runtime").notNull().default("takos.worker"),
  ...
  uniqAccountName: uniqueIndex("service_runtimes_account_id_name_key").on(
    table.accountId,
    table.name,
  ),
  idxBundleDeployment: index("service_runtimes_bundle_deployment_id_idx").on(
    table.bundleDeploymentId,
  ),
...
export const serviceRuntimesForInfra = serviceRuntimesTable;
export const infraWorkers = serviceRuntimesForInfra;
...
export const shortcuts = sqliteTable("shortcuts", {
  id: text("id").primaryKey(),
  userAccountId: text("user_account_id").notNull().references(() =>
    accounts.id
  ),
  accountId: text("account_id").notNull().references(() => accounts.id),
  ...
  uniqUserResourceTypeId: uniqueIndex("idx_shortcuts_user_resource_type_id").on(
    table.userAccountId,
    table.resourceType,
    table.resourceId,
  ),
  idxUser: index("idx_shortcuts_user_account_id").on(table.userAccountId),
  idxResourceTypeId: index("idx_shortcuts_resource_type_id").on(
    table.resourceType,
    table.resourceId,
  ),
...
export const uiExtensions = sqliteTable("ui_extensions", {
  ...
  uniqAccountPath: uniqueIndex("idx_ui_extensions_account_path").on(
    table.accountId,
    table.path,
  ),
