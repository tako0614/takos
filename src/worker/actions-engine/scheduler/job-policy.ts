 ...
 * 定義されていなければ null を返す（呼び出し側が既定の出力集約を選択する）。
 */
...
export function finalizeJobResult(
  ...
  // 未定義なら step 出力をフラット化する。同名出力は後続 step が上書きする。
  if (options.job && options.jobContext) {
    const stepsContext = buildStepsContext(result.steps);
    let resolvedOutputs: Record<string, string> | null;
    try {
      resolvedOutputs = evaluateJobOutputs(
        options.job,
        stepsContext,
        options.jobContext,
      );
