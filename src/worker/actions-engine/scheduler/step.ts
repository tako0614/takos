...
export class StepRunner {
  ...
  private async runAction(
    ...
    if (!step.uses) {
      throw new Error("runAction invariant violated: step.uses is required");
    }
  ...
  private async runShell(
    ...
    if (!step.run) {
      throw new Error("runShell invariant violated: step.run is required");
    }
    // コマンド文字列を補間
  ...
  private resolveShell(
    ...
    // job/workflow defaults.run.shell は GHA 互換で未検証の任意文字列。
    // step-level の enum (workflow-models.ts / validator.ts) だけが厳格な
    // source of truth であり、ここでは値を検証・正規化せず素通しする。
    const jobShell = jobDefaults?.run?.shell;
    if (jobShell) {
      return jobShell as Step["shell"];
    }
    const workflowShell = this.options.defaults?.workflow?.run?.shell;
    if (workflowShell) {
      return workflowShell as Step["shell"];
    }
    ...
    if (constructorJobShell) {
      return constructorJobShell as Step["shell"];
    }
