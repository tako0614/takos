...
test("job policy helpers - normalizes job needs definitions", () => {
  expect(normalizeNeedsInput(
      {
...
test("job policy helpers - creates completed and in-progress job result shapes", () => {
  ...
  expect(inProgress.id).toEqual("test");
  expect(inProgress.name).toEqual("Test");
  expect(inProgress.status).toEqual("in_progress");
  expect(inProgress.outputs).toEqual({});
  expect(inProgress.steps).toEqual([]);
  expect(inProgress.startedAt instanceof Date).toBeTruthy();
});
test("job policy helpers - returns dependency skip reason for non-success dependency outcomes", () => {
  ...
  expect(getDependencySkipReason(["setup"], results)).toEqual('Dependency "setup" failure');
  ...
  expect(getDependencySkipReason(["setup"], results)).toEqual('Dependency "setup" cancelled');
  ...
  expect(getDependencySkipReason(["setup"], results)).toEqual('Dependency "setup" skipped');
  ...
  expect(getDependencySkipReason(["setup"], results)).toEqual('Dependency "setup" did not succeed');
});
test("job policy helpers - returns null when dependencies are successful or missing", () => {
  const results = new Map<string, JobResult>([
    [
      "setup",
      {
  ...
  expect(getDependencySkipReason(["setup"], results)).toEqual(null);
  expect(getDependencySkipReason(["unknown"], results)).toEqual(null);
});
test("job policy helpers - ignores continue-on-error for failure control", () => {
...
test("job policy helpers - finalizes job status/conclusion and aggregates outputs from named steps", () => {
