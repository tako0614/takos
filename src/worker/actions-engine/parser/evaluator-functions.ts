...
function toExpressionString(value: unknown): string | undefined {
...
function normalizeExpressionString(value: unknown): string | undefined {
  return toExpressionString(value)?.toLowerCase();
}
...
export function fnContains(args: unknown[]): boolean {
  ...
  if (Array.isArray(search)) {
    return search.some((entry) => Object.is(entry, item));
  }

  const searchText = normalizeExpressionString(search);
  const itemText = normalizeExpressionString(item);
  if (searchText === undefined || itemText === undefined) {
    return false;
  }
...
export function fnStartsWith(args: unknown[]): boolean {
  const [searchString, searchValue] = args;
  const searchText = normalizeExpressionString(searchString);
  const valueText = normalizeExpressionString(searchValue);
  if (searchText === undefined || valueText === undefined) {
    return false;
  }
  return searchText.startsWith(valueText);
}
...
export function fnEndsWith(args: unknown[]): boolean {
  ...
  return searchText.endsWith(valueText);
}
