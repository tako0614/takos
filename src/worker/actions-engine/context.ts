...
/**
 * 名前が有効でも `__proto__` / `constructor` / `prototype` は、後段の
 * `Object.assign(sharedEnv, parsed)` で prototype pollution を起こすため
 * 取り込まない。式評価器の BLOCKED_PROPERTY_KEYS と同じ集合に揃える。
 */
...
export function parseGitHubEnvFile(content: string): Record<string, string> {
  // null-prototype so a dangerous key can never reach Object.prototype even if
  // a future change misses the explicit BLOCKED_ENV_KEYS guard below.
