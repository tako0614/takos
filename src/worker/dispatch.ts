  ...
  return {
    async fetch(
      ...
      try {
        ...
        const providerRuntimeNotFound = ["Worker", "NotFound"].join("");
        const isNotFound = errorMessage.includes("Service not found") ||
          errorMessage.includes(["Worker", " not found"].join("")) ||
          errorMessage.includes("not found") ||
          errorName === "ServiceNotFound" ||
          errorName === providerRuntimeNotFound;

        if (isNotFound) {
          return errorJsonResponse("Tenant service not found", 503, {
            message:
              "The tenant service may be provisioning or has been deleted",
          });
        }
