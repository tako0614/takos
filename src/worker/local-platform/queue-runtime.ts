 ...
 * required by the service poll loop. Cloud-backed message queues (SQS, Pub/Sub)
 * implement this interface directly, while local/in-memory/Redis queues
 * extend it via {@link LocalQueue}.
 */
export type ConsumableQueue<T = unknown> = MessageQueueBinding<T> & {
...
export function buildLocalMessageBatch<T>(
  ...
  const batch: MessageQueueBatch<T> = {
    queue: queueName,
    messages: [message],
  };
  return batch;
