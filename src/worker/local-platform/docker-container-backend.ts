...
export class DockerContainerBackend implements ContainerBackend {
  async pullImage(imageRef: string): Promise<void> {
    ...
    if (atIndex >= 0) {
      // Digest ref (e.g. registry/img@sha256:abc): the digest is the tag value.
      fromImage = imageRef.slice(0, atIndex);
      tag = imageRef.slice(atIndex + 1);
    } else {
      // Only treat a colon in the last path segment as a tag separator so a
      // registry host with a port (e.g. registry:5000/img) is not mis-split.
      const lastSlash = imageRef.lastIndexOf("/");
      const tagColon = imageRef.indexOf(":", lastSlash + 1);
      if (tagColon >= 0) {
        fromImage = imageRef.slice(0, tagColon);
        tag = imageRef.slice(tagColon + 1);
      } else {
        fromImage = imageRef;
        tag = "latest";
      }
    ...
    // The Docker Engine /images/create endpoint returns 200 even when the pull
    // fails; the failure arrives as an NDJSON line with an error/errorDetail
    // field inside the streamed body.
    for (const line of result.body.split("\n")) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      let parsed: { error?: unknown; errorDetail?: { message?: string } };
      try {
        parsed = JSON.parse(trimmed);
      } catch {
        // Tolerate non-JSON / partial progress lines.
        continue;
      }
      if (parsed.error !== undefined || parsed.errorDetail !== undefined) {
        const message = parsed.errorDetail?.message ??
          (typeof parsed.error === "string"
            ? parsed.error
            : String(parsed.error));
        throw new Error(`Docker pull failed: ${message}`);
      }
    }
  }
