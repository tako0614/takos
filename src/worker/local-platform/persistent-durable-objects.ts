  ...
  namespace.get = (id: { toString(): string }) => {
    ...
    void loadRegistry().then((state) => {
      if (!state.ids.includes(key)) {
        state.ids.push(key);
        // Idempotent best-effort: errors are swallowed via .catch below
        // because the in-memory state already reflects the new id.
        flushRegistry().catch((err) =>
          logWarn("Failed to flush registry on get", {
      ...
      logWarn("Failed to load registry on get", {
        module: "persistent-durable-objects",
        error: err instanceof Error ? err.message : String(err),
      })
    );
