...
function createDeployHandler(deps: OciOrchestratorRouteDeps) {
  return async (c: Context) => {
    ...
    // Acquire the state gate only for the brief disk read-modify-write; the
    // slow container work above ran outside the lock. Reload fresh state inside
    // the gate so `previous` (created_at carry-over and old-container cleanup)
    // reflects any concurrent deploy/remove of the same route_ref.
    const { record, previous } = await withState((state) => {
      const priorRecord = state.services[key];
      const nextRecord: OciServiceRecord = {
...
function createRemoveServiceHandler(deps: OciOrchestratorRouteDeps) {
  return async (c: Context) => {
    ...
    try {
      ...
      // Reload fresh state inside the gate so a concurrent deploy of the same
      // route_ref is not clobbered by a stale snapshot.
      const updated = await withState((state) => {
        const current = state.services[key] ?? record;
        const next: OciServiceRecord = {
          ...current,
          status: "removed",
          container_id: null,
