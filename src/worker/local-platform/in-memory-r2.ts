/**
 * In-memory implementation of object store (S3-compatible object storage).
 *
 * Used for local development and testing where a real object store binding is not
...
export type LocalObjectStoreMultipartUpload = {
  ...
  uploadPart(
    ...
    options?: { ssecKey?: ArrayBuffer | string },
...
export type LocalObjectStoreBinding =
  & Omit<
    ObjectStoreBinding,
  ...
  & {
    ...
    ): Promise<LocalObjectStoreMultipartUpload>;
...
function createObjectStoreObject(
  ...
  const object: ObjectStoreObjectBody = {
    key,
    version: "local",
...
export function createInMemoryObjectStore(): LocalObjectStoreBinding {
  ...
  const bucket: LocalObjectStoreBinding = {
    async head(key: string) {
      const record = objects.get(key);
      if (!record) return null;
      const object = createObjectStoreObject(
        key,
        record.bytes,
    ...
    async get(
      ...
      return createObjectStoreObject(
        key,
        bytes,
        record.customMetadata,
        record.httpMetadata,
    ...
    async put(
      ...
      if (!head) {
        throw new Error(
          `in-memory object store head returned null right after put for key=${key}`,
        );
      }
      return head;
    },
      ...
      return {
        objects: await Promise.all(
          page.map(async (key) => {
            ...
            if (!head) {
              throw new Error(
                `in-memory object store head returned null for listed key=${key}`,
              );
            }
            return head;
          }),
    ...
    async createMultipartUpload(
      ...
      return {
        ...
        async complete(
          ...
          if (!result) {
            throw new Error(
              `in-memory multipart complete put returned null for key=${key}`,
            );
          }
          return result;
        },
    ...
    resumeMultipartUpload(key: string, uploadId: string) {
      ...
      return {
        ...
        async complete(
          ...
          if (!result) {
            throw new Error(
              `in-memory multipart resume complete put returned null for key=${key}`,
            );
          }
          return result;
        },
