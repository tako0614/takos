  ...
  switch (binding.queue_backend) {
    case "sqs": {
      if (!binding.queue_url) {
        throw new Error(
          `Message queue binding "${binding.name}" requires queue_url for SQS`,
        );
      }
...
export async function createLocalTenantRuntimeRegistry(
  ...
  async function getOrCreateWorker(
    ...
    const created = (async () => {
      ...
      // Bindings that own background timers/state and must be released when
      // the surrounding deployment runtime is disposed. Today this carries
      // the analytics-engine binding (which holds a setTimeout-driven flush
      // batch); other timer-owning bindings should register here as they
      // are added.
      const disposableBindings: Array<{ dispose(): void }> = [];
      ...
      for (const binding of bindingsSnapshot) {
        ...
        switch (bindingType) {
          ...
          case "analytics_engine": {
            ...
            disposableBindings.push(analyticsBinding);
      ...
      return {
        ...
        async dispose(): Promise<void> {
          ...
          // Release timer-owning bindings before tearing down miniflare so
          // pending setTimeout flushes do not fire against a torn-down
          // closure. Each dispose() is synchronous and must not throw.
          for (const entry of disposableBindings) {
            try {
              entry.dispose();
            } catch {
              // Best-effort: a misbehaving dispose() must not block the
              // rest of the runtime teardown.
            }
          }
          await mf.dispose();
        },
