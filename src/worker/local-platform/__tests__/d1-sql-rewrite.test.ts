...
Deno.test("normalizePostgresMigrationSql preserves delimiters after index statements", () => {
...
Deno.test("normalizePostgresMigrationSql drops sqlite trigger bodies", () => {
  const normalized = normalizePostgresMigrationSql(
...
Deno.test("normalizePostgresMigrationSql cascades legacy worker mirror drops", () => {
  const normalized = normalizePostgresMigrationSql(
...
Deno.test("normalizePostgresMigrationSql rewrites sqlite json helper migrations", () => {
  for (
    const fileName of [
...
Deno.test("normalizePostgresMigrationSql skips sqlite table-rebuild repair", () => {
  assertEquals(
    normalizePostgresMigrationSql(
...
Deno.test("splitSqlStatements handles doubled-quote escapes (no backslash escape)", () => {
...
Deno.test("splitSqlStatements treats trailing backslash in a literal as data, not an escape", () => {
...
Deno.test("splitSqlStatements handles doubled double-quote in identifiers", () => {
  assertEquals(
