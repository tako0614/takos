...
localBootstrapTest(
  "local bootstrap - rejects self-hosted production with local defaults",
  async () => {
    const guardedKeys = [
      "ENVIRONMENT",
      "OIDC_ISSUER_URL",
...
localBootstrapTest(
  "local bootstrap - rejects deployment ids that do not belong to the requested tenant service",
  async () => {
    await withLocalBootstrapEnv(async ({ tempDataDir }) => {
      void tempDataDir;
      const env = await createNodeWebEnv();
  ...
  async () => {
    await withLocalBootstrapEnv(async ({ tempDataDir }) => {
      ...
      const upstreamFetch = stubGlobalFetch(async (request) => {
        ...
        assertEquals(body.serviceId, "worker-forward");
