...
function withQueueHelpers(messages: Array<MessageQueueMessage>): {
  wrappedMessages: Array<MessageQueueMessage & { ack(): void; retry(): void }>;
  explicitAcks: string[];
  ...
  const wrappedMessages = messages.map((message) => {
    const _msg = message as MessageQueueMessage & {
      ack: () => void;
      retry: () => void;
    };
    return {
...
class DurableObjectStateBinding {
  readonly storage = new Map<string, unknown>();
}

class DurableObjectNamespace {
  private readonly stateById = new Map<string, DurableObjectStateBinding>();
  ...
  constructor(
    private readonly clazz: {
      new (
        state: DurableObjectStateBinding,
        env: Record<string, unknown>,
      ): unknown;
    },
  ...
  private getInstance(
    ...
    const state = this.stateById.get(id) ?? new DurableObjectStateBinding();
    ...
    return {
      ...
      queue: async (
        ...
        const { wrappedMessages, explicitAcks, retryMessages } =
          withQueueHelpers(normalized as Array<MessageQueueMessage>);
