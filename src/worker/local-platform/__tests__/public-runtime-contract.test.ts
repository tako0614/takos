...
function assertSourceMatches(source: string, pattern: RegExp): void {
  assert(pattern.test(source), `Expected source to match ${pattern}`);
}

test("local public runtime contract - keeps bootstrap and local runtime exports free of loader registration and shim imports", async () => {
  ...
  const sourceIndex = await read("local-platform/index.ts", sourcePackageRoot);
...
test("local public runtime contract - publishes canonical local runtime entrypoints from src/worker", async () => {
  ...
  await assertRejects(() =>
    access(path.join(takosRoot, "src/local-platform"), constants.F_OK),
  );
  await assertRejects(() =>
    access(
      path.join(sourcePackageRoot, "local-platform-entrypoints"),
      constants.F_OK,
    ),
  );
  await assertRejects(() =>
...
test("local public runtime contract - removes the public local runner shim entirely", async () => {
  ...
  for (const shim of removedShims) {
    await assertRejects(() =>
      access(path.join(sourcePackageRoot, shim), constants.F_OK),
    );
    await assertRejects(() =>
      access(
...
test("local public runtime contract - routes compose and canonical scripts through direct package imports instead of bin indirection or *-node exports", async () => {
  const rootPackage = JSON.parse(
    await readFile(path.join(takosRoot, "package.json"), "utf8"),
  ) as {
    scripts?: Record<string, string>;
  ...
  const composeConfig = parseYaml(compose) as {
    services?: Record<
      string,
      {
        command?: string[];
  ...
  assert(
    !JSON.stringify(rootPackage.scripts ?? {}).includes(
      "local-platform-entrypoints",
    ),
  );
  assert(
    !JSON.stringify(rootPackage.scripts ?? {}).includes(
      "takos-worker-local-platform",
    ),
  );
  assertEquals(rootPackage.dependencies?.["@hono/node-server"], undefined);

  assertEquals(composeConfig.services?.["takos-worker"]?.command, [
    "bun",
    "run",
    "dev",
  ]);
  assertEquals(composeConfig.services?.takosumi?.command, [
    "bun",
    "core/index.ts",
  ]);
