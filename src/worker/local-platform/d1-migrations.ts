...
export async function ensureServerPostgresMigrations(
  ...
  for (const fileName of files) {
    ...
    try {
      // Wrap each migration's statements + its applied-marker insert in a
      // single transaction. On failure, ROLLBACK and rethrow without marking
      // applied -- that prevents a partially-applied migration from being
      // recorded as complete, which would skip the remaining statements on
      // the next boot.
