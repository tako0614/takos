/**
 * File-backed persistent implementation of object store (S3-compatible object storage).
 *
 ...
 * where a real object store binding is not available.
 */

import { createHash } from "node:crypto";
import { Buffer } from "node:buffer";
import type { ObjectStoreObjectBody } from "../shared/types/bindings.ts";
import type {
...
export function createPersistentObjectStore(
  filePath: string,
): LocalObjectStoreBinding {
  ...
  async function putObject(
    ...
    if (!head) {
      throw new Error(
        `persistent object store head returned null right after put for key=${key}`,
      );
    }
    return head;
  }
      ...
      return {
        objects: await Promise.all(
          page.map(async (key) => {
            ...
            if (!head) {
              throw new Error(
                `persistent object store head returned null for listed key=${key}`,
              );
            }
            return head;
          }),
