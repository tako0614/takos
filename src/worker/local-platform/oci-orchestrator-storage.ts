...
export async function loadState(): Promise<OciOrchestratorState> {
  try {
    ...
    if (nodeError && nodeError.code === "ENOENT") return { services: {} };
    // Do NOT interpret on-disk corruption (truncated/invalid JSON) as "no
    // services" — that would silently drop the entire service ledger. Fail
    // loud, mirroring the sibling routing-store.ts ENOENT-vs-parse pattern.
    throw error;
  }
}
...
export async function saveState(state: OciOrchestratorState): Promise<void> {
  ...
  // Crash-atomic write: write to a temp file then rename() (POSIX-atomic) so a
  // crash/disk-full mid-write cannot truncate state.json. The rename is already
  // serialized inside withState, so no extra locking is needed.
...
/**
 * Serialize the load -> mutate -> save read-modify-write of state.json so that
 * concurrent deploy/remove requests cannot lose each other's writes. The gate
 * is per-process (single reference deployment); it does not coordinate across
 * multiple orchestrator replicas sharing the same state.json over a volume.
 *
 * Callers MUST keep slow work (image pulls, container start, health checks)
 * OUTSIDE this critical section and only acquire it for the brief disk
 * read-modify-write, reloading fresh state inside `fn`.
 */
const stateGate = createSerializationGate();

export async function withState<T>(
  fn: (state: OciOrchestratorState) => Promise<T> | T,
): Promise<T> {
  const release = await stateGate.acquire();
  try {
