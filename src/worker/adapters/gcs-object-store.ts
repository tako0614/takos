...
import type {
  ObjectStoreObjects,
  ObjectStoreRangeLike,
} from "./r2-compat-types.ts";
import {
  normaliseBody,
  type ObjectStoreObjectMeta,
  toObjectStoreObject as toObjectStoreObjectShared,
  toObjectStoreObjectBody as toObjectStoreObjectBodyShared,
...
/**
 * Map GCS FileMetadata to backend-independent metadata, then build ObjectStoreObject.
 */
function gcsToObjectStoreObject(
  key: string,
  metadata: FileMetadata,
  range?: ObjectStoreRangeLike,
): ObjectStoreObject {
  ...
  const meta: ObjectStoreObjectMeta = {
    key,
    etag,
  ...
  return toObjectStoreObjectShared(meta);
}
...
/**
 * Wrap a GCS download result as an ObjectStoreObjectBody.
 */
function gcsToObjectStoreObjectBody(
  key: string,
  buffer: Uint8Array,
  metadata: FileMetadata,
  range?: ObjectStoreRangeLike,
): ObjectStoreObjectBody {
  const base = gcsToObjectStoreObject(key, metadata, range);
  const bytes = bufferToArrayBuffer(buffer);
  return toObjectStoreObjectBodyShared(base, bytes);
}
...
export function createGcsObjectStore(
  ...
  const store: ObjectStoreBinding = {
    // ----- head -----
    async head(key: string): Promise<ObjectStoreObject | null> {
      try {
        ...
        return gcsToObjectStoreObject(key, metadata);
      } catch (err: unknown) {
    ...
    async get(
      ...
      try {
        ...
        let rangeInfo: ObjectStoreRangeLike | undefined;
        if (options?.range) {
        ...
        return gcsToObjectStoreObjectBody(key, buffer, metadata, rangeInfo);
      } catch (err: unknown) {
        if (isGcsError(err) && err.code === 404) {
          return null;
        }
    ...
    async delete(keys: string | string[]): Promise<void> {
      if (typeof keys === "string") {
        try {
          ...
          // Ignore 404 on delete (matches object store behavior)
    ...
    async list(
      ...
      const objects: ObjectStoreObject[] = files.map((file) =>
        gcsToObjectStoreObject(file.name, file.metadata)
      );
      ...
      return {
        ...
        async complete(
          ...
          if (sourceKeys.length === 0) {
            ...
            const onlyKey = sourceKeys[0];
            if (!onlyKey) {
              throw new Error("sourceKeys[0] missing after length === 1 check");
            }
            await bucket.file(onlyKey).copy(destFile);
            ...
            while (currentKeys.length > COMBINE_LIMIT) {
              ...
              for (let i = 0; i < currentKeys.length; i += COMBINE_LIMIT) {
                ...
                if (batch.length === 1) {
                  const onlyBatchKey = batch[0];
                  if (!onlyBatchKey) {
                    throw new Error(
                      "batch[0] missing after length === 1 check",
                    );
                  }
                  nextKeys.push(onlyBatchKey);
                } else {
