...
export function createDynamoKvStore(
  config: DynamoKvStoreConfig,
): KvStoreBinding {
    ...
    async list(
      ...
      const keys = (result.Items ?? [])
        ...
        .map((item) => {
          ...
          if (!pkValue) {
            throw new Error(
              `DynamoDB item missing pk.S after filter (table=${config.tableName}, itemKeys=[${
