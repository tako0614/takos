...
async function resolveApiConfig(
  ...
  if (!resolvedApiServer) {
    throw new Error(
      "k8s secret requires K8S_API_SERVER or in-cluster Kubernetes service env",
    );
  }
  if (!resolvedToken) {
    throw new Error(
      "k8s secret requires K8S_BEARER_TOKEN or in-cluster service account token",
    );
  }
  if (!resolvedNamespace) {
    throw new Error(
      "k8s secret requires K8S_NAMESPACE or in-cluster service account namespace",
    );
  }
...
// K8s apiserver reads are on tenant request paths; cap stalls at 10s so
// handlers fail fast rather than hanging on a slow / unreachable apiserver.
const K8S_REQUEST_TIMEOUT_MS = 10_000;
...
async function requestJson<T>(
  ...
  const responseText = await new Promise<string>((resolve, reject) => {
    ...
    req.on("timeout", () => {
      req.destroy(
        new Error(
          `Kubernetes API request timed out after ${K8S_REQUEST_TIMEOUT_MS}ms`,
        ),
      );
    });
    if (payload) {
  ...
  return {
    async ensureSecret(name: string, value: string): Promise<string> {
      ...
      const requestPath = `/api/v1/namespaces/${
        encodeURIComponent(apiConfig.namespace)
    ...
    async getSecretValue(name: string): Promise<string> {
      ...
      if (!encoded) {
        throw new Error(
          `Kubernetes secret "${name}" does not contain key "value"`,
        );
      }
