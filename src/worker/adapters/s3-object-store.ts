...
import type { ObjectStoreObjects } from "./r2-compat-types.ts";
import {
  normaliseBody,
...
// ---------------------------------------------------------------------------
// Local type helpers – avoid adding SDK or provider ambient type packages just
...
/**
 * Map an S3 head / get response to backend-independent metadata, then build ObjectStoreObject.
 */
function s3ToObjectStoreObject(
  key: string,
...
/**
 * Wrap an S3 GetObjectCommand output as an ObjectStoreObjectBody.
 */
async function s3ToObjectStoreObjectBody(
  key: string,
  output: GetObjectCommandOutput,
  range?: { offset: number; length: number },
): Promise<ObjectStoreObjectBody> {
  const base = s3ToObjectStoreObject(key, output, range);
  const bytes = await consumeBody(output.Body);
  return toObjectStoreObjectBodyShared(base, bytes);
}

/**
 * Build a Range header string from object-store-style range options.
 */
  ...
  const store: ObjectStoreBinding = {
    ...
    async head(key: string): Promise<ObjectStoreObject | null> {
      try {
        ...
        return s3ToObjectStoreObject(key, output);
      } catch (err: unknown) {
    ...
    async get(
      ...
      try {
        ...
        return s3ToObjectStoreObjectBody(key, output, rangeInfo);
      } catch (err: unknown) {
        const sdkErr = err as Partial<AwsSdkError>;
        if (
    ...
    async put(
      ...
      // the canonical ObjectStoreObject shape.
    ...
    async list(
      ...
      const objects: ObjectStoreObject[] = (output.Contents ?? []).map(
        (item) => {
          const etag = (item.ETag ?? "").replace(/"/g, "");
          return toObjectStoreObjectShared({
            key: item.Key ?? "",
            etag,
