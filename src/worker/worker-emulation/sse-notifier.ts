...
export async function createSseNotifierService(
  ...
  const service: SseNotifierService = {
    ...
    subscribe(
      ...
      // 失敗してもローカル配信は動作するため非致命だが、観測のため log する
      ensureRedisSubscription(channel).catch((err) =>
        logWarn("Failed to ensure Redis subscription (non-critical)", {
          module: "sse-notifier",
          event: "sse.bg.subscribe_failed",
          channel,
