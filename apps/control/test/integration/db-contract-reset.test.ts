...
const docsDatabase = docsDatabasePath
  ? readFileSync(docsDatabasePath, "utf8")
  : "";
...
function expectDocsTable(table: string): void {
  if (table === "service_runtimes") {
    assert(
      /CREATE TABLE (service_runtimes|infra_workers)\s*\(/.test(docsDatabase),
    );
    return;
  }
  if (table === "service_mcp_endpoints") {
    assert(
      /CREATE TABLE (service_mcp_endpoints|worker_mcp_endpoints)\s*\(/.test(
        docsDatabase,
      ),
    );
    return;
...
function dbContractTest(name: string, fn: () => void): void {
  Deno.test({
    ...
    ignore: docsDatabase.length === 0,
    fn,
  });
}

dbContractTest(
  "keeps the reset-critical tables aligned across docs and baseline SQL",
  () => {
...
dbContractTest("keeps unified account tables present in all sources", () => {
...
dbContractTest(
  "documents the canonical table inventory that previously drifted out of the database reference",
  () => {
...
dbContractTest("removes legacy tables from docs and baseline SQL", () => {
...
dbContractTest(
  "keeps critical column contracts aligned across docs and baseline SQL",
  () => {
