...
function getTerminalRunStatusFromTimelineEvent(
  type: string,
  ...
  if (direct === "completed" || direct === "failed" || direct === "cancelled") {
    return direct;
  }
  const run = isRecord(data.run) ? data.run : undefined;
  const nested = run?.status;
  if (nested === "completed" || nested === "failed" || nested === "cancelled") {
    return nested;
  }
  return undefined;
}
...
function isRunInRootTree(
  runId: string,
  rootRunId: string,
  byId: Map<string, { id: string; parent_run_id: string | null }>,
): boolean {
