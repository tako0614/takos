...
const mocks = {
  checkWeeklyRuntimeLimit: spy((..._args: any[]) => undefined) as any,
};

// [Deno] vi.mock removed - manually stub imports from '@/services/billing/billing'
import {
  planGateDeps,
  requireWeeklyRuntimeLimitForAgent,
} from "@/middleware/plan-gates";

type Vars = { user?: User };
type TestEnv = { Bindings: Env; Variables: Vars };
const originalPlanGateDeps = { ...planGateDeps };
...
Deno.test("requireWeeklyRuntimeLimitForAgent - skips read-only methods", async () => {
  ...
  mocks.checkWeeklyRuntimeLimit = spy(async () => ({
    allowed: true,
    usedSeconds: 10,
    limitSeconds: 18_000,
  ...
  planGateDeps.checkWeeklyRuntimeLimit = mocks
    .checkWeeklyRuntimeLimit as typeof planGateDeps.checkWeeklyRuntimeLimit;
  try {
    const app = createApp();
    ...
    Object.assign(planGateDeps, originalPlanGateDeps);
  }
});
...
Deno.test("requireWeeklyRuntimeLimitForAgent - returns 402 when weekly runtime is exceeded", async () => {
  ...
  try {
    ...
    assertObjectMatch(payload, {
      error: {
        message: "Weekly runtime limit exceeded",
    ...
    assertStringIncludes(
      String(payload.error.details.reason),
      "Weekly runtime limit reached",
    );
  } finally {
    Object.assign(planGateDeps, originalPlanGateDeps);
  }
