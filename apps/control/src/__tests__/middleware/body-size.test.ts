...
Deno.test("pre-configured body limits - generalApiBodyLimit skips group deployment snapshot paths", async () => {
  const app = new Hono<{ Bindings: Env }>();
  app.use("*", generalApiBodyLimit);
  app.post(
    "/api/spaces/sp-1/group-deployment-snapshots",
    async (c) => c.json({ ok: true }),
  );

  const largeBody = "x".repeat(2 * 1024 * 1024);
  const res = await app.fetch(
    new Request("http://localhost/api/spaces/sp-1/group-deployment-snapshots", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  // Should not be 413 because group deployment snapshots are skipped
  assertEquals(res.status, 200);
});
