...
function cfEndpoint(
  name: string,
  routeRef: string,
  routes: StoredHttpEndpoint["routes"],
): StoredHttpEndpoint {
  return {
    name,
    routes,
...
Deno.test("selectRouteRefFromRoutingTarget - selects a deployment candidate by weight", () => {
  ...
  assertEquals(
    selectRouteRefFromRoutingTarget(target, { random: () => 0.1 }),
    "worker-a",
  );
  assertEquals(
    selectRouteRefFromRoutingTarget(target, { random: () => 0.95 }),
    "worker-b",
  );
});
...
Deno.test("parseRoutingValue - fails closed for legacy workerName envelopes", () => {
  const parsed = parseRoutingValue(
    JSON.stringify({ workerName: "worker-legacy" }),
  );
  assertEquals(parsed.target, null);
  assertEquals(parsed.rawFormat, "unknown");
});
...
Deno.test("selectRouteRefFromHttpEndpointSet - matches match-all endpoint (empty routes)", () => {
  ...
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/anything", "GET"),
    "worker-a",
  );
});
...
Deno.test("selectRouteRefFromHttpEndpointSet - matches endpoint with pathPrefix", () => {
  ...
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/api/users", "GET"),
    "worker-api",
  );
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/other", "GET"),
    "worker-all",
  );
});
...
Deno.test("selectRouteRefFromHttpEndpointSet - uses longest prefix match", () => {
  ...
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/api/v2/users", "GET"),
    "worker-v2",
  );
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/api/v1/users", "GET"),
    "worker-api",
  );
});
...
Deno.test("selectRouteRefFromHttpEndpointSet - respects method filter", () => {
  const endpoints = [
    cfEndpoint("write", "worker-write", [{
      pathPrefix: "/data",
      methods: ["POST", "PUT"],
    }]),
    cfEndpoint("read", "worker-read", [{
      pathPrefix: "/data",
      methods: ["GET"],
    }]),
  ];
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/data", "GET"),
    "worker-read",
  );
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/data", "POST"),
    "worker-write",
  );
});
...
Deno.test("selectRouteRefFromHttpEndpointSet - returns null for http-url targets", () => {
  ...
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/any", "GET"),
    null,
  );
});

Deno.test("selectRouteRefFromHttpEndpointSet - matches segment prefixes without leaking into siblings", () => {
  const endpoints: StoredHttpEndpoint[] = [
    {
      name: "api",
      routes: [{ pathPrefix: "/api" }],
      target: { kind: "service-ref", ref: "worker-api" },
    },
  ];
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/api", "GET"),
    "worker-api",
  );
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/api/users", "GET"),
  ...
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/apiary", "GET"),
    null,
  );
});
...
Deno.test("selectRouteRefFromHttpEndpointSet - returns matching http.url endpoints for direct forwarding", () => {
  ...
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/api/test", "GET"),
    null,
  );
  assertEquals(
    selectHttpEndpointFromHttpEndpointSet(endpoints, "/api/test", "GET"),
    endpoints[0],
  );
});
...
Deno.test("selectRouteRefFromHttpEndpointSet - match-all only wins if no longer prefix match exists", () => {
  ...
  assertEquals(
    selectRouteRefFromHttpEndpointSet(endpoints, "/home", "GET"),
    "worker-fallback",
  );
});
