...
const mocks = ({
  ...
  sanitizeRepoName: (name: string) => name.toLowerCase().replace(/[^a-z0-9_-]/g, '-'),
});
...
const sourceRepo = {
  ...
  const drizzle = createDrizzleMock();
    drizzle._.get
       ...
       = (async () => ({ ...sourceRepo, id: 'fork-id', accountId: 'ws-target', forkedFromId: 'source-1' })) as any; // forked repo
    mocks.getDb = (() => drizzle) as any;
  ...
  const drizzle = createDrizzleMock();
    drizzle._.get
       = (async () => ({ ...sourceRepo, id: 'fork-1', forkedFromId: 'source-1' })) as any
       = (async () => sourceRepo) as any;
    mocks.getDb = (() => drizzle) as any;
