...
import { assert, assertStringIncludes } from 'jsr:@std/assert';
  ...
  assertStringIncludes(DEFAULT_CORE_PROMPT, "You are Takos's universal agent.");
    assertStringIncludes(DEFAULT_CORE_PROMPT, 'Only use tools that are explicitly listed');
    assertStringIncludes(DEFAULT_CORE_PROMPT, 'keep moving until the task is actually complete');
    assertStringIncludes(DEFAULT_CORE_PROMPT, 'Default to spawning sub-agents');
    assertStringIncludes(DEFAULT_CORE_PROMPT, 'Infer the target product from thread context');
    assertStringIncludes(SYSTEM_PROMPTS.default, 'built-in official skills');
    assertStringIncludes(SYSTEM_PROMPTS.default, 'Treat the request as work to complete');
    assertStringIncludes(SYSTEM_PROMPTS.default, 'spawn sub-agents early');
    assertStringIncludes(SYSTEM_PROMPTS.default, 'infer it from the thread, docs, and repo context first');
})
  ...
  const prompt = buildAvailableToolsPrompt(SYSTEM_PROMPTS.default, [
    ...
    assertStringIncludes(prompt, '`workspace_files_read`');
  ...
  assertStringIncludes(SYSTEM_PROMPTS.assistant, "You are Takos's universal agent.");
    assertStringIncludes(SYSTEM_PROMPTS.assistant, '## Assistant Mode');
    assertStringIncludes(SYSTEM_PROMPTS.planner, "You are Takos's universal agent.");
