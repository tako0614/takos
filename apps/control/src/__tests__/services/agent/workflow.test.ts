...
import {
  ...
  const context: WorkflowContext = {
    ...
    await await assertRejects(async () => { await executeReview(context, 'pr-1', 'self'); }, 'OpenAI API key');
})
  ...
  const mockDb = {
    ...
    await await assertRejects(async () => { await executeReview(context, 'pr-999', 'self'); }, 'PR not found');
})
