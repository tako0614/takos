...
import {
  type MessagePersistenceDeps,
  persistMessage,
} from "@/services/agent/message-persistence";
...
Deno.test("persistMessage - throws after max retries exhausted", async () => {
  ...
  await assertRejects(async () => {
    await persistMessage(deps, { role: "user", content: "Fail" });
  }, "SQLITE_BUSY");
});
