...
import {
  applyDelta,
  readPackfileAsync,
} from "@/services/git-smart/protocol/packfile-reader";
import { getObject, putBlob } from "@/services/git-smart/core/object-store";
import { concatBytes, sha1Bytes } from "@/services/git-smart/core/sha1";
...
Deno.test("readPackfileAsync - throws on invalid signature", async () => {
  ...
  await assertRejects(async () => {
    await readPackfileAsync(bad, bucket as any);
