import { MockR2Bucket, MockD1Database } from '../../../../test/integration/setup.ts';
...
function createMockDrizzle(overrides: Record<string, unknown> = {}) {
  ...
  const env = { DB: db, RUNTIME_HOST: { fetch: ((..._args: any[]) => undefined) as any } } as never;
  ...
  const drizzle = createMockDrizzle();
      ...
      const env = { DB: db, RUNTIME_HOST: { fetch: ((..._args: any[]) => undefined) as any }, GIT_OBJECTS: storage } as never;
      const mgr = new RuntimeSessionManager(env, db as never, storage as never, spaceId, sessionId);
      mgr.setRepositoryInfo('repo-1', 'main');

      await await assertRejects(async () => { await mgr.initSession(); }, 'Session not found');
})
  ...
  const selectChain = {
      ...
      await await assertRejects(async () => { await mgr.initSession(); }, 'Session is already initialized');
})
  ...
  const env = { DB: db, RUNTIME_HOST: { fetch: ((..._args: any[]) => undefined) as any }, GIT_OBJECTS: storage } as never;
      ...
      await await assertRejects(async () => { await mgr.initSession({ skipDbLock: true }); }, 
        'repo_id is required',
      );
})  
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => new Response(JSON.stringify({ success: true, targetDir: '/tmp/repo', branch: 'main' }), { status: 200 }),) as any;
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => new Response(JSON.stringify({ error: 'clone failed' }), { status: 500 }),) as any;
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => new Response(JSON.stringify({ success: true, committed: true, commitHash: 'abc123' }), { status: 200 }),) as any;
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => new Response(JSON.stringify({ success: true, branch: 'main' }), { status: 200 }),) as any;
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => new Response(JSON.stringify({ error: 'push failed' }), { status: 500 }),) as any;
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => new Response('{}', { status: 200 })) as any;
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => new Response('error', { status: 500 })) as any;
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  mocks.callRuntimeRequest = (async () => { throw new Error('network error'); }) as any;
