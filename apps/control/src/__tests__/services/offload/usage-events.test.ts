...
// ---------------------------------------------------------------------------
  ...
  assertEquals(usageSegmentKey('run-x', 42), 'runs/run-x/usage/000042.jsonl.gz');
})
// ---------------------------------------------------------------------------
// writeUsageEventSegmentToR2
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
  ...
  const bucket = new MockR2Bucket();
    ...
    mocks.gzipDecompressToString
       = (async () => events1.map(e => JSON.stringify(e)).join('\n') + '\n') as any
       = (async () => events2.map(e => JSON.stringify(e)).join('\n') + '\n') as any;
