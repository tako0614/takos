...
import { assertEquals, assertNotEquals } from 'jsr:@std/assert';
  ...
  assertEquals(messageR2Key('thread-1', 'msg-1'), 
...
// ---------------------------------------------------------------------------
  ...
  const longContent = 'x'.repeat(MESSAGE_OFFLOAD_CONTENT_THRESHOLD_CHARS + 1);
    assertEquals(shouldOffloadMessage({ role: 'assistant', content: longContent }), true);
})
  ...
  const shortContent = 'x'.repeat(MESSAGE_OFFLOAD_CONTENT_THRESHOLD_CHARS);
    assertEquals(shouldOffloadMessage({ role: 'assistant', content: shortContent }), false);
})
