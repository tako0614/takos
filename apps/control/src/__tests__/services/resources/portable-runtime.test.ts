...
Deno.test("portable managed resource runtime - does not materialize marker files for takos-runtime logical resources", async () => {
  ...
  try {
    ...
    const durablePath = path.join(
      ...
      "managed-resources",
      "durable-namespace",
      "portable-counter.json",
    );
    const secretPath = path.join(
      tempDir,
      "managed-resources",
      "secret",
      "portable-secret.json",
    );
    ...
    await assertRejects(async () => {
      await access(durablePath);
    });
    await assertEquals(await access(secretPath), undefined);

    const secretState = JSON.parse(await readFile(secretPath, "utf-8")) as {
      value?: string;
    };
...
Deno.test("portable managed resource runtime - requires pgvector bootstrap for portable vector indexes", async () => {
  ...
  try {
    ...
    await assertRejects(async () => {
      await ensurePortableManagedResource(vector, "vector_index");
