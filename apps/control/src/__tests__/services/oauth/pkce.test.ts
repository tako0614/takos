...
import { assertEquals, assertNotEquals, assert, assertRejects } from 'jsr:@std/assert';
  ...
  await await assertRejects(async () => { await generateCodeChallenge('verifier', 'plain' as never); }, 
  ...
  assertEquals(isValidCodeVerifier('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopq'), true);
  ...
  assertEquals(isValidCodeChallenge('A'.repeat(43)), true);
    assertEquals(isValidCodeChallenge('abcdefghijklmnopqrstuvwxyz0123456789_-ABCDE'), true);
})
  ...
  const id = generateId();
    assert(/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/.test(id));
})
