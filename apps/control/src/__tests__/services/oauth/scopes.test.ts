...
import { assertEquals, assertStringIncludes } from 'jsr:@std/assert';
  ...
  assertEquals(parseScopes('openid profile email'), ['openid', 'profile', 'email']);
})
  ...
  assertEquals(areScopesAllowed(['openid', 'profile'], ['openid', 'profile', 'email']), true);
})
  ...
  assertEquals(areScopesAllowed(['openid', 'spaces:write'], ['openid', 'profile']), false);
})
  ...
  const summary = getScopeSummary(['openid', 'profile', 'spaces:read', 'files:write']);
    assertStringIncludes(summary.identity, 'OpenID Connect identity');
    assertStringIncludes(summary.identity, 'User profile (name, picture)');
    assertStringIncludes(summary.resources, 'Read workspaces');
    assertStringIncludes(summary.resources, 'Write files');
})
