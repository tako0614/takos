...
Deno.test("listMemories - returns mapped memories from the database", async () => {
  ...
  memoryServiceDeps.getDb = (() => drizzle) as any;
  drizzle._.all = (async () => [
    {
      id: "m-1",
      accountId: "space-1",
...
Deno.test("createMemory - inserts a new memory and retrieves it", async () => {
  ...
  const drizzle = createDrizzleMock();
  memoryServiceDeps.getDb = (() => drizzle) as any;
  memoryServiceDeps.generateId = (() => "new-mem-id") as any;
  memoryServiceDeps.now = (() => "2026-01-01T00:00:00.000Z") as any;
  drizzle._.get = (async () => ({
    id: "new-mem-id",
    accountId: "space-1",
    authorAccountId: "user-1",
...
Deno.test("createReminder - creates a reminder and returns it", async () => {
  ...
  const drizzle = createDrizzleMock();
  memoryServiceDeps.getDb = (() => drizzle) as any;
  memoryServiceDeps.generateId = (() => "rem-id") as any;
  memoryServiceDeps.now = (() => "2026-01-01T00:00:00.000Z") as any;
  drizzle._.get = (async () => ({
    id: "rem-id",
    accountId: "space-1",
...
Deno.test("triggerReminder - sets status to triggered and updates triggeredAt", async () => {
  ...
  const drizzle = createDrizzleMock();
  memoryServiceDeps.getDb = (() => drizzle) as any;
  memoryServiceDeps.now = (() => "2026-01-15T00:00:00.000Z") as any;
  drizzle._.get = (async () => ({
    id: "rem-1",
    accountId: "space-1",
    ownerAccountId: "user-1",
