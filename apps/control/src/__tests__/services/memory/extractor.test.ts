...
import { shouldAutoExtract, MemoryExtractor } from '@/services/memory/extractor';
  ...
  /* mocks cleared (no-op in Deno) */ void 0;
  const insertMock = ((..._args: any[]) => undefined) as any
         = (() => ({
  ...
  const mockLLMClient = { chat: ((..._args: any[]) => undefined) as any };
      mocks.LLMClient = () => mockLLMClient as any;
      mocks.chatAndParseJsonArray = (async () => { throw new Error('LLM API error'); }) as any;
  ...
  const drizzle = createDrizzleMock();
      ...
      assertStringIncludes(result[0].content, 'database password is stored in vault');
})
