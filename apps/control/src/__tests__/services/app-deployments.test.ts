...
const removedMessage =
  /App deployment API is not available in the current implementation|Legacy bundle deployment pipeline has been removed/;
...
for (
  const [name, call] of [
    ["deployFromRepoRef", () =>
      service.deployFromRepoRef("space-1", "user-1", {
        repoId: "repo-1",
        ref: "main",
      })],
  ...
  Deno.test(`${name} rejects as removed`, async () => {
    try {
      ...
      assert(removedMessage.test(error.message));
    }
  });
}
