...
Deno.test("agent tools - creates a delegation packet before spawning a delegated run", async () => {
  ...
  mockGet =
    ...
    (async () => ({
      content: "Refine the delegation model for Takos",
    })) as any =
      (async () => ({
        input: JSON.stringify({
          task: "Parent task",
          goal: "Ship the delegation improvement",
        }),
...
Deno.test("agent tools - archives the child thread when run creation fails after thread creation", async () => {
  ...
  await assertRejects(async () => {
    await spawnAgentHandler({ task: "write tests" }, makeContext());
  }, "Cannot spawn sub-agent: Too many concurrent child runs");
...
Deno.test("agent tools - returns the child final response when the run is completed", async () => {
  ...
  const result = JSON.parse(
    await waitAgentHandler({ run_id: "child-run" }, makeContext()),
  );
...
Deno.test("agent tools - returns a timeout payload when the child is still running", async () => {
  ...
  const result = JSON.parse(
    await waitAgentHandler(
      { run_id: "child-run", timeout_ms: 1 },
      makeContext(),
    ),
  );
...
Deno.test("agent tools - rejects runs that are not children of the current run", async () => {
  ...
  await assertRejects(async () => {
    await waitAgentHandler({ run_id: "other-run" }, makeContext());
  }, "Child run not found or not owned by this parent run");
});
