...
function makeContext(registry?: CapabilityRegistry): ToolContext {
  ...
  registry.register(makeDescriptor({
      ...
      assertEquals(result.results.every((r: { name: string }) => r.name !== 'hidden_tool'), true);
})
  ...
  const result = JSON.parse(await capabilityFamiliesHandler({}, ctx));
      assert(result.families.some((item: any) => JSON.stringify(item) === JSON.stringify({ family: 'file.ops', count: 1 })));
      assert(result.families.some((item: any) => JSON.stringify(item) === JSON.stringify({ family: 'browser.nav', count: 1 })));
  ...
  registry = new CapabilityRegistry();
    ...
    ctx = makeContext(registry);
  await await assertRejects(async () => { await capabilityInvokeHandler({}, ctx); }, 'tool_name is required');
})
  ...
  registry = new CapabilityRegistry();
    ...
    ctx = makeContext(registry);
  await await assertRejects(async () => { await capabilityInvokeHandler({ tool_name: 'file_read' }, ctx); }, 'Tool executor not available');
})
