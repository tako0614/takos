...
Deno.test("runtimeDelete - does not throw on 404", async () => {
  ...
  const env = createMockEnv();
  await assertEquals(
    await runtimeDelete(env, "/endpoint", "space-1"),
    undefined,
  );
});
...
Deno.test("getStepDisplayName - uses step.name when available", () => {
  /* mocks cleared (no-op in Deno) */ void 0;
  assertEquals(
    getStepDisplayName({ name: "Build project" } as any, 1),
    "Build project",
  );
});
...
Deno.test("getStepDisplayName - prefers name over uses", () => {
  ...
  assertEquals(
    getStepDisplayName(
      { name: "Checkout", uses: "actions/checkout@v4" } as any,
      1,
    ),
    "Checkout",
  );
...
Deno.test("getStepDisplayName - prefers uses over run", () => {
  ...
  assertEquals(
    getStepDisplayName(
      { uses: "actions/checkout@v4", run: "echo hi" } as any,
      1,
    ),
    "actions/checkout@v4",
  );
...
Deno.test("getSpaceIdFromRepoId - throws when repository not found", async () => {
  ...
  await assertRejects(async () => {
    await getSpaceIdFromRepoId({} as any, "missing");
...
Deno.test("getSpaceIdFromRepoId - throws when accountId is null", async () => {
  ...
  await assertRejects(async () => {
    await getSpaceIdFromRepoId({} as any, "repo-no-account");
...
Deno.test("buildSkippedStepResultsFromDb - returns step results from DB records", async () => {
  ...
  const results = await buildSkippedStepResultsFromDb(
    {} as any,
    "job-1",
    "fallback",
  );
