...
function createJobQueueContext(
  overrides: Partial<JobQueueContext> = {},
): JobQueueContext {
  return {
    env: {
      DB: {} as any,
...
Deno.test("handleJobSkipped - returns false when job has no if condition", async () => {
  ...
  mocks.buildRunNotifierEmitRequest =
    (() =>
      new Request("https://notifier.example.test", { method: "POST" })) as any;
...
Deno.test("handleJobSkipped - returns true and marks job as skipped when condition is false", async () => {
  ...
  assertStringIncludes(
    state.logs,
    "Job skipped (condition not met): ${{ env.MISSING_VAR }}",
  );
...
Deno.test("completeJobFailure - sets job conclusion to failure and stores logs", async () => {
  ...
  assertSpyCallArgs(engine.storeJobLogs, 0, [
    "job-1",
    expect.stringContaining("build broke"),
  ]);
  assertSpyCallArgs(engine.onJobComplete, 0, ["job-1", {
    conclusion: "failure",
  }]);
