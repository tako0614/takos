...
const originalUserCacheDeps = { ...userCacheDeps };

function restoreUserCacheDeps() {
  Object.assign(userCacheDeps, originalUserCacheDeps);
}
...
Deno.test("user cache guards (issue 182) - rejects invalid user IDs before DB query", async () => {
  ...
  const getDbSpy = spy(() => undefined);
  userCacheDeps.getDb = getDbSpy as any;
...
Deno.test("user cache guards (issue 182) - maps DB row and stores it in context cache", async () => {
  ...
  const getDbSpy = spy(() => drizzleMock);
  userCacheDeps.getDb = getDbSpy as any;
  ...
  try {
    ...
    assert(cachedUser);
    assertObjectMatch(cachedUser, { id: "user-2" });
  } finally {
    restoreUserCacheDeps();
  }
});
Deno.test("user cache guards (issue 182) - normalizes user ID before DB lookup", async () => {
  ...
  try {
    const user = await getCachedUser(ctx, "  user-2  ");
    assert(user);
    assertObjectMatch(user, { id: "user-2" });
  } finally {
    restoreUserCacheDeps();
  }
});
