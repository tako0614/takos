...
function createMockEnv(
  opts: { assetsAvailable?: boolean; indexHtmlOk?: boolean } = {},
) {
  ...
  return {
    ASSETS: {
      fetch: spy(async (request: Request) => {
        lastRequest = request;
        return indexHtmlOk
...
Deno.test("serveSpaFallback - fetches /index.html based on the provided request URL", async () => {
  ...
  const fetchedRequest = env.ASSETS.fetch.calls[0].args[0] as
    | Request
    | undefined;
  assertNotEquals(fetchedRequest, undefined);
  if (!fetchedRequest) {
    throw new Error("expected ASSETS.fetch to receive a Request");
  }
