...
const SALT = 'test-salt';
  ...
  const bad: EncryptedData = {
    ...
    await await assertRejects(async () => { await decrypt(bad, MASTER_SECRET, SALT); }, 'Unsupported encryption format');
})
  ...
  const encrypted = await encrypt('secret data', MASTER_SECRET, SALT);
    ...
    await await assertRejects(async () => { await decrypt(encrypted, wrongSecret, SALT); });
})
  Deno.test('encrypt / decrypt - fails to decrypt with wrong salt', async () => {
  const encrypted = await encrypt('secret data', MASTER_SECRET, SALT);
    await await assertRejects(async () => { await decrypt(encrypted, MASTER_SECRET, 'wrong-salt'); });
})
  ...
  await await assertRejects(async () => { await decryptEnvVars('not json', MASTER_SECRET, SALT); }, 
