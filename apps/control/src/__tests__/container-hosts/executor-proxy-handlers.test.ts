...
function makeMockR2(): any {
  return {
    get: (async () => null),
    put: (async () => ({ key: 'k', size: 0, etag: 'etag', uploaded: new Date() })),
...
// ---------------------------------------------------------------------------
  ...
  const env = makeEnv();
    env.DB._stmt.first = (async () => { throw new Error('SQLITE_CONSTRAINT: UNIQUE'); }) as any;
...
// ---------------------------------------------------------------------------
  ...
  const bucket = makeMockR2();
    bucket.list = (async () => ({ objects: [{ key: 'a' }], truncated: false })) as any;
  ...
  const bucket = makeMockR2();
    bucket.get = (async () => { throw new Error('fetch failed'); }) as any;
...
// ---------------------------------------------------------------------------
  ...
  const env = makeEnv();
    env.VECTORIZE.query = (async () => ({ matches: [{ id: 'vec-1', score: 0.9 }] })) as any;
...
// ---------------------------------------------------------------------------
  ...
  const env = makeEnv();
    env.AI.run = (async () => { throw new TypeError('invalid input'); }) as any;
