...
function createApp(user?: User) {
  ...
  Object.assign(profilesRepoRouteDeps, {
    findRepoByUsernameAndName: (...args: any[]) =>
      mocks.findRepoByUsernameAndName(...args),
    getDb: (...args: any[]) => mocks.getDb(...args),
    checkSpaceAccess: (...args: any[]) => mocks.checkWorkspaceAccess(...args),
    listBranches: (...args: any[]) => mocks.listBranches(...args),
    resolveReadableCommitFromRef: (...args: any[]) =>
      mocks.resolveReadableCommitFromRef(...args),
    listDirectory: (...args: any[]) => mocks.listDirectory(...args),
    getBlobAtPath: (...args: any[]) => mocks.getBlobAtPath(...args),
    getCommitsFromRef: (...args: any[]) => mocks.getCommitsFromRef(...args),
    getDefaultBranch: (...args: any[]) => mocks.getDefaultBranch(...args),
  });
...
Deno.test("profiles repo routes - GET /:username/:repoName - returns 404 when repo not found", async () => {
  ...
  mocks.findRepoByUsernameAndName = createRecordedMock(async () => null) as any;
...
Deno.test("profiles repo routes - GET /:username/:repoName - skips reserved paths (repos, stars, followers, following)", async () => {
  ...
  for (const reserved of ["repos", "stars", "followers", "following"]) {
    ...
    assertEquals(res.status, 404);
    assertEquals(mocks.findRepoByUsernameAndName.calls.length, 0);
  }
});
Deno.test("profiles repo routes - GET /:username/:repoName - returns repository data when found", async () => {
  ...
  mocks.listBranches =
    (async () => [{ name: "main", is_default: true }]) as any;
