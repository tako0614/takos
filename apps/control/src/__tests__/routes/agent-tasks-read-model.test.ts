...
import { agentTaskHandlerDeps } from "../../../../../packages/control/src/server/routes/agent-tasks-handlers.ts";
...
Deno.test("agent task read-model routes - returns latest run summary and resume target for each task", async () => {
  ...
  try {
    ...
    agentTaskRouteDeps.checkSpaceAccess = mocks
      .checkWorkspaceAccess as unknown as typeof agentTaskRouteDeps.checkSpaceAccess;
