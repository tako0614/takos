...
Deno.test('createPrefixedKvNamespace - prefixes kv operations per managed resource and strips prefixes from list results', async () => {
  const baseKv = {
      get: spy(async (key: string) => (key === 'managed-ns:hello' ? 'world' : null)),
      getWithMetadata: spy(async (key: string) => ({
      ...
      list: spy(async (options?: { prefix?: string }) => ({
        keys: [
    ...
    assertSpyCallArgs(baseKv.list, 0, [{
      prefix: 'managed-ns:he',
      cursor: undefined,
      limit: undefined,
    }]);
