...
import { smartHttpRouteDeps } from "@/routes/smart-http";
...
const mocks = {
  ...
  triggerPushWorkflows: createRecordedMock((..._args: any[]) =>
    undefined
  ) as any,
...
function createApp(user?: User) {
  ...
  smartHttpRouteDeps.optionalGitAuth = (async (c: any, next: any) => {
    const result = mocks.optionalGitAuth.mockResult;
    if (result) c.set("user", result);
    await next();
  }) as any;
  smartHttpRouteDeps.requireGitAuth = (async (c: any, next: any) => {
    const result = mocks.requireGitAuth.mockResult;
    if (result) c.set("user", result);
    await next();
  }) as any;
  if (user) {
...
function mockResolveRepo(result: Record<string, unknown> | null) {
  const results = result === null ? [undefined, undefined] : [
    result.account ?? { id: "acc-1", type: "user" },
...
Deno.test("smart-http routes - GET /git/:owner/:repo/info/refs - rejects invalid service parameter", async () => {
  ...
  const res = await app.fetch(
    new Request(
      "http://localhost/git/testuser/repo.git/info/refs?service=invalid",
    ),
...
Deno.test("smart-http routes - GET /git/:owner/:repo/info/refs - requires auth for receive-pack on any repo", async () => {
  ...
  const res = await app.fetch(
    new Request(
      "http://localhost/git/testuser/repo.git/info/refs?service=git-receive-pack",
    ),
    env as unknown as Env,
    {} as ExecutionContext,
  );
...
Deno.test("smart-http routes - POST /git/:owner/:repo/git-upload-pack - succeeds for public repo without auth", async () => {
  ...
  mocks.handleUploadPack = createRecordedMock(async () => responseData) as any;
  ...
  assertEquals(mocks.handleUploadPack.calls[0].args[2], "repo-1");
  assertEquals(mocks.handleUploadPack.calls.length > 0, true);
});
...
Deno.test("smart-http routes - POST /git/:owner/:repo/git-receive-pack - triggers push workflows on successful push with updated refs", async () => {
  ...
  assertEquals(mocks.triggerPushWorkflows.calls.length > 0, true);
});
Deno.test("smart-http routes - POST /git/:owner/:repo/git-receive-pack - releases push lock even when receive-pack throws", async () => {
  ...
  mocks.checkWorkspaceAccess = (async () => true) as any;
  mocks.handleReceivePackFromStream = (async () => {
    throw new Error("Packfile corrupted");
  }) as any;
