...
const modelsSource = await Deno.readTextFile(
  new URL(
    "../../../../../packages/control/src/shared/types/services-resources.ts",
    import.meta.url,
  ),
);
...
Deno.test("shared DB model types - does not expose removed project_id placeholders on canonical shared models", () => {
  assert(
    !(/export interface SpaceFile[\s\S]*\bproject_id\b/).test(modelsSource),
  );
