...
async function main() {
  ...
  const getUrl =
    `https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/dispatch/namespaces/${dispatchNamespace}/scripts/${workerName}/settings`;
  ...
  if (!currentSettings.ok) {
    console.error(
      ...
      await currentSettings.text(),
    );
    process.exit(1);
  }

  const settingsData = await currentSettings.json() as {
    result: { bindings: Array<{ name: string; type: string; text?: string }> };
  };
  console.log(
    "Current bindings:",
    settingsData.result.bindings.map((b) => b.name),
  );
