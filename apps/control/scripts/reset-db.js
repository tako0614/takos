...
function printUsage() {
  console.log('Usage: node scripts/reset-db.js --env <staging|production> [--include-accounts]');
...
function executeSql(sql) {
  const result = spawnSync('npx', ['wrangler', 'd1', 'execute', 'DB', '--remote', '--env', env, '--command', sql], {
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  if (result.status !== 0) {
    const message = (result.stderr || result.stdout || `exit code ${result.status ?? 1}`).trim();
