...
for (let i = 1; i < rawArgs.length; i += 1) {
  ...
  if (arg === '--env') {
    ...
    if (envName !== 'staging' && envName !== 'production') {
      console.error('Usage: node scripts/fix-worker-bindings.js <route-ref> [--local|--env staging|production]');
      process.exit(1);
    }
...
if (!ROUTE_REF_PATTERN.test(routeRef)) {
  console.error('Invalid route ref. Only letters, numbers, hyphen, and underscore are allowed.');
  process.exit(1);
}
...
async function main() {
  ...
  const dbResult = await new Promise((resolve, reject) => {
    const sql = `SELECT config FROM services WHERE route_ref = ${sqlString(routeRef)}`;
    const args = executionTarget.kind === 'local'
      ? ['wrangler', 'd1', 'execute', 'DB', '--local', '--command', sql, '--json']
      : ['wrangler', 'd1', 'execute', 'DB', '--remote', '--env', executionTarget.env, '--command', sql, '--json'];
    ...
    proc.on('close', code => {
      ...
      try {
        ...
        reject(new Error(`Failed to parse wrangler output: ${error instanceof Error ? error.message : String(error)}`));
      }
    });
  });
