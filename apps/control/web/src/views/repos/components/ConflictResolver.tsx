...
export function ConflictResolver(props: ConflictResolverProps) {
  ...
  const [resolutions, setResolutions] = createSignal<Map<string, Resolution>>(
    new Map(),
  );
  ...
  const fetchConflicts = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/repos/${props.repoId}/pulls/${props.prNumber}/conflicts`,
      );
  ...
  const selectVersion = (
    path: string,
    source: "ours" | "theirs" | "delete",
  ) => {
    const conflict = conflicts().find((c) => c.path === path);
    if (!conflict) return;
    const newResolutions = new Map(resolutions());
    if (source === "delete") {
      newResolutions.set(path, {
        path,
        content: "",
        delete: true,
        source: "ours",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 401 redirect handling and consistent error envelope parsing.
      const res = await fetch(
        `/api/repos/${props.repoId}/pulls/${props.prNumber}/resolve`,
        {
          method: "POST",
  ...
  const selectedConflict = () =>
    conflicts().find((c) => c.path === selectedFile());
  const selectedResolution = () =>
    selectedFile() ? resolutions().get(selectedFile()!) : null;
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: "0.5rem" }}>
              ...
              >
                {submitting() ? t("merging") : t("commitMerge", {
                  ...
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button
                      ...
                      style={{
                        ...
                        "font-size": "0.75rem",
                        cursor: "pointer",
                        border: selectedResolution()?.source === "ours"
                          ? "2px solid var(--color-primary)"
                          : "1px solid var(--color-border)",
                        background: selectedResolution()?.source === "ours"
                    ...
                    <button
                      ...
                      style={{
                        ...
                        "font-size": "0.75rem",
                        cursor: "pointer",
                        border: selectedResolution()?.source === "theirs"
                          ? "2px solid var(--color-primary)"
                          : "1px solid var(--color-border)",
                        background: selectedResolution()?.source === "theirs"
                          ? "var(--color-primary-bg, #eff6ff)"
                          : "transparent",
                      }}
                    >
                    ...
                    <Show when={conflict().type === "delete-modify"}>
                      <button
                        ...
                        style={{
                          ...
                          "font-size": "0.75rem",
                          cursor: "pointer",
                          border: selectedResolution()?.delete
                            ? "2px solid var(--color-error)"
                            : "1px solid var(--color-border)",
                          background: selectedResolution()?.delete
                            ? "#fef2f2"
                            : "transparent",
                          color: "var(--color-error, #dc2626)",
                        }}
                  ...
                  <div style={{ display: "flex", "flex-direction": "column" }}>
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setManualContent(conflict().path, editContent())}
                        style={{
                          "font-size": "0.6875rem",
                    ...
                    <textarea
                      ...
                      style={{
                        ...
                        "border-radius": "0 0 4px 4px",
                        resize: "vertical",
                        background: selectedResolution()?.source === "manual"
                          ? "var(--color-primary-bg, #eff6ff)"
                          : "transparent",
                      }}
                    />
