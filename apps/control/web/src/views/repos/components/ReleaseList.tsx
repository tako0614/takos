...
export function ReleaseList(props: ReleaseListProps) {
  ...
  const [editingRelease, setEditingRelease] = createSignal<Release | null>(
    null,
  );
  ...
  const fetchReleases = async () => {
    ...
    try {
      ...
      if (seq !== releasesSeq || repoId !== props.repoId) return;
      setReleases(data.releases || []);
    } catch (err: unknown) {
      if (seq !== releasesSeq || repoId !== props.repoId) return;
      setError(err instanceof Error ? err.message : t("unknownError"));
    } finally {
      if (seq === releasesSeq && repoId === props.repoId) {
        setLoading(false);
      }
    }
  };
  ...
  const handleSubmit: JSX.EventHandler<HTMLFormElement, SubmitEvent> = async (
    ...
    try {
      ...
      showToast(
        "error",
        err instanceof Error ? err.message : t("failedToSaveRelease"),
      );
    } finally {
      setSaving(false);
    }
    ...
    <>
                  ...
                  >
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <Show
                          when={index() === 0 && !release.is_prerelease &&
                            !release.is_draft}
                        >
                          <Badge
                            variant="default"
                      ...
                      <Show when={release.downloads > 0}>
                        <div class="flex items-center gap-1 mt-2 text-xs text-zinc-500">
                          ...
                          <span>
                            {t("downloadsCount", { count: release.downloads })}
                          </span>
                        </div>
                      </Show>
                    </div>
      ...
      >
        <form onSubmit={handleSubmit} class="flex flex-col gap-4">
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onInput={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    isPrerelease: e.currentTarget.checked,
                  }))}
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              <input
                ...
                onInput={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    isDraft: e.currentTarget.checked,
                  }))}
                class="w-4 h-4 rounded accent-zinc-900"
                style={{ "border-color": "var(--color-border-primary)" }}
              />
          ...
          <ModalFooter
            style={{ margin: "1rem -1.5rem -1.5rem", padding: "1rem 1.5rem" }}
          >
            <Button
              type="button"
              variant="secondary"
