...
export function RunDetail(props: RunDetailProps) {
  ...
  return (
    <Card padding="none">
      <div class="flex items-center justify-between px-4 py-3 border-b" style={{ "border-color": 'var(--color-border-primary)' }}>
        <span class="text-sm" style={{ color: 'var(--color-text-secondary)' }}>{t('runDetailsTitle')}</span>
        <Show when={props.run}>
          {(run) => (
            <div class="flex items-center gap-2">
              ...
              <Show when={run().status !== 'completed' && run().status !== 'cancelled'}>
                <Button
                  variant="ghost"
                  size="sm"
      ...
      <Show when={props.loadingRun}>
        <div class="flex items-center gap-3 px-4 py-6" style={{ color: 'var(--color-text-secondary)' }}>
      ...
      <Show when={!props.loadingRun && !props.run}>
        <div class="px-4 py-10 text-sm" style={{ color: 'var(--color-text-secondary)' }}>{t('selectRunToSeeDetails')}</div>
      </Show>
        ...
        {(run) => (
          <div class="px-4 py-4 space-y-4">
            <div class="flex items-center justify-between">
              <div>
                <div class="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>{run().workflow_path}</div>
            ...
            <div class="space-y-2">
              <Show when={run().jobs.length === 0}>
                <div class="text-sm" style={{ color: 'var(--color-text-secondary)' }}>{t('noJobsRecorded')}</div>
              </Show>
              <For each={run().jobs}>{(job) => (
                <JobCard
                  job={job}
