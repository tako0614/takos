...
export function ForkModal(props: ForkModalProps) {
  ...
  createEffect(on(
    () => [auth.spaces, auth.spacesLoaded, targetSpaceId()],
    () => {
  ...
  const isSelfFork = () =>
    targetSpaceId() === props.repo.space_id &&
    effectiveName() === props.repo.name;
  ...
  const handleSubmit: JSX.EventHandler<HTMLFormElement, SubmitEvent> = async (
    ...
    try {
      ...
      if (data.repository) {
        showToast(
          "success",
          t("forkedSuccess", { name: data.repository.name }),
        );
        props.onSuccess(data.repository);
      } else {
        showToast(
          "success",
          t("forkedSuccess", { name: customName() || props.repo.name }),
        );
        props.onClose();
      }
    } catch (err: unknown) {
          ...
          <div style={fieldGroupStyle}>
            ...
            <div class="relative" ref={dropdownRef}>
              <button
                ...
                disabled={!auth.spacesLoaded}
              >
                <span class="flex items-center gap-2">
                  <Show
                    when={selectedSpace()}
                    fallback={<span>{t("selectSpace")}</span>}
                  >
                    {(space) => (
                      <>
                        {space().is_personal
