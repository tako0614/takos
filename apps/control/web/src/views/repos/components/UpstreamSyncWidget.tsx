...
import type { SyncResult, SyncStatus } from "../../../types/index.ts";
...
export function UpstreamSyncWidget(props: UpstreamSyncWidgetProps) {
  ...
  const fetchSyncStatus = async () => {
    ...
    try {
      ...
      if (seq !== syncStatusSeq || repoId !== props.repoId) return;
      setStatus(data);
    } catch (err) {
      if (seq !== syncStatusSeq || repoId !== props.repoId) return;
      setError(err instanceof Error ? err.message : t("syncStatusError"));
    } finally {
      if (seq === syncStatusSeq && repoId === props.repoId) {
        setLoading(false);
      }
    }
  };
