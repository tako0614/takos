    ...
    <div class="flex text-sm font-mono">
      <Show when={props.blameEnabled}>
        <div class="flex flex-col py-3 px-3 bg-zinc-50 dark:bg-zinc-800 border-r border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 select-none w-64">
          <Show when={props.blameError} fallback={
            <For each={props.lines}>{(_, index) => {
              ...
              return (
                <div
                  ...
                  title={title()}
                >
                  <span class="font-mono text-zinc-400 w-14 flex-shrink-0">{shaShort()}</span>
                  <span class="truncate text-zinc-500 dark:text-zinc-400">{author()}</span>
                </div>
              );
            }}</For>
          }>
      ...
      <div class="flex flex-col py-3 px-3 bg-zinc-50 dark:bg-zinc-800 border-r border-zinc-200 dark:border-zinc-700 text-right text-zinc-500 dark:text-zinc-400 select-none">
        <For each={props.lines}>{(_, index) => (
          <span
            class={`leading-6 ${props.initialLine === index() + 1 ? 'text-zinc-900 dark:text-zinc-100 font-semibold' : ''}`}
          >
            {index() + 1}
          </span>
        )}</For>
      </div>
      <pre class={`flex-1 py-3 px-4 overflow-x-auto language-${props.language}`}>
