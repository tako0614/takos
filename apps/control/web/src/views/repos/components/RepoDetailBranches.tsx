...
export function RepoDetailBranches(props: RepoDetailBranchesProps) {
  ...
  createEffect(() => {
    ...
    function handleClickOutside(event: MouseEvent): void {
      if (
        branchDropdownRef && !branchDropdownRef.contains(event.target as Node)
      ) {
                ...
                {(branch) => (
                  ...
                  >
                    ...
                    <Show when={branch.is_default}>
                      <span class="px-1.5 py-0.5 text-xs rounded bg-zinc-200 dark:bg-zinc-600 text-zinc-600 dark:text-zinc-400">
                        {t("default")}
                      </span>
                    </Show>
                  </button>
