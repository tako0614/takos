  ...
  return (
    ...
    >
      <div class="flex items-center justify-between gap-4 mb-4">
        <div class="min-w-0">
          <div class="text-sm text-zinc-500 dark:text-zinc-400">{t('path')}</div>
          <div class="font-mono text-sm text-zinc-900 dark:text-zinc-100 truncate">{props.filePath}</div>
        </div>
      ...
      <Show when={!props.loading && props.error}>
        <div class="flex flex-col items-center justify-center py-10 text-zinc-500">
          ...
          <span class="mt-3 text-zinc-700 dark:text-zinc-300">{props.error}</span>
        </div>
      </Show>

      <Show when={!props.loading && !props.error && (props.data?.commits?.length ?? 0) === 0}>
        <div class="text-zinc-500 text-sm">{t('noHistoryEntries')}</div>
      </Show>

      <Show when={!props.loading && !props.error && (props.data?.commits?.length ?? 0) > 0}>
        <div class="divide-y divide-zinc-200 dark:divide-zinc-700">
          <For each={props.data?.commits || []}>{(cmt) => (
            <div class="py-3 flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <span class="text-sm text-zinc-900 dark:text-zinc-100 truncate">{cmt.message}</span>
                </div>
