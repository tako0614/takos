...
import type { ChatTimelineEventType } from "./chat-types.ts";
...
const TIMELINE_EVENT_TYPES: ReadonlySet<ChatTimelineEventType> = new Set<
...
export function getTerminalRunStatusFromTimelineEvent(
  ...
  const isTerminalRunStatus = (
    ...
    TERMINAL_RUN_STATUSES.has(status as RunTerminalStatus)
  );
...
export function summarizeEvent(
  ...
  switch (type) {
    ...
    case "progress":
      return {
        message: payload.message || payload.content || t("timelineProgress"),
      };
    case "message":
      return {
        message: t("timelineResponseUpdated"),
