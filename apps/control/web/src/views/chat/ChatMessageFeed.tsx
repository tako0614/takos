  ...
  return (
    <div class="py-2 px-4">
      ...
      <div class="space-y-0.5">
        <For each={props.toolCalls}>{(tc) => {
          const isRunning = tc.status === 'running' || tc.status === 'pending';
          const isError = tc.status === 'error';
          return (
            <div class="flex items-center gap-2 text-sm">
              <span class={`text-xs ${isError ? 'text-red-500' : isRunning ? 'text-zinc-400 dark:text-zinc-500' : 'text-green-500'}`}>
                {isError ? '\u2715' : isRunning ? '\u25CF' : '\u2713'}
              </span>
              <span
                class={
  ...
  return (
    <div class="flex-1 overflow-y-auto flex flex-col">
      <div class="w-full max-w-3xl mx-auto flex-1 flex flex-col">
        ...
        <For each={uniqueMessages()}>{(message) => (
          <MessageBubble
            message={message}
            showToolExecutions
        ...
        <Show when={!hasToolCalls() && !props.streaming.currentMessage && props.streaming.thinking}>
          <div class="py-2 px-4">
            <p class={`text-sm text-zinc-500 dark:text-zinc-400 italic${props.isLoading ? ' animate-pulse' : ''}`}>
              {props.streaming.thinking}
            </p>
          </div>
        </Show>

        <Show when={props.isLoading && !hasToolCalls() && !props.streaming.thinking}>
          <div class="py-2 px-4">
        ...
        <Show when={props.streaming.currentMessage && props.streaming.currentMessage !== lastAssistantMessage()?.content}>
          <div class="py-3 px-4">
