  ...
  return (
    <div class="bg-white dark:bg-zinc-900 px-3 py-3 md:px-6 lg:px-8 pb-[calc(0.75rem+var(--spacing-safe-bottom))] w-full max-w-3xl mx-auto">
      <form onSubmit={handleSubmit} class="mx-auto">
        <Show when={props.attachedFiles.length > 0}>
          <div class="flex flex-wrap gap-2 mb-3">
            <For each={props.attachedFiles}>
              {(file, index) => (
                <div class="flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-sm text-zinc-700 dark:text-zinc-300">
        ...
        <div
          class={`flex items-end gap-2 md:gap-3 bg-zinc-100 dark:bg-zinc-900 rounded-2xl border ${borderClass()} p-2 transition-colors`}
        >
          <button
            type="button"
          ...
          />
          <Show
            when={props.isLoading && props.onCancel}
            fallback={
              <button
                type="submit"
          ...
          >
            ...
            >
              <Show
                ...
                fallback={<Icons.Square class="w-5 h-5" />}
              >
