...
export function SettingsBilling(props: { user: User | null }) {
  ...
  const [billingSummary, setBillingSummary] = createSignal<
    BillingSummary | null
  >(null);
    ...
    const loadBilling = async () => {
      ...
      try {
        ...
        try {
          ...
          const invoicesData = await rpcJson<{ invoices: BillingInvoice[] }>(
            invoicesRes,
          );
          if (cancelled) {
            return;
  ...
  const handleSubscribePlus = async () => {
    ...
    try {
      ...
      showToast(
        "error",
        err instanceof Error ? err.message : t("billingSubscribeFailed"),
      );
  ...
  const handleTopupCheckout = async (packId: string) => {
    ...
    try {
      ...
      showToast(
        "error",
        err instanceof Error ? err.message : t("billingTopupFailed"),
      );
      setBillingAction(null);
    }
  };
  ...
  const handleSendInvoice = async (invoiceId: string) => {
    ...
    try {
      ...
      showToast(
        "error",
        err instanceof Error ? err.message : t("billingInvoiceSendFailed"),
      );
    } finally {
      setBillingAction(null);
    }
          ...
          <div class="space-y-4">
            <div class="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
              ...
              <div class="grid gap-3 text-sm text-zinc-600 dark:text-zinc-300 sm:grid-cols-2">
                ...
                <div>
                  <div class="text-zinc-500 dark:text-zinc-400">
                    {t("billingRuntimeLimit")}
                  </div>
                  <div class="mt-1 font-medium text-zinc-900 dark:text-zinc-100">
                    {t("billingRuntimeLimitValue", {
                      hours: String(
                        Math.round(bs.runtime_limit_7d_seconds / 3600),
                      ),
                    })}
                  </div>
                </div>
            ...
            <div class="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-800">
              ...
              <div class="grid gap-3 md:grid-cols-3">
                ...
                >
                  <div class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                    {t("billingPlanPlusTitle")}
                  </div>
                  <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
                    {t("billingPlanPlusDesc")}
                  </p>
                  <div class="mt-4">
                ...
                >
                  <div class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                    {t("billingPlanProTitle")}
                  </div>
                  <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">
                    {t("billingPlanProDesc")}
                  </p>
              ...
              {bs.has_payment_account && !invoiceLoading() &&
                ...
                <div class="space-y-3">
                  {billingInvoices().map((invoice: BillingInvoice) => (
                    <div class="flex flex-col gap-3 rounded-2xl border border-zinc-200 p-3 dark:border-zinc-800 md:flex-row md:items-center md:justify-between">
                      <div>
                        <div class="text-sm font-medium text-zinc-900 dark:text-zinc-100">
