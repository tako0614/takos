...
import { DEPLOY_NAV_SECTIONS, type DeploySection, type Resource, type User, type UserSettings, type Worker, type Workspace } from '../../../types';
...
import { useWorkspaceWorkers } from '../../../hooks/useWorkspaceWorkers';
...
export function DeployPanel({
  ...
  const [selectedTakopackWorkspaceId, setSelectedTakopackWorkspaceId] = useState<string | null>(spaceId);

  const { cfWorkers: workers = [], setCfWorkers, loadingCfWorkers: loadingWorkers, refreshWorkers, deleteWorker } = useWorkspaceWorkers(spaceId);
  const [selectedWorker, setSelectedWorker] = useState<Worker | null>(null);
  const [workerTab, setWorkerTab] = useState<import('../../workers/types').WorkerDetailTab>('overview');

  const { resources = [], loadingResources, refreshResources } = useWorkspaceResources(spaceId);
        ...
        return (
          <>
            <TakopackSection
              ...
              selectedWorkspaceId={selectedTakopackWorkspaceId}
              setSelectedWorkspaceId={setSelectedTakopackWorkspaceId}
            />
          </>
        );
      }
