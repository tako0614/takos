...
function getTypeLabel(type: Memory['type'], t: (key: any) => string) {
  switch (type) {
...
export function MemoryList({
  ...
  const [activeFilter, setActiveFilter] = createSignal<'all' | 'episode' | 'semantic' | 'procedural'>('all');
          ...
          filteredMemories.map(memory => (
            <Card padding="md">
              <div style={{ display: 'flex', "align-items": 'center', gap: '0.75rem', "margin-bottom": '0.5rem' }}>
                ...
                <span style={{ color: 'var(--color-text-tertiary)', "font-size": '0.875rem', "margin-left": 'auto' }} title={t('memoryImportance')}>
        ...
        <form onSubmit={handleCreateMemory}>
          <div style={{ display: 'flex', "flex-direction": 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', "flex-direction": 'column', gap: '0.5rem' }}>
              <label style={{ "font-size": '0.875rem', "font-weight": 500, color: 'var(--color-text-secondary)' }}>{t('memoryContent')}</label>
              <Textarea
            ...
            <div style={{ display: 'flex', "flex-direction": 'column', gap: '0.5rem' }}>
              <label style={{ "font-size": '0.875rem', "font-weight": 500, color: 'var(--color-text-secondary)' }}>{t('memoryType')}</label>
              <Select
                value={memoryType()}
            ...
            <div style={{ display: 'flex', "flex-direction": 'column', gap: '0.5rem' }}>
              <label style={{ "font-size": '0.875rem', "font-weight": 500, color: 'var(--color-text-secondary)' }}>{t('memoryCategory')}</label>
              <Input
