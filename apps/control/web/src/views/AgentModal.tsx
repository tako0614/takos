...
const TAB_CONFIG: { id: AgentTab; icon: ReactNode; labelKey: TranslationKey; descriptionKey: TranslationKey }[] = [
...
export function AgentModal({
  ...
  const { t } = useI18n();
  const [activeTab, setActiveTab] = useState<AgentTab>('work');
  const [selectedToolsWsId, setSelectedToolsWsId] = useState<string>(spaceId);
  ...
  return (
    ...
    >
      <div
        className="bg-zinc-50 dark:bg-zinc-900 rounded-t-xl md:rounded-xl w-full md:max-w-4xl lg:max-w-5xl h-[90vh] md:max-h-[85vh] overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
        style={{ paddingBottom: 'var(--spacing-safe-bottom)' }}
      >
        <div className="flex items-start justify-between gap-4 px-4 md:px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 shrink-0">
          <div className="space-y-1">
            <p className="text-xs font-medium uppercase tracking-[0.16em] text-zinc-500 dark:text-zinc-400">
              {t('agentSettings')}
            </p>
            <h3 id="agent-modal-title" className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
          ...
          <button
            type="button"
            className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-white/10 transition-colors"
            onClick={onClose}
            aria-label={t('close')}
          >
            <Icons.X />
        ...
        <div className="flex overflow-x-auto border-b border-zinc-200 dark:border-zinc-700 shrink-0" role="tablist" aria-label="Agent settings">
          {TAB_CONFIG.map(({ id, icon, labelKey }) => (
            <button
              key={id}
              id={`agent-tab-${id}`}
              type="button"
              className={`flex items-center gap-2 px-4 min-h-[44px] min-w-max text-sm font-medium transition-colors whitespace-nowrap ${activeTab === id ? 'text-zinc-900 dark:text-zinc-100 border-b-2 border-zinc-900 dark:border-zinc-100' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100'}`}
