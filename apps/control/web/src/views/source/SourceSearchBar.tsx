...
import type { SourceItem, CatalogSuggestionUser, CatalogSuggestionRepo } from '../../hooks/useSourceData';
...
export function SourceSearchBar({
  ...
  return (
    <div class="relative">
      ...
      <input
        ref={searchRef as HTMLInputElement | undefined}
        type="text"
        value={query}
        onInput={(e) => setQuery(e.target.value)}
        onFocus={() => { setSearchFocused(true); onFocusSearch(); }}
      ...
      {searchFocused && query.trim() && (suggestions.users.length > 0 || suggestions.repos.length > 0) && (
        <div class="absolute z-20 left-0 right-0 mt-2 rounded-2xl bg-white dark:bg-zinc-900 shadow-xl border border-zinc-100 dark:border-zinc-800 overflow-hidden">
          {suggestions.users.length > 0 && (
            <div class="py-1">
              <p class="px-4 pt-2 pb-1 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">{t('usersLabel')}</p>
              {suggestions.users.map((user) => (
                ...
                >
                  {user.avatar_url
                    ? <img src={user.avatar_url} alt="" class="w-6 h-6 rounded-full" />
                    : <div class="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center text-[10px]">{user.username.charAt(0).toUpperCase()}</div>
                  }
                  <span class="text-sm font-medium text-zinc-800 dark:text-zinc-200">@{user.username}</span>
                  {user.name && <span class="text-xs text-zinc-400">{user.name}</span>}
                </div>
              ))}
            </div>
          )}
          {suggestions.repos.length > 0 && (
            <div class={suggestions.users.length > 0 ? 'border-t border-zinc-100 dark:border-zinc-800 py-1' : 'py-1'}>
              <p class="px-4 pt-2 pb-1 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">{t('repositories')}</p>
              {suggestions.repos.map((repo) => (
                ...
                >
                  <p class="text-sm font-medium text-zinc-900 dark:text-zinc-100">{repo.owner.username}/{repo.name}</p>
                  {repo.description && <p class="text-xs text-zinc-400 truncate">{repo.description}</p>}
                </button>
              ))}
            </div>
          )}
