...
export function ServerCard({
  ...
  const [tools, setTools] = createSignal<{ name: string; description: string }[] | null>(null);
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-xl hover:border-zinc-200 dark:hover:border-zinc-700 transition-colors">
      <div class="flex items-center gap-4 p-4">
        ...
        <div class="relative w-10 h-10 rounded-xl flex items-center justify-center bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 flex-shrink-0">
          ...
          <span
            class={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white dark:border-zinc-900 ${statusColors[status]}`}
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            <h4 class="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate">{server.name}</h4>
          ...
          <p class="text-xs text-zinc-500 dark:text-zinc-400 truncate mt-0.5">{server.url}</p>
        </div>
      ...
      {expanded() && (
        <div class="border-t border-zinc-100 dark:border-zinc-800 px-4 py-3">
          <div class="flex items-center justify-between mb-2">
            ...
            >
              <Icons.RefreshCw class={`w-3.5 h-3.5 ${toolsLoading() ? 'animate-spin' : ''}`} />
            </button>
          </div>

          {toolsLoading() && !tools() && (
            <p class="text-xs text-zinc-400 dark:text-zinc-500 italic">{t('mcpFetchingTools')}</p>
          )}
          ...
          {tools() && tools()!.length === 0 && !toolsLoading() && (
            <p class="text-xs text-zinc-400 dark:text-zinc-500">{t('mcpNoTools')}</p>
          )}
            ...
            <div class="space-y-1.5 max-h-64 overflow-y-auto">
              {tools()!.map((tool: { name: string; description: string }) => (
                <div class="flex items-start gap-2 py-1">
                  ...
                  <div class="min-w-0">
                    <span class="text-xs font-mono font-medium text-zinc-700 dark:text-zinc-300">{tool.name}</span>
                    {tool.description && (
                      <p class="text-xs text-zinc-400 dark:text-zinc-500 line-clamp-2 mt-0.5">{tool.description}</p>
                    )}
                  </div>
                </div>
              ))}
