...
export function MemoryList({
  ...
  const [activeFilter, setActiveFilter] = createSignal<'all' | Memory['type']>('all');

  const filteredMemories = createMemo(() => memories.filter(m => {
    ...
    <div class="flex-1 overflow-y-auto p-6">
      <div class="flex gap-2 mb-6 border-b border-zinc-200 dark:border-zinc-700">
        <button type="button"
          ...
          onClick={() => onShowRemindersChange(false)}
        >
          <Icons.HardDrive />
        ...
        <button type="button"
          ...
          onClick={() => onShowRemindersChange(true)}
        >
          <Icons.Bell />
              ...
              {filteredMemories().map(memory => (
                <div class="bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 flex flex-col gap-3 hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors">
                  <div class="flex items-center justify-between gap-2">
                    ...
                    {memory.category && (
                      <span class="px-2 py-0.5 bg-zinc-100 dark:bg-zinc-700 rounded text-xs text-zinc-500 dark:text-zinc-400">{memory.category}</span>
                    )}
                  </div>
                  ...
                  <div class="flex items-center justify-between gap-2 pt-2 border-t border-zinc-200 dark:border-zinc-700">
                    <span class="text-xs text-zinc-600 dark:text-zinc-400" title={t('memoryImportance')}>
          ...
          {reminders.length === 0 ? (
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {reminders.map(reminder => (
                <div class={`bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 flex flex-col gap-3 hover:border-zinc-300 dark:hover:border-zinc-600 transition-colors ${getPriorityClasses(reminder.priority)}`}>
                  <div class="flex items-center justify-between gap-2">
