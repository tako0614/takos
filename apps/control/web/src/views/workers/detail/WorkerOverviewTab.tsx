...
export function WorkerOverviewTab(props: WorkerOverviewTabProps) {
  const { t } = useI18n();
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {workerUrl()
            ? (
              <a
                ...
                class="text-sm text-zinc-900 dark:text-zinc-100 hover:underline"
              >
                {workerHostname()}
              </a>
            )
        ...
        <div class="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
          ...
          <span class="text-sm text-zinc-900 dark:text-zinc-100">
            {props.worker.created_at
              ? new Date(props.worker.created_at).toLocaleDateString()
              : "-"}
          </span>
        </div>
        <div class="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
          ...
          <span class="text-sm text-zinc-900 dark:text-zinc-100">
            {props.worker.updated_at
              ? new Date(props.worker.updated_at).toLocaleDateString()
              : "-"}
          </span>
        </div>
      </div>
