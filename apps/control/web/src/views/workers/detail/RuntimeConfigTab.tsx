...
export function RuntimeConfigTab(props: RuntimeConfigTabProps) {
  const { t } = useI18n();
    ...
    <div class="space-y-4">
      <div class="space-y-2">
        ...
        <input
          ...
          value={props.runtimeConfig.compatibility_date || ""}
          onInput={(e) =>
            props.onRuntimeConfigChange({
              ...props.runtimeConfig,
              compatibility_date: e.currentTarget.value,
            })}
        />
      </div>
      <div class="space-y-2">
        <label class="block text-sm font-medium text-zinc-900 dark:text-zinc-100">
          {t("compatibilityFlags")}
        </label>
        <input
          type="text"
          class="w-full px-3 py-2 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-900/50"
          value={(props.runtimeConfig.compatibility_flags || []).join(", ")}
          onInput={(e) =>
            props.onRuntimeConfigChange({
              ...props.runtimeConfig,
              compatibility_flags: e.currentTarget.value.split(",").map((f) =>
                f.trim()
              ).filter(Boolean),
            })}
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center gap-2">
          <input
            ...
            value={props.runtimeConfig.cpu_ms || ""}
            onInput={(e) =>
              props.onRuntimeConfigChange({
                ...props.runtimeConfig,
                cpu_ms: parseInt(e.currentTarget.value, 10) || undefined,
              })}
            placeholder="50"
            min="10"
            max="30000"
      ...
      <div class="space-y-2">
        ...
        <input
          type="number"
          class="w-32 px-3 py-2 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-900/50"
          value={props.runtimeConfig.subrequests || ""}
          onInput={(e) =>
            props.onRuntimeConfigChange({
              ...props.runtimeConfig,
              subrequests: parseInt(e.currentTarget.value, 10) || undefined,
            })}
          placeholder="50"
          min="1"
          max="1000"
      ...
      <button
        ...
        onClick={props.onSaveRuntimeConfig}
        disabled={props.savingWorkerSettings}
      >
        {props.savingWorkerSettings ? t("saving") : t("saveRuntime")}
      </button>
    </div>
  );
}
