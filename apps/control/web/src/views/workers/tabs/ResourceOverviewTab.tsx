...
export function ResourceOverviewTab({ resource }: ResourceOverviewTabProps) {
  ...
  return (
    <div className="space-y-6" role="region" aria-label={t('overview')}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
          ...
          <span className="text-sm text-zinc-900 dark:text-zinc-100">{resource.created_at ? new Date(resource.created_at).toLocaleDateString() : '-'}</span>
        </div>
      </div>

      <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
        <h4 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 mb-4 flex items-center gap-2">
          <Icons.Link className="w-4 h-4" aria-hidden="true" />
          {t('connectionInfo')}
        </h4>
      ...
      <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
        <h4 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 mb-4 flex items-center gap-2">
          <Icons.Key className="w-4 h-4" aria-hidden="true" />
          {t('accessTokens')}
        </h4>
        <AccessTokensList
          tokens={tokens}
...
function ConnectionInfoDisplay({
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400" role="status">
  ...
  if (!connectionInfo) {
    return <p className="text-sm text-zinc-500 dark:text-zinc-400">{t('connectionInfoNotAvailable')}</p>;
  }
        ...
        <div key={key} className="flex items-center gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              <code className="flex-1 text-sm text-zinc-900 dark:text-zinc-100 font-mono bg-zinc-100 dark:bg-zinc-700 px-3 py-2 rounded-lg truncate">
                {value}
              </code>
              <button
...
function AccessTokensList({
  ...
  return (
    <div className="space-y-4">
      ...
      {tokens.length > 0 && (
        <ul className="space-y-2" aria-label="Access tokens">
          {tokens.map((token) => (
            <li
              key={token.id}
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <Icons.Key className="w-4 h-4 text-zinc-500 dark:text-zinc-400" aria-hidden="true" />
                  <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{token.name}</span>
...
function CreateTokenModal({
  ...
  if (newToken) {
    return (
      <Modal isOpen={isOpen} onClose={handleClose} title={t('tokenCreatedTitle')} size="md">
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800" role="alert">
            <div className="flex items-start gap-3">
              <Icons.Check className="w-5 h-5 text-green-600 dark:text-green-400 mt-0.5" aria-hidden="true" />
              <div>
                <p className="text-sm font-medium text-green-800 dark:text-green-200">{t('tokenCreatedSuccessfully')}</p>
          ...
          <div>
            <label htmlFor="new-token-value" className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-2">
              {t('accessTokenLabel')}
            </label>
            <div className="flex items-center gap-2">
              <code id="new-token-value" className="flex-1 text-sm text-zinc-900 dark:text-zinc-100 font-mono bg-zinc-100 dark:bg-zinc-700 px-3 py-2 rounded-lg break-all">
                {newToken}
              </code>
              <button
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={t('generateAccessTokenTitle')} size="md">
      <div className="space-y-4">
        <div>
          <label htmlFor="token-name" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
            {t('tokenNameLabel')}
          </label>
          <input
            id="token-name"
            ...
            className="w-full px-3 py-2 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 dark:focus:ring-zinc-100/10"
          />
        </div>

        <div>
          <label htmlFor="token-permission" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
            {t('permissionLabel')}
          </label>
          <select
            id="token-permission"
        ...
        <div>
          <label htmlFor="token-expiration" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
            {t('expirationLabel')}
          </label>
          <select
            id="token-expiration"
