...
export function RepoSummaryCard({
  ...
  return (
    <div
      className="p-4 rounded-lg bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:border-zinc-300 dark:hover:border-zinc-600 cursor-pointer transition-colors"
      onClick={() => onSelect?.()}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <h3 className="flex items-center gap-2 text-zinc-900 dark:text-zinc-100 font-medium hover:underline">
      ...
      {description && (
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400 line-clamp-2">{description}</p>
      )}
