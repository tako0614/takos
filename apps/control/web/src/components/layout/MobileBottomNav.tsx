...
export function MobileBottomNav(props: MobileBottomNavProps) {
  ...
  const navItems: { id: NavItem; icon: JSX.Element; label: string }[] = [
    ...
    { id: 'chat', icon: <Icons.MessageSquare class="w-5 h-5" />, label: t('chat') },
    { id: 'store', icon: <Icons.ShoppingBag class="w-5 h-5" />, label: t('store') },
  ];

  return (
    <nav
      aria-label={t('primaryNavigation')}
      class={`fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-zinc-900/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 supports-[backdrop-filter]:dark:bg-zinc-900/80 border-t border-zinc-200 dark:border-zinc-800 pb-[var(--spacing-safe-bottom)] h-[calc(var(--nav-height-mobile)+var(--spacing-safe-bottom))] ${props.className ?? ''}`}
    >
