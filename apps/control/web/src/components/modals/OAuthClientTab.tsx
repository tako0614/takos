...
export function OAuthClientTab(props: OAuthClientTabProps) {
  ...
  const [formClientType, setFormClientType] = createSignal<
    "confidential" | "public"
  >("confidential");
  ...
  const [createdClient, setCreatedClient] = createSignal<
  ...
  async function handleDeleteClient(clientId: string): Promise<void> {
    ...
    try {
      const res = await rpc.me.oauth.clients[":clientId"].$delete({
        param: { clientId },
      });
      await rpcJson(res);
    ...
    try {
      const res = await rpc.me.oauth.clients.$post({
        json: {
          ...
          token_endpoint_auth_method: formClientType() === "public"
            ? "none"
            : "client_secret_post",
        },
      });
      const data = await rpcJson<{ client_id: string; client_secret?: string }>(
        res,
      );
        ...
        fallback={
          <div>
            <div class="flex items-center justify-between mb-4">
              <p class="text-sm text-[var(--color-text-secondary)] m-0">
                {t("developerAppsDesc")}
              </p>
              <Button
                variant="primary"
                        ...
                        <div class="flex items-start justify-between gap-4">
                          <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                              <h4 class="font-medium text-[var(--color-text-primary)] m-0">
                                {client.name}
                              </h4>
                              <Badge
                                variant={client.status === "active"
                                  ? "success"
                                  : "error"}
                              >
                                {client.status}
                            ...
                            <p class="text-xs text-[var(--color-text-tertiary)] mt-1">
                              ...
                              <code class="px-1 py-0.5 rounded-md bg-[var(--color-surface-secondary)] font-mono">
                                {client.client_id}
                              </code>
                            </p>
                            <p class="text-xs text-[var(--color-text-tertiary)] mt-1">
                          ...
                          <Button
                            ...
                            isLoading={deleting() === client.client_id}
                          >
                            {t("delete")}
                          </Button>
                        </div>
            ...
            >
              <div class="text-center py-12 text-[var(--color-text-tertiary)]">
                ...
                <p class="font-medium text-[var(--color-text-primary)] m-0">
                  {t("noDeveloperApps")}
                </p>
            ...
            <div class="flex flex-col gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={t("oauthClientNamePlaceholder")}
                />
              </div>
              <div>
                <label class="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    {(uri, i) => (
                      <div class="flex gap-2">
                        <Input
                          ...
                          value={uri}
                          onInput={(e) => {
                            const u = [...formRedirectUris()];
                            u[i()] = e.currentTarget.value;
                            setFormRedirectUris(u);
                          }}
                          placeholder={t("oauthRedirectUriPlaceholder")}
                          class="flex-1"
                        />
                  ...
                  <button
                    type="button"
                    class="bg-transparent border-none p-0 text-sm text-[var(--color-text-primary)] cursor-pointer text-left"
                    onClick={() => setFormRedirectUris((prev) => [...prev, ""])}
                  >
                    + {t("addRedirectUri")}
                  </button>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2">
                  <For each={["confidential", "public"] as const}>
                    {(type) => (
                      <button
                        type="button"
                        class="p-3 rounded-lg text-left cursor-pointer transition-colors"
                        style={{
                          border: `1px solid ${
                            formClientType() === type
                          ...
                          "background-color": formClientType() === type
                            ? "var(--color-surface-secondary)"
                            : "var(--color-surface-primary)",
                        }}
                        onClick={() => setFormClientType(type)}
                      >
                        <div class="font-medium text-[var(--color-text-primary)]">
                          {t(
                            type === "confidential"
                              ? "oauthClientTypeConfidential"
                              : "oauthClientTypePublic",
                          )}
                        </div>
                        <div class="text-xs text-[var(--color-text-tertiary)] mt-1">
                          {t(
                            type === "confidential"
                              ? "oauthClientTypeConfidentialDesc"
                              : "oauthClientTypePublicDesc",
                          )}
                        </div>
                      </button>
                    )}
                    ...
                    {(scope) => (
                      <label
                        ...
                        style={{
                          ...
                          "background-color": formScopes().includes(scope)
                            ? "var(--color-surface-secondary)"
                            : "var(--color-surface-primary)",
                        }}
                      >
                ...
                <div>
                  ...
                  <div class="flex gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleCopy(client().client_secret!, "secret")}
                    >
                      {copied() === "secret"
                        ? t("copied")
                        : t("copyToClipboard")}
