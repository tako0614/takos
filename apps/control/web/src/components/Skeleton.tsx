...
function Skeleton(props: SkeletonProps) {
  return (
    <div
      class={`animate-pulse bg-zinc-200/50 dark:bg-zinc-700/50 rounded ${props.class ?? ''}`}
      aria-hidden="true"
    />
  );
}
...
function SkeletonCard(props: SkeletonProps) {
  return (
    <div class={`bg-zinc-100 dark:bg-zinc-900 rounded-lg p-4 border border-zinc-200 dark:border-zinc-800 ${props.class ?? ''}`}>
...
export function SkeletonList(props: SkeletonProps & { count?: number }) {
  return (
    <div class={`space-y-3 ${props.class ?? ''}`} role="status" aria-label="Loading content">
