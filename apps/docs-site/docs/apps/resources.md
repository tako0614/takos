...
local tenant runtime は `durableObject` と `queue` producer binding を materialize します。`vectorize` は PostgreSQL + pgvector が利用可能であれば materialize されます（`POSTGRES_URL` + `PGVECTOR_ENABLED=true` が必要）。`workflow` は resource 管理と worker binding/invocation が分かれています。
