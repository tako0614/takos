...
`deploy-group` で複数の Worker / Container をデプロイしている場合、ロールバックにはいくつか注意点があります。
...
git checkout v1.0.0
takos deploy-group --env production --worker api
```
...
takos deploy-group --env production --worker browser-worker
```
