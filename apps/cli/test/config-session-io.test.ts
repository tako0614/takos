...
const validSessionIdPermission = validSessionId;
...
Deno.test("findSessionFile - finds session file in current directory", () => {
  ...
  try {
    ...
    createSessionFile(
      dir,
      JSON.stringify({
        session_id: validSessionIdPermission,
...
if (platform() !== "win32") {
  Deno.test("findSessionFile permission checks - rejects session file with world-readable permissions", () => {
    originalCwd = process.cwd();
    tempDirs = [];
    try {
      const dir = createTempDir();
      const _sessionPath = createSessionFile(
        dir,
        JSON.stringify({
          session_id: validSessionId,
          workspace_id: "ws-test",
  ...
  Deno.test("findSessionFile permission checks - rejects session file with group-readable permissions", () => {
    originalCwd = process.cwd();
    tempDirs = [];
    try {
      const dir = createTempDir();
  ...
  Deno.test("findSessionFile permission checks - accepts session file with mode 600", () => {
    originalCwd = process.cwd();
    tempDirs = [];
    try {
      const dir = createTempDir();
  ...
  Deno.test("setSecurePermissions - does not throw for nonexistent file", () => {
    originalCwd = process.cwd();
    tempDirs = [];
    try {
      const dir = createTempDir();
  ...
  Deno.test("setSecurePermissions - sets file to mode 600", () => {
    originalCwd = process.cwd();
    tempDirs = [];
    try {
      const dir = createTempDir();
