...
export function generateContainerWranglerConfig(
  ...
  const groupName = options.groupName || options.manifest?.metadata?.name ||
    "app";
  ...
  if (!service.container) {
    throw new Error(
      `Service "${serviceName}" is missing container configuration`,
    );
  }
...
export function serializeContainerWranglerToml(
  config: ContainerWranglerConfig,
): string {
  const lines: string[] = [];
  ...
  if (config.containers) {
    for (const container of config.containers) {
      ...
      lines.push(
        `image_build_context = ${
          JSON.stringify(container.image_build_context)
        }`,
      );
...
export function generateContainerHostEntry(
  serviceName: string,
  service: ContainerServiceDef,
): string {
  const className = `${toPascalCase(serviceName)}Container`;
  ...
  if (options.dryRun) {
    ...
    if (!container) {
      return {
        ...
        type: "container",
        status: "failed",
        error: "Missing container configuration",
      };
    }
    return {
      name: serviceName,
      ...
      error:
        `[dry-run] would deploy container (Dockerfile: ${container.dockerfile}, Port: ${
          container.port || 8080
        }, Instance Type: ${
          container.instanceType || "basic"
        }, Max Instances: ${container.maxInstances || 10})`,
    };
  }
