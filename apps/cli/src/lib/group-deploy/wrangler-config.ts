...
export function generateWranglerConfig(
  ...
  if (service.type !== "worker" || !service.build) {
    throw new Error(
      `Cannot generate wrangler config for non-worker service: ${serviceName}`,
    );
  }
  ...
  if (service.containers && service.containers.length > 0) {
    const containerConfig: ContainerWranglerConfig = {
      ...
      migrations: [{
        tag: "v1",
        new_classes: service.containers.map((c) =>
          `${toPascalCase(c.name)}Container`
        ),
      }],
    };
  ...
  if (service.bindings?.services && service.bindings.services.length > 0) {
    config.services = service.bindings.services.map((target) => {
      ...
      const targetScript = options.namespace
        ? `${options.groupName}-${targetName}`
        : targetName;
      return { binding: toBinding(targetName), service: targetScript };
    });
  }
