...
export async function deployWorkers(
  ...
  for (const [workerName, worker] of Object.entries(workers)) {
    if (
      ...
      !effectiveFilter.includes(workerName)
    ) continue;

    try {
      const resolvedContainers: WorkerContainerSpec[] =
        (worker.containers || []).map((cRef) => {
      ...
      const legacyService = {
        ...
        containers: resolvedContainers.length > 0
          ? resolvedContainers
          : undefined,
      };
      ...
      if (dryRun) {
        const dryRunInfo = resolvedContainers.length > 0
          ? ` (with ${resolvedContainers.length} CF container(s): ${
            resolvedContainers.map((c) => c.name).join(", ")
          })`
          : "";
        result.services.push({
          ...
          ...(dryRunInfo
            ? { error: `[dry-run] would deploy worker${dryRunInfo}` }
            : {}),
        });
        result.bindings.push(
          ...collectWorkerBindingResults(workerName, worker, "bound"),
        );
        continue;
      }
      ...
      const toml = isContainerConfig
        ? serializeContainerWranglerToml(
          wranglerConfig as ContainerWranglerConfig,
        )
      ...
      if (wranglerResult.success) {
        ...
        result.bindings.push(
          ...collectWorkerBindingResults(workerName, worker, "failed"),
        );
      }
    } catch (error) {
      result.services.push({
