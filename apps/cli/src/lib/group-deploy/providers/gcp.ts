...
export class GCPProvider implements ResourceProvider {
  ...
  constructor(opts?: { project?: string; region?: string }) {
    this.project = opts?.project || Deno.env.get("GOOGLE_CLOUD_PROJECT") ||
      Deno.env.get("GCLOUD_PROJECT") || "";
    this.region = opts?.region || Deno.env.get("GOOGLE_CLOUD_REGION") ||
      "us-central1";
  }

  /** Run a gcloud CLI command with graceful failure handling. */
  private async gcloud(
    args: string[],
    ...
    try {
      ...
      if (exitCode !== 0) {
        if (String(exitCode) === "ENOENT") {
          return {
            ok: false,
            stdout,
            error: `gcloud CLI not available: ${stderr || "command not found"}`,
          };
        }
        ...
        return {
          ok: false,
          stdout,
          error: stderr || `gcloud exited with code ${exitCode}`,
        };
      }
      ...
      return {
        ok: false,
        stdout: "",
        error: `gcloud CLI not available: ${
  ...
  async createObjectStorage(name: string): Promise<ProvisionResult> {
    ...
    try {
      const { stdout: _stdout, stderr, exitCode } = await execCommand(
        "gsutil",
        ["mb", "-l", this.region, `gs://${bucketName}`],
      );
      if (exitCode !== 0) {
        if (String(exitCode) === "ENOENT") {
          return {
            ...
            type: "object-storage",
            status: "failed",
            error: `gsutil not available: ${stderr || "command not found"}`,
          };
        }
        if (isAlreadyExistsError(GCP_ALREADY_EXISTS_PATTERNS, stderr)) {
          return {
        ...
        return {
          ...
          type: "object-storage",
          status: "failed",
          error: stderr || `gsutil exited with code ${exitCode}`,
        };
      }
      return {
        name,
  ...
  async createVectorIndex(
    ...
    return {
      ...
      type: "vectorize",
      status: "skipped",
      error: "Vector index provisioning is not supported on GCP provider",
    };
  }
