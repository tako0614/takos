...
export class AWSProvider implements ResourceProvider {
      ...
      if (exitCode !== 0) {
        if (String(exitCode) === "ENOENT") {
          return {
            ok: false,
            stdout,
            error: `aws CLI not available: ${stderr || "command not found"}`,
          };
        }
        ...
        return {
          ok: false,
          stdout,
          error: stderr || `aws exited with code ${exitCode}`,
        };
      }
  ...
  async createVectorIndex(
    ...
    return {
      ...
      type: "vectorize",
      status: "skipped",
      error: "Vector index provisioning is not supported on AWS provider",
    };
  }
