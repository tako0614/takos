...
const ja: Strings = {
  ...
  install: {
    ...
    lede: [
      ...
      { t: ' 上で動くので、Takosumi から 1-click で install できる。自分の cloud / 自前 VM で動かすなら、Git source を install lifecycle に渡す。' },
    ],
    cards: [
      {
        kind: 'use',
        title: 'Use Takos',
        body: '一般ユーザー向け。Takosumi の account-plane で Account / Space / launch を進める。public managed offering が閉じている間は launch readiness gate で止まります。',
        cta: 'Use Takos',
      },
      {
        kind: 'git',
      ...
      {
        kind: 'self',
        title: 'Self-host — takosumi CLI',
        body: '自前の Takosumi substrate に install したい人向け。provider target と PlatformService inventory は operator distribution 側で選び、source は同じ Git URL / ref で扱える。',
      },
    ],
  },
  footer: {
...
const en: Strings = {
  ...
  install: {
    ...
    lede: [
      { t: 'Takos runs on ' },
      { t: 'Takosumi', code: true },
      { t: ', so you can install it in one click from Takosumi. To run it on your own cloud or VM, hand a Git source to the install lifecycle.' },
    ],
    cards: [
      {
        kind: 'use',
        title: 'Use Takos',
        body: 'For everyday users. Move through Account / Space / launch in the Takosumi account-plane. While the public managed offering is closed, this stops at the launch readiness gate.',
        cta: 'Use Takos',
      },
      {
        kind: 'git',
      ...
      },
      {
        kind: 'self',
        title: 'Self-host — takosumi CLI',
        body: 'For installing onto your own Takosumi substrate. The operator distribution chooses the provider target and PlatformService inventory; the source is the same Git URL / ref.',
      },
    ],
  },
  footer: {
