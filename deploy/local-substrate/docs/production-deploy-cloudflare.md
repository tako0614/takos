...
3. **API token** with: `Workers Scripts:Edit`, `Workers Routes:Edit`, `D1:Edit`, `Pages:Edit`, `DNS:Edit` for
   `takosumi.com`.
...
The local-substrate runs the **same bundled Worker file** that `wrangler deploy` ships:

```
takosumi-cloud/deploy/cloudflare/.wrangler/dist/takosumi-cloud-accounts-worker.mjs
```

The build container produces it; Miniflare runs it locally with an emulated D1 (SQLite). The difference between local
and prod is the D1 backend (SQLite vs Cloudflare's distributed DB) and the bindings' values (local issuer / client
URLs). Code path is identical.

If `cloud.takosumi.test/.well-known/openid-configuration` returns 200 in local-substrate, the same endpoint at
`cloud.takosumi.com` will too, modulo Cloudflare-side configuration.
