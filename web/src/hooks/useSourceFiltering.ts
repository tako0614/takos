...
export function useSourceFiltering({
  ...
  const effectiveSpaceId = createMemo(() => {
    ...
    if (!isAuthenticated() || id === null) return null;
    return spaceIds().has(id) ? id : null;
  });
  ...
  createEffect(() => {
    ...
    const currentSelectedId = selectedSpaceId();
    if (currentSelectedId !== null && spaceIds().has(currentSelectedId)) {
      return;
    }
  ...
  createEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        setSuggesting(true);
        const response = await rpcPath(rpc, "explore", "catalog", "suggest")
          .$get({
            query: { q, limit: "6" },
          });
