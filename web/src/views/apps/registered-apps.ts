...
/**
 * Resets the process-local per-tab registered-apps cache. Exported solely so
 * tests can isolate cases; it is not part of the runtime UI surface.
 */
...
export function useRegisteredApps(spaceId: Accessor<string>) {
  ...
  const fetchApps = async () => {
    ...
    try {
      ...
      registeredAppsCache.set(currentSpaceId, items);
      setApps(items);
    } catch (err) {
      if (requestId !== requestSeq || spaceId() !== currentSpaceId) return;
