...
export type ResolvedHandler =
  | { type: "local"; localId: "text-editor" | "image-viewer" }
...
export const LOCAL_TEXT_EDITOR = "text-editor" as const;
export const LOCAL_IMAGE_VIEWER = "image-viewer" as const;
...
function handlerMatchesFile(h: FileHandler, file: StorageFile): boolean {
  ...
  if (
    mimeType && h.mime_types.some((m) => {
      if (m.endsWith("/*")) return mimeType.startsWith(m.slice(0, -1));
      return m === mimeType;
    })
  ) return true;
  return false;
...
/**
 * Collect all handlers that can open a file (local + app handlers).
 * Local handlers are always appended at the end as fallbacks.
 */
export function resolveHandlers(
  file: StorageFile,
  appHandlers: FileHandler[],
): ResolvedHandler[] {
  ...
  if (isImage) {
    result.push({ type: "local", localId: LOCAL_IMAGE_VIEWER });
  }

  // Local text editor (always available as fallback)
  result.push({ type: "local", localId: LOCAL_TEXT_EDITOR });
...
function getHandlerKey(h: ResolvedHandler): string {
  return h.type === "local" ? `__local_${h.localId}__` : h.handler.id;
}
...
export function handlerDisplayName(
  h: ResolvedHandler,
