...
export function PersistedToolCalls(props: { toolExecutions: ToolExecution[] }) {
  ...
  const totalDuration = () =>
    props.toolExecutions.reduce((sum, te) => sum + (te.duration_ms || 0), 0);
      ...
      <div class="mb-3">
        ...
        >
          ...
          <span class="text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <Show when={hasErrors()}>
              <span class="text-red-500 dark:text-red-400 font-medium ml-1">
                {t("withErrors")}
              </span>
            </Show>
          </span>
        ...
        <Show when={isExpanded()}>
          <div class="mt-1 ml-1 pl-4 border-l-2 border-zinc-200 dark:border-zinc-700 space-y-0.5">
            <For each={props.toolExecutions}>
              {(te, index) => {
                const hasDetails = () =>
                  Object.keys(te.arguments || {}).length > 0 || !!te.result ||
                  !!te.error;
                return (
                  <div>
                    ...
                    <Show when={expandedTools().has(index()) && hasDetails()}>
                      <div class="ml-5 pb-2 text-xs space-y-2">
                        <Show when={Object.keys(te.arguments || {}).length > 0}>
                          <div>
                            <div class="text-zinc-500 dark:text-zinc-400 font-medium mb-1">
                              {t("toolArguments")}
                            </div>
                        ...
                        <Show
                          when={te.result && !te.error ? te.result : undefined}
                        >
                          {(result) => (
                            <div>
                              <div class="text-zinc-500 dark:text-zinc-400 font-medium mb-1">
