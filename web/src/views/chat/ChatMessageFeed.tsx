...
function getToolLabel(
  ...
  if (normalized.includes("search")) return t("toolStatus_searching");
  if (normalized.includes("fetch") || normalized.includes("browse")) {
    return t("toolStatus_fetching");
  }
  if (normalized.includes("read")) return t("toolStatus_reading");
  if (normalized.includes("write")) return t("toolStatus_writing");
  if (normalized.includes("list")) return t("toolStatus_listing");
  if (normalized.includes("exec")) return t("toolStatus_executing");
  if (normalized.includes("deploy")) return t("toolStatus_deploying");
  if (
    normalized.includes("memory") || normalized.includes("remember") ||
    normalized.includes("recall")
  ) return t("toolStatus_memory");
  return `${name}`;
}
  ...
  const entries = createMemo<ActivityDisplayEntry[]>(() => {
    ...
    for (const toolCall of props.toolCalls) {
      ...
      items.push({
        ...
        message: isRunning ? getToolLabel(toolCall.name, t) : toolCall.name,
