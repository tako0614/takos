import { createMemo, type JSX } from "solid-js";
import { useI18n } from "../../store/i18n.ts";
import type { Resource } from "../../types/index.ts";
import type { ResourceDetailTab } from "./worker-models.ts";
import { getResourceStatusLabel } from "./utils/resourceUtils.tsx";
import { Breadcrumb } from "../../components/ui/Breadcrumb.tsx";
import { ResourceOverviewTab } from "./tabs/ResourceOverviewTab.tsx";
import { D1ExplorerTab } from "./tabs/D1ExplorerTab.tsx";
import { R2BrowserTab } from "./tabs/R2BrowserTab.tsx";
import { ResourceBindingsTab } from "./tabs/ResourceBindingsTab.tsx";
import { ResourceSettingsTab } from "./tabs/ResourceSettingsTab.tsx";

export interface ResourceDetailProps {
  resource: Resource;
  tab: ResourceDetailTab;
  onBack: () => void;
  onTabChange: (tab: ResourceDetailTab) => void;
  getResourceTypeIcon: (type: Resource["type"]) => JSX.Element;
  getResourceTypeName: (type: Resource["type"]) => string;
  getResourceStatusBgClass: (status: Resource["status"]) => string;
  onDeleteResource: () => void;
}

export function ResourceDetail(props: ResourceDetailProps) {
  const { t } = useI18n();
  const statusLabel = createMemo(() =>
    getResourceStatusLabel(props.resource.status, t)
  );

  const tabClass = (tabId: ResourceDetailTab): string =>
    `px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${
      props.tab === tabId
        ? "bg-zinc-100 dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100"
        : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-700"
    }`;

  const breadcrumbItems = createMemo(() => [
    { label: t("resources"), onClick: props.onBack },
    { label: props.resource.name },
  ]);

  return (
    <div class="flex flex-col h-full bg-white dark:bg-zinc-900">
      <header class="flex flex-col gap-3 px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800">
        <Breadcrumb items={breadcrumbItems()} />
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-3 flex-1">
            <span
              class="w-10 h-10 rounded-xl bg-zinc-100 dark:bg-zinc-700 flex items-center justify-center text-xl text-zinc-900 dark:text-zinc-100"
              aria-hidden="true"
            >
              {props.getResourceTypeIcon(props.resource.type)}
            </span>
            <div>
              <h1 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
                {props.resource.name}
              </h1>
              <span class="text-sm text-zinc-500 dark:text-zinc-400">
                {props.getResourceTypeName(props.resource.type)}
              </span>
            </div>
          </div>
          <div
            class="flex items-center gap-2"
            aria-label={t("resourceStatusLabel", {
              status: statusLabel(),
            })}
          >
            <span
              class={`w-2 h-2 rounded-full ${
                props.getResourceStatusBgClass(props.resource.status)
              }`}
              aria-hidden="true"
            />
            <span class="text-sm text-zinc-500 dark:text-zinc-400">
              {statusLabel()}
            </span>
          </div>
        </div>
      </header>

      <nav
        class="flex gap-1 px-6 pt-4 border-b border-zinc-200 dark:border-zinc-700"
        role="tablist"
        aria-label={t("resourceSections")}
      >
        <button
          type="button"
          role="tab"
          aria-selected={props.tab === "overview"}
          aria-controls="tabpanel-overview"
          class={tabClass("overview")}
          onClick={() => props.onTabChange("overview")}
        >
          {t("overview")}
        </button>
        {props.resource.type === "sql" && (
          <button
            type="button"
            role="tab"
            aria-selected={props.tab === "explorer"}
            aria-controls="tabpanel-explorer"
            class={tabClass("explorer")}
            onClick={() => props.onTabChange("explorer")}
          >
            {t("explorer")}
          </button>
        )}
        {props.resource.type === "object-store" && (
          <button
            type="button"
            role="tab"
            aria-selected={props.tab === "browser"}
            aria-controls="tabpanel-browser"
            class={tabClass("browser")}
            onClick={() => props.onTabChange("browser")}
          >
            {t("browser")}
          </button>
        )}
        <button
          type="button"
          role="tab"
          aria-selected={props.tab === "bindings"}
          aria-controls="tabpanel-bindings"
          class={tabClass("bindings")}
          onClick={() => props.onTabChange("bindings")}
        >
          {t("bindings")}
        </button>
        <button
          type="button"
          role="tab"
          aria-selected={props.tab === "settings"}
          aria-controls="tabpanel-settings"
          class={tabClass("settings")}
          onClick={() => props.onTabChange("settings")}
        >
          {t("settings")}
        </button>
      </nav>

      <div
        class="flex-1 overflow-auto p-6"
        role="tabpanel"
        id={`tabpanel-${props.tab}`}
        aria-label={t(props.tab)}
      >
        {props.tab === "overview" && (
          <ResourceOverviewTab resource={props.resource} />
        )}
        {props.tab === "explorer" && props.resource.type === "sql" && (
          <D1ExplorerTab resource={props.resource} />
        )}
        {props.tab === "browser" && props.resource.type === "object-store" && (
          <R2BrowserTab resource={props.resource} />
        )}
        {props.tab === "bindings" && (
          <ResourceBindingsTab resource={props.resource} />
        )}
        {props.tab === "settings" && (
          <ResourceSettingsTab onDeleteResource={props.onDeleteResource} />
        )}
      </div>
    </div>
  );
}
