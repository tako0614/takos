...
export function getWorkerDisplayName(
  worker: Pick<Worker, "id" | "slug" | "service_name">,
): string {
  return worker.slug?.trim() || worker.service_name?.trim() || worker.id;
}

export function getWorkerDisplayHostname(
  worker: Pick<Worker, "hostname">,
): string {
...
export function getWorkerStatusIndicatorClass(
