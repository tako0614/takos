...
export function GeneralTab(props: GeneralTabProps) {
  ...
  const platformDomain = () =>
    props.worker.hostname?.includes(".")
      ? props.worker.hostname.split(".").slice(1).join(".")
      : "";
    ...
    <div class="space-y-6">
      <div class="space-y-2">
        ...
        <button
          ...
          onClick={props.onSaveSlug}
          disabled={props.savingSlug || !props.editSlug.trim() ||
            props.editSlug === (props.worker.slug ?? "")}
        >
          {props.savingSlug ? t("saving") : t("saveSubdomain")}
        </button>
      </div>
        ...
        <div class="flex items-center gap-2">
          <span
            class={`w-2 h-2 rounded-full ${
              getWorkerStatusIndicatorClass(props.worker.status)
            }`}
          />
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center gap-2">
          <code class="px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded text-xs text-zinc-500 dark:text-zinc-400 font-mono">
            {props.worker.id}
          </code>
        </div>
      </div>
