...
import { repoBlob, repoBranches, rpc, rpcJson } from "../../../lib/rpc.ts";
...
async function resolveReadmeContent(
  repoId: string,
  branch: string,
): Promise<string | null> {
  for (const filename of README_CANDIDATES) {
    try {
      ...
      if (
        res.status === 404 &&
        (apiError === "File not found" || apiError === "Path not found")
      ) {
        continue;
      }
      ...
      if (
        ...
        NON_RETRYABLE_ERRORS.has(apiError)
      ) {
        break;
      }
    } catch {
...
export function RepoDetail(props: RepoDetailProps) {
  ...
  const [selectedFile, setSelectedFile] = createSignal<SelectedFile>({
    path: props.initialFilePath || null,
    line: props.initialFileLine ?? null,
  });
  const [star, setStar] = createSignal<StarState>({
    starred: props.repo.is_starred ?? false,
    count: props.repo.stars,
  });
  ...
  const [readmeState, setReadmeState] = createSignal<ReadmeState>({
    status: "idle",
    content: null,
  });
  let readmeSeq = 0;
  ...
  const fetchBranches = async () => {
    ...
    try {
      const data = await repoBranches(repoId);
  ...
  createEffect(on(
    () => [props.repo.id, props.initialRef],
    () => {
      setCurrentBranch(props.initialRef || props.repo.default_branch);
    },
  ));

  createEffect(on(
    () => [props.initialFilePath, props.initialFileLine],
    () => {
      setSelectedFile({
        path: props.initialFilePath || null,
        line: props.initialFileLine ?? null,
  ...
  createEffect(on(
    ...
    () => {
      ...
      const fallbackBranch = br.find((branch) => branch.is_default)?.name ||
        br[0].name;
  ...
  const toggleStar = async () => {
    ...
    setStar({
      ...
      count: prev.count + (prev.starred ? -1 : 1),
    });
    ...
    try {
      if (prev.starred) {
        ...
        await rpc.repos[":repoId"].star.$post({
          param: { repoId: props.repo.id },
        });
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await rpc.repos[":repoId"].$delete({
        param: { repoId: props.repo.id },
      });
      await rpcJson(res);
  ...
  const tabs: { id: TabType; label: string; icon: JSX.Element }[] = [
    { id: "code", label: t("code"), icon: <Icons.Code class="w-4 h-4" /> },
    {
      id: "search",
      label: t("search"),
      icon: <Icons.Search class="w-4 h-4" />,
    },
    {
      id: "commits",
      label: t("commits"),
      icon: <Icons.Clock class="w-4 h-4" />,
    },
    {
      id: "branches",
      label: t("branches"),
      icon: <Icons.GitMerge class="w-4 h-4" />,
    },
    {
      id: "pull-requests",
                  ...
                  <button
                    ...
                    onClick={() =>
                      showToast(
                        "info",
                        `${t("upstreamSync")}: ${
                          forkedFrom().owner_username || t("unknown")
                        }/${forkedFrom().name}`,
                      )}
                  >
                    {forkedFrom().owner_username ||
                      forkedFrom().owner_name || t("unknown")}/{forkedFrom()
                      .name}
                  </button>
                </div>
              )}
