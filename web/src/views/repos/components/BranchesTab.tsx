...
import { repoBranches, rpc, rpcJson, rpcPath } from "../../../lib/rpc.ts";
...
export function BranchesTab(props: BranchesTabProps) {
  ...
  const fetchBranches = async () => {
    ...
    try {
      ...
      if (seq !== branchesSeq || repoId !== props.repoId) return;
      setBranches(data.branches);
    } catch (err) {
      if (seq !== branchesSeq || repoId !== props.repoId) return;
      setError(err instanceof Error ? err.message : t("failedToLoad"));
    } finally {
      if (seq === branchesSeq && repoId === props.repoId) {
        setLoading(false);
      }
    }
  };
  ...
  const handleSetDefault = async (branchName: string) => {
    ...
    try {
      ...
      showToast(
        "error",
        err instanceof Error ? err.message : t("operationFailed"),
      );
    } finally {
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                      <Show when={!branch.is_default}>
                        ...
                        >
                          <Show
                            when={actionLoading() === branch.name}
                            fallback={<Icons.Star class="w-4 h-4" />}
                          >
