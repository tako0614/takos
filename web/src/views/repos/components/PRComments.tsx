...
import type { PRComment, PRReview, PullRequest } from "../../../types/index.ts";
import { formatDateTime } from "../../../lib/format.ts";
import { authorAvatarInitial, displayAuthorName } from "../../../lib/author.ts";
...
function renderAvatar(
  ...
  return (
    <div
      class={`${sizeClass} rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center text-zinc-700 dark:text-zinc-300 font-medium`}
    >
      {initial}
    </div>
  );
  ...
  return (
    <div class="mx-auto space-y-4">
        ...
        <Card padding="md">
          <div class="flex gap-4">
            <div class="flex-shrink-0">
              {renderAvatar(
                props.pr.author,
                t("avatarAlt", { name: displayAuthorName(props.pr.author, t) }),
                authorAvatarInitial(props.pr.author, t),
              )}
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 text-sm">
                <span class="font-medium text-zinc-900 dark:text-zinc-100">
                  {displayAuthorName(props.pr.author, t)}
                </span>
                <span class="text-zinc-500">
                  {formatDateTime(props.pr.created_at)}
                </span>
              </div>
          ...
          <Card padding="md">
            <div class="flex gap-4">
              <div class="flex-shrink-0">
                {renderAvatar(
                  review.author,
                  t("avatarAlt", { name: displayAuthorName(review.author, t) }),
                  authorAvatarInitial(review.author, t),
                )}
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 text-sm flex-wrap">
                  <span class="font-medium text-zinc-900 dark:text-zinc-100">
                    {displayAuthorName(review.author, t)}
                  </span>
            ...
            <div class="flex gap-4">
              <div class="flex-shrink-0">
                {renderAvatar(
                  comment.author,
                  t("avatarAlt", {
                    name: displayAuthorName(comment.author, t),
                  }),
                  authorAvatarInitial(comment.author, t),
                )}
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 text-sm">
                  <span class="font-medium text-zinc-900 dark:text-zinc-100">
                    {displayAuthorName(comment.author, t)}
                  </span>
                  <span class="text-zinc-500">
      ...
      <Card padding="md">
        <Textarea
          ...
          onInput={(e: Event & { currentTarget: HTMLTextAreaElement }) =>
            props.onNewCommentChange(e.currentTarget.value)}
          rows={3}
          style={{ resize: "none" }}
        />
      ...
      <Show when={props.pr.status === "open" && props.showReviewForm}>
        <Card padding="md">
          <h4 class="text-sm font-medium text-zinc-900 mb-3">
            {t("prSubmitReview")}
          </h4>
          <Textarea
            ...
            onInput={(e: Event & { currentTarget: HTMLTextAreaElement }) =>
              props.onReviewCommentChange(e.currentTarget.value)}
            rows={3}
            style={{ resize: "none" }}
          />
