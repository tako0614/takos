...
import type { SourceInstallSource } from "../../hooks/sourceInstall.ts";
...
export function SourcePage(props: SourcePageProps) {
  ...
  const [gitUrlInstallSeed, setGitUrlInstallSeed] = createSignal<
    {
      gitUrl: string;
      ref: string;
      revision?: {
  ...
  const openGitUrlInstall = (
    ...
    const installationId = item?.installation?.installation_id ?? null;
    setGitUrlInstallSeed(
      source
        ? {
          gitUrl: source.repository_url,
          ref: source.ref,
          ...(installationId
            ? { revision: { installationId, operation: "upgrade" } }
            : {}),
        }
        : null,
    );
    setShowGitUrlInstallModal(true);
  };
  ...
  const {
    ...
    onInstallGitUrl: (source, item) => openGitUrlInstall(source, item),
  });
    ...
    <div class="h-full flex flex-col bg-zinc-50 dark:bg-zinc-900 overflow-hidden">
      ...
      {showGitUrlInstallModal() && (
        <GitUrlInstallModal
          ...
          initialGitUrl={gitUrlInstallSeed()?.gitUrl ?? null}
          initialRef={gitUrlInstallSeed()?.ref ?? null}
          revision={gitUrlInstallSeed()?.revision ?? null}
          onClose={() => {
            setShowGitUrlInstallModal(false);
            setGitUrlInstallSeed(null);
          }}
          onApplied={refresh}
        />
      )}
