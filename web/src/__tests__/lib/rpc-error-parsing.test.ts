...
import { deepStrictEqual as assertEquals, ok as assertOk } from "node:assert/strict";
...
async function getRejection<T>(fn: () => Promise<T>): Promise<unknown> {
  try {
    await fn();
  } catch (error) {
    return error;
...
test("rpcJson - flattens takos common envelope { error: { code, message } }", async () => {
  const res = makeResponse(
    { error: { code: "NOT_FOUND", message: "Repository not found" } },
    404,
  );
  const err = await getRejection(() => rpcJson(res));
  assertOk(err instanceof Error);
  assertEquals(err.message, "Repository not found");
});

test("rpcJson - falls back to envelope code when message is absent", async () => {
...
test("rpcJson - reads OAuth error_description (RFC 6749)", async () => {
...
test("rpcJson - falls back to OAuth error code when description is absent", async () => {
...
test("rpcJson - rejects arbitrary plain-string error messages", async () => {
...
test("rpcJson - returns 'Request failed' when body is empty", async () => {
...
test("rpcJson - returns parsed JSON on 2xx", async () => {
  const res = makeResponse({ hello: "world" }, 200);
  const data = await rpcJson<{ hello: string }>(res);
  assertEquals(data.hello, "world");
});

test("apiJson - fetches JSON with an Accept header", async () => {
...
test("apiJson - times out hung requests", async () => {
  const originalFetch = globalThis.fetch;
  let aborted = false;
  try {
    ...
    const err = await getRejection(() => apiJson("/api/hangs", { timeoutMs: 10 }));
    assertOk(err instanceof Error);
