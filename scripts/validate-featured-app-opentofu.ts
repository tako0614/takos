...
const featuredApps: readonly FeaturedAppSource[] = [
  { name: "takos-office", path: "takos-apps/takos-office" },
  { name: "takos-computer", path: "takos-apps/takos-computer" },
  { name: "takos-storage", path: "takos-storage" },
  { name: "takos-git", path: "takos-git" },
  { name: "yurucommu", path: "yurucommu", requiresPostApply: true },
  { name: "yurumeet", path: "yurumeet", requiresPostApply: true },
];
...
for (const app of featuredApps) {
  results.push(await validateFeaturedApp(app));
}

console.log(JSON.stringify({ ok: true, apps: results }, null, 2));

async function validateFeaturedApp(app: FeaturedAppSource): Promise<{
  name: string;
  manifestName?: string;
  publications: number;
  serviceExports: number;
  ...
  const workdir = await runtime.makeTempDir({
    prefix: `takos-featured-${app.name}-`,
  });
  try {
    ...
    const runtimeProjection = parseRuntimeProjectionOutputs(outputJson, source);
    const launcherExport = runtimeProjection.exports.find((serviceExport) =>
    ...
    if (hasAppDeploymentOutput) {
      ...
      for (const [index, publication] of manifest.publish.entries()) {
        if (LEGACY_PUBLICATION_TYPES.has(publication.type)) {
          throw new Error(
            `${app.name}: publish[${index}].type must use canonical runtime projection capability, got ${publication.type}`,
          );
        }
      }
    ...
    return {
      ...
      publications,
      serviceExports: runtimeProjection.exports.length,
      serviceBindings: runtimeProjection.bindings.length,
...
function parseRuntimeProjectionOutputs(
  raw: string,
  source: string,
  ...
  return value.map((entry, index) => {
    ...
    for (const capability of record.capabilities) {
      if (LEGACY_PUBLICATION_TYPES.has(capability)) {
        throw new Error(
          `${source}.service_exports[${index}].capabilities must use canonical runtime projection capability, got ${capability}`,
        );
      }
    }
    return {
