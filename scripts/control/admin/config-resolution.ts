...
export function inferDefaultR2Buckets(
...
export function inferR2BucketAliases(
  ...
  const envPrefix = environment === "production"
    ? "env.production"
    : "env.staging";
  const envEntries = readTomlArraySections(
    wranglerToml,
    `${envPrefix}.r2_buckets`,
  );
...
export function inferAccountId(
  ...
  const envPrefix = environment === "production"
    ? "env.production"
    : "env.staging";
  const varsSection = readTomlSection(wranglerToml, `${envPrefix}.vars`) ||
    readTomlSection(wranglerToml, "vars");
...
export function resolveEnvironment(args: string[]): DeployEnvironment {
  ...
  if (hasLocal) {
    fail(
      "Local mode is not supported by admin-cli. Use Cloudflare API targets (production/staging).",
    );
  }
...
export function parseGlobalOptions(
  rawArgs: string[],
): { remainingArgs: string[]; options: GlobalOptions } {
  const args = [...rawArgs];
...
export function resolveConfig(options: GlobalOptions): ResolvedConfig {
  ...
  if (!accountId) {
    fail(
      "CF_ACCOUNT_ID is required (env var, --account-id, or wrangler.toml).",
    );
  }

  if (!apiToken) {
    fail("CF_API_TOKEN is required (env var or --api-token).");
  }
