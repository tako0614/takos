...
export function hasValidWhereClause(
...
export function requireApprovalId(
...
export function enforceTenantSqlAccessPolicy(
  sql: string,
  options: GlobalOptions,
): void {
  ...
  requireApprovalId(
    options,
    `Tenant/workspace D1 access (${matchedTokens.join(", ")})`,
  );
  ...
  if (!scopeWorkspaceId && !scopeUserId) {
    fail(
      "Tenant/workspace D1 access requires --scope-workspace-id or --scope-user-id.",
    );
  }
  ...
  if (scopeWorkspaceId) {
    if (!/\bworkspace_id\b/.test(normalized)) {
      fail(
        "Tenant/workspace D1 access with --scope-workspace-id requires workspace_id predicate in SQL.",
      );
    }
  ...
  if (scopeUserId) {
    if (!/\buser_id\b/.test(normalized)) {
      fail(
        "Tenant/workspace D1 access with --scope-user-id requires user_id predicate in SQL.",
      );
    }
...
export function enforceTenantR2AccessPolicy(
  options: GlobalOptions,
  operation: string,
  pathOrPrefix: string,
): void {
  ...
  if (!scopePrefix) {
    fail(
      `Tenant/workspace R2 access (${operation}) requires --scope-r2-prefix.`,
    );
  }
  ...
  if (!normalizedTarget) {
    fail(
      `R2 ${operation} requires a target prefix within --scope-r2-prefix (${scopePrefix}).`,
    );
  }

  if (
    !(normalizedTarget === scopePrefix ||
      normalizedTarget.startsWith(`${scopePrefix}/`))
  ) {
    fail(
      `R2 ${operation} target must stay within --scope-r2-prefix (${scopePrefix}).`,
    );
  }
}
