...
const fixtureCases = [
  {
    target: 'aws' as const,
    inputPath: 'deploy/terraform/testdata/helm-values/aws-output.json',
    expectedPath: 'deploy/terraform/testdata/helm-values/aws-values.yaml',
  },
  {
    target: 'gcp' as const,
    inputPath: 'deploy/terraform/testdata/helm-values/gcp-output.json',
    expectedPath: 'deploy/terraform/testdata/helm-values/gcp-values.yaml',
  },
];
...
function generateHelmValues(outputs: TerraformOutputs, options: GenerateOptions): string {
  ...
  return [
    '# Generated by scripts/terraform-output-to-helm-values.ts.',
    `# Source: ${options.source}`,
    '# Contains only non-sensitive Terraform outputs. Secrets stay in takos-private or external secrets.',
    ...renderYamlObject(values, 0),
    '',
  ].join('\n');
}

function queueBindings(outputs: TerraformOutputs, target: TargetId): JsonRecord {
...
function objectStorageBuckets(outputs: TerraformOutputs, target: TargetId): JsonRecord {
...
function parseTerraformOutputs(text: string, source: string): TerraformOutputs {
  const parsed = JSON.parse(text) as unknown;
  if (!isJsonRecord(parsed)) {
    throw new Error(`${source} must be a Terraform output JSON object`);
  }

  const outputs: TerraformOutputs = {};
  for (const [key, value] of Object.entries(parsed)) {
    if (!isJsonRecord(value)) {
      throw new Error(`${source}.${key} must be a Terraform output record`);
    }
    outputs[key] = {
...
function targetOutput(outputs: TerraformOutputs): TargetId | null {
  const value = optionalStringOutput(outputs, ['target']);
  if (!value) return null;
  if (value === 'aws' || value === 'gcp') return value;
  throw new Error(`Terraform output target must be aws or gcp, got ${value}`);
}

function inferTarget(outputs: TerraformOutputs): TargetId {
  if (
...
function requiredStringOutput(outputs: TerraformOutputs, keys: readonly string[]): string {
  const value = optionalStringOutput(outputs, keys);
  if (value) return value;

  if (keys.includes('database_endpoint') && outputs.database_url?.sensitive) {
    throw new Error(
      'Terraform output database_endpoint is required. Refusing to bridge sensitive database_url into Helm values.',
    );
  }

  throw new Error(`Missing required Terraform output: ${keys.join(' or ')}`);
}

function optionalStringOutput(outputs: TerraformOutputs, keys: readonly string[]): string | null {
  for (const key of keys) {
    const value = outputValue(outputs, key);
    if (value === null) continue;
    const string = jsonString(value);
    if (string === null) {
      throw new Error(`Terraform output ${key} must be a string`);
    }
    return string;
  }
  return null;
...
function optionalStringMapOutput(outputs: TerraformOutputs, key: string): JsonRecord | null {
  const value = outputValue(outputs, key);
  if (value === null) return null;
  if (!isJsonRecord(value)) {
    throw new Error(`Terraform output ${key} must be an object`);
  }
  ...
  for (const [entryKey, entryValue] of Object.entries(value)) {
    ...
    if (string === null) {
      throw new Error(`Terraform output ${key}.${entryKey} must be a string`);
    }
...
function optionalCanonicalMap(
  ...
  const result: JsonRecord = {};
  for (const [terraformKey, helmKey] of Object.entries(keyMap)) {
    const entry = value[terraformKey];
    if (entry === undefined || entry === null) continue;
    result[helmKey] = canonicalOutputValue(entry, `${key}.${terraformKey}`);
  }
  return result;
}

function outputValue(outputs: TerraformOutputs, key: string): JsonValue | null {
  const output = outputs[key];
  if (!output) return null;
  if (output.sensitive) {
    throw new Error(`Terraform output ${key} is sensitive and cannot be written to Helm values`);
  }
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      if (typeof item !== 'string') {
        throw new Error(`Terraform output ${label} must contain only strings`);
      }
    }
    return value;
  }
  throw new Error(`Terraform output ${label} must be a string or string array`);
}
...
async function terraformOutputJson(terraformDir: string): Promise<string> {
  ...
  if (!output.success) {
    ...
    throw new Error(`terraform output -json failed in ${terraformDir}:\n${stderr}`);
  }
...
function parseArgs(values: readonly string[]): Args {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (value) {
      ...
      case '--terraform-dir':
        parsed.terraformDir = requiredArgValue(values, index, value);
        index += 1;
        break;
      case '--help':
      case '-h':
