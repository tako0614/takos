...
const opentofuRoot = `${root}/deploy/opentofu/cloudflare`;
...
async function runPlan(target: TargetId): Promise<PlanResult> {
  ...
  console.error(
    `cloudflare-opentofu-plan-gate: ${planCase.label}: ${counts.add} add, ${counts.change} change, ${counts.destroy} destroy; wrote ${
...
async function opentofu(input: OpenTofuCommand): Promise<CommandOutput> {
  console.error(`cloudflare-opentofu-plan-gate: ${input.name}: ${[args.opentofuBin, ...input.args].join(' ')}`);
...
function renderMarkdownSummary(results: PlanResult[]): string {
  const lines = [
    '# Cloudflare OpenTofu Plan Gate',
    '',
    'Staging plans are generated from the `deploy/opentofu/cloudflare` adapter with `opentofu_plan_mode = true` and a temporary CI-only Cloudflare token env placeholder.',
...
function usage(): string {
  return [
    'Usage:',
    '  bun run cloudflare:opentofu:plan-gate',
    '  bun run cloudflare:opentofu:plan-gate --target cloudflare --out-dir .opentofu-plan',
    '  TAKOS_OPENTOFU_BIN=/path/to/tofu bun run cloudflare:opentofu:plan-gate',
  ].join('\n');
}
