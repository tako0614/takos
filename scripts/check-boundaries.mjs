...
const checks = [
  ...
  {
    name: "control app script/test deep control package source import",
    files:
      /^apps\/control\/(?:(?:src\/__tests__|test|scripts)\/|src\/.*\.test\.)/,
    pattern: /\bpackages\/control\/src\b/g,
    allowFile: isAllowedControlAppTestPackageSourceImport,
  },
  {
    name: "runtime app script/test deep runtime package source import",
    files:
      /^apps\/runtime\/(?:(?:src\/__tests__|test|scripts)\/|src\/.*\.test\.)/,
    pattern: /\bpackages\/runtime-service\/src\b/g,
    allowFile: isAllowedRuntimeAppTestPackageSourceImport,
  },
  {
    name: "control app script/test cross-package source import",
    files:
      /^apps\/control\/(?:(?:src\/__tests__|test|scripts)\/|src\/.*\.test\.)/,
    pattern: /\bpackages\/runtime-service\/src\b/g,
  },
  {
    name: "runtime app script/test cross-package source import",
    files:
      /^apps\/runtime\/(?:(?:src\/__tests__|test|scripts)\/|src\/.*\.test\.)/,
    pattern: /\bpackages\/control\/src\b/g,
  },
...
function isAllowedControlAppTestPackageSourceImport(relPath) {
  return (
    relPath.startsWith("apps/control/src/__tests__/") ||
    relPath.startsWith("apps/control/test/")
  );
}

function isAllowedRuntimeAppTestPackageSourceImport(relPath) {
  return (
    relPath.startsWith("apps/runtime/src/__tests__/") ||
    relPath.startsWith("apps/runtime/test/")
  );
}
...
for await (const filePath of walk(repoRoot)) {
  ...
  for (const check of checks) {
    ...
    if (check.allowFile?.(relPath)) continue;
...
if (violations.length > 0) {
  for (const violation of violations) {
    console.error(
      `${violation.check}: ${violation.path}:${violation.lines.join(",")}`,
    );
  }
  Deno.exit(1);
}
