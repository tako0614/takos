...
const checks = [
  {
    name: 'package to PaaS app source dependency',
    files: /^packages\//,
    pattern: /\bapps\/paas\/src\b/g,
  },
  {
    name: 'PaaS app deep contract source import',
...
for (const check of appWrapperChecks) {
  ...
  for await (const filePath of walk(appRoot)) {
    ...
    violations.push({
      check: isThinCompatibilityModule(text) ? 'disallowed app compatibility module' : 'app source duplication',
