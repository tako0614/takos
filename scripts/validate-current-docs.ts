...
async function validateRemovedHistoricalDocs(): Promise<string[]> {
  ...
  for (const path of forbiddenPaths) {
    if (await exists(path)) {
      errors.push(
        `${path}: historical or no-user clean-cut docs must not be kept as current Takos docs`,
      );
    }
  }
  return errors;
...
async function validateVitePressExcludesNonCurrentDocs(): Promise<string[]> {
  ...
  for (const required of ['"contributing/**"', '"releases/**"']) {
    if (!config.includes(required)) {
      errors.push(
        `docs/.vitepress/config.ts: srcExclude must include ${required}`,
      );
    }
  }
  return errors;
...
async function validateContributingIndex(): Promise<string[]> {
  ...
  return forbidden
    ...
    .map(
      (term) =>
        `docs/contributing/index.md: remove non-current docs reference '${term}'`,
    );
}
...
async function validateCurrentInstallDocs(): Promise<string[]> {
  ...
  for (const path of files) {
    ...
    for (const forbidden of [
      ...
      if (text.includes(forbidden)) {
        errors.push(
          `${path}: remove stale current install wording '${forbidden}'`,
        );
      }
    }
  }
  const upgradeExport = await runtime.readTextFile(
    "docs/platform/upgrade-export.md",
  );
  for (const required of [
    ...
    if (!includesRequiredText(upgradeExport, required)) {
      errors.push(
        `docs/platform/upgrade-export.md: missing current revision boundary '${required}'`,
      );
    }
  }
  ...
  for (const path of files) {
    ...
    for (const term of forbidden) {
      if (text.includes(term)) {
        errors.push(
          `${path}: remove stale Takos split layout reference '${term}'`,
        );
      }
    }
  }
  const currentState = await runtime.readTextFile(
    "docs/contributing/current-state.md",
  );
  for (const term of required) {
    if (!currentState.includes(term)) {
      errors.push(
        `docs/contributing/current-state.md: missing canonical layout reference '${term}'`,
      );
    }
  }
  return errors;
