...
- Does not own: Takosumi Accounts, control plane, runner, or Run ledger / Git object/ref hosting, clone/fetch/push, and collaboration owned by takos-git / Reusable Rust agent engine owned by takos-agent-engine
- Hazards: The Takos Worker is an OIDC client and resource server, never an issuer. / Worker-native Git compatibility is quarantined fail-closed until legacy repositories are explicitly migrated to takos-git. / The complete Cloudflare adapter is the only current install surface; the retained Takoform Provider 1.x projection is not selectable and does not prove current Forms cover Takos. / Operator configuration and secrets stay outside the repository.
