...
export type ParseResult<T> = { ok: true; payload: T } | {
  ok: false;
  error: string;
};
 ...
 */
function extractNonEmptyString(val: unknown): string | undefined {
  return typeof val === "string" && val.length > 0 ? val : undefined;
}

function extractExecutorTier(val: unknown): 1 | 2 | 3 | undefined {
...
export function parseStartPayload(value: unknown): ParseResult<StartPayload> {
  ...
  const serviceId = extractNonEmptyString(v.serviceId);
  if (!serviceId) {
    return { ok: false, error: "Missing required field: serviceId" };
  }

  const workerId = extractNonEmptyString(v.workerId);
  if (!workerId) {
    return { ok: false, error: "Missing required field: workerId" };
  }

  const controlRpcToken = extractNonEmptyString(v.controlRpcToken);
  ...
  if (v.executorTier !== undefined && !extractExecutorTier(v.executorTier)) {
    return {
      ok: false,
      error: "executorTier must be 1, 2, or 3 when provided",
    };
  }
  ...
  return {
    ...
    payload: {
      ...
      executorTier: extractExecutorTier(v.executorTier),
      executorContainerId: v.executorContainerId as string | undefined,
      controlRpcToken,
...
export function installGracefulShutdown(
  ...
  function shutdown(signal: string): void {
    logger.info(
      `[${tag}] Received ${signal}, shutting down (${concurrency.activeRuns} active runs)`,
    );
    ...
    const forceExitTimer = setTimeout(() => {
      logger.warn(
        `[${tag}] Force exit after ${gracePeriodMs}ms (${concurrency.activeRuns} runs still active)`,
      );
      process.exit(1);
    }, gracePeriodMs);
