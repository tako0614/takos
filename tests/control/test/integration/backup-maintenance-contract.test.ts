...
Deno.test("daily SQL backup skips when TAKOS_OFFLOAD is not configured", async () => {
...
Deno.test("daily SQL backup skips when the current UTC day is already backed up", async () => {
  const now = new Date();
  const result = await runD1DailyBackup({
    DB: {} as never,
