...
The package exposes multiple subpath imports so consumers can import only
what they need:
...
`isPrivateIP` covers RFC 1918 ranges, loopback, link-local, carrier-grade
NAT (100.64/10), documentation ranges (TEST-NET-1/2/3), multicast, and
...
```typescript
import { parseIntEnv, parseFloatEnv, parseIntEnvRequired } from 'takos-common/env-parse';
...
**Service Token Authentication** -- RS256 JWT verification for service-to-service calls:
...
app.use('*', createServiceTokenMiddleware({
...
```typescript
import { createErrorHandler, notFoundHandler } from 'takos-common/middleware/hono';
