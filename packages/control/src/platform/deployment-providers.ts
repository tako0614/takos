...
function createWorkersDispatchConfig(env: EnvRecord): WorkersDispatchDeployProviderConfig | null {
  ...
  return {
    ...
    config: {
      ...
      apiToken,
      dispatchNamespace,
      ...(getEnvString(env, 'CF_ZONE_ID') ? { zoneId: getEnvString(env, 'CF_ZONE_ID')! } : {}),
    },
  };
}

function createOciConfig(env: EnvRecord): OciDeployProviderConfig | null {
...
function createEcsConfig(env: EnvRecord): EcsDeployProviderConfig | null {
  const clusterArn = getEnvString(env, 'AWS_ECS_CLUSTER_ARN');
  const taskDefinitionFamily = getEnvString(env, 'AWS_ECS_TASK_DEFINITION_FAMILY');
  const region = getEnvString(env, 'AWS_ECS_REGION') ?? getEnvString(env, 'AWS_REGION');
  ...
  return {
    ...
    config: {
      ...
      ...(getEnvStringList(env, 'AWS_ECS_SECURITY_GROUP_IDS')
        ? { securityGroupIds: getEnvStringList(env, 'AWS_ECS_SECURITY_GROUP_IDS')! }
        : {}),
...
function createCloudRunConfig(env: EnvRecord): CloudRunDeployProviderConfig | null {
  const projectId = getEnvString(env, 'GCP_PROJECT_ID');
  const region = getEnvString(env, 'GCP_CLOUD_RUN_REGION') ?? getEnvString(env, 'GCP_REGION');
  if (!projectId || !region) {
    return null;
  }
  return {
    ...
    config: {
      ...
      ...(getEnvString(env, 'GCP_CLOUD_RUN_INGRESS')
        ...
        : {}),
      ...(getEnvBoolean(env, 'GCP_CLOUD_RUN_ALLOW_UNAUTHENTICATED') !== undefined
        ? { allowUnauthenticated: getEnvBoolean(env, 'GCP_CLOUD_RUN_ALLOW_UNAUTHENTICATED')! }
        : {}),
      ...
      ...(getEnvString(env, 'GCP_ARTIFACT_REGISTRY_REPO')
        ? { artifactRegistryRepo: getEnvString(env, 'GCP_ARTIFACT_REGISTRY_REPO')! }
        : {}),
    },
  };
}

function createK8sConfig(env: EnvRecord): K8sDeployProviderConfig | null {
  const namespace = getEnvString(env, 'K8S_NAMESPACE') ?? 'default';
  const deploymentName = getEnvString(env, 'K8S_DEPLOYMENT_NAME');
  const imageRegistry = getEnvString(env, 'K8S_IMAGE_REGISTRY');
  const hasSignal = getEnvString(env, 'K8S_NAMESPACE') || deploymentName || imageRegistry;
  if (!hasSignal) {
    return null;
  }
  return {
...
const PROVIDER_DEFAULT_ORDER: Array<PlatformDeployProviderConfig['name']> = [
  'workers-dispatch',
  'ecs',
  'cloud-run',
  'k8s',
...
export function resolveDeploymentProviderConfigsFromEnv(
  env: EnvRecord,
): PlatformDeployProviderConfig[] {
  return [
    ...
    createK8sConfig(env),
    createOciConfig(env),
  ].filter((config): config is PlatformDeployProviderConfig => config != null);
}
