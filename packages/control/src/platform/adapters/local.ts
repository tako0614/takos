...
function createOciDeploymentRegistry(env: Record<string, unknown>) {
  const orchestratorUrl = typeof env.OCI_ORCHESTRATOR_URL === 'string' ? env.OCI_ORCHESTRATOR_URL : undefined;
  const orchestratorToken = typeof env.OCI_ORCHESTRATOR_TOKEN === 'string' ? env.OCI_ORCHESTRATOR_TOKEN : undefined;
  if (!orchestratorUrl || orchestratorUrl.length === 0) {
    return undefined;
  }
...
function buildLocalPlatform<TBindings extends object>(env: TBindings & PlatformEnvRecord): ControlPlatform<TBindings> {
  ...
  const services = createPlatformServices({
    ...
    deploymentProviders: createOciDeploymentRegistry(env),
  });

  return buildPlatform('local', env, config, services);
}

export function buildLocalWebPlatform(env: Env): ControlPlatform<Env> {
  return buildLocalPlatform(env as Env & PlatformEnvRecord);
}

export function buildLocalDispatchPlatform(env: DispatchEnv): ControlPlatform<DispatchEnv> {
  return buildLocalPlatform(env as DispatchEnv & PlatformEnvRecord);
}

export function buildLocalWorkerPlatform(env: WorkerEnv): ControlPlatform<WorkerEnv> {
  return buildLocalPlatform(env as WorkerEnv & PlatformEnvRecord);
}
