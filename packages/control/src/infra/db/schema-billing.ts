...
export const billingAccounts = sqliteTable("billing_accounts", {
  ...
  /** Active payment processor for this account ('stripe' for now). */
  processorName: text("processor_name").notNull().default("stripe"),
  /** Processor-side customer identifier (e.g. Stripe `cus_*`). */
  processorCustomerId: text("processor_customer_id"),
  /** Processor-side recurring subscription identifier (e.g. Stripe `sub_*`). */
  ...
  idxProcessorCustomer: index("idx_billing_accounts_processor_customer_id").on(
    table.processorCustomerId,
  ),
...
export const billingTransactions = sqliteTable("billing_transactions", {
  ...
  idxReference: index("idx_billing_transactions_reference_id").on(
    table.referenceId,
  ),
  idxCreatedAt: index("idx_billing_transactions_created_at").on(
    table.createdAt,
  ),
  idxBillingAccount: index("idx_billing_transactions_billing_account_id").on(
...
export const usageEvents = sqliteTable("usage_events", {
  ...
  idempotencyKey: text("idempotency_key").unique(
    "idx_usage_events_idempotency_key",
  ),
  billingAccountId: text("billing_account_id").notNull(),
  ...
  idxBillingAccount: index("idx_usage_events_billing_account_id").on(
    table.billingAccountId,
  ),
...
// after the billing system was generalized to a `PaymentProcessor` interface.
// When a non-Stripe processor is added, give it its own dedup table or rename
...
export const stripeWebhookEvents = sqliteTable("stripe_webhook_events", {
  ...
  idxReceivedAt: index("idx_stripe_webhook_events_received_at").on(
    table.receivedAt,
  ),
}));
...
export const usageRollups = sqliteTable("usage_rollups", {
  ...
  uniqBillingScope: uniqueIndex("idx_usage_rollups_billing_scope").on(
    table.billingAccountId,
    table.scopeType,
    table.accountId,
    table.meterType,
  ...
  idxBillingAccount: index("idx_usage_rollups_billing_account_id").on(
    table.billingAccountId,
  ),
