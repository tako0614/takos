...
export const oauthAuthorizationCodes = sqliteTable(
  "oauth_authorization_codes",
  {
    ...
    codeChallengeMethod: text("code_challenge_method").notNull().default(
      "S256",
    ),
  ...
  (table) => ({
    idxExpiresAt: index("idx_oauth_authorization_codes_expires_at").on(
      table.expiresAt,
    ),
    idxCodeHash: index("idx_oauth_authorization_codes_code_hash").on(
      table.codeHash,
    ),
    idxClient: index("idx_oauth_authorization_codes_client_id").on(
      table.clientId,
    ),
  }),
);
...
export const oauthClients = sqliteTable("oauth_clients", {
  ...
  idxOwner: index("idx_oauth_clients_owner_account_id").on(
...
export const oauthConsents = sqliteTable("oauth_consents", {
  ...
  status: text("status").notNull().default("active"),
  grantedAt: text("granted_at").notNull().$defaultFn(() =>
    new Date().toISOString()
  ),
  ...updatedAtColumn,
}, (table) => ({
  uniqAccountClient: uniqueIndex("idx_oauth_consents_account_client").on(
    table.accountId,
    table.clientId,
  ),
...
export const oauthDeviceCodes = sqliteTable("oauth_device_codes", {
  ...
  idxUserCodeHash: index("idx_oauth_device_codes_user_code_hash").on(
    table.userCodeHash,
  ),
  ...
  idxDeviceCodeHash: index("idx_oauth_device_codes_device_code_hash").on(
...
export const oauthTokens = sqliteTable("oauth_tokens", {
  ...
  idxAccountRevoked: index("idx_oauth_tokens_account_revoked").on(
    table.accountId,
    table.revoked,
  ),
