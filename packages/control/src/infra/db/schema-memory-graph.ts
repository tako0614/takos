...
export const memoryClaims = sqliteTable('memory_claims', {
  ...
  idxAccountSubject: index('idx_memory_claims_account_subject').on(table.accountId, table.subject),
  idxAccountStatus: index('idx_memory_claims_account_status').on(table.accountId, table.status),
}));
...
export const memoryEvidence = sqliteTable('memory_evidence', {
  ...
  idxSource: index('idx_memory_evidence_source').on(table.sourceType, table.sourceRef),
}));
...
export const memoryPaths = sqliteTable('memory_paths', {
  ...
  idxAccountStart: index('idx_memory_paths_account_start').on(table.accountId, table.startClaimId),
  idxAccountEnd: index('idx_memory_paths_account_end').on(table.accountId, table.endClaimId),
}));
