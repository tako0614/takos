...
function isConditionalCacheHit(c: SearchContext, headers: Headers): boolean {
  ...
  if (Number.isNaN(ifModifiedDate.getTime()) || Number.isNaN(lastModifiedDate.getTime())) return false;
...
async function matchSearchCache(c: SearchContext, cacheKey: Request): Promise<Response | null> {
  ...
  try {
    ...
    if (c.req.method === 'GET' && isConditionalCacheHit(c, headers)) return new Response(null, { status: 304, headers });
...
async function storeSearchCache(c: SearchContext, cacheKey: Request, response: Response): Promise<void> {
  const cache = getDefaultCache();
  if (!cache) return;
  const cachePut = cache.put(cacheKey, response.clone()).catch((err) => { logError('Failed to store search cache', err, { module: 'routes/search' }); });
  const ctx = c.executionCtx;
  if (ctx && typeof ctx.waitUntil === 'function') { ctx.waitUntil(cachePut); } else { await cachePut; }
}
...
async function createCacheableJsonResponse(body: unknown): Promise<Response> {
  ...
  return new Response(payload, { status: 200, headers: new Headers({
    'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': `private, max-age=${CacheTTL.SEARCH}`,
    'Cache-Tag': CacheTags.SEARCH, 'ETag': etag, 'Last-Modified': new Date().toUTCString(), 'X-Cache': 'MISS',
  }) });
}

async function resolveCachedSearchResponse(
  c: SearchContext,
  cacheKeyPayload: { scope: 'workspace' | 'quick'; spaceId: string; userId: string; query: string; type?: string | null; fileTypes?: string[]; limit?: number | null },
...
search.post('/spaces/:spaceId/search', spaceAccess(), async (c) => {
  ...
  if (!body.query || body.query.trim().length === 0) throw new BadRequestError('Query is required');
