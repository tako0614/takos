...
export default new Hono<AuthenticatedRouteEnv>()
  .get('/:spaceId/common-env', async (c) => {
    ...
    try {
      ...
      const env = await service.listSpaceCommonEnv(access.space.id);
      return c.json({ env });
    } catch (error) {
  ...
  .put('/:spaceId/common-env', async (c) => {
    ...
    try {
      ...
      await service.reconcileWorkersForEnvKey(access.space.id, body.name, 'workspace_env_put');
  ...
  .delete('/:spaceId/common-env/:name', async (c) => {
    ...
    try {
      const service = createCommonEnvService(c.env);
      const actor = await buildCommonEnvActor(c, user.id);
      const deleted = await service.deleteSpaceCommonEnv(access.space.id, envName, actor);
      if (!deleted) {
        throw new NotFoundError('Common env');
      }
      await service.reconcileWorkersForEnvKey(access.space.id, envName, 'workspace_env_delete');
      return c.json({ success: true });
    } catch (error) {
      if (error instanceof AppError) throw error;
