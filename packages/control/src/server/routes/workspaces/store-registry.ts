...
export default new Hono<AuthenticatedRouteEnv>()
  ...
  .get('/:spaceId/store-registry', async (c) => {
    ...
    try {
      const entries = await listRegisteredStores(c.env.DB, access.workspace.id);
      return c.json({ stores: entries.map(formatEntry) });
    } catch (error) {
      logError('Failed to list store registry', error, { module: 'routes/store-registry' });
      return internalError(c, 'Failed to list store registry');
    }
  })
  ...
  .post('/:spaceId/store-registry',
    ...
    async (c) => {
      ...
      try {
        const entry = await addRemoteStore(c.env.DB, access.workspace.id, {
  ...
  .delete('/:spaceId/store-registry/:entryId', async (c) => {
    ...
    try {
      const deleted = await removeRemoteStore(c.env.DB, access.workspace.id, c.req.param('entryId'));
      if (!deleted) {
      ...
      return internalError(c, 'Failed to remove store');
    }
  })
  ...
  .patch('/:spaceId/store-registry/:entryId',
    ...
    async (c) => {
      ...
      try {
        ...
        const existing = await getRegistryEntry(c.env.DB, access.workspace.id, entryId);
        if (!existing) {
          return notFound(c, 'Store registry entry');
        }

        if (body.is_active !== undefined) {
          await setActiveStore(c.env.DB, access.workspace.id, body.is_active ? entryId : null);
        }

        if (body.subscription_enabled !== undefined) {
          await setSubscription(c.env.DB, access.workspace.id, entryId, body.subscription_enabled);
        }

        // Re-fetch to get updated state
        const entry = await getRegistryEntry(c.env.DB, access.workspace.id, entryId);
  ...
  .post('/:spaceId/store-registry/:entryId/refresh', async (c) => {
    ...
    try {
      const entry = await refreshRemoteStore(c.env.DB, access.workspace.id, c.req.param('entryId'));
      if (!entry) {
        return notFound(c, 'Store registry entry');
      }
      return c.json({ store: formatEntry(entry) });
  ...
  .get('/:spaceId/store-registry/:entryId/repositories', async (c) => {
    ...
    const entry = await getRegistryEntry(c.env.DB, access.workspace.id, c.req.param('entryId'));
    if (!entry) {
      return notFound(c, 'Store registry entry');
    }
  ...
  .post('/:spaceId/store-registry/:entryId/install',
    ...
    async (c) => {
      ...
      try {
        const result = await installFromRemoteStore(c.env.DB, access.workspace.id, {
  ...
  .get('/:spaceId/store-registry/updates', async (c) => {
    ...
    try {
      ...
      const result = await getStoreUpdates(c.env.DB, access.workspace.id, {
        unseenOnly,
        limit,
        offset,
      });
      ...
      return internalError(c, 'Failed to get store updates');
    }
  })
  ...
  .post('/:spaceId/store-registry/updates/mark-seen',
    ...
    async (c) => {
      ...
      try {
        if (body.all) {
          await markAllUpdatesSeen(c.env.DB, access.workspace.id);
        } else if (body.update_ids?.length) {
          await markUpdatesSeen(c.env.DB, access.workspace.id, body.update_ids);
        }
        return c.json({ success: true });
      } catch (error) {
        logError('Failed to mark updates seen', error, { module: 'routes/store-registry' });
        return internalError(c, 'Failed to mark updates seen');
      }
    })
