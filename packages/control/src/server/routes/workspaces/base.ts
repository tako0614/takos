...
import { badRequest, notFound } from '../../../shared/utils/error-response';
...
export default new Hono<AuthenticatedRouteEnv>()
  ...
  .get('/me', async (c) => {
    ...
    const { workspace, repository } = await getWorkspaceWithRepository(c.env, access.workspace);
  ...
  .get('/:spaceId/export', async (c) => {
    ...
    const accessibleResourceIds = await db.select({ resourceId: resourceAccess.resourceId, permission: resourceAccess.permission })
      .from(resourceAccess)
      .where(eq(resourceAccess.accountId, access.workspace.id))
      .all();
    ...
    const [repoRows, threadRows, resourceRows] = await Promise.all([
      ...
      db.select({
        ...
        and(
          ...
          or(
            and(
              eq(resources.accountId, access.workspace.id),
    ...
    return c.json({
      space: toWorkspaceResponse(access.workspace),
  ...
  .patch('/:spaceId',
    ...
    if (body.ai_model) {
      ...
      if (!normalizedModel) {
        return badRequest(c, 'Invalid model');
      }
      ...
      if (providerOverride && providerOverride !== inferredProvider) {
        return badRequest(c, 'Provider does not match model');
      }
    ...
    if (body.ai_provider) {
      ...
      if (!body.ai_model) {
        const existingModel = normalizeModelId(access.workspace.ai_model) || DEFAULT_MODEL_ID;
    ...
    const workspace = await updateWorkspace(c.env.DB, access.workspace.id, updates);
    if (!workspace) {
      return badRequest(c, 'No valid updates provided');
    }
  ...
  .get('/:spaceId/model', async (c) => {
    ...
    const workspace = await getWorkspaceModelSettings(c.env.DB, access.workspace.id);
  ...
  .patch('/:spaceId/model',
    ...
    if (body.tier !== undefined) {
      ...
      if (!tierCfg) {
        return badRequest(c, 'Invalid tier');
      }
      await updateWorkspaceModel(c.env.DB, access.workspace.id, tierCfg.model, tierCfg.provider);
      return c.json({
    ...
    if (!requestedModel) {
      return badRequest(c, 'Model or tier is required');
    }
    ...
    await updateWorkspaceModel(c.env.DB, access.workspace.id, model, provider);
  ...
  .delete('/:spaceId', async (c) => {
    ...
    await deleteWorkspace(c.env.DB, access.workspace.id);
  ...
  .get('/:spaceId/sidebar-items', async (c) => {
    ...
    const items = await getUISidebarItems(c.env.DB, access.workspace.id);
    return c.json({ items });
  });
