...
oauthConsentApi.get("/authorize/context", async (c) => {
  ...
  if (!validation.valid) {
    ...
    throw new BadRequestError(validation.errorDescription ?? errorCode, {
  ...
  if (!client) {
    throw new InternalError("Client missing after validation");
  }
  ...
  const alreadyConsented = await hasFullConsent(
    c.env.DB,
    user.id,
    client.client_id,
    requestedScopes,
...
oauthConsentApi.get("/device/context", async (c) => {
  ...
  if (deviceAuth.status === "approved") {
    return c.json({
      status: "result" as const,
      title: "Approved",
      message: "Already approved. Return to your device.",
    });
  }
