...
export function createCustomDomainsRoute(
  ...
  route.onError((err, c) => {
    if (isAppError(err)) {
      return c.json(err.toResponse(), toStatusCode(err.statusCode));
    }

    logError("[custom-domains]", err, { module: "custom-domains" });
    const response = new InternalError("Internal server error").toResponse();
    return c.json(response, toStatusCode(500));
  });
