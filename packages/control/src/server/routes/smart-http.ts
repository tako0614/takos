...
import type { Env, Repository, User } from "../../shared/types/index.ts";
import { optionalGitAuth, requireGitAuth } from "../middleware/git-auth.ts";
...
} from "../../application/services/git-smart/smart-http/receive-pack.ts";
...
const optionalGitAuthMiddleware = async (
  c: Parameters<typeof optionalGitAuth>[0],
  next: Parameters<typeof optionalGitAuth>[1],
) => smartHttpRouteDeps.optionalGitAuth(c, next);

const requireGitAuthMiddleware = async (
  c: Parameters<typeof requireGitAuth>[0],
  next: Parameters<typeof requireGitAuth>[1],
...
async function resolveRepo(
  ...
  const repo = await drizzle.select().from(repositories).where(
    and(
      eq(repositories.accountId, account.id),
      eq(repositories.name, cleanName),
    ),
  ).get();
...
async function acquireRepoPushLock(
  env: Env,
  repoId: string,
): Promise<{ token: string } | null> {
  if (!env.GIT_PUSH_LOCK) return null;

  const lockNamespace = env.GIT_PUSH_LOCK;
...
async function releaseRepoPushLock(
  env: Env,
  repoId: string,
  ...
  await stub.fetch("https://git-push-lock/release", {
    ...
    logError("Failed to release push lock", err, {
      action: "release_push_lock",
      repoId,
    });
  });
}
...
smartHttpRoutes.get(
  ...
  async (c) => {
    ...
    if (service === "git-receive-pack") {
      ...
      const access = await smartHttpRouteDeps.checkSpaceAccess(
        c.env.DB,
        result.spaceId,
        user.id,
        ["owner", "admin", "editor"],
    ...
    const data = await smartHttpRouteDeps.handleInfoRefs(
      c.env.DB,
      result.repo.id,
      service,
    );
...
smartHttpRoutes.post(
  ...
  async (c) => {
    ...
    const data = await smartHttpRouteDeps.handleUploadPack(
      c.env.DB,
      bucket,
      result.repo.id,
      body,
...
smartHttpRoutes.post(
  ...
  async (c) => {
    ...
    try {
      ...
      return c.text(
        `${
          err instanceof Error ? err.message : "Repository push lock failed"
        }\n`,
        409,
      );
    }
    ...
    if (contentLength !== null && contentLength > MAX_GIT_REQUEST_BODY_BYTES) {
      ...
      return c.text(
        `Request body exceeds ${MAX_GIT_REQUEST_BODY_BYTES} bytes\n`,
        413,
      );
    }
    ...
    try {
      ...
      let receivePack: {
        response: Uint8Array;
        updatedRefs: { oldSha: string; newSha: string; refName: string }[];
      };
      try {
        if (stream) {
          receivePack = await smartHttpRouteDeps.handleReceivePackFromStream(
            c.env.DB,
            bucket,
            result.repo.id,
            stream,
          ...
          receivePack = await smartHttpRouteDeps.handleReceivePack(
            c.env.DB,
            bucket,
            result.repo.id,
            new Uint8Array(),
      ...
      if (updatedRefs.length > 0 && c.env.WORKFLOW_QUEUE) {
        c.executionCtx.waitUntil((async () => {
          for (const ref of updatedRefs) {
            if (
              !ref.refName.startsWith("refs/heads/") || ref.newSha === ZERO_SHA
            ) {
              continue;
            }
            ...
            try {
              ...
              logError("Failed to trigger push workflows", err, {
                action: "trigger_push_workflows",
                repoId: result.repo.id,
                refName: ref.refName,
              });
            }
