...
function parseRunListCursor(
  cursor: string,
): { createdAt: string; runId: string | null } | null {
...
export function registerRunListRoutes(app: RunRouteApp) {
  app.get(
    ...
    async (c) => {
      ...
      const { limit } = parsePagination(runsQuery, {
        limit: 50,
        maxLimit: 200,
      });
      ...
      let parsedCursor: { createdAt: string; runId: string | null } | null =
        null;
      if (cursor) {
      ...
      const runsList = result.map((row) =>
        runRowToApi(asRunRow({ ...row, spaceId: row.accountId }))
      );
