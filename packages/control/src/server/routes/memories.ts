...
export default new Hono<{ Bindings: Env; Variables: BaseVariables }>()
  ...
  .get(
    ...
    async (c) => {
      ...
      const access = await memoriesRouteDeps.requireSpaceAccess(
        c,
        spaceId,
        user.id,
      );
      ...
      const memoryList = await memoriesRouteDeps.listMemories(
        c.env.DB,
        access.space.id,
        {
          type,
  ...
  .get("/memories/:id", async (c) => {
    ...
    const access = await memoriesRouteDeps.checkSpaceAccess(
      c.env.DB,
      memory.space_id,
      user.id,
    );
  ...
  .get(
    ...
    async (c) => {
      ...
      const reminders = await memoriesRouteDeps.listReminders(
        c.env.DB,
        access.space.id,
        {
          status,
  ...
  .get("/reminders/:id", async (c) => {
    ...
    const reminder = await memoriesRouteDeps.getReminderById(
      c.env.DB,
      reminderId,
    );
    if (!reminder) {
  ...
  .patch(
    ...
    async (c) => {
      ...
      const updated = await memoriesRouteDeps.updateReminder(
        c.env.DB,
        reminderId,
        {
          content: body.content,
  ...
  .post("/reminders/:id/trigger", async (c) => {
    ...
    const updated = await memoriesRouteDeps.triggerReminder(
      c.env.DB,
      reminderId,
    );
