...
// ---------------------------------------------------------------------------
// OAuth callback — no authentication, protected by the state CSRF token
// ---------------------------------------------------------------------------
...
mcpRoutes.get('/oauth/callback', async (c) => {
  ...
  if (!pending) {
    return c.html(errorPage('Invalid or expired OAuth state. Please try again.'), 400);
  }
...
// ---------------------------------------------------------------------------
// Authenticated MCP server management
// ---------------------------------------------------------------------------
...
mcpRoutes.get('/servers', spaceAccess({ roles: MCP_LIST_ROLES }), async (c) => {
  ...
  return c.json({
    data: servers.map(serializeMcpServer),
  });
});
...
mcpRoutes.get('/servers/:id/tools', spaceAccess({ roles: MCP_LIST_ROLES }), async (c) => {
  ...
  if (server.sourceType === 'external') {
    // External server with OAuth tokens
    ...
    // Managed server — no auth token needed
    accessToken = null;
  }
...
function successPage(serverName: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>MCP Server Connected</title>
<style>
  body { font-family: system-ui, sans-serif; display: flex; align-items: center; justify-content: center;
         min-height: 100vh; margin: 0; background: #f0fdf4; }
  .card { background: white; border-radius: 12px; padding: 2rem 3rem; box-shadow: 0 4px 24px rgba(0,0,0,.08);
...
<div class="card">
  <h1>✓ Connected</h1>
  <p>MCP server <strong>${escapeHtml(serverName)}</strong> has been successfully authorized.</p>
  <p>You can close this window and continue in the agent.</p>
</div>
</body></html>`;
}
...
function errorPage(message: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>MCP Authorization Error</title>
<style>
  body { font-family: system-ui, sans-serif; display: flex; align-items: center; justify-content: center;
