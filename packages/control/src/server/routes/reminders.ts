...
import { z } from "zod";
import type { Env, ReminderStatus } from "../../shared/types/index.ts";
...
export default new Hono<{ Bindings: Env; Variables: BaseVariables }>()
  ...
  .get(
    ...
    async (c) => {
      ...
      const access = await remindersRouteDeps.requireSpaceAccess(
        c,
        spaceId,
        user.id,
      );
      ...
      const reminders = await remindersRouteDeps.listReminders(
        c.env.DB,
        access.space.id,
        {
          status,
  ...
  .get("/reminders/:id", async (c) => {
    ...
    const reminder = await remindersRouteDeps.getReminderById(
      c.env.DB,
      reminderId,
    );
    if (!reminder) {
    ...
    const access = await remindersRouteDeps.checkSpaceAccess(
      c.env.DB,
      reminder.space_id,
      user.id,
    );
  ...
  .patch(
    ...
    async (c) => {
      ...
      const updated = await remindersRouteDeps.updateReminder(
        c.env.DB,
        reminderId,
        {
          content: body.content,
  ...
  .post("/reminders/:id/trigger", async (c) => {
    ...
    const updated = await remindersRouteDeps.triggerReminder(
      c.env.DB,
      reminderId,
    );
