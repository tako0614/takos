...
async function listWorkflowFiles(
  ...
  const workflowsDir = await gitStore.listDirectory(
    bucket,
    commit.tree,
    ".takos/workflows",
  );
...
const workflowsRouter = new Hono<AuthenticatedRouteEnv>()
  .get("/repos/:repoId/workflows", async (c) => {
    ...
    if (bucket) {
      ...
      try {
        const gitFiles = await listWorkflowFiles(
          c.env.DB,
          bucket,
          repoId,
          refName,
  ...
  .get("/repos/:repoId/workflows/:path{.+}", async (c) => {
    ...
    if (!cached || refresh) {
      ...
      const refName = resolveRefName(branch, repoAccess);
      const blob = await resolveWorkflowBlob(
        c.env.DB,
        bucket,
        repoId,
        refName,
      ...
      const workflowId = await upsertWorkflowCache(
        db,
        repoId,
        path,
        content,
  ...
  .post("/repos/:repoId/workflows/sync-all", async (c) => {
    ...
    for (const file of workflowFiles) {
      try {
        ...
        const content = new TextDecoder().decode(blob);
        const { name, triggers, errors: parseErrors } = parseWorkflowContent(
          content,
        );
        ...
        const existing = await db.select({ id: workflows.id })
          .from(workflows)
          .where(
            and(eq(workflows.repoId, repoId), eq(workflows.path, file.path)),
          )
          .get();

        await upsertWorkflowCache(
          db,
          repoId,
          file.path,
          content,
