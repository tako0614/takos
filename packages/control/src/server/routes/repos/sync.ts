...
const repoSync = new Hono<AuthenticatedRouteEnv>()
  ...
  try {
    ...
    for (const branchName of targetBranches) {
      ...
      if (!refUpdateResult.success) {
        return c.json({
          error: refUpdateResult.error || 'Failed to update remote-tracking branch',
    ...
    if (limitError) {
      return c.json({
        error: 'Repository tree is too large to fetch from upstream',
  ...
  try {
    ...
    if (!has_merge_base) {
      return c.json({
        ...
        merge_base: null,
        message: 'No merge base between fork and upstream.',
      }, 409);
    }
    ...
    const mergeCommit = await gitStore.createCommit(c.env.DB, bucket, repoId, {
    ...
    if (!updateResult.success) {
      return c.json({
        error: 'Failed to update branch after merge',
    ...
    if (limitError) {
      return c.json({
        error: 'Repository tree is too large to sync with upstream',
        code: limitError.code,
        detail: limitError.detail,
      }, 413);
    }
  ...
  try {
    ...
    const status = await gitStore.checkSyncStatus(c.env.DB, bucket, repoId, branchName);
    ...
    if (limitError) {
      return c.json({
        ...
        has_merge_base: true,
        conflict: false,
        error: 'Repository tree is too large to evaluate sync status',
        code: limitError.code,
        detail: limitError.detail,
      }, 413);
    }
    logError('Failed to check sync status', err, { module: 'routes/repos/sync' });
    return c.json({
      can_sync: false,
      can_fast_forward: false,
      commits_behind: 0,
