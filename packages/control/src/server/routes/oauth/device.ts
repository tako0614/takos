...
oauthDevice.post("/device/code", async (c) => {
  ...
  if (!clientId || !scope) {
    return c.json(
      {
        error: "invalid_request",
        error_description: "client_id and scope are required",
      },
      400,
    );
  }
    ...
    return c.json(
      {
        ...
        error_description: "Client does not support device_code grant",
      },
      400,
    );
  }
  ...
  const verificationUriComplete = `${verificationUri}?user_code=${
    encodeURIComponent(created.userCode)
  }`;
...
oauthDevice.get("/device", async (c) => {
  ...
  if (isDeviceUserCodeLimited(normalizedUserCode)) {
    return c.html(
      deviceResultPage({
        title: "\u5236\u9650",
        message:
          "\u3053\u306e\u30b3\u30fc\u30c9\u3078\u306e\u8a66\u884c\u56de\u6570\u304c\u4e0a\u9650\u306b\u9054\u3057\u307e\u3057\u305f\u3002",
      }),
    );
  }

  const deviceAuth = await getDeviceAuthorizationByUserCode(
    c.env.DB,
    normalizedUserCode,
  );
  if (!deviceAuth) {
  ...
  if (deviceAuth.status === "approved") {
    return c.html(deviceResultPage({
      title: "\u627f\u8a8d\u6e08\u307f",
      message:
        "\u627f\u8a8d\u6e08\u307f\u3067\u3059\u3002\u30c7\u30d0\u30a4\u30b9\u5074\u306b\u623b\u3063\u3066\u304f\u3060\u3055\u3044\u3002",
    }));
  }
  ...
  if (hasConsent) {
    const ok = await approveDeviceAuthorization(c.env.DB, {
      id: deviceAuth.id,
      userId: user.id,
    });
    if (!ok) {
...
oauthDevice.post("/device", async (c) => {
  ...
  const user = await loadSessionUser(c);
  if (!user) {
    const returnUrl = `/oauth/device?user_code=${
      encodeURIComponent(rawUserCode)
    }`;
    return c.redirect(`/auth/login?return_to=${encodeURIComponent(returnUrl)}`);
  }
  ...
  if (action === "deny") {
    const ok = await denyDeviceAuthorization(c.env.DB, {
      id: deviceAuth.id,
      userId: user.id,
    });
    if (!ok) {
