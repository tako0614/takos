...
import { getBodyValue, tryLogOAuthEvent } from "./request-utils.ts";
...
async function handleAuthorizationCodeGrant(
  ...
  if (!exchangedCode) {
    return c.json(
      {
        error: "server_error",
        error_description: "Code exchange succeeded but code data is missing",
      },
      500,
    );
  }
...
async function handleRefreshTokenGrant(
  ...
  const { token: storedToken, isReuse } = await getRefreshTokenWithReuseCheck(
    c.env.DB,
    refreshToken,
  );
  ...
  if (isReuse) {
    logWarn("SECURITY: Refresh token reuse detected!", {
      module: "routes/oauth/token",
      ...{
    ...
    if (storedToken.token_family) {
      const revokedCount = await revokeTokenFamily(
        c.env.DB,
        storedToken.token_family,
        "reuse_detected",
      );
      logWarn(
        `SECURITY: Revoked ${revokedCount} tokens in family ${storedToken.token_family}`,
        { module: "routes/oauth/token" },
      );
    }
    ...
    return c.json(
      {
        error: "invalid_grant",
        error_description:
          "Refresh token has been invalidated for security reasons",
      },
      400,
    );
  }
...
async function handleDeviceCodeGrant(c: TokenContext, body: TokenRequestBody) {
  ...
  const poll = await pollDeviceAuthorization(c.env.DB, {
    deviceCode,
    clientId,
  });
    ...
    return c.json(
      {
        ...
        error_description: poll.slowDown
          ? "Slow down polling"
...
async function handleClientCredentialsGrant(
  ...
  if (
    ...
    !areScopesAllowed(requestedScopes, allowedScopes)
  ) {
    return c.json({
      error: "invalid_scope",
  ...
  const finalScope = requestedScopes.length > 0
