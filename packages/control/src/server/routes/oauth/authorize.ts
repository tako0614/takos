...
import {
  getScopeSummary,
  parseScopes,
} from "../../../application/services/oauth/scopes.ts";
import {
  buildErrorRedirect,
...
async function loadSessionUser(c: Context<PublicRouteEnv>) {
  ...
  const userData = await db.select().from(accounts).where(
    eq(accounts.id, session.user_id),
  ).get();
...
function getFormTextValue(
  ...
  if (Array.isArray(value)) {
    const firstTextValue = value.find((entry): entry is string =>
      typeof entry === "string"
    );
...
oauthAuthorize.get("/authorize", async (c) => {
  ...
  if (!client) {
    return c.json({
      error: "server_error",
      error_description: "Validation succeeded but client is missing",
    }, 500);
  }

  const user = await loadSessionUser(c);
  if (!user) {
    const returnUrl = `/oauth/authorize?${
  ...
  if (
    !validatedScope || !validatedRedirectUri || !validatedCodeChallenge ||
    !validatedCodeChallengeMethod || !validatedState
  ) {
    return c.json({
      error: "server_error",
      error_description: "Validation succeeded but required fields are missing",
    }, 500);
  }
  ...
  const validLogoUri = isValidLogoUrl(client.logo_uri)
    ? escapeHtml(client.logo_uri!)
    : null;

  const csrfToken = crypto.randomUUID();
  c.header(
    "Set-Cookie",
    `__Host-csrf=${csrfToken}; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age=600`,
  );
...
oauthAuthorize.post("/authorize", async (c) => {
  ...
  const authorizationRequest: Partial<AuthorizationRequest> = {
    ...
    code_challenge_method: getFormTextValue(body.code_challenge_method) as
  ...
  if (!validation.valid) {
    ...
    const oauthErrorDescription = validation.errorDescription ??
  ...
  if (
    !postClient || !postRedirectUri || !postScope || !postState ||
    !postCodeChallenge || !postCodeChallengeMethod
  ) {
    return c.json({
      error: "server_error",
      error_description: "Validation succeeded but required fields are missing",
