...
const expensiveIndexRateLimiter = new InMemoryRateLimiter({
  ...
  windowMs: 15 * 60 * 1000,
  message:
    "Too many indexing requests. Please wait before triggering another reindex.",
...
indexRoutes.post(
  "/spaces/:spaceId/index/vectorize",
  expensiveIndexRateLimiter.middleware(),
  async (c) => {
