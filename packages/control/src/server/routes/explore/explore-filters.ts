...
export const CATEGORY_FILTER_OPTS = {
  maxLen: 32,
  pattern: /^[a-z0-9_-]+$/,
} as const;
export const LANG_LICENSE_FILTER_OPTS = {
  maxLen: 64,
...
export function parseSinceDateToIsoStart(
  value: string | undefined,
): string | undefined {
  if (!value) return undefined;
...
export function parseExploreFilters(
  ...
  return {
    category: normalizeSimpleFilter(
      c.req.query("category"),
      CATEGORY_FILTER_OPTS,
    ),
    language: normalizeSimpleFilter(
      c.req.query("language"),
      LANG_LICENSE_FILTER_OPTS,
    ),
    license: normalizeSimpleFilter(
      c.req.query("license"),
      LANG_LICENSE_FILTER_OPTS,
    ),
...
export function validateExploreFilters(
  ...
  if (
    filters.category &&
    !(EXPLORE_CATEGORIES as ReadonlyArray<string>).includes(filters.category)
  ) {
