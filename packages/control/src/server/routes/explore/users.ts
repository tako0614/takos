...
export default new Hono<{ Bindings: Env; Variables: Variables }>()
  ...
  .get(
    ...
    async (c) => {
      ...
      const repoCountMap = new Map(
        repoCounts.map((r) => [r.accountId, r.count]),
      );
  ...
  .get("/users/:username", async (c) => {
    ...
    if (currentUser && repos.length > 0) {
      const stars = await db.select({ repoId: repoStars.repoId }).from(
        repoStars,
      ).where(
        and(
          eq(repoStars.accountId, currentUser.id),
          inArray(repoStars.repoId, repos.map((r) => r.id)),
        ),
      ).all();
