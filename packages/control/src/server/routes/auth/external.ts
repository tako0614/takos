...
import {
  errorPage,
  externalLoginPage,
  externalTokenPostRedirectPage,
} from "./html.ts";
import { AuthorizationError, BadRequestError } from "takos-common/errors";
import {
  getPlatformConfig,
  getPlatformServices,
...
function resolveAuthPublicBaseUrl(
  adminDomain: string,
  ...
  if (configuredBaseUrl) {
    try {
      ...
      logWarn("Invalid AUTH_PUBLIC_BASE_URL ignored", {
        module: "routes/auth/external",
...
externalAuthRouter.get("/external", async (c) => {
  ...
  const configuredAllowedDomains = getOptionalEnvBinding(
    c.env,
    "AUTH_ALLOWED_REDIRECT_DOMAINS",
  );

  if (!redirectUri) {
    return c.html(
      errorPage("エラー", "redirect_uri が指定されていません"),
      400,
    );
  }
  ...
  if (
    ...
    await auditLog("external_auth_invalid_redirect", {
      redirect_uri: redirectUri,
    });
    return c.html(
      errorPage("エラー", "許可されていないリダイレクト先です"),
      400,
    );
  }
  ...
  const googleOAuthUrl =
    `https://${platformConfig.adminDomain}/auth/external/google?state=${oauthState}`;
  ...
  const html = externalLoginPage({
    ...
    homeLabel: resolveAuthPublicHost(
      authPublicBaseUrl,
      platformConfig.adminDomain,
    ),
  });
...
externalAuthRouter.get("/external/google", async (c) => {
  ...
  const redirectUri =
    `https://${platformConfig.adminDomain}/auth/external/callback`;
...
externalAuthRouter.get("/external/callback", async (c) => {
  ...
  if (error) {
    ...
    return c.html(
      errorPage("認証エラー", "外部認証プロバイダーでエラーが発生しました"),
      400,
    );
  }
  ...
  if (!code || !state) {
    return c.html(
      errorPage("認証エラー", "認証コードが取得できませんでした"),
      400,
    );
  }

  if (
    !dbBinding || !sessionStore || !platformConfig.googleClientId ||
    !platformConfig.googleClientSecret
  ) {
  ...
  if (
    ...
    await auditLog("external_oauth_invalid_redirect_after_state", {
  ...
  if (!userInfoResponse.ok) {
    return c.html(
      errorPage("認証エラー", "ユーザー情報の取得に失敗しました"),
      400,
    );
  }
  ...
  if (!user) {
    ...
    const authPublicHost = resolveAuthPublicHost(
      authPublicBaseUrl,
      platformConfig.adminDomain,
    );
    return c.html(
  ...
  await auditLog("external_oauth_success", {
    userId: user.id,
