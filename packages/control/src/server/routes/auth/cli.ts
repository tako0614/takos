...
import { errorPage, escapeHtml, warningPage } from "./html.ts";
...
const CLI_CALLBACK_STYLE =
  "body{font-family:system-ui,sans-serif;padding:24px;}";
const CLI_CALLBACK_AUTOSUBMIT_SCRIPT =
...
<html lang="en">
  ...
  <body>
    ...
    <form id="cli-callback-form" method="POST" action="${
    escapeHtml(cliCallback)
  }">
      ${hiddenFields}
      <noscript>
...
authCliRouter.get("/cli/callback", async (c) => {
  ...
  if (!stateResult.valid || !stateResult.cliCallback) {
    return c.html(
      errorPage("認証エラー", "無効な CLI コールバックです。"),
      400,
    );
  }

  const cliCallback = stateResult.cliCallback;
  const cliState = typeof stateResult.returnTo === "string" &&
      CLI_STATE_PATTERN.test(stateResult.returnTo)
  ...
  if (!code) {
    return respondToCliWithPost(c, cliCallback, {
  ...
  if (!identity) {
    return respondToCliWithPost(c, cliCallback, {
      error: "Please complete registration on the web first",
    }, cliState);
  }
