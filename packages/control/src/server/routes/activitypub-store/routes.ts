...
export function setActivitypubStoreTestDeps(
  overrides: Partial<typeof activitypubStoreDeps>,
): void {
...
async function handleInbox(
  ...
  if (signatureHeader) {
    try {
      const sigResult = await activitypubStoreDeps.verifyHttpSignature(
        c.req.raw,
      );
  ...
  if (type === "Follow") {
    await activitypubStoreDeps.addFollower(c.env.DB, targetActorUrl, actorUrl);
    return activityJson(c, {
      "@context": "https://www.w3.org/ns/activitystreams",
      type: "Accept",
  ...
  if (type === "Undo") {
    ...
    if (innerObject && String(innerObject.type ?? "") === "Follow") {
      await activitypubStoreDeps.removeFollower(
        c.env.DB,
        targetActorUrl,
        actorUrl,
      );
...
async function handleFollowers(
  ...
  const result = await activitypubStoreDeps.listFollowers(
    c.env.DB,
    targetActorUrl,
    {
      limit,
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    if (kind === "repo" && repoOwner && repoName && domain) {
      ...
      const repo = await activitypubStoreDeps.findCanonicalRepo(
        c.env,
        repoOwner,
        repoName,
      );
    ...
    const storeRecord = await activitypubStoreDeps.findStoreBySlug(c.env, slug);
    if (!storeRecord) {
      return c.json({ error: "Actor not found" }, 404);
    }
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    const storeRecord = await activitypubStoreDeps.findStoreBySlug(
      c.env,
      store,
    );
    if (!storeRecord) {
...
activitypubStore.post("/ap/stores/:store/inbox", async (c) => {
  const store = c.req.param("store");
  const storeRecord = await activitypubStoreDeps.findStoreBySlug(c.env, store);
  if (!storeRecord) {
    return c.json({ error: "Store not found" }, 404);
  }
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    return handleFollowers(
      c,
      buildStoreActorId(getOrigin(c), storeRecord.slug),
    );
  },
);
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    const pageNum = Number.isFinite(pageNumParsed) && pageNumParsed > 0
      ? pageNumParsed
      : 1;
    const { limit } = parsePagination(c.req.query());
    ...
    const result = await activitypubStoreDeps.listStoreRepositories(
      c.env,
      storeRecord.slug,
      {
        limit,
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    if (!page) {
      const result = await activitypubStoreDeps.searchStoreRepositories(
        c.env,
        storeRecord.slug,
        query,
        {
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    const explicit = await activitypubStoreDeps.hasExplicitInventory(
      c.env.DB,
    ...
    if (explicit) {
      ...
      if (!page) {
        ...
        const invResult = await activitypubStoreDeps.listInventoryActivities(
          c.env.DB,
          storeRecord.accountId,
          storeRecord.slug,
          { limit: 1, offset: 0 },
        );
        const activeItems = await activitypubStoreDeps.listInventoryItems(
          c.env.DB,
          storeRecord.accountId,
          storeRecord.slug,
          { limit: 100, offset: 0 },
        ...
        const announceResult = localRepoIds.length > 0
          ? await activitypubStoreDeps.listPushActivitiesForRepoIds(
            c.env.DB,
            localRepoIds,
            { limit: 1, offset: 0 },
          )
      ...
      const invActivities: { ts: string; activity: Record<string, unknown> }[] =
        invResult.items.map((item) => ({
          ts: item.createdAt,
          activity: {
            "@context": "https://www.w3.org/ns/activitystreams",
      ...
      let announceActivities: {
        ts: string;
        activity: Record<string, unknown>;
      }[] = [];
      if (localRepoIds.length > 0) {
        const pushResult = await activitypubStoreDeps
          .listPushActivitiesForRepoIds(c.env.DB, localRepoIds, {
            limit,
            offset: (pageNum - 1) * limit,
          });
        ...
        const repoIdToItem = new Map(
          activeItems.items.filter((i) => i.localRepoId).map((
            i,
          ) => [i.localRepoId!, i]),
        );
        ...
        announceActivities = pushResult.items
          ...
          .map((push) => {
            ...
            return {
              ...
              activity: {
                "@context": activityContext(),
                id: `${actorId}/activities/announce/${
                  encodeURIComponent(push.createdAt)
                }`,
                type: "Announce",
                actor: actorId,
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    const publicRepo = await activitypubStoreDeps.findCanonicalRepo(
      c.env,
      owner,
      repoName,
    );
    ...
    const repo = await activitypubStoreDeps.findCanonicalRepoIncludingPrivate(
      c.env,
      owner,
      repoName,
    );
    ...
    const hasGrant = await activitypubStoreDeps.checkGrant(
      c.env.DB,
      repo.id,
      requestActorUrl,
      "visit",
    ...
    return activityJson(
      c,
      buildRepoActor(getOrigin(c), repo, { omitPushUri: true }),
    );
  },
);
...
activitypubStore.post("/ap/repos/:owner/:repoName/inbox", async (c) => {
  ...
  return handleInbox(
    c,
    buildRepoActorId(getOrigin(c), repo.ownerSlug, repo.name),
  );
});
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    const pushResult = await activitypubStoreDeps.listPushActivities(
      c.env.DB,
      repo.id,
      {
        limit,
    ...
    const activities: Record<string, unknown>[] = pushResult.items.map(
      (push) => {
        ...
        if (push.ref === DELETE_REF) {
          return {
            "@context": activityContext(),
            id: `${repoActorId}/activities/delete/${
              encodeURIComponent(push.createdAt)
            }`,
            type: "Delete",
            actor: repoActorId,
        ...
        return {
          "@context": activityContext(),
          id: `${repoActorId}/activities/push/${
            encodeURIComponent(push.createdAt)
          }`,
          type: "Push",
          actor: repoActorId,
...
activitypubStore.get(
  ...
  async (c) => {
    ...
    const storeRecords = await activitypubStoreDeps.listStoresForRepo(
      c.env,
      repo.ownerId,
    );
    const storeUris = storeRecords.map((s) =>
      buildStoreActorId(origin, s.slug)
    );
...
activitypubStore.get("/ap/stores/:store/repositories/:owner/:repoName", (c) => {
  ...
  const repoName = c.req.param("repoName");
  return c.redirect(
    `${getOrigin(c)}/ap/repos/${enc(owner)}/${enc(repoName)}`,
    301,
  );
});
