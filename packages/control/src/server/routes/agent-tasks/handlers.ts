...
function toIsoTimestamp(value: TimestampValue): string | null {
...
export function toApiTask(row: AgentTaskRow): AgentTaskBase {
  return {
    ...
    plan: row.plan ?? null,
    due_at: toIsoTimestamp(row.dueAt),
    started_at: toIsoTimestamp(row.startedAt),
    completed_at: toIsoTimestamp(row.completedAt),
    created_at: toRequiredIsoTimestamp(row.createdAt),
    updated_at: toRequiredIsoTimestamp(row.updatedAt),
  };
}
