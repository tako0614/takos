...
type TrustTierContext = Context<
  { Bindings: Env; Variables: TrustTierVariables }
>;
...
function meetsMinTier(
  userTier: string,
  requiredTier: "new" | "normal" | "trusted",
): boolean {
 ...
 */
export function requireTrustTier(
  minTier: "normal" | "trusted",
): MiddlewareHandler<{ Bindings: Env; Variables: TrustTierVariables }> {
  return async (c, next) => {
