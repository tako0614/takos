...
const SPACE_ROUTE_PARAM_PATTERN =
  /^(?:me|[a-z0-9](?:[a-z0-9-]{0,31})|[A-Za-z0-9_-]{1,128})$/;
...
export const validateApiOpaqueRouteParams: MiddlewareHandler<{
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    logWarn(
      `Rejected malformed route parameter "${key}" on ${c.req.method} ${c.req.path}`,
      { module: "middleware/param-validation" },
    );
