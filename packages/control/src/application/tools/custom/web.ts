...
export const WEB_FETCH: ToolDefinition = {
  ...
  parameters: {
    ...
    properties: {
      ...
      render: {
        type: "boolean",
        description: "Deprecated. Render mode is not supported in core Takos.",
      },
      timeout_ms: {
        type: "number",
...
export const webFetchHandler: ToolHandler = async (args, context) => {
  ...
  if (render) {
    return "Render mode via web_fetch is no longer supported in core Takos.";
  }
  ...
  if (!response.ok) {
    ...
    try {
      ...
      logWarn("Failed to parse web_fetch error response", {
        module: "tools/custom/web",
        url,
