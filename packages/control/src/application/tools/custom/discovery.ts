...
export const capabilitySearchHandler: ToolHandler = async (args, ctx) => {
  ...
  const results = registry.search(query, { limit }).filter((d) =>
    d.discoverable
  );
  ...
  parameters: {
    ...
    properties: {
      ...
      arguments: {
        type: "object",
        description:
          "Arguments to pass to the tool. Check capability_search results for tool descriptions.",
      },
    },
    required: ["tool_name"],
  },
...
export const capabilityInvokeHandler: ToolHandler = async (args, ctx) => {
  ...
  if (registry) {
    ...
    if (descriptor && !descriptor.discoverable) {
      throw new Error(
        `capability_invoke: tool "${toolName}" is not available for invocation.`,
      );
    }
  }
