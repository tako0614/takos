...
export const repoListHandler: ToolHandler = async (_args, context) => {
  const sessionId = requireContainerSession(
    context,
    "listing mounted repositories",
  );
  ...
  const lines = repos.map((repo) =>
    `- ${repo.repoName} (${repo.repoId}) @ ${repo.branch || "default"} -> ${
      repo.mountPath || "/"
...
export const repoStatusHandler: ToolHandler = async (_args, context) => {
  ...
  const primary = await db.select({
    ...
    .where(
      and(
        eq(sessionRepos.sessionId, sessionId),
  ...
  return `Active repository:\n${primary.repoName} (${primary.repoId}) @ ${
...
export const repoSwitchHandler: ToolHandler = async (args, context) => {
  ...
  await db.update(sessionRepos).set({ isPrimary: false }).where(
    and(
      eq(sessionRepos.sessionId, sessionId),
      eq(sessionRepos.isPrimary, true),
    ),
  );
  await db.update(sessionRepos).set({ isPrimary: true }).where(
    eq(sessionRepos.id, target.id),
  );
  ...
  return `Active repository switched to ${target.repoName} (${target.repoId}) @ ${
