...
export const storeSearchHandler: ToolHandler = async (args, context) => {
  ...
  const items = result.items.map((item) => ({
    ...
    package_available: item.package.available,
    package_app_id: item.package.app_id,
    package_version: item.package.latest_version,
    package_category: item.package.category,
    package_tags: item.package.tags,
    package_publish_status: item.package.publish_status,
    installed_in_current_space: item.installation?.installed ?? false,
    installation_group_deployment_snapshot_id:
