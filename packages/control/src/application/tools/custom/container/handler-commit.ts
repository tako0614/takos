...
import { type and as _and, asc, eq } from "drizzle-orm";
import {
  callSessionApi,
  checkSessionHealth,
  validateStringInput,
...
export const containerCommitHandler: ToolHandler = async (args, context) => {
  ...
  if (!health.isHealthy) {
    if (
      health.reason === "session_dead" || health.reason === "heartbeat_timeout"
    ) {
      throw new Error(
        `Container is DEAD (runtime session lost). Last heartbeat: ${
          health.session.last_heartbeat || "never"
        }. Cannot commit changes.`,
      );
    }
    throw new Error(
      `Container is not running (status: ${health.session.status}).`,
    );
  }
  ...
  if (targetRepoId) {
    reposToCommit = reposToCommit.filter((repo) =>
      repo.repoId === targetRepoId
    );
    if (reposToCommit.length === 0) {
  ...
  if (failures.length > 0) {
    ...
    const committedLines = results
      ...
      .map((result) =>
        `- ${result.repoName} (${result.repoId}) @ ${result.branch}: ${result.commitHash}`
      )
      .join("\n") || "None";
    throw new Error(
  ...
  try {
    ...
    logError("Failed to destroy runtime session", e, {
      module: "tools/custom/container/handler-commit",
    });
  }
