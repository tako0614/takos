...
import { normalizeMountPath, validateStringInput } from "./session.ts";
...
export const containerStartHandler: ToolHandler = async (args, context) => {
  ...
  const repoIdsInput = Array.isArray(args.repo_ids)
    ...
    : [];
  const mountsInput = Array.isArray(args.mounts)
    ? (args.mounts as Array<Record<string, unknown>>)
    : [];
  ...
  try {
    if (!context.env.RUNTIME_HOST) {
      throw new Error(
        "RUNTIME_HOST binding is required. Cannot start container.",
      );
    }
    ...
    if (mountsInput.length > 0) {
      ...
      let repo = await db.select({
        ...
        .from(repositories).where(
          and(
            eq(repositories.accountId, context.spaceId),
            eq(repositories.name, "main"),
          ),
        ).get() ?? undefined;
      ...
      if (!repo) {
        throw new Error(
          "No repository found in this space. Use create_repository to create one first.",
        );
      }
    ...
    if (repos.length !== repoIds.length) {
      throw new Error(
        "One or more repositories were not found or do not belong to this space.",
      );
    }
    ...
    for (const request of mountRequests) {
      ...
      if (!repo) {
        throw new Error(
          "Repository not found or does not belong to this space.",
        );
      }

      let mountPath = request.mountPath
        ? normalizeMountPath(request.mountPath)
        : "";
      if (!mountPath) {
    ...
    const primaryRepo = sessionReposList.find((repo) => repo.isPrimary) ||
      sessionReposList[0];
    ...
    logInfo(
      `Starting container ${newSessionId} for run ${context.runId} (git mode)`,
      { module: "tools/custom/container/handler-start" },
    );
    ...
    logInfo(
      `Container ${newSessionId} started for run ${context.runId} (git: ${initResult.branch})`,
      { module: "tools/custom/container/handler-start" },
    );

    const repoLines = sessionReposList
      .map((repo) =>
        `- ${repo.repoName} (${repo.repoId}) @ ${repo.branch} -> ${
          repo.mountPath || "/"
