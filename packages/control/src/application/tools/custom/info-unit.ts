...
function formatVectorMatch(
  ...
  const segmentInfo = typeof meta.segmentIndex === "number" &&
      typeof meta.segmentCount === "number"
    ? ` (${meta.segmentIndex + 1}/${meta.segmentCount})`
    : "";
  const repoInfo =
    includeRepoInfo && Array.isArray(meta.repoIds) && meta.repoIds.length > 0
      ? ` repos:${meta.repoIds.join(",")}`
      : "";
  return `${index + 1}. [${
    match.score.toFixed(3)
  }] run:${runId}${segmentInfo}${repoInfo}\n${snippet}`;
}
...
function formatTextSearchResult(
  ...
  const snippet = unit.content.length > 200
  ...
  const owned = await db.select({ id: repositories.id })
    .from(repositories).where(
      and(
        ...
        eq(repositories.accountId, spaceId),
      ),
    ).all();
...
export const infoUnitSearchHandler: ToolHandler = async (args, context) => {
  ...
  if (context.env.AI && context.env.VECTORIZE) {
    ...
    const searchResult = await context.env.VECTORIZE.query(
    ...
    const lines = matches.map((
      match: { score: number; id: string; metadata?: Record<string, unknown> },
      index: number,
    ) => formatVectorMatch(match, index, false));
  ...
  const lines = results.map((unit, index) =>
    formatTextSearchResult(
...
export const repoGraphSearchHandler: ToolHandler = async (args, context) => {
  ...
  const limit = Math.min((args.limit as number) || 5, 20);
  const minScore = typeof args.min_score === "number" ? args.min_score : 0.5;
  const repoIds = Array.isArray(args.repo_ids)
    ? (args.repo_ids as string[])
    : [];
  ...
  const db = getDb(context.db);
  const allowedRepoIds = await resolveAccessibleRepoIds(
    db,
    context.spaceId,
    repoIds,
  );
  ...
  if (context.env.AI && context.env.VECTORIZE) {
    ...
    let matches = searchResult.matches.filter((match: { score: number }) =>
    ...
    if (allowedRepoIds.length > 0) {
      matches = matches.filter(
        (match: { score: number; metadata?: Record<string, unknown> }) => {
          const meta = match.metadata as Record<string, unknown>;
          const metaRepoIds = Array.isArray(meta.repoIds)
            ? meta.repoIds as string[]
            : [];
    ...
    const lines = matches.map((
      match: { score: number; id: string; metadata?: Record<string, unknown> },
      index: number,
    ) => formatVectorMatch(match, index, true));
  ...
  const filtered = allowedRepoIds.length > 0
    ? results.filter((unit) =>
      allowedRepoIds.some((id) => unit.metadata.includes(`"repo_id":"${id}"`))
    )
    : results;
  ...
  const lines = filtered.slice(0, limit).map((unit, index) =>
    formatTextSearchResult(
      { run_id: unit.runId, kind: unit.kind, content: unit.content },
      index,
    )
...
async function traverseGraph(
  ...
  for (let d = 0; d < depth; d += 1) {
    ...
    const targetNodeMap = new Map<
      string,
      { label: string | null; type: string; refId: string }
    >();
    if (targetNodeIds.length > 0) {
      const targetNodeRows = await db.select({
        id: nodes.id,
        label: nodes.label,
        type: nodes.type,
        refId: nodes.refId,
    ...
    for (const edge of edgeRows) {
      const sourceLabel = edge.sourceLabel ||
        `${edge.sourceType}:${edge.sourceRefId}`;
      const targetNode = targetNodeMap.get(edge.targetId);
      const targetLabel = targetNode
        ? (targetNode.label || `${targetNode.type}:${targetNode.refId}`)
        : edge.targetId;
      lines.push(
        `${edge.sourceId} (${sourceLabel}) -[${edge.type}]-> ${edge.targetId} (${targetLabel})`,
      );
...
export const repoGraphNeighborsHandler: ToolHandler = async (args, context) => {
  ...
  if (!nodeId && infoUnitId) {
    const node = await db.select({ id: nodes.id })
      .from(nodes).where(
        and(
          eq(nodes.accountId, context.spaceId),
          eq(nodes.type, "info_unit"),
          eq(nodes.refId, infoUnitId),
        ),
      ).get();
    nodeId = node?.id;
  }
...
export const repoGraphLineageHandler: ToolHandler = async (args, context) => {
  ...
  const lines = await traverseGraph(
    db,
    context.spaceId,
    node.id,
    depth,
