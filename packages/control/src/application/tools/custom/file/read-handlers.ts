...
export const fileReadHandler: ToolHandler = async (args, context) => {
  ...
  const result = await handleSessionApiResponse<
    { content: string; size: number; is_binary?: boolean }
  >(response, "read file");
...
export const fileListHandler: ToolHandler = async (args, context) => {
  ...
  const response = await callSessionApi(context, "/session/file/list", {
    path,
  });
  ...
  if (result.entries.length === 0) {
    return `No files found in ${path || mountPath || "space root"}`;
  }
