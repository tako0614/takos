...
async function ensureServiceInWorkspace(
  serviceId: string,
  spaceId: string,
  d1: Parameters<ToolHandler>[1]["db"],
): Promise<void> {
  const db = getDb(d1);
  const worker = await db.select({ id: services.id })
    .from(services).where(
      and(eq(services.id, serviceId), eq(services.accountId, spaceId)),
    ).get();
...
export const deploymentGetHandler: ToolHandler = async (args, context) => {
  ...
  try {
    ...
    logWarn("Failed to load masked env vars", {
      module: "deployment_get",
  ...
  try {
    ...
    logWarn("Failed to load bindings", {
      module: "deployment_get",
      error: error instanceof Error ? error.message : String(error),
    });
  }
...
export const deploymentRollbackHandler: ToolHandler = async (args, context) => {
  ...
  const targetVersion = typeof args.target_version === "number" &&
      Number.isFinite(args.target_version)
