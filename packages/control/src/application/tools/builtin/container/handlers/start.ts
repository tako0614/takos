...
export const containerStartHandler: ToolHandler = async (args, context) => {
  ...
  try {
    ...
    logInfo(`Starting container ${newSessionId} for run ${context.runId} (git mode)`, { module: 'tools/builtin/container/handlers/start' });
    ...
    logInfo(`Container ${newSessionId} started for run ${context.runId} (git: ${initResult.branch})`, { module: 'tools/builtin/container/handlers/start' });
