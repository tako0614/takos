...
export const containerStopHandler: ToolHandler = async (args, context) => {
  ...
  if (context.env.RUNTIME_HOST) {
    try {
      ...
      logError('Failed to destroy runtime session', e, { module: 'tools/builtin/container/handlers/stop' });
    }
  }
  ...
  if (context.storage && context.sessionId) {
    try {
      ...
      if (listed.objects && listed.objects.length > 0) {
        ...
        logInfo(`Cleaned up ${listed.objects.length} R2 session files on stop`, { module: 'tools/builtin/container/handlers/stop' });
      }
    } catch (e) {
      logError('Failed to cleanup R2 session files', e, { module: 'tools/builtin/container/handlers/stop' });
    }
  }
