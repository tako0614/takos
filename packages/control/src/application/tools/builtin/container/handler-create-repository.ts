...
export const createRepositoryHandler: ToolHandler = async (args, context) => {
  ...
  logInfo(`Creating repository '${name}' for workspace ${context.spaceId}`, { module: 'tools/builtin/container/handler-create-repository' });
  try {
    ...
    logInfo(`Created repository '${repository.name}' with id ${repository.id}`, { module: 'tools/builtin/container/handler-create-repository' });
