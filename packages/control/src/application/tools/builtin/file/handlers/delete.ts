...
export const fileDeleteHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === 'rejected') {
    logWarn(`R2 backup delete failed for ${path}`, { module: 'tools/builtin/file/handlers/delete', detail: r2Result.reason });
  }

  if (runtimeResult.status === 'rejected') {
    const r2Succeeded = r2Result.status === 'fulfilled';
    logError('Runtime delete failed', runtimeResult.reason, { module: 'tools/builtin/file/handlers/delete' });
