...
export const fileWriteHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === 'rejected') {
    logWarn(`R2 backup write failed for ${path}`, { module: 'tools/builtin/file/handler-write', detail: r2Result.reason });
  }

  if (runtimeResult.status === 'rejected') {
    const r2Succeeded = r2Result.status === 'fulfilled';
    logError('Runtime write failed', runtimeResult.reason, { module: 'tools/builtin/file/handler-write' });
