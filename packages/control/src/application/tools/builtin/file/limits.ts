...
const FILE_LIMITS: Record<FileCategory, FileLimits> = {
  source: { maxSize: 10 * 1024 * 1024, description: 'Source code files (10MB)' },
  config: { maxSize: 5 * 1024 * 1024, description: 'Configuration files (5MB)' },
  asset: { maxSize: 25 * 1024 * 1024, description: 'Image and font assets (25MB)' },
...
function getFileCategory(path: string): FileCategory {
  ...
  const sourceExtensions = new Set([
    ...
    '.vue', '.svelte', '.astro', '.php', '.lua', '.sh', '.bash',
  ...
  if (configExtensions.has(ext) || configFilenames.has(filename)) return 'config';
...
export function validateBinaryContent(base64Content: string, path: string): void {
