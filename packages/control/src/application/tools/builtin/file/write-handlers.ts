...
export const fileWriteHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === 'rejected') {
    logWarn(`R2 backup write failed for ${path}`, { module: 'tools/builtin/file/write-handlers', detail: r2Result.reason });
  }

  if (runtimeResult.status === 'rejected') {
    logError('Runtime write failed', runtimeResult.reason, { module: 'tools/builtin/file/write-handlers' });
...
export const fileWriteBinaryHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === 'rejected') {
    logWarn(`R2 backup binary write failed for ${path}`, { module: 'tools/builtin/file/write-handlers', detail: r2Result.reason });
  }

  if (runtimeResult.status === 'rejected') {
    logError('Runtime binary write failed', runtimeResult.reason, { module: 'tools/builtin/file/write-handlers' });
...
export const fileCopyHandler: ToolHandler = async (args, context) => {
  ...
  if (context.storage) {
    try {
      ...
      logWarn(`R2 backup copy write failed for ${destPath}`, { module: 'tools/builtin/file/write-handlers', detail: err });
    }
  }
