...
export function requireContainer(context: ToolContext): void {
  if (!context.sessionId) {
    throw new Error(buildContainerUnavailableMessage(context, 'using file operations'));
  }
}
...
export async function resolveMountPath(
  ...
  if (repoId) {
    const entry = await db.select({ mountPath: sessionRepos.mountPath })
      .from(sessionRepos).where(and(eq(sessionRepos.sessionId, context.sessionId), eq(sessionRepos.repoId, repoId))).get();
    if (!entry) {
  ...
  const entry = await db.select({ mountPath: sessionRepos.mountPath })
    .from(sessionRepos).where(and(eq(sessionRepos.sessionId, context.sessionId), eq(sessionRepos.repoId, session.repoId))).get();
