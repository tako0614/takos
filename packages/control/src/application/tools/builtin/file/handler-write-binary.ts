...
export const fileWriteBinaryHandler: ToolHandler = async (args, context) => {
  ...
  if (r2Result.status === 'rejected') {
    logWarn(`R2 backup binary write failed for ${path}`, { module: 'tools/builtin/file/handler-write-binary', detail: r2Result.reason });
  }

  if (runtimeResult.status === 'rejected') {
    const r2Succeeded = r2Result.status === 'fulfilled';
    logError('Runtime binary write failed', runtimeResult.reason, { module: 'tools/builtin/file/handler-write-binary' });
