...
export async function createCloudflareCustomHostname(
  ...
  try {
    const result = await cfClient.zonePost<{ id: string }>(
      "/custom_hostnames",
      {
        hostname: domain,
        ssl: {
...
export async function deleteCloudflareCustomHostname(
  ...
  try {
    ...
    logError("Failed to delete custom hostname", err, {
      module: "services/platform/custom-domains",
    });
  }
}
...
export async function getCloudflareCustomHostnameStatus(
  ...
  try {
    const result = await cfClient.zoneGet<
      { status: string; ssl?: { status: string } }
    >(
    ...
    logError("Failed to get custom hostname status", err, {
      module: "services/platform/custom-domains",
    });
    return null;
  }
