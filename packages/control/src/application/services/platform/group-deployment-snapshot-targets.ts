...
export async function readManifestFromRepoTarget(
  ...
  if (!manifestPath) {
    throw new NotFoundError("Deploy manifest (.takos/app.yml)");
  }

  const rawManifest = await readRepoTextFileAtTarget(env, target, manifestPath);
  if (!rawManifest) {
    throw new NotFoundError(`Deploy manifest not found at ${manifestPath}`);
  }
  ...
  try {
    ...
    throw new BadRequestError(
      error instanceof Error ? error.message : "Invalid deploy manifest",
    );
  }
}
...
export async function buildSourceFromRow(
  env: Env,
  row: GroupDeploymentSnapshotRow,
): Promise<GroupDeploymentSnapshotSource> {
