...
import { and, eq, type sql as _sql } from "drizzle-orm";
...
import {
  type accountMemberships as _accountMemberships,
  accounts,
  getDb,
...
async function encryptManagedToken(
  ...
  const encrypted = await encrypt(
    ...
    buildManagedTokenSalt(serviceId, envName),
  );
  return JSON.stringify(encrypted);
}

async function decryptManagedToken(
  env: Pick<Env, "ENCRYPTION_KEY">,
  row: ManagedTakosTokenRow,
): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `Failed to parse encrypted token for service ${row.service_id}, env ${row.env_name}: ${
  ...
  return decrypt(
    encrypted,
    getCommonEnvSecret(env),
    buildManagedTokenSalt(row.service_id, row.env_name),
  );
}

function toManagedTokenRow(
  row: SelectOf<typeof serviceManagedTakosTokens>,
): ManagedTakosTokenRow {
  return {
    id: row.id,
...
export async function deleteManagedTakosTokenConfig(params: {
  ...
  const existing = await listManagedRow(
    params.env.DB,
    params.spaceId,
    serviceId,
    envName,
...
export async function upsertManagedTakosTokenConfig(params: {
  ...
  const tokenEncrypted = await encryptManagedToken(
    params.env,
    serviceId,
    envName,
    issued.token,
  ...
  await drizzle.insert(serviceManagedTakosTokens)
    ...
    .onConflictDoUpdate({
      target: [
        ...
        serviceManagedTakosTokens.envName,
      ],
      set: {
        subjectAccountId: resolved.subjectUserId,
        subjectMode: resolved.subjectMode,
...
> {
  ...
  if (
    row.subject_user_id !== resolved.subjectUserId ||
    row.subject_mode !== resolved.subjectMode
  ) {
...
export async function listTakosManagedStatuses(params: {
  ...
  const tokenLinkState =
    params.linkStateByName?.get(TAKOS_ACCESS_TOKEN_ENV_NAME) || null;
  ...
  return {
    ...
    [TAKOS_ACCESS_TOKEN_ENV_NAME]: {
      ...
      sync_state: tokenLinkState
        ? (tokenLinkState.syncState === "missing_common"
          ? "missing_included"
          : tokenLinkState.syncState)
        : (managedToken ? "managed" : "pending"),
      sync_reason: tokenLinkState?.syncReason ??
        (managedToken ? null : "missing_config"),
    },
  };
}
