...
function buildR2Key(spaceId: string, fileId: string): string {
...
async function updateDescendantPaths(
  ...
  await d1.prepare(
    ...
    .bind(newPfx, oldPfx.length + 1, now, spaceId, `${escapeSqlLike(oldPfx)}%`)
    .run();
}

async function revertParentUpdate(
  db: Database,
  spaceId: string,
  fileId: string,
  oldName: string,
...
export async function listStorageFiles(
  ...
  if (normalizedPath !== "/") {
    const parent = await db.select({ id: accountStorageFiles.id }).from(
      ...
      .where(
        and(
          eq(accountStorageFiles.accountId, spaceId),
          eq(accountStorageFiles.path, normalizedPath),
          eq(accountStorageFiles.type, "folder"),
        ),
      ).get();
  ...
  const files = (truncated ? results.slice(0, MAX_LIST_ITEMS) : results).map(
    toApiResponse,
  );
...
export async function getStorageItem(
  ...
  const db = sourceServiceDeps.getDb(d1);
  const file = await db.select().from(accountStorageFiles).where(
    and(
      eq(accountStorageFiles.id, fileId),
      eq(accountStorageFiles.accountId, spaceId),
    ),
...
export async function getStorageItemByPath(
  ...
  const file = await db.select().from(accountStorageFiles).where(
    and(
      eq(accountStorageFiles.accountId, spaceId),
      eq(accountStorageFiles.path, normalizePath(path)),
    ),
  ).get();
  return file ? toApiResponse(file) : null;
}
...
export async function createFolder(
  ...
  if (!sourceServiceDeps.validatePathSegment(name)) {
    throw new StorageError("Invalid folder name", "VALIDATION");
  }
...
export async function createFileRecord(
  ...
  if (!sourceServiceDeps.validatePathSegment(name)) {
    throw new StorageError("Invalid file name", "VALIDATION");
  }
...
export async function confirmUpload(
  ...
  const db = sourceServiceDeps.getDb(d1);
  const fileRecord = await db.select({ r2Key: accountStorageFiles.r2Key }).from(
    accountStorageFiles,
  )
    .where(
      and(
  ...
  if (file.type === "folder") {
    ...
    const descendants = await db.select({ r2Key: accountStorageFiles.r2Key })
      ...
      .where(
        and(
          eq(accountStorageFiles.accountId, spaceId),
          sql`${accountStorageFiles.path} LIKE ${descendantPattern} ESCAPE '\\'`,
          eq(accountStorageFiles.type, "file"),
        ),
      ).all();
    ...
    const rec = await db.select({ r2Key: accountStorageFiles.r2Key }).from(
      accountStorageFiles,
    ).where(
      and(
        eq(accountStorageFiles.id, fileId),
...
export async function renameStorageItem(
  ...
  const { name } = input;
  if (!sourceServiceDeps.validatePathSegment(name)) {
    throw new StorageError("Invalid name", "VALIDATION");
  }
  const file = await getStorageItem(d1, spaceId, fileId);
  if (!file) throw new StorageError("File or folder not found", "NOT_FOUND");
    ...
    try {
      ...
      try {
        ...
        sourceServiceDeps.logWarn(
          "Rename rollback of parent update failed (non-critical)",
          {
...
export async function moveStorageItem(
  ...
  const newParentId = await resolveParentId(
    db,
    spaceId,
    normalizedParentPath,
  ...
  if (file.type === "folder") {
    try {
      ...
      try {
        ...
        sourceServiceDeps.logWarn(
          "Move rollback of parent update failed (non-critical)",
          {
            module: "space-storage",
            error: err instanceof Error ? err.message : String(err),
          },
...
> {
  const db = sourceServiceDeps.getDb(d1);
  const fileRecord = await db.select().from(accountStorageFiles).where(
    and(
      eq(accountStorageFiles.id, fileId),
      eq(accountStorageFiles.accountId, spaceId),
      eq(accountStorageFiles.type, "file"),
  ...
  if (!fileRecord.r2Key) {
    throw new StorageError("File has no storage content", "STORAGE_ERROR");
  }
...
export async function writeFileContent(
  ...
  if (!fileRecord) throw new StorageError("File not found", "NOT_FOUND");
  if (!fileRecord.r2Key) {
    throw new StorageError("File has no storage key", "STORAGE_ERROR");
  }
...
export async function createFileWithContent(
  ...
  if (!sourceServiceDeps.validatePathSegment(fileName)) {
    throw new StorageError("Invalid file name", "VALIDATION");
  }
