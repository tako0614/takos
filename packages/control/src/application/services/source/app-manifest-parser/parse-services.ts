...
function parseServiceProvider(value: unknown): ServiceProvider | undefined {
  return value === "oci" || value === "ecs" || value === "cloud-run" ||
      value === "k8s"
    ? value
    : undefined;
}
...
function parseServiceTriggers(
  name: string,
  serviceSpec: Record<string, unknown>,
): { triggers: AppService["triggers"] } | undefined {
  ...
  const schedules = schedulesRaw == null ? undefined : (() => {
    if (!Array.isArray(schedulesRaw)) {
      throw new Error(
        `spec.services.${name}.triggers.schedules must be an array`,
      );
    }
    return schedulesRaw.map((entry, index) => {
      ...
      return {
        cron: asRequiredString(
          record.cron,
          `spec.services.${name}.triggers.schedules[${index}].cron`,
        ),
        export: asRequiredString(
          record.export,
          `spec.services.${name}.triggers.schedules[${index}].export`,
        ),
      };
    });
  })();
...
export function parseServices(
  ...
  for (const [name, value] of Object.entries(servicesRecord)) {
    ...
    const serviceHealthCheck = parseHealthCheck(
      s.healthCheck,
      `spec.services.${name}`,
    );
    const serviceVolumes = parseVolumes(s.volumes, `spec.services.${name}`);
    const serviceDependsOn = asStringArray(
      s.dependsOn,
      `spec.services.${name}.dependsOn`,
    );
    const serviceTriggers = parseServiceTriggers(name, s);

    const serviceBindings = s.bindings
      ? parseWorkloadBindings(s.bindings, `spec.services.${name}.bindings`)
      : undefined;

    const dockerfile = asString(
      s.dockerfile,
      `spec.services.${name}.dockerfile`,
    );
    const imageRef = asString(s.imageRef, `spec.services.${name}.imageRef`);
    const artifactRecord = asRecord(s.artifact);
    const artifactKind = asString(
      artifactRecord.kind,
      `spec.services.${name}.artifact.kind`,
    );
    const artifact = artifactKind === "image"
      ? {
        kind: "image" as const,
        imageRef: asRequiredString(
          artifactRecord.imageRef,
          `spec.services.${name}.artifact.imageRef`,
        ),
    ...
    if (!dockerfile && !imageRef && !artifact) {
      throw new Error(
        `spec.services.${name} must define dockerfile, imageRef, or artifact.kind=image`,
      );
    }
    ...
    services[name] = {
      ...
      ...(((): { env?: Record<string, string> } => {
        const v = asStringMap(s.env, `spec.services.${name}.env`);
        return v ? { env: v } : {};
      })()),
...
export function parseMcpServers(
  ...
  return mcpServersRaw.map((entry, index) => {
    ...
    const endpoint = asString(
      server.endpoint,
      `spec.mcpServers[${index}].endpoint`,
    );
    const route = asString(server.route, `spec.mcpServers[${index}].route`);
    if (!endpoint && !route) {
      throw new Error(
        `spec.mcpServers[${index}].endpoint or spec.mcpServers[${index}].route is required`,
      );
    }
    const authSecretRef = asString(
      server.authSecretRef,
      `spec.mcpServers[${index}].authSecretRef`,
    );
    return {
      ...
      ...((() => {
        const v = asString(
          server.transport,
          `spec.mcpServers[${index}].transport`,
        );
        return v ? { transport: v as "streamable-http" } : {};
      })()),
...
export function parseFileHandlers(
  specRecord: Record<string, unknown>,
): AppFileHandler[] | undefined {
  ...
  return fileHandlersRaw.map((entry, index) => {
    ...
    return {
      ...
      ...((() => {
        const v = asStringArray(
          handler.mimeTypes,
          `spec.fileHandlers[${index}].mimeTypes`,
        );
        return v ? { mimeTypes: v } : {};
      })()),
      ...((() => {
        const v = asStringArray(
          handler.extensions,
          `spec.fileHandlers[${index}].extensions`,
        );
        return v ? { extensions: v } : {};
      })()),
      openPath: asRequiredString(
        handler.openPath,
        `spec.fileHandlers[${index}].openPath`,
      ),
    };
  });
}
