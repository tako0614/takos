...
import { parseWorkloadBindings } from "../app-manifest-bindings.ts";
...
function parseScaling(
  raw: unknown,
  _prefix: string,
): WorkerScaling | undefined {
  if (!raw) return undefined;
  const record = asRecord(raw);
  return {
    ...(record.minInstances != null
      ? { minInstances: Number(record.minInstances) }
      : {}),
    ...(record.maxConcurrency != null
      ? { maxConcurrency: Number(record.maxConcurrency) }
      : {}),
  };
}
...
function parseWorkerBuild(
  ...
  if (
    buildSpec.command != null || buildSpec.output != null ||
    buildSpec.cwd != null || workerSpec.entry != null
  ) {
    throw new Error(
      `spec.workers.${workerName} local build fields are not supported; use build.fromWorkflow`,
    );
  }
  if (Object.keys(buildSpec).length > 0) {
    if (Object.keys(fromWorkflow).length === 0) {
      throw new Error(
        `spec.workers.${workerName}.build.fromWorkflow is required`,
      );
    }
    const workflowPath = normalizeRepoPath(
      asRequiredString(
        fromWorkflow.path,
        `spec.workers.${workerName}.build.fromWorkflow.path`,
      ),
    );
    if (!workflowPath.startsWith(".takos/workflows/")) {
      throw new Error(
        `spec.workers.${workerName}.build.fromWorkflow.path must be under .takos/workflows/`,
      );
    }
    return {
      build: {
        fromWorkflow: {
          ...
          job: asRequiredString(
            fromWorkflow.job,
            `spec.workers.${workerName}.build.fromWorkflow.job`,
          ),
          artifact: asRequiredString(
            fromWorkflow.artifact,
            `spec.workers.${workerName}.build.fromWorkflow.artifact`,
          ),
          artifactPath: normalizeRepoPath(
            asRequiredString(
              fromWorkflow.artifactPath,
              `spec.workers.${workerName}.build.fromWorkflow.artifactPath`,
            ),
          ),
        },
      },
  ...
  if (artifactSpec.kind === "bundle") {
    const deploymentId = typeof artifactSpec.deploymentId === "string" &&
        artifactSpec.deploymentId.trim().length > 0
      ? artifactSpec.deploymentId.trim()
      : undefined;
    const artifactRef = typeof artifactSpec.artifactRef === "string" &&
        artifactSpec.artifactRef.trim().length > 0
      ? artifactSpec.artifactRef.trim()
      : undefined;
    if (!deploymentId && !artifactRef) {
      throw new Error(
        `spec.workers.${workerName}.artifact.bundle requires deploymentId or artifactRef`,
      );
    }
    return {
      artifact: {
  ...
  throw new Error(
    `spec.workers.${workerName} must define build.fromWorkflow or artifact.kind=bundle`,
  );
}
...
function parseWorkerBindings(
  ...
  const bindings = parseWorkloadBindings(
    workerSpec.bindings,
    `spec.workers.${workerName}.bindings`,
  );
  return {
    bindings: bindings ?? {},
  };
...
function parseWorkerTriggers(
  ...
  const schedules = schedulesRaw == null ? undefined : (() => {
    if (!Array.isArray(schedulesRaw)) {
      throw new Error(
        `spec.workers.${workerName}.triggers.schedules must be an array`,
      );
    }
    return schedulesRaw.map((entry, index) => {
      ...
      return {
        cron: asRequiredString(
          record.cron,
          `spec.workers.${workerName}.triggers.schedules[${index}].cron`,
        ),
        export: asRequiredString(
          record.export,
          `spec.workers.${workerName}.triggers.schedules[${index}].export`,
        ),
      };
    });
  })();
  const queues = queuesRaw == null ? undefined : (() => {
    if (!Array.isArray(queuesRaw)) {
      throw new Error(
        `spec.workers.${workerName}.triggers.queues must be an array`,
      );
    }
    return queuesRaw.map((entry, index) => {
      const record = asRecord(entry);
      return {
        queue: asRequiredString(
          record.queue,
          `spec.workers.${workerName}.triggers.queues[${index}].queue`,
        ),
        export: asRequiredString(
          record.export,
          `spec.workers.${workerName}.triggers.queues[${index}].export`,
        ),
      };
    });
  })();
...
export function parseWorkers(
  ...
  if (workerNames.length === 0) return workers;
  ...
  for (const [workerName, workerValue] of Object.entries(workersRecord)) {
    ...
    if (containersRaw != null) {
      if (!Array.isArray(containersRaw)) {
        throw new Error(
          `spec.workers.${workerName}.containers must be an array of container names`,
        );
      }
      containerRefs = containersRaw.map((entry, i) => {
        const ref = asRequiredString(
          entry,
          `spec.workers.${workerName}.containers[${i}]`,
        );
        if (!containers[ref]) {
          throw new Error(
            `spec.workers.${workerName}.containers[${i}] references unknown container: ${ref}`,
          );
        }
        return ref;
      });
    ...
    const workerHealthCheck = parseHealthCheck(
      workerSpec.healthCheck,
      `spec.workers.${workerName}`,
    );
    const workerScaling = parseScaling(
      workerSpec.scaling,
      `spec.workers.${workerName}`,
    );
    const workerDependsOn = asStringArray(
      workerSpec.dependsOn,
      `spec.workers.${workerName}.dependsOn`,
    );
    workers[workerName] = {
      ...(containerRefs && containerRefs.length > 0
        ? { containers: containerRefs }
        : {}),
      ...build,
      ...(((): { env?: Record<string, string> } => {
        const v = asStringMap(workerSpec.env, `spec.workers.${workerName}.env`);
        return v ? { env: v } : {};
      })()),
      ...(workerSpec.bindings
        ? parseWorkerBindings(workerName, workerSpec)
        : {}),
      ...(workerSpec.triggers
        ? parseWorkerTriggers(workerName, workerSpec)
        : {}),
