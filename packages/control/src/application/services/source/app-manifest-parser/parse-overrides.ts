...
function parsePartialContainers(
  raw: unknown,
): Record<string, Partial<AppContainer>> {
  ...
  for (const [name, value] of Object.entries(containersRecord)) {
    ...
    result[name] = {
      ...(c.dockerfile
        ? { dockerfile: normalizeRepoPath(String(c.dockerfile)) }
        : {}),
      ...(c.port != null ? { port: Number(c.port) } : {}),
      ...(c.instanceType ? { instanceType: String(c.instanceType) } : {}),
      ...(c.maxInstances != null
        ? { maxInstances: Number(c.maxInstances) }
        : {}),
      ...(((): { env?: Record<string, string> } => {
        const v = asStringMap(c.env, `overrides.containers.${name}.env`);
        return v ? { env: v } : {};
      })()),
    };
  }
...
function parsePartialWorkers(raw: unknown): Record<string, Partial<AppWorker>> {
  ...
  for (const [name, value] of Object.entries(workersRecord)) {
    ...
    result[name] = {
      ...(((): { env?: Record<string, string> } => {
        const v = asStringMap(w.env, `overrides.workers.${name}.env`);
        return v ? { env: v } : {};
      })()),
      ...(w.bindings
        ? {
          bindings: parseWorkloadBindings(
            w.bindings,
            `overrides.workers.${name}.bindings`,
          ),
        }
        : {}),
    };
...
function parsePartialServices(
  raw: unknown,
): Record<string, Partial<AppService>> {
  ...
  for (const [name, value] of Object.entries(servicesRecord)) {
    ...
    result[name] = {
      ...(s.dockerfile
        ? { dockerfile: normalizeRepoPath(String(s.dockerfile)) }
        : {}),
      ...(s.port != null ? { port: Number(s.port) } : {}),
      ...(s.instanceType ? { instanceType: String(s.instanceType) } : {}),
      ...(s.maxInstances != null
        ? { maxInstances: Number(s.maxInstances) }
        : {}),
      ...(s.ipv4 === true ? { ipv4: true } : {}),
      ...(((): { env?: Record<string, string> } => {
        const v = asStringMap(s.env, `overrides.services.${name}.env`);
        return v ? { env: v } : {};
      })()),
      ...(s.bindings
        ? {
          bindings: parseWorkloadBindings(
            s.bindings,
            `overrides.services.${name}.bindings`,
          ),
        }
        : {}),
    };
...
export function parseOverrides(
  ...
  for (const [envName, envOverrides] of Object.entries(record)) {
    ...
    result[envName] = {
      ...(envRecord.containers
        ? { containers: parsePartialContainers(envRecord.containers) }
        : {}),
      ...(envRecord.workers
        ? { workers: parsePartialWorkers(envRecord.workers) }
        : {}),
      ...(envRecord.services
        ? { services: parsePartialServices(envRecord.services) }
        : {}),
    };
  }
  return result;
