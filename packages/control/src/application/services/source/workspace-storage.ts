 ...
 *
 * Provides file and folder storage capabilities for workspaces.
 * Uses R2 for object storage and D1 for metadata.
 */

import type { D1Database, R2Bucket } from '../../../shared/types/bindings.ts';
import type { WorkspaceStorageFileType } from '../../../shared/types';
...
export interface StorageFileResponse {
  ...
  type: WorkspaceStorageFileType; size: number; mime_type: string | null; sha256: string | null;
...
function toApiResponse(file: StorageFileRow): StorageFileResponse {
  return {
    ...
    path: file.path, type: file.type as WorkspaceStorageFileType, size: file.size,
