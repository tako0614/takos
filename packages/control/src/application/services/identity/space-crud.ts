...
export async function listWorkspacesForUser(
  ...
  return memberships.map((membership) => ({
    ...
    kind: (membership.spaceType === "user" ? "user" : "team") as
      | "user"
      | "team",
    ...
    security_posture: membership.spaceSecurityPosture === "restricted_egress"
      ? "restricted_egress"
      : "standard",
    created_at: membership.spaceCreatedAt,
    updated_at: membership.spaceUpdatedAt,
    member_role: membership
      .memberRole as import("../../../shared/types/index.ts").SpaceRole,
...
export async function getWorkspaceWithRepository(
  ...
  return {
    workspace,
    repository: await findLatestRepositoryBySpaceId(env.DB, workspace.id).then(
      (repo) => {
        if (!repo) return null;
        return {
          id: repo.id,
...
export async function getPersonalWorkspace(
  ...
  return {
    ...
    security_posture: userAccount.securityPosture === "restricted_egress"
      ? "restricted_egress"
      : "standard",
