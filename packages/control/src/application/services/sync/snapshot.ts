...
import type { BlobFetcher, Snapshot, SnapshotTree } from "./models.ts";
...
export interface SnapshotDeps {
  ...
  logInfo: typeof realLogInfo;
  logWarn: typeof realLogWarn;
}
...
function parseParentIds(
  raw: string | null,
  snapshotId?: string,
  logError = snapshotDeps.logError,
): string[] {
  if (!raw) return [];
  try {
    ...
    if (snapshotId) {
      logError(`Failed to parse parent_ids for snapshot ${snapshotId}`, err, {
...
export class SnapshotManager {
  ...
  constructor(
    ...
    private deps: SnapshotDeps = snapshotDeps,
  ) {
  ...
  async createSnapshot(
    ...
    const db = this.deps.getDb(this.env.DB);
    const snapshotId = this.deps.generateId();
  ...
  async completeSnapshot(snapshotId: string): Promise<void> {
    ...
    if ((result.meta.changes ?? 0) === 0) {
      this.deps.logWarn(
        `Snapshot ${snapshotId} was not in pending status or not found`,
        { module: "services/sync/snapshot" },
      );
    }
  }
  ...
  async failSnapshot(snapshotId: string): Promise<void> {
    ...
    try {
      ...
      this.deps.logInfo(`Cleaned up failed snapshot ${snapshotId}`, {
        module: "services/sync/snapshot",
      });
    } catch (err) {
      this.deps.logError(`Failed to clean up snapshot ${snapshotId}`, err, {
        module: "services/sync/snapshot",
      });
  ...
  async cleanupPendingSnapshots(maxAgeMinutes: number = 30): Promise<number> {
    return cleanupPendingSnapshotsImpl(
      this,
      this.env,
      this.spaceId,
      maxAgeMinutes,
  ...
  async getTree(snapshotId: string): Promise<SnapshotTree> {
    ...
    try {
      ...
      if (
        ...
        parseError.message.includes("not configured")
      ) {
        throw parseError;
      }
      this.deps.logError(
        `Failed to parse tree JSON for snapshot ${snapshotId}`,
        parseError,
        { module: "services/sync/snapshot" },
      );
      return {};
    ...
    if (!tree) {
      this.deps.logError(
        `Tree object not found for snapshot ${snapshotId}: ${snapshot.tree_key}`,
        undefined,
        { module: "services/sync/snapshot" },
      );
      try {
        await db.delete(snapshots)
          ...
          .run();
        this.deps.logInfo(
          `Cleaned up orphaned snapshot record: ${snapshotId}`,
          { module: "services/sync/snapshot" },
        );
      } catch (cleanupError) {
        this.deps.logError(
          `Failed to cleanup orphaned snapshot`,
          cleanupError,
          { module: "services/sync/snapshot" },
        );
      }
    ...
    if (!this.validateTree(tree)) {
      this.deps.logError(
        `Invalid tree structure for snapshot ${snapshotId}`,
        undefined,
        { module: "services/sync/snapshot" },
      );
      return {};
    ...
    if (!this.storage.validateTreeKeyIntegrity(snapshot.tree_key, snapshotId)) {
      this.deps.logError(
        `Snapshot integrity check failed: tree_key ${snapshot.tree_key} does not match expected prefix`,
        undefined,
        { module: "services/sync/snapshot" },
      );
      return {};
