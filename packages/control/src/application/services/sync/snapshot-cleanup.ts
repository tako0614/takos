...
export async function getReachableSnapshots(
    ...
    while (toFetch.length > 0) {
        if (iterations >= MAX_DAG_DEPTH) {
            logWarn(`DAG traversal limit reached (${MAX_DAG_DEPTH} snapshots) for workspace ${spaceId}`, { module: 'services/sync/snapshot' });
            break;
        }
...
export async function runGC(
    ...
    env: Env,
    spaceId: string,
): Promise<{ deletedBlobs: number; deletedSnapshots: number; deletedSessionFiles: number }> {
    const db = getDb(env.DB);
    let deletedBlobs = 0;
