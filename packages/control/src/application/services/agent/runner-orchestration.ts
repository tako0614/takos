...
export interface OrchestrationDeps {
  ...
  emitEvent: (type: AgentEvent['type'], data: Record<string, unknown>, options?: { skipDb?: boolean }) => Promise<void>;
...
export async function initToolExecutor(deps: OrchestrationDeps): Promise<void> {
  const toolExecutor = await RemoteToolExecutor.create(deps.context.runId, {
    getToolCatalog: (input: { runId: string }) => deps.runIo.getToolCatalog(input),
    executeTool: (input: { runId: string; toolCall: ToolCall }) => deps.runIo.executeTool(input),
    cleanupToolExecutor: (input: { runId: string }) => deps.runIo.cleanupToolExecutor(input),
  });
...
export async function enqueuePostRunJobs(deps: OrchestrationDeps): Promise<void> {
  const enqueueInfoUnit = async () => {
    ...
    try {
      ...
      logWarn(`Failed to enqueue info unit job for run ${deps.context.runId}`, { module: 'info_unit', detail: err });
    }
  };
  ...
  const enqueueThreadContext = async () => {
    ...
    try {
      ...
      logWarn(`Failed to enqueue thread context job for thread ${deps.context.threadId}`, { module: 'thread_context', detail: err });
    }
  };
...
export async function prepareRunExecution(deps: OrchestrationDeps): Promise<{
  ...
  const engine: 'langgraph' | 'simple' | 'none' = !deps.llm.client
    ? 'none'
    : deps.llm.modelProvider === 'openai' && !!deps.llm.openAiKey
      ? 'langgraph'
      : 'simple';
  ...
  if (!deps.llm.client) {
    await deps.emitEvent('thinking', {
      message: `Warning: No API key configured for ${deps.llm.modelProvider} (model: ${deps.llm.aiModel}). Running in limited mode without LLM.`,
      warning: true,
    });
  }
  ...
  await resolveAndApplySkills(deps.skillDeps, deps.skillState, history, toolExecutor, deps.emitEvent);
...
async function runWithLangGraph(deps: OrchestrationDeps, history: AgentMessage[]): Promise<void> {
...
async function runSimpleLoop(deps: OrchestrationDeps): Promise<void> {
  ...
  await runWithSimpleLoop({
    ...
    updateRunStatus: (status, output, error) => deps.updateRunStatus(status, output, error),
    buildTerminalEventPayload: (status, details) => deps.buildTerminalEventPayload(status, details),
...
export async function cleanupAfterRun(deps: OrchestrationDeps): Promise<void> {
  try {
    ...
    const currentRunStatus = await deps.runIo.getRunStatus({ runId: deps.context.runId });

    if (normalizeRunStatus(currentRunStatus) === 'running') {
      logWarn(`Run ${deps.context.runId} was left in running state - marking as failed`, { module: 'agentrunner' });
      await deps.updateRunStatus('failed', undefined, 'Run terminated unexpectedly during cleanup');
    }
  } catch (cleanupError) {
...
export async function handleRunCancelled(deps: OrchestrationDeps): Promise<void> {
  await handleCancelledRun(buildLifecycleDeps(deps));
}

export async function handleRunFailed(deps: OrchestrationDeps, error: unknown): Promise<void> {
