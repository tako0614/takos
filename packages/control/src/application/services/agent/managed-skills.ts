...
export interface ManagedSkillDefinition {
  ...
  locales: Record<SkillLocale, ManagedSkillLocaleContent>;
}

export interface LocalizedManagedSkill {
  id: string;
  version: string;
  locale: SkillLocale;
  category: ManagedSkillCategory;
...
export const CATEGORY_LABELS: Record<
  ManagedSkillCategory | "custom",
  { label: string; description: string }
> = {
  research: {
    label: "Research",
    description: "Investigate topics, gather evidence, and summarize findings.",
  },
  writing: {
    label: "Writing",
    description: "Draft, rewrite, and polish written content.",
  },
  planning: {
    label: "Planning",
    description: "Organize goals, milestones, and next steps.",
  },
  slides: {
    label: "Slides",
    description: "Design slide decks and presentation structures.",
  },
  software: {
    label: "Software",
    description: "Build, deploy, and manage software assets.",
  },
  custom: { label: "Custom", description: "Space custom skills." },
};

export function getCategoryLabel(
  cat: ManagedSkillCategory | "custom",
): { label: string; description: string } {
  return CATEGORY_LABELS[cat] ?? CATEGORY_LABELS.custom;
}

const JAPANESE_TEXT_RE = /[\u3040-\u30ff\u3400-\u9fff]/;
const VALID_DURABLE_OUTPUT_HINTS: DurableOutputHint[] = [
  "artifact",
  "reminder",
  "repo",
  "app",
...
const VALID_CATEGORIES: ManagedSkillCategory[] = [
  "research",
  "writing",
  "planning",
  "slides",
...
export function isSkillLocale(
  value: string | undefined | null,
): value is SkillLocale {
...
export function resolveSkillLocale(input?: {
  ...
  const normalizedAcceptLanguage = String(input?.acceptLanguage || "")
    .toLowerCase();
...
function cloneExecutionContract(
...
export function normalizeCustomSkillMetadata(
  input: unknown,
): CustomSkillMetadata {
  ...
  const activationTags = Array.isArray(raw.activation_tags)
    ? raw.activation_tags.map((item) => String(item).trim()).filter(Boolean)
      .slice(0, 20)
    : [];

  const executionRaw =
    raw.execution_contract && typeof raw.execution_contract === "object" &&
      !Array.isArray(raw.execution_contract)
      ? raw.execution_contract as Record<string, unknown>
      : {};

  const preferredTools = Array.isArray(executionRaw.preferred_tools)
    ? executionRaw.preferred_tools.map((item) => String(item).trim()).filter(
      Boolean,
    ).slice(0, 20)
    : [];
  const durableOutputs = Array.isArray(executionRaw.durable_output_hints)
    ? executionRaw.durable_output_hints
      ...
      .filter((item): item is DurableOutputHint =>
        VALID_DURABLE_OUTPUT_HINTS.includes(item as DurableOutputHint)
      )
      .slice(0, 10)
    : [];
  const outputModes = Array.isArray(executionRaw.output_modes)
    ? normalizeSkillOutputModes(executionRaw.output_modes).slice(0, 10)
    : [];
  const requiredMcpServers = Array.isArray(executionRaw.required_mcp_servers)
    ? executionRaw.required_mcp_servers.map((item) => String(item).trim())
  ...
  return {
    ...
    category: typeof raw.category === "string" &&
        VALID_CATEGORIES.includes(raw.category as ManagedSkillCategory)
      ? raw.category as ManagedSkillCategory
      : undefined,
...
export function validateCustomSkillMetadata(
  input: unknown,
): CustomSkillMetadataValidationResult {
  ...
  if (
    ...
    !isSkillLocale(typeof raw.locale === "string" ? raw.locale : null)
  ) {
  ...
  if (
    ...
    fieldErrors.category =
      "category must be one of research, writing, planning, slides, software";
  }

  if (
    raw.activation_tags !== undefined && !Array.isArray(raw.activation_tags)
  ) {
  ...
  if (executionRaw === "invalid") {
    ...
    if (
      executionRaw.preferred_tools !== undefined &&
      !Array.isArray(executionRaw.preferred_tools)
    ) {
      fieldErrors["execution_contract.preferred_tools"] =
        "preferred_tools must be an array of strings";
    }
    if (
      executionRaw.durable_output_hints !== undefined &&
      (!Array.isArray(executionRaw.durable_output_hints) ||
        executionRaw.durable_output_hints.some((item) =>
          !VALID_DURABLE_OUTPUT_HINTS.includes(
            String(item).trim() as DurableOutputHint,
          )
        ))
    ) {
      fieldErrors["execution_contract.durable_output_hints"] =
        "durable_output_hints contains an invalid value";
    }
    if (
      executionRaw.output_modes !== undefined &&
      (!Array.isArray(executionRaw.output_modes) ||
        executionRaw.output_modes.some((item) =>
          normalizeSkillOutputMode(item) === null
        ))
    ) {
      fieldErrors["execution_contract.output_modes"] =
        "output_modes contains an invalid value";
    }
    if (
      executionRaw.required_mcp_servers !== undefined &&
      !Array.isArray(executionRaw.required_mcp_servers)
    ) {
      fieldErrors["execution_contract.required_mcp_servers"] =
        "required_mcp_servers must be an array of strings";
    }
    if (
      executionRaw.template_ids !== undefined &&
      !Array.isArray(executionRaw.template_ids)
    ) {
...
export function localizeManagedSkill(
  skill: ManagedSkillDefinition,
  locale: SkillLocale,
): LocalizedManagedSkill {
...
export function listManagedSkillDefinitions(): ManagedSkillDefinition[] {
...
export function listLocalizedManagedSkills(
  locale: SkillLocale,
): LocalizedManagedSkill[] {
  return MANAGED_SKILLS.map((skill) => localizeManagedSkill(skill, locale));
}

export function getManagedSkillById(
  skillId: string,
  locale: SkillLocale,
): LocalizedManagedSkill | null {
  const skill = MANAGED_SKILLS.find((entry) => entry.id === skillId);
  if (!skill) {
    return null;
  }

  return localizeManagedSkill(skill, locale);
}
