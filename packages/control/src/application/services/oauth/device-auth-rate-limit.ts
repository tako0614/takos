...
export function isDeviceUserCodeLimited(userCode: string, now = Date.now()): boolean {
...
export function recordDeviceUserCodeAttempt(userCode: string, now = Date.now()): void {
  cleanupExpiredDeviceCodeAttempts(now);
  const state = deviceUserCodeAttempts.get(userCode);
  if (!state) {
