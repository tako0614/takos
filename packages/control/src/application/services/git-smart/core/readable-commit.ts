...
export type ResolveReadableCommitFailureReason =
  | "ref_not_found"
  | "commit_not_found"
  | "tree_not_found";
...
export async function resolveReadableCommitFromRef(
  ...
  const primary = await readableCommitDeps.getCommit(
    db,
    bucket,
    repoId,
    refCommitSha,
  ...
  const history = await readableCommitDeps.getCommitLog(
    db,
    bucket,
    repoId,
    refCommitSha,
  ...
  for (const candidate of history) {
    ...
    return {
      ok: true,
      refCommitSha,
      resolvedCommitSha: candidate.sha,
      degraded: true,
      commit: candidate,
    };
  }
