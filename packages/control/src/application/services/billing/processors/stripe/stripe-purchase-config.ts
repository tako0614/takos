 ...
 *
 * If a future processor needs its own top-up catalog, that processor should ship
 * its own equivalent of this file under `processors/<name>/`.
 */
...
export function getAvailableActions(account: {
  ...
  const hasActiveSubscription = Boolean(account.processorSubscriptionId);
...
function parseConfiguredTopupPack(
  value: unknown,
  index: number,
): BillingTopupPack {
  if (!isRecord(value)) {
  ...
  const priceId = typeof value.price_id === "string"
    ? value.price_id.trim()
    : "";
  const creditsCentsRaw = value.credits_cents;
  const featured = value.featured;
  const badge = typeof value.badge === "string"
    ? value.badge.trim() || null
    : null;
  ...
  if (
    typeof creditsCentsRaw !== "number" || !Number.isInteger(creditsCentsRaw) ||
    creditsCentsRaw <= 0
  ) {
...
export function getConfiguredProTopupPacks(env: Env): BillingTopupPack[] {
  ...
  if (rawCatalog) {
    ...
    try {
      ...
      throw new Error(
        `STRIPE_PRO_TOPUP_PACKS_JSON is invalid JSON: ${String(err)}`,
      );
    }
    ...
    return parsed.map((entry, index) => {
      ...
      if (seenIds.has(pack.id)) {
        throw new Error(`Duplicate top-up pack id: ${pack.id}`);
      }
      seenIds.add(pack.id);
      return pack;
    });
...
export function resolveConfiguredProTopupPack(
  env: Env,
  packId: string,
): BillingTopupPack {
  const pack = getConfiguredProTopupPacks(env).find((entry) =>
    entry.id === packId
  );
  if (!pack) {
