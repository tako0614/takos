 ...
 * shapes into the processor-agnostic types from `payment-processor.ts`.
 *
 * No code outside `processors/stripe/` should import from `stripe.ts` directly.
 */
...
type KnownStripeWebhookEvent = {
  [K in StripeWebhookEventType]: StripeWebhookEvent<K>;
}[StripeWebhookEventType];

const KNOWN_STRIPE_WEBHOOK_EVENT_TYPES = new Set<string>([
...
function isKnownStripeWebhookEvent(
  event: StripeWebhookEvent,
): event is KnownStripeWebhookEvent {
  return KNOWN_STRIPE_WEBHOOK_EVENT_TYPES.has(event.type);
}

function toCustomerIdString(
...
function toCheckoutMode(
  mode: "subscription" | "payment" | undefined,
...
): CompletedCheckoutSession["paymentStatus"] {
  if (
...
function normalizeWebhookEvent(event: StripeWebhookEvent): BillingWebhookEvent {
  ...
  if (!isKnownStripeWebhookEvent(event)) {
    return {
      kind: "unhandled",
      eventId,
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      const obj: StripeWebhookCheckoutSession = event.data.object;
      return {
    ...
    case "invoice.paid": {
      const obj: StripeWebhookInvoice = event.data.object;
      const customerId = toCustomerIdString(obj.customer);
      if (!customerId) {
        return { kind: "unhandled", eventId, processorEventType: event.type };
      }
    ...
    case "customer.subscription.updated": {
      const obj: StripeSubscription = event.data.object;
      const customerId = toCustomerIdString(obj.customer);
      if (!customerId) {
        return { kind: "unhandled", eventId, processorEventType: event.type };
      }
...
export function createStripeProcessor(env: Env): PaymentProcessor {
