...
import type { BillingAccountWithPlan, MeterType } from "./billing-types.ts";

// ---------------------------------------------------------------------------
// Plan ID constants and resolution
// ---------------------------------------------------------------------------

export const CANONICAL_BILLING_PLAN_IDS = [
  "plan_free",
  "plan_plus",
  "plan_payg",
] as const;
...
export function resolveCanonicalBillingPlanId(
  ...
  return LEGACY_BILLING_PLAN_ALIAS_MAP[
    normalized as keyof typeof LEGACY_BILLING_PLAN_ALIAS_MAP
  ] ?? null;
}

export function resolveCanonicalBillingPlanIdFromName(
...
export const DEFAULT_BILLING_QUOTAS: Record<
  string,
  Partial<Record<MeterType, number>>
> = {
  plan_free: {
...
export function hasExpectedBillingCatalog(plan: {
  ...
  const planId = resolveCanonicalBillingPlanId(plan.id) ??
    resolveCanonicalBillingPlanIdFromName(plan.name);
  if (!planId) {
    return false;
  }
...
export async function ensureDefaultBillingCatalog(db: Database): Promise<void> {
  for (const plan of DEFAULT_BILLING_PLANS) {
    // Upsert plan
    const existing = await db.select().from(billingPlans).where(
      eq(billingPlans.id, plan.id),
    ).get();
    if (existing) {
    ...
    for (
      const [quotaKey, limitValue] of Object.entries(
        DEFAULT_BILLING_QUOTAS[plan.id] ?? {},
      )
    ) {
      const existingQuota = await db.select().from(billingPlanQuotas)
        .where(
          and(
            eq(billingPlanQuotas.planId, plan.id),
            eq(billingPlanQuotas.quotaKey, quotaKey),
          ),
        )
        .get();
      if (existingQuota) {
        ...
        await db.insert(billingPlanQuotas).values({
          planId: plan.id,
          quotaKey,
          limitValue,
        });
    ...
    for (
      const [meterType, rateCents] of Object.entries(
        DEFAULT_BILLING_RATES[plan.id] ?? {},
      )
    ) {
      const existingRate = await db.select().from(billingPlanRates)
        .where(
          and(
            eq(billingPlanRates.planId, plan.id),
            eq(billingPlanRates.meterType, meterType),
          ),
        )
        .get();
      if (existingRate) {
        ...
        await db.insert(billingPlanRates).values({
          planId: plan.id,
          meterType,
          rateCents,
        });
...
export async function loadBillingAccountWithPlan(
  db: Database,
  accountIdFilter: { byAccountId?: string; byId?: string },
): Promise<BillingAccountWithPlan | null> {
  let account;
  if (accountIdFilter.byAccountId) {
    account = await db.select().from(billingAccounts).where(
      eq(billingAccounts.accountId, accountIdFilter.byAccountId),
    ).get();
  } else if (accountIdFilter.byId) {
    account = await db.select().from(billingAccounts).where(
      eq(billingAccounts.id, accountIdFilter.byId),
    ).get();
  }
  if (!account) return null;

  const plan = await db.select().from(billingPlans).where(
    eq(billingPlans.id, account.planId),
  ).get();
  if (!plan) return null;

  const quotas = await db.select().from(billingPlanQuotas).where(
    eq(billingPlanQuotas.planId, account.planId),
  ).all();
  const rates = await db.select().from(billingPlanRates).where(
    eq(billingPlanRates.planId, account.planId),
  ).all();
  const features = await db.select().from(billingPlanFeatures).where(
    eq(billingPlanFeatures.planId, account.planId),
  ).all();
  ...
  return {
    ...
    processorName: account.processorName,
    processorCustomerId: account.processorCustomerId ?? null,
    processorSubscriptionId: account.processorSubscriptionId ?? null,
    ...
    billingPlan: {
      ...
      description: plan.description,
      isDefault: plan.isDefault,
      billingPlanQuotas: quotas.map((q) => ({
        quotaKey: q.quotaKey,
        limitValue: q.limitValue,
      })),
      billingPlanRates: rates.map((r) => ({
        meterType: r.meterType,
        rateCents: r.rateCents,
      })),
      billingPlanFeatures: features.map((f) => ({
