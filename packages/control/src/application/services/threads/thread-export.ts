...
export async function exportThread(
  ...
  const allowedRoles =
    params.includeInternal && params.includeInternalRolesAllowed
  ...
  }).from(messages).where(eq(messages.threadId, params.threadId)).orderBy(
    asc(messages.sequence),
  ).all();
  const filteredMessages = messageRows.filter((m) =>
  ...
  if (params.format === "json") {
    ...
    headers.set(
      "Content-Disposition",
      `attachment; filename="${safeTitle}-${thread.id}.json"`,
    );
  ...
  if (params.format === "markdown" || params.format === "md") {
    ...
    headers.set(
      "Content-Disposition",
      `attachment; filename="${safeTitle}-${thread.id}.md"`,
    );
    headers.set("Cache-Control", "no-store");
  ...
  if (params.format === "pdf") {
    if (!params.renderPdf) {
      return errorJsonResponse("PDF export renderer is not configured", 503);
    }
  ...
  <body>
    ...
    ${
      filteredMessages.map((message) => (
        `<div class="msg"><div class="hdr">#${message.sequence} [${message.role}] ${message.createdAt}</div><pre>${
    ...
    try {
      ...
      headers.set(
        "Content-Disposition",
        `attachment; filename="${safeTitle}-${thread.id}.pdf"`,
      );
      headers.set("Cache-Control", "no-store");
      return new Response(pdf, { status: 200, headers });
    } catch (err) {
      threadExportDeps.logError("PDF export failed", err, {
        module: "services/threads/threads/thread-export",
      });
      const message = err instanceof Error ? err.message : String(err);
      const status = message.includes("not supported") ? 501 : 500;
      return threadExportDeps.errorJsonResponse(
