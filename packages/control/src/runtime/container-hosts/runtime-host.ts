...
export function buildRuntimeContainerEnv(
  env: Pick<Env, "ADMIN_DOMAIN" | "JWT_PUBLIC_KEY" | "PROXY_BASE_URL">,
  ...
  async generateSessionProxyToken(
    ...
    if (!this.cachedTokens) {
      const stored = await this.ctx.storage.get<
        Record<string, RuntimeProxyTokenInfo>
      >("proxyTokens");
    ...
    await this.ctx.storage.put(
      "proxyTokens",
      Object.fromEntries(this.cachedTokens),
    );
    return token;
  }
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      return await stub.fetch(
        await buildRuntimeForwardRequest(request, env, stub),
      );
    } catch (err) {
