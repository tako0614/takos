...
export async function resolveSecretValues(
  ...
  for (const secret of secretRecords) {
    try {
      ...
      const value = await decrypt(
        encrypted,
        encryptionKey,
        `secret:${repoId}:${secret.name}`,
      );
      secrets[secret.name] = value;
    } catch (err) {
      logError(`Failed to decrypt secret ${secret.name}`, err, {
  ...
  if (requiredSecretNames.length > 0) {
    const missingSecrets = requiredSecretNames.filter((name) =>
      secrets[name] === undefined
    );
    if (missingSecrets.length > 0) {
      throw new Error(
        `Missing referenced secrets: ${missingSecrets.join(", ")}`,
      );
    }
  }
...
export function collectReferencedSecretNamesFromEnv(
