...
export class GitPushLockDO {
  ...
  private async acquire(request: Request): Promise<Response> {
    const body = await request.json().catch((err) => {
      logWarn("Failed to parse acquire request body, using defaults", {
        module: "git-push-lock",
    ...
    const leaseMs = Math.max(
      1_000,
      Math.min(DEFAULT_LEASE_MS, Number(body.leaseMs) || DEFAULT_LEASE_MS),
    );
    const now = Date.now();
  ...
  private async release(request: Request): Promise<Response> {
    const body = await request.json().catch((err) => {
      logWarn("Failed to parse release request body, using defaults", {
        module: "git-push-lock",
        error: err instanceof Error ? err.message : String(err),
      });
      return {};
