...
export async function handleVectorize(
  ...
  if (indexResult.chunks > 0) {
    try {
      const workspace = await getDb(env.DB).select({
      ...
      logError(`${TAG} Billing recording failed for job ${jobId}`, billingErr, {
        module: "indexer",
      });
    }
  }
...
export async function handleIndexJobDlq(
  ...
  try {
    ...
    logError(`${TAG} Failed to persist DLQ entry`, persistErr, {
      module: "indexer",
    });
    throw persistErr;
  }
