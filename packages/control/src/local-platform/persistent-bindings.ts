...
export async function createSqliteD1Database(dbPath: string, migrationsDir: string) {
  const mod = await import('./persistent-d1.ts');
  return mod.createSqliteD1Database(dbPath, migrationsDir);
}

export async function createPostgresD1Database(connectionString: string) {
  const mod = await import('./persistent-d1.ts');
  return mod.createPostgresD1Database(connectionString);
}
