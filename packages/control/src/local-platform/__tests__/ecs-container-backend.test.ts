...
function findCall(calls: CommandCall[], first: string, second: string): CommandCall {
  const call = calls.find(({ args }) => args[0] === first && args[1] === second);
  assert(call);
  return call;
}
...
Deno.test('EcsContainerBackend - registers a task definition revision, creates a service when missing, and tails logs', async () => {
  ...
  assertEquals(registerCall.options, { env: { AWS_DEFAULT_REGION: 'us-east-1' } });
  ...
  assertEquals(createServiceCall.options, { env: { AWS_DEFAULT_REGION: 'us-east-1' } });
  assertEquals(
...
Deno.test('EcsContainerBackend - updates an existing service instead of creating a new one', async () => {
  ...
  assertEquals(updateServiceCall.options, { env: { AWS_DEFAULT_REGION: 'us-east-1' } });
  ...
  assertEquals(calls.some(({ args }) => args[0] === 'ecs' && args[1] === 'create-service'), false);
});
