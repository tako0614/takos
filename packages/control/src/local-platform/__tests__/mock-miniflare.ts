...
type WorkerModule = {
  default?: {
    fetch?: (request: Request, env: Record<string, unknown>, ctx?: unknown) => unknown;
    scheduled?: (controller: unknown, env: Record<string, unknown>, ctx?: unknown) => unknown;
    queue?: (batch: unknown, env: Record<string, unknown>, ctx?: unknown) => unknown;
  };
  [name: string]: unknown;
};

type QueueProducerConfig = string | { queueName: string; deliveryDelay?: number };
type DurableObjectConfig = { className: string; scriptName?: string; useSQLite?: boolean };
type QueueMessage = {
  id: string;
  timestamp: Date;
  attempts: number;
...
type MiniflareWorkerOptions = {
  ...
  serviceBindings?: Record<string, string | ((request: Request) => Promise<Response>)>;
...
function normalizeRequest(input: Request | string | URL, init?: RequestInit): Request {
...
function withQueueHelpers(messages: Array<QueueMessage>): {
  ...
  const retryMessages: Array<Record<string, unknown>> = [];
    ...
    const wrappedMessages = messages.map((message) => {
      const _msg = message as QueueMessage & { ack: () => void; retry: () => void };
      return {
        ...message,
        ack: () => {
...
function createWorkflowServiceHandler() {
  ...
  return async (requestOrInput: Request | string | URL, init?: RequestInit): Promise<Response> => {
    ...
    switch (method) {
      case 'create': {
        const state = store.create('workflow');
      ...
      default:
        return Response.json({ error: `Unknown Workflow method: ${method}` }, { status: 404 });
    }
  };
}

function createSimpleServiceBinding(fetcher: string | ((request: Request) => Promise<Response>) | undefined) {
  if (typeof fetcher === 'function') {
    return {
      fetch: (input: Request | string | URL, init?: RequestInit) => fetcher(normalizeRequest(input, init)),
    };
  }
  return {
    fetch: (input: Request | string | URL, init?: RequestInit) => globalThis.fetch(normalizeRequest(input, init)),
  };
}
...
class DurableObjectNamespace {
  ...
  private readonly instances = new Map<string, { fetch: (request: Request, env: Record<string, unknown>, ctx: Record<string, unknown>) => unknown }>();

  constructor(
    private readonly clazz: { new (state: DurableObjectState, env: Record<string, unknown>): unknown },
  ...
  private getInstance(id: string): { fetch: (request: Request, env: Record<string, unknown>, ctx: Record<string, unknown>) => unknown } {
    ...
    const instanceFetch = typeof (raw as { fetch?: unknown }).fetch === 'function'
      ? (raw as { fetch: (request: Request, env: Record<string, unknown>, ctx: Record<string, unknown>) => unknown }).fetch
      : null;
    if (!instanceFetch) {
      throw new Error('Durable object class does not implement fetch(request)');
    }
    const obj = {
      fetch: async (request: Request, env: Record<string, unknown>, ctx: Record<string, unknown>) => instanceFetch.call(raw, request, env, ctx),
    };
  ...
  get(id: string | { toString(): string }) {
    ...
    return {
      ...
      fetch: async (input: string | URL | Request, init?: RequestInit) => {
        ...
        return stub.fetch(request, this.env, {});
      },
    };
  }
}
...
export class MockMiniflare {
  ...
  private async initialize(): Promise<void> {
    if (!this.options.scriptPath) throw new Error('Miniflare scriptPath is required in mock');

    const rootPath = this.options.rootPath
      ? path.resolve(this.options.rootPath)
      : (this.options.modulesRoot ? path.resolve(this.options.modulesRoot) : process.cwd());
    ...
    this.env = {
      ...plainBindings,
      ...Object.fromEntries(Object.keys(this.options.d1Databases ?? {}).map((name) => [name, createMockD1()])),
      ...Object.fromEntries(Object.keys(this.options.kvNamespaces ?? {}).map((name) => [name, createMockResourceStore()])),
      ...Object.fromEntries(Object.keys(this.options.r2Buckets ?? {}).map((name) => [name, createMockResourceStore()])),
    };

    for (const [name, producer] of Object.entries(queueProducers)) {
      if (typeof producer === 'string' || producer?.constructor?.name === 'String') {
    ...
    for (const [name, durable] of Object.entries(durableObjects)) {
      ...
      this.env[name] = new DurableObjectNamespace(ctor as { new (state: DurableObjectState, env: Record<string, unknown>): unknown }, this.env);
    }
  ...
  async getWorker(): Promise<{
    ...
    scheduled: (controller: { scheduledTime?: string | number | Date; cron: string }) => Promise<QueueResult>;
    queue: (queueName: string, messages: Array<{ id: string; timestamp: Date; attempts: number; body?: unknown }>) => Promise<DispatchQueueResult>;
    connect(): never;
  }> {
    await this.ready;
    ...
    const workerBinding = workerModule.default as NonNullable<WorkerModule['default']>;
    return {
      fetch: async (input: Request | string | URL, init?: RequestInit): Promise<Response> => {
        ...
        const request = normalizeRequest(input, init);
        const response = await Promise.resolve(workerBinding.fetch(request, this.env, this.createContext()));
        if (response instanceof Response) return response;
        return new Response(response as never);
      },
      scheduled: async (controller: { scheduledTime?: string | number | Date; cron: string }) => {
        ...
        const scheduledTime = controller.scheduledTime instanceof Date
          ...
          : typeof controller.scheduledTime === 'number'
            ...
            : controller.scheduledTime
              ...
              : Date.now();
        await Promise.resolve(workerBinding.scheduled({ ...controller, scheduledTime }, this.env, this.createContext()));
        return { outcome: 'ok', noRetry: false };
      },
      queue: async (queueName: string, messages: Array<{ id: string; timestamp: Date; attempts: number; body?: unknown; serializedBody?: ArrayBuffer | ArrayBufferView }>) => {
        ...
        const { wrappedMessages, explicitAcks, retryMessages } = withQueueHelpers(normalized as Array<QueueMessage>);
        await Promise.resolve(workerBinding.queue({
          queue: queueName,
          messages: wrappedMessages,
        }, this.env, this.createContext()));
        return {
          outcome: 'ok',
          noRetry: false,
