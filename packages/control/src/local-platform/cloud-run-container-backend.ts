...
export class CloudRunContainerBackend implements ContainerBackend {
  ...
  async createAndStart(opts: ContainerCreateOpts): Promise<ContainerCreateResult> {
    const serviceName = sanitizeCloudRunServiceName(this.serviceId ?? opts.name);
    const args = [
      'run',
      'deploy',
      serviceName,
    ...
    if (result.exitCode !== 0) {
      throw new Error(result.stderr || `gcloud run deploy exited with code ${result.exitCode}`);
    }
    ...
    const runtimeBaseUrl = this.baseUrl
      ...
      ?? (opts.requestedEndpoint?.kind === 'http-url' ? opts.requestedEndpoint.base_url : null);
    if (!runtimeBaseUrl) {
      throw new Error('Cloud Run deployment did not return a service URL');
    }

    const healthUrl = new URL(opts.healthPath ?? '/health', runtimeBaseUrl).toString();
  ...
  async remove(containerId: string): Promise<void> {
    ...
    if (result.exitCode !== 0 && !result.stderr.includes('was not found')) {
      throw new Error(result.stderr || `gcloud run services delete exited with code ${result.exitCode}`);
    }
  }
  ...
  async getLogs(containerId: string, tail = 100): Promise<string> {
    const result = await this.gcloud([
      'logging',
      'read',
      `resource.type="cloud_run_revision" AND resource.labels.service_name="${sanitizeCloudRunServiceName(containerId)}"`,
      '--limit',
      String(tail),
      '--order=desc',
