...
import { assertEquals, assert, assertStringIncludes } from 'jsr:@std/assert';
  ...
  const { app } = createBrowserServiceApp();
    ...
    const body = await res.json();
    assertStringIncludes(body.error, 'localhost');
})
  ...
  const { app } = createBrowserServiceApp();
    ...
    assertEquals(res.status, 500);
    const body = await res.json();
    assertStringIncludes(body.error, 'private');
})
  ...
  const { app } = createBrowserServiceApp();
    ...
    assertEquals(res.status, 500);
    const body = await res.json();
    assertStringIncludes(body.error, 'HTTP/HTTPS');
})
  ...
  const { app } = createBrowserServiceApp();
    ...
    assertEquals(res.status, 500);
    const body = await res.json();
    assertStringIncludes(body.error, 'Invalid URL');
})
