...
export const SPACE_SCOPE_MISMATCH_ERROR =
...
export function getSpaceIdFromBody(
...
export function collectRequestedSpaceIds(
...
export function hasSpaceScopeMismatch(
  c: Context<RuntimeEnv>,
  spaceId: unknown,
): boolean {
...
export function hasAnySpaceScopeMismatch(
  c: Context<RuntimeEnv>,
  spaceIds: readonly unknown[],
): boolean {
...
export function enforceSpaceScopeMiddleware(
  ...
  return async (
    ...
    if (new Set(spaceIds).size > 1) {
      return forbidden(c, "Conflicting space identifiers in request");
    }
