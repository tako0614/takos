...
export async function setupNode(
  ...
  if (!nodeVersion && inputs['node-version-file']) {
    ...
    if (!allowedVersionFiles.includes(requestedFile)) {
      pushLog(context.logs, `Warning: node-version-file must be one of: ${allowedVersionFiles.join(', ')}`);
    } else {
      ...
      try {
        ...
        pushLog(context.logs, `Read Node.js version from file: ${requestedFile}`);
      } catch {
        pushLog(context.logs, `Warning: Could not read version file: ${requestedFile}`);
      }
    }
  }
  ...
  if (!nodeVersion) {
    ...
    for (const versionFile of [nvmrcPath, nodeVersionPath]) {
      try {
        ...
        nodeVersion = stripVersionPrefix(content);
        pushLog(context.logs, `Read Node.js version from ${path.basename(versionFile)}: ${nodeVersion}`);
        break;
      } catch {
  ...
  if (!currentVersion.startsWith(nodeVersion.split('.')[0])) {
    pushLog(context.logs, `Warning: Requested major version ${nodeVersion} differs from installed ${currentVersion}`);
  }
