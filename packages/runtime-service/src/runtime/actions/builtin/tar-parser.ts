...
function parsePaxAttributes(data: Uint8Array): Record<string, string> {
  ...
  while (offset < data.length) {
    ...
    const lengthText = Buffer.from(data.subarray(offset, spaceIndex)).toString('utf8').trim();
    ...
    const recordBytes = data.subarray(spaceIndex + 1, offset + recordLength - 1);
...
function resolveEntryAndResetState(
  ...
  const resolvedPath = state.pendingLongPath ?? state.pendingPax.path ?? combinedName;
  const resolvedLinkPath = state.pendingLongLinkPath ?? state.pendingPax.linkpath ?? linkName;
...
export function parseTarEntriesFromGzipArchive(archiveData: Uint8Array): ParsedTarEntry[] {
  ...
  while (state.offset + 512 <= tarData.length) {
    ...
    entries.push(resolveEntryAndResetState(combinedName, linkName, typeFlag, state));
    state.offset = nextOffset;
  }
