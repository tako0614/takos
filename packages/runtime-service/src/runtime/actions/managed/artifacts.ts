...
import { isPathWithinBase, resolvePathWithin } from "../../paths.ts";
...
function logTransferSummary(
  ...
  pushLog(
    context.logs,
    `${verb} ${count} file(s), ${formatBytes(totalBytes)} total`,
  );
}
...
function validateArtifactName(rawName: string): string {
  ...
  if (
    artifactName.includes("..") || artifactName.includes("/") ||
...
export async function uploadArtifact(
  ...
  for (const artifactPath of artifactPaths) {
    try {
      ...
      if (ifNoFiles === "error") {
        throw new Error(
          `Artifact path is invalid or not found: ${artifactPath} (${
            getErrorMessage(err)
          })`,
        );
      }
      if (ifNoFiles === "warn") {
        pushLog(
          context.logs,
          `Warning: Artifact path is invalid or not found: ${artifactPath}`,
        );
      }
    }
  }
  ...
  for (const file of filesToUpload) {
    try {
      ...
      pushLog(
        context.logs,
        `Warning: Failed to upload ${file.relativePath}: ${err}`,
      );
    }
  }
  ...
  do {
    ...
    for (const obj of listResult.Contents || []) {
      ...
      if (relativePath.includes("..") || path.isAbsolute(relativePath)) {
        pushLog(
          context.logs,
          `Warning: Skipping unsafe artifact path: ${relativePath}`,
        );
        continue;
      }
      ...
      try {
        ...
        if (getResult.Body) {
          ...
          if (
            !isPathWithinBase(resolvedDownload, resolvedLocal, {
              allowBase: false,
            })
          ) {
            pushLog(
              context.logs,
              `Warning: Skipping artifact that escapes download directory: ${relativePath}`,
            );
            continue;
          }
        ...
        pushLog(
          context.logs,
          `Warning: Failed to download ${relativePath}: ${err}`,
        );
      }
    }
