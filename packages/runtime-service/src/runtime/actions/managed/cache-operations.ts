...
async function cleanupTempFile(
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
      ...
      pushLog(
        context.logs,
        `Warning: Failed to clean up temp file (${reason}): ${tempPath} - ${message}`,
      );
    }
  }
}
...
function validateCacheKey(rawKey: string, label: string): string {
  ...
  if (rawKey.length > CACHE_MAX_KEY_LENGTH) {
    throw new Error(
      `Invalid ${label}: exceeds ${CACHE_MAX_KEY_LENGTH} characters`,
    );
  }
...
function buildCacheNamespace(context: ActionContext): CacheNamespaceInfo {
  const spaceId = firstNonEmpty([
    context.env.TAKOS_SPACE_ID,
    context.env.SPACE_ID,
  ]);
...
function buildNamespacedCacheObjectKey(
  cacheKey: string,
...
function validateCacheArchiveEntries(archiveData: Uint8Array): void {
  ...
  for (const entry of entries) {
    ...
    const entryPath = assertSafeArchivePath(
      entry.path,
      "Archive entry path",
      true,
    );
    if (entryPath === "." && entry.type !== "5") {
      throw new Error(
        'Archive entry path "." is only allowed for directory entries',
      );
    }
    ...
    if (entry.type === "1" || entry.type === "2") {
      ...
      assertSafeArchivePath(
        entry.linkPath,
        `Archive link target for "${entryPath}"`,
        true,
      );
    }
  }
...
export async function cache(
  ...
  for (const restoreKey of inputs["restore-keys"] || []) {
    try {
      ...
      pushLog(
        context.logs,
        `Warning: Ignoring invalid restore key: ${getErrorMessage(err)}`,
      );
    }
  }
    ...
    for (const candidate of buildCacheObjectCandidates(key, cacheNamespace)) {
      try {
        ...
        try {
          ...
          throw new Error(
            `Rejected cache archive "${candidate.r2Key}": ${
              getErrorMessage(err)
            }`,
          );
        } finally {
        ...
        pushLog(
          context.logs,
          `Warning: Cache restore failed for key "${key}" (${candidate.mode}): ${
            getErrorMessage(err)
          }`,
        );
      }
