...
export function isValidDomainPattern(domain: string): boolean {
  ...
  const pattern = /^\*?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)+(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)$|^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*$/;
...
export function isPrivateIPv6(host: string): boolean {
  ...
  const collapsed = normalized.replace(/(^|:)0+(?=\d)/g, '$1').replace(/(:0)+:/, '::');
...
export async function assertOutboundUrlAllowed(targetUrl: URL, allowedDomains: string[]): Promise<void> {
  if (!ALLOWED_PROTOCOLS.has(targetUrl.protocol)) {
    throw new Error(`Network access denied: protocol ${targetUrl.protocol} is not allowed`);
  }
  ...
  for (const ip of resolvedIps) {
    if (isPrivateOrLocalHost(ip)) {
      throw new Error(`Network access denied: ${hostname} resolved to private/local IP ${ip}`);
    }
  }
}
