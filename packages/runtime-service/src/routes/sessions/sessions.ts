...
app.post('/sessions', async (c) => {
  try {
    ...
    const workDir = await sessionStore.getSessionDir(session_id, space_id, ownerSub, proxyTokenHeader || undefined);
    let fileCount = 0;
    let gitMode = false;
    ...
    if (repoGitPath) {
      ...
      if (!cloneResult.success) {
        const isBranchNotFound = branch
          && (cloneResult.output.includes('Remote branch') || cloneResult.output.includes('not found'));

        if (!isBranchNotFound) {
          return internalError(c, 'Failed to clone repository', { output: cloneResult.output });
        }
        ...
        if (!defaultResult.success) {
          return internalError(c, 'Failed to clone repository', { output: defaultResult.output });
        }

        const checkoutResult = await runGitCommand(['checkout', '-b', branch], workDir);
        if (checkoutResult.exitCode !== 0) {
          c.get('log')?.warn('Failed to create branch', { branch, output: checkoutResult.output });
        }
      }
      ...
      const branchResult = await runGitCommand(['rev-parse', '--abbrev-ref', 'HEAD'], workDir);
      clonedBranch = branchResult.output.trim() || branch || 'main';

      try {
        const items = await fs.readdir(workDir, { recursive: true, withFileTypes: true });
        fileCount = items.filter((item) => item.isFile()).length;
      } catch {
        fileCount = 0;
      }

      c.get('log')?.info('Git clone completed', { repoGitPath: resolvedRepoGitPath, workDir, branch: clonedBranch });
...
app.post('/sessions/:id/commit', async (c) => {
  try {
    ...
    let session;
    try {
      session = sessionStore.getSessionWithValidation(sessionId, space_id, getSessionOwnerSub(c));
    } catch (err) {
      if (err instanceof OwnerBindingError) {
        ...
        if (msg.includes('not found')) {
          ...
          return forbidden(c, 'Session does not belong to the specified workspace');
        } else {
    ...
    try {
      ...
      return c.json({ success: true, git_mode: false, message: 'Not a git repository, no commit needed' });
    }
    ...
    if (!statusResult.output.trim()) {
      return c.json({ success: true, git_mode: true, committed: false, message: 'No changes to commit' });
    }
    ...
    const commitResult = await runGitCommand(['commit', '-m', commitMessage], workDir);
    ...
    if (pushResult.exitCode !== 0) {
      const pushUpstreamResult = await runGitCommand(['push', '-u', 'origin', branch], workDir);
      if (pushUpstreamResult.exitCode !== 0) {
        return internalError(c, 'Failed to push changes', { committed: true, commitHash, output: pushUpstreamResult.output });
      }
    }
