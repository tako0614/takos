...
app.post("/repos/:repoId/import", publicControlRepoIdRoute);
app.post("/repos/:repoId/export", publicControlRepoIdRoute);
app.post("/repos/:repoId/commit", publicControlRepoIdRoute);
app.put("/repos/:repoId/content/*", publicControlRepoIdRoute);
...
): { spaceId: string } | { error: Response } {
  const session = sessionStore.getSessionByWorkDir(resolvedWorkDir);
  const inferredSpaceId = session?.spaceId;
  const spaceId = requestedSpaceId ?? inferredSpaceId;

  if (!spaceId) {
    return { error: badRequest(c, "space_id is required for repo operations") };
  }

  if (session && requestedSpaceId && session.spaceId !== requestedSpaceId) {
    return {
      error: forbidden(c, "workDir does not belong to the specified space"),
    };
  }

  if (hasSpaceScopeMismatch(c, spaceId)) {
    return { error: forbidden(c, SPACE_SCOPE_MISMATCH_ERROR) };
  }
...
app.post("/repos/commit", async (c) => {
  try {
    const { workDir, message, author, space_id } = await c.req.json() as {
      workDir: string;
      message: string;
      space_id?: string;
    ...
    const spaceResult = resolveWorkspaceSpaceId(c, resolvedWorkDir, space_id);
...
app.post("/repos/push", async (c) => {
  try {
    const { workDir, branch, space_id } = await c.req.json() as {
      workDir: string;
      branch?: string;
      space_id?: string;
    };
...
app.post("/repos/merge", async (c) => {
  try {
    const { spaceId, repoName, sourceBranch, targetBranch, message } = await c
      .req.json() as {
        spaceId: string;
        repoName: string;
        sourceBranch: string;
    ...
    if (!spaceId || !repoName || !sourceBranch || !targetBranch) {
      return badRequest(
        c,
        "spaceId, repoName, sourceBranch, and targetBranch are required",
      );
    }
    ...
    try {
      ...
      if (cloneResult.exitCode !== 0) {
        return internalError(c, "Failed to clone repository for merge", {
      ...
      if (checkoutResult.exitCode !== 0) {
        return badRequest(
          c,
          `Failed to checkout target branch: ${targetBranch}`,
      ...
      if (message) {
        if (typeof message !== "string" || message.length > 4096) {
          return badRequest(
            c,
            "Merge message must be a string of at most 4096 characters",
          );
        }
        if (hasControlCharacters(message)) {
          return badRequest(
            c,
            "Merge message contains invalid control characters",
          );
        }
      ...
      if (mergeResult.exitCode !== 0) {
        ...
        const conflictFiles = statusLines.filter(isConflictLine).map((line) =>
          line.slice(3).trim()
        );
        ...
        if (conflictFiles.length > 0) {
          ...
          return c.json({
            error: {
              ...
              details: { conflicts: conflictFiles, output: mergeResult.output },
            },
          }, 409);
        }

        return internalError(c, "Failed to merge branches", {
      ...
      if (pushResult.exitCode !== 0) {
        return internalError(
          c,
          "Merge succeeded but failed to push to origin",
          {
            commitHash,
